...
test(same_all) :-
    ga(A), gb(B), compare_same_cells(A, B, Sames),
    msort(Sames, Sorted),
    Sorted = [r(0,0), r(0,1), r(1,0), r(1,1)].

test(same_partial) :-
    g1(A), g2(B), compare_same_cells(A, B, Sames),
    % 8 cells are 0 in both; center differs.
    length(Sames, N), N =:= 8.

test(same_none) :-
    p1(A), p2(B), compare_same_cells(A, B, Sames),
    Sames = [].
...
test(added_2_in_center) :-
    g1(A), g2(B), compare_added_color(A, B, 2, Added),
    Added = [r(1,1)].

test(added_nothing) :-
    g1(A), g2(B), compare_added_color(A, B, 9, Added),
    Added = [].

test(added_row) :-
    gc(A), gd(B), compare_added_color(A, B, 1, Added),
    msort(Added, Sorted),
    Sorted = [r(0,0), r(0,1), r(0,2)].
...
test(removed_1_from_center) :-
    g1(A), g2(B), compare_removed_color(A, B, 1, Removed),
    Removed = [r(1,1)].

test(removed_nothing) :-
    g1(A), g2(B), compare_removed_color(A, B, 0, Removed),
    Removed = [].

test(removed_row) :-
    gd(A), gc(B), compare_removed_color(A, B, 1, Removed),
...
test(changed_to_2) :-
    g1(A), g2(B), compare_changed_to(A, B, 2, Changed),
    Changed = [r(1,1)].

test(changed_to_absent) :-
    ga(A), gb(B), compare_changed_to(A, B, 9, Changed),
    Changed = [].
...
test(changed_from_1) :-
    g1(A), g2(B), compare_changed_from(A, B, 1, Changed),
    Changed = [r(1,1)].

test(changed_from_absent) :-
    ga(A), gb(B), compare_changed_from(A, B, 9, Changed),
    Changed = [].
...
test(similarity_identical) :-
    ga(A), gb(B), compare_similarity(A, B, EqCount-Total),
    EqCount =:= 4, Total =:= 4.

test(similarity_one_diff) :-
    g1(A), g2(B), compare_similarity(A, B, EqCount-Total),
    EqCount =:= 8, Total =:= 9.

test(similarity_all_diff) :-
    p1(A), p2(B), compare_similarity(A, B, EqCount-Total),
    EqCount =:= 0, Total =:= 4.
...
test(region_diff_basic) :-
    compare_region_diff([r(0,0), r(0,1), r(1,0)], [r(0,0)], Diff),
    msort(Diff, Sorted),
    Sorted = [r(0,1), r(1,0)].

test(region_diff_empty_result) :-
    compare_region_diff([r(0,0)], [r(0,0), r(1,1)], Diff),
    Diff = [].

test(region_diff_no_overlap) :-
    compare_region_diff([r(0,0)], [r(1,1)], Diff),
    Diff = [r(0,0)].
...
test(region_intersect_one) :-
    compare_region_intersect([r(0,0), r(0,1)], [r(0,0), r(1,1)], Inter),
    Inter = [r(0,0)].

test(region_intersect_empty) :-
    compare_region_intersect([r(0,0)], [r(1,1)], Inter),
    Inter = [].

test(region_intersect_all) :-
    compare_region_intersect([r(0,0), r(1,1)], [r(0,0), r(1,1)], Inter),
    msort(Inter, Sorted),
    Sorted = [r(0,0), r(1,1)].
...
test(region_union_combined) :-
    compare_region_union([r(0,0)], [r(1,1)], Union),
    msort(Union, Sorted),
    Sorted = [r(0,0), r(1,1)].

test(region_union_no_dup) :-
    compare_region_union([r(0,0), r(1,1)], [r(1,1), r(2,2)], Union),
    msort(Union, Sorted),
    Sorted = [r(0,0), r(1,1), r(2,2)].

test(region_union_identical) :-
    compare_region_union([r(0,0)], [r(0,0)], Union),
    Union = [r(0,0)].
...
test(equal_same_order) :-
    compare_region_equal([r(0,0), r(1,1)], [r(0,0), r(1,1)]).

test(equal_diff_order) :-
    compare_region_equal([r(1,1), r(0,0)], [r(0,0), r(1,1)]).

test(not_equal) :-
    \+ compare_region_equal([r(0,0)], [r(1,1)]).
...
test(equal_grids) :-
    ga(A), gb(B), compare_grids_equal(A, B).

test(not_equal_grids) :-
    g1(A), g2(B), \+ compare_grids_equal(A, B).
...
test(shift_one_cell) :-
    g1(A), g2(B), compare_color_shift(A, B, Pairs),
    Pairs = [1-2].

test(shift_none) :-
    ga(A), gb(B), compare_color_shift(A, B, Pairs),
    Pairs = [].

test(shift_all_cells) :-
    p1(A), p2(B), compare_color_shift(A, B, Pairs),
    msort(Pairs, Sorted),
