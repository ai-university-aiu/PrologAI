...
:- module(colormap, [
    cm_apply/3, cm_apply_default/4,
    cm_invert/3, cm_compose/3,
    cm_from_grids/3, cm_identity/2,
    cm_remap/3, cm_remap_list/3,
    cm_palette/2, cm_used/2,
    cm_has_key/2, cm_lookup/3,
    cm_restrict/3, cm_expand/4
]).
...
% cm_apply(+Map, +Grid, -Grid2): substitute each cell value using Map (list of Old-New pairs).
...
% Apply per-row substitution using maplist/3 with a row helper capturing Map.
    maplist(cm_apply_row_(Map), Grid, Grid2).
% Per-row helper applies substitution to every cell.
cm_apply_row_(Map, Row, Row2) :-
    maplist(cm_apply_cell_(Map), Row, Row2).
% Per-cell substitution: replace via memberchk lookup; preserve on miss.
cm_apply_cell_(Map, V, V2) :-
    (memberchk(V-V2, Map) -> true ; V2 = V).

% cm_apply_default(+Map, +Bg, +Grid, -Grid2): substitute cells; replace unmatched with Bg.
cm_apply_default(Map, Bg, Grid, Grid2) :-
% Apply per-row substitution with default using maplist/3.
    maplist(cm_apply_default_row_(Map, Bg), Grid, Grid2).
% Per-row helper applies substitution with default to every cell.
cm_apply_default_row_(Map, Bg, Row, Row2) :-
    maplist(cm_apply_default_cell_(Map, Bg), Row, Row2).
% Per-cell substitution with default: memberchk lookup or Bg.
cm_apply_default_cell_(Map, Bg, V, V2) :-
    (memberchk(V-V2, Map) -> true ; V2 = Bg).

% cm_invert(+Map, +Grid, -Grid2): apply the inverse of Map (swap Old-New to New-Old).
cm_invert(Map, Grid, Grid2) :-
% Build the inverse map by swapping key-value in every pair.
    findall(N-O, member(O-N, Map), InvMap),
    cm_apply(InvMap, Grid, Grid2).

% cm_compose(+MapA, +MapB, -MapAB): compose two maps: MapAB[k] = MapB[MapA[k]].
...
% cm_from_grids(+GridA, +GridB, -Map): build a map from cell correspondences.
...
% cm_identity(+Palette, -Map): build the identity map V-V for every value in Palette.
cm_identity(Palette, Map) :-
% Each palette value maps to itself.
    findall(V-V, member(V, Palette), Map).

% cm_remap(+OldVals, +NewVals, -Map): build a map from parallel old/new value lists.
...
% cm_remap_list(+Map, +List, -List2): apply Map to a 1D list (not a grid).
% Cells with no matching key are left unchanged.
cm_remap_list(Map, List, List2) :-
% Apply per-element substitution using maplist/3.
    maplist(cm_apply_cell_(Map), List, List2).

% cm_palette(+Map, -Palette): sorted list of all source (key) values in Map.
cm_palette(Map, Palette) :-
% Collect all keys from the map and sort to deduplicate.
    findall(K, member(K-_, Map), Ks),
    sort(Ks, Palette).

% cm_used(+Grid, -UsedVals): sorted list of all distinct values actually used in Grid.
cm_used(Grid, UsedVals) :-
% Collect all cell values from the grid then sort to deduplicate.
    findall(V, (member(Row, Grid), member(V, Row)), Vs),
    sort(Vs, UsedVals).

% cm_has_key(+Map, +K): succeeds iff K appears as a key in Map.
cm_has_key(Map, K) :-
% memberchk provides deterministic first-match lookup.
    memberchk(K-_, Map).

% cm_lookup(+Map, +K, -V): look up the value for key K in Map.
% Fails if K is not present.
cm_lookup(Map, K, V) :-
% memberchk is deterministic and cuts on first match.
    memberchk(K-V, Map).

% cm_restrict(+Map, +Keys, -Map2): keep only entries whose key is in Keys.
...
% cm_expand(+Map, +Bg, +Palette, -Map2): extend Map with K-Bg entries for each
