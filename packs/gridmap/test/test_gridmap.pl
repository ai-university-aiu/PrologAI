...
test('AC-GMP-001: remap two colors on pal3') :-
    gmp_remap([[r,g,r],[g,b,g],[r,g,r]], [r-x, g-y], R),
    R = [[x,y,x],[y,b,y],[x,y,x]].

test('AC-GMP-002: remap with partial match leaves unmatched unchanged') :-
    gmp_remap([[r,g,r],[g,b,g],[r,g,r]], [r-x], R),
    R = [[x,g,x],[g,b,g],[x,g,x]].
...
% Apply [r-x,g-y], then invert the map and apply back.
    G = [[r,g,r],[g,b,g],[r,g,r]],
    Map = [r-x, g-y],
    gmp_remap(G, Map, Mapped),
    gmp_invert_map(Map, InvMap),
    gmp_remap(Mapped, InvMap, Restored),
