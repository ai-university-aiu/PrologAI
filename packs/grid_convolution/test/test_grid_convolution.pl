:- use_module('../prolog/grid_convolution').
...
test(count_in_corner, []) :-
    ...
    grid_convolution_count_in(G, 0, 0, 1, r, 4).

test(count_in_zero, []) :-
    g3x3_all_x(G),
    grid_convolution_count_in(G, 1, 1, 1, r, 0).

test(count_in_dot, []) :-
    g5x5_dot(G),
    % N=0: 1x1 window at center (2,2) = 1 r
    grid_convolution_count_in(G, 2, 2, 0, r, 1).

% --- grid_convolution_density_map ---
test(density_map_dot, []) :-
    g5x5_dot(G),
    % N=1: center cell (2,2) has 1 r in 3x3 window; edges have 0 or 1
    grid_convolution_density_map(G, 1, r, DMap),
...
% --- grid_convolution_find_pattern ---
test(find_pattern_identity, []) :-
    g2x2_r(G),
    % Pattern = grid itself -> found at (0,0)
    grid_convolution_find_pattern(G, G, Positions),
    Positions = [0-0].

test(find_pattern_1x1, []) :-
    g3x3_rb(G),
    % Find 1x1 pattern [[r]] in g3x3_rb -> 4 positions
    grid_convolution_find_pattern(G, [[r]], Positions),
    length(Positions, 4).

test(find_pattern_not_found, []) :-
    g3x3_all_x(G),
    grid_convolution_find_pattern(G, [[r]], []).

test(find_pattern_2x2_in_5x5, []) :-
    g5x5_twoblock(G),
    % Find 2x2 r block: [[r,r],[r,r]] in g5x5_twoblock
    % Block at (1,1) and (3,3)
    grid_convolution_find_pattern(G, [[r,r],[r,r]], Positions),
    length(Positions, 2).

% --- grid_convolution_count_pattern ---
test(count_pattern_one, []) :-
    g3x3_rb(G),
    grid_convolution_count_pattern(G, [[r,r],[r,r]], 1).

test(count_pattern_zero, []) :-
    g3x3_all_x(G),
    grid_convolution_count_pattern(G, [[r]], 0).

% --- grid_convolution_has_pattern ---
test(has_pattern_yes, []) :-
    g3x3_rb(G),
    grid_convolution_has_pattern(G, [[r]]).

test(has_pattern_no, []) :-
    g3x3_all_x(G),
    \+ grid_convolution_has_pattern(G, [[r]]).

% --- grid_convolution_hot_spots ---
test(hot_spots_center, []) :-
    ...
    grid_convolution_hot_spots(G, 1, r, 1, Cells),
    length(Cells, 9).

test(hot_spots_none, []) :-
    g3x3_all_x(G),
    grid_convolution_hot_spots(G, 1, r, 1, []).
...
test(hot_spots_high_threshold, []) :-
    ...
    grid_convolution_hot_spots(G, 1, r, 9, Cells),
    Cells = [1-1].

% --- grid_convolution_dilate_sq ---
test(dilate_sq_n0_identity, []) :-
    g3x3_rb(G),
    grid_convolution_dilate_sq(G, r, 0, G).

test(dilate_sq_n1_dot_fills, []) :-
    g5x5_dot(G),
    grid_convolution_dilate_sq(G, r, 1, Result),
...
test(dilate_sq_uniform_unchanged, []) :-
    g3x3_all_r(G),
    grid_convolution_dilate_sq(G, r, 1, G).

% --- grid_convolution_erode_sq ---
test(erode_sq_n0_identity, []) :-
    g3x3_all_r(G),
    grid_convolution_erode_sq(G, r, 0, G).
...
test(erode_sq_n1_block_to_center, []) :-
    ...
    grid_convolution_erode_sq(G, r, 1, Result),
    findall(RC, (nth0(Row,Result,RowL), nth0(Col,RowL,r), RC=Row-Col), Cells),
    Cells = [2-2].

test(erode_sq_uniform_unchanged, []) :-
    g3x3_all_r(G),
    grid_convolution_erode_sq(G, r, 1, G).

% --- grid_convolution_replace_pattern ---
test(replace_pattern_first_match, []) :-
    g5x5_twoblock(G),
    % Replace first 2x2 r block at (1,1) with [[a,a],[a,a]]
    grid_convolution_replace_pattern(G, [[r,r],[r,r]], [[a,a],[a,a]], Result),
    nth0(1, Result, Row1), nth0(1, Row1, a),
    nth0(1, Result, Row1), nth0(2, Row1, a).

test(replace_pattern_no_match_unchanged, []) :-
    g3x3_all_x(G),
    grid_convolution_replace_pattern(G, [[r]], [[a]], G).

% --- Combined tests ---
test(find_and_count_consistent, []) :-
    g5x5_twoblock(G),
    grid_convolution_find_pattern(G, [[r,r],[r,r]], Positions),
    grid_convolution_count_pattern(G, [[r,r],[r,r]], Count),
...
test(density_map_matches_count_in, []) :-
    ...
    grid_convolution_count_in(G, 1, 1, 1, r, V).

test(dilate_sq_increases_fg, []) :-
    g5x5_dot(G),
    grid_convolution_dilate_sq(G, r, 1, D),
...
test(erode_sq_decreases_fg, []) :-
    g5x5_block(G),
    grid_convolution_erode_sq(G, r, 1, E),
    findall(1, (member(Row,G), member(r,Row)), Orig),
