:- use_module('../prolog/scene_pair').
:- use_module(library(plunit)).

:- begin_tests(scene_pair).
...
test(same_n_objs_equal) :-
    ...
    scene_pair_same_n_objs([R, B], [B, R]).

test(same_n_objs_unequal) :-
    red_dot(R), blue_dot(B),
    \+ scene_pair_same_n_objs([R, B], [R]).

test(same_n_objs_empty) :-
    scene_pair_same_n_objs([], []).

% --- scene_pair_added_objs/3 ---

test(added_objs_one) :-
    red_dot(R), blue_dot(B),
    scene_pair_added_objs([R], [R, B], Added),
    Added == [B].

test(added_objs_none) :-
    red_dot(R), blue_dot(B),
    scene_pair_added_objs([R, B], [R, B], []).

test(added_objs_all) :-
    blue_dot(B),
    scene_pair_added_objs([], [B], Added),
    Added == [B].
...
test(removed_objs_one) :-
    red_dot(R), blue_dot(B),
    scene_pair_removed_objs([R, B], [R], Removed),
    Removed == [B].

test(removed_objs_none) :-
    red_dot(R),
    scene_pair_removed_objs([R], [R], []).

test(removed_objs_all) :-
    red_dot(R),
    scene_pair_removed_objs([R], [], [R]).

% --- scene_pair_n_added/3 and scene_pair_n_removed/3 ---

test(n_added_basic) :-
    red_dot(R), blue_dot(B),
    scene_pair_n_added([R], [R, B], 1).

test(n_removed_basic) :-
    red_dot(R), blue_dot(B),
    scene_pair_n_removed([R, B], [R], 1).

test(n_added_zero) :-
    red_dot(R),
    scene_pair_n_added([R], [R], 0).

% --- scene_pair_color_set_before/2 and scene_pair_color_set_after/2 ---

test(color_set_before_basic) :-
    red_dot(R), blue_dot(B),
    scene_pair_color_set_before([R, B], Colors),
    Colors == [b, r].

test(color_set_after_basic) :-
    green_dot_from_red(G), blue_dot(B),
    scene_pair_color_set_after([G, B], Colors),
    Colors == [b, g].

test(color_set_single) :-
    red_dot(R),
    scene_pair_color_set_before([R], [r]).
...
test(same_color_set_yes) :-
    red_dot(R), blue_dot(B),
    red_bar(RB),
    scene_pair_same_color_set([R, B], [RB, B]).

test(same_color_set_no) :-
    red_dot(R), blue_dot(B), green_dot(G),
    \+ scene_pair_same_color_set([R, B], [R, G]).

test(same_color_set_empty) :-
    scene_pair_same_color_set([], []).
...
test(is_recolor_yes) :-
    red_dot(R), blue_dot(B),
    green_dot_from_red(G),
    scene_pair_is_recolor([R, B], [G, B]).

test(is_recolor_no_two_changes) :-
    red_dot(R), blue_dot(B),
    green_dot_from_red(G), lshape(L),
    \+ scene_pair_is_recolor([R, B], [G, L]).

test(is_recolor_no_length_diff) :-
    red_dot(R), blue_dot(B),
    \+ scene_pair_is_recolor([R, B], [R]).
...
test(is_recolor_all_yes) :-
    ...
    scene_pair_is_recolor_all([R, RB], [G, GB]).

test(is_recolor_all_already_one_color) :-
    green_dot_from_red(G),
    scene_pair_is_recolor_all([G], [G]).

test(is_recolor_all_no) :-
    red_dot(R), blue_dot(B),
    green_dot_from_red(G),
    % After has r and g — not recolor_all
    \+ scene_pair_is_recolor_all([R, B], [G, R]).

% --- scene_pair_is_shift/3 ---

test(is_shift_yes) :-
    red_dot(R), blue_dot(B),
    red_dot_shifted(RS), blue_dot_shifted(BS),
    scene_pair_is_shift([R, B], [RS, BS], DR, DC),
    DR == 1, DC == 1.

test(is_shift_zero) :-
    red_dot(R),
    scene_pair_is_shift([R], [R], DR, DC),
    DR == 0, DC == 0.

test(is_shift_no) :-
    red_dot(R), red_bar(RB),
    \+ scene_pair_is_shift([R], [RB], _, _).
...
test(color_delta_swap) :-
    ...
    red_dot(R), blue_dot(B),
    scene_pair_color_delta([R, B], [B, R], Delta),
    msort(Delta, Sorted),
    Sorted == [b-r, r-b].
...
test(added_objs_form_match) :-
    ...
    red_dot(R),
    scene_pair_added_objs([R], [R], []).

test(is_shift_multi_obj_bar) :-
    red_bar(RB), blue_bar(BB),
    red_bar_shifted(RBS),
    % shift [RB, BB] down by 1: expect scene_pair_is_shift to detect DR=1, DC=0
    blue_bar_shifted(BBS),
    scene_pair_is_shift([RB, BB], [RBS, BBS], DR, DC),
    DR == 1, DC == 0.

:- end_tests(scene_pair).
