...
:- module(variance_statistics, [
    variance_statistics_sum/2,
    variance_statistics_mean_floor/2,
    variance_statistics_mean_round/2,
    variance_statistics_deviation/2,
    variance_statistics_abs_deviation/2,
    variance_statistics_row_sums/2,
    variance_statistics_col_sums/2,
    variance_statistics_row_means/2,
    variance_statistics_col_means/2,
    variance_statistics_global_mean/2,
    variance_statistics_above_mean/2,
    variance_statistics_below_mean/2,
    variance_statistics_row_above_mean/2,
    variance_statistics_col_above_mean/2
]).
...
% variance_statistics_sum(+List, -Sum): Sum is the integer total of all values in List.
% Delegates to sum_list/2 from library(lists).
variance_statistics_sum(List, Sum) :-
% sum_list is the standard SWI-Prolog predicate for totaling a list of numbers.
    sum_list(List, Sum).

% variance_statistics_mean_floor(+List, -Mean): Mean is the floor of the arithmetic mean of List.
% Computed as sum(List) // length(List). Fails when List is empty.
variance_statistics_mean_floor(List, Mean) :-
% Sum the list values.
    sum_list(List, Sum),
% Count the elements.
    length(List, N),
...
% variance_statistics_mean_round(+List, -Mean): Mean is the rounded arithmetic mean of List.
% Uses floating-point division then SWI-Prolog round/1 (banker's rounding).
% Fails when List is empty.
variance_statistics_mean_round(List, Mean) :-
% Sum the list.
    sum_list(List, Sum),
% Count the elements.
    length(List, N),
...
% variance_statistics_deviation(+List, -Devs): Devs[I] = List[I] - floor_mean(List) for each I.
% Signed integer deviation; negative when below mean, positive when above.
variance_statistics_deviation(List, Devs) :-
% Compute the floor mean of the list.
    variance_statistics_mean_floor(List, Mean),
% For each element, subtract the mean.
    findall(D, (member(V, List), D is V - Mean), Devs).

% variance_statistics_abs_deviation(+List, -AbsDevs): AbsDevs[I] = |List[I] - floor_mean(List)|.
% Non-negative integer absolute deviation from the floor mean.
variance_statistics_abs_deviation(List, AbsDevs) :-
...
% variance_statistics_row_sums(+Grid, -Sums): Sums[R] is the integer sum of all values in row R.
% Sums has the same length as Grid.
variance_statistics_row_sums(Grid, Sums) :-
% Apply sum_list to every row and collect results in order.
    findall(S, (member(Row, Grid), sum_list(Row, S)), Sums).

% variance_statistics_col_sums(+Grid, -Sums): Sums[C] is the integer sum of all values in column C.
% Sums has the same length as the number of columns. Returns [] on empty grid.
variance_statistics_col_sums(Grid, Sums) :-
...
% variance_statistics_row_means(+Grid, -Means): Means[R] is the floor mean of row R.
% Each mean is computed independently by variance_statistics_mean_floor.
variance_statistics_row_means(Grid, Means) :-
% Compute floor mean of every row and collect in order.
    findall(M, (member(Row, Grid), variance_statistics_mean_floor(Row, M)), Means).

% variance_statistics_col_means(+Grid, -Means): Means[C] is the floor mean of column C.
% Each column mean is computed from all values in that column.
variance_statistics_col_means(Grid, Means) :-
...
% For each column, collect values and compute floor mean.
    findall(M, (between(0, W1, C),
        findall(V, (member(Row, Grid), nth0(C, Row, V)), ColVals),
        variance_statistics_mean_floor(ColVals, M)), Means).

% variance_statistics_global_mean(+Grid, -Mean): Mean is the floor mean of all cell values in Grid.
% All cells across all rows contribute to a single pooled mean.
variance_statistics_global_mean(Grid, Mean) :-
...
% Compute the floor mean of the pooled list.
    variance_statistics_mean_floor(Vals, Mean).

% variance_statistics_above_mean(+Grid, -Cells): Cells is the sorted R-C list of positions
% whose value is strictly greater than the global floor mean.
% Cells are enumerated in row-major order (R outer, C inner).
variance_statistics_above_mean(Grid, Cells) :-
...
% variance_statistics_below_mean(+Grid, -Cells): Cells is the sorted R-C list of positions
% whose value is strictly less than the global floor mean.
% Cells are enumerated in row-major order.
variance_statistics_below_mean(Grid, Cells) :-
% Compute global floor mean.
    variance_statistics_global_mean(Grid, Mean),
% Determine grid dimensions.
    length(Grid, H), H1 is H - 1,
...
% variance_statistics_row_above_mean(+Grid, -Cells): Cells is the sorted R-C list of positions
% whose value is strictly greater than the floor mean of their own row.
% Each row's mean is computed independently.
variance_statistics_row_above_mean(Grid, Cells) :-
% Determine grid dimensions.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
% For each row, compute the row mean, then collect cells above it.
    findall(R-C, (between(0, H1, R),
        nth0(R, Grid, Row),
        variance_statistics_mean_floor(Row, RowMean),
        between(0, W1, C),
        nth0(C, Row, V),
        V > RowMean), Cells).

% variance_statistics_col_above_mean(+Grid, -Cells): Cells is the sorted R-C list of positions
...
variance_statistics_col_above_mean(Grid, Cells) :-
% Determine grid dimensions.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
% Enumerate in row-major order; for each (R,C), compute column mean and compare.
    findall(R-C, (between(0, H1, R),
        ...
        variance_statistics_mean_floor(ColVals, ColMean),
        nth0(R, Grid, Row),
        nth0(C, Row, V),
        V > ColMean), Cells).
