% State the fact: name(actors).
name(actors).
% State the fact: version('1.0.0').
version('1.0.0').
% State the fact: title('PrologAI Actor Framework — cyclic_actor, receptor, pub-sub (PRs 6-8)').
title('PrologAI Actor Framework — cyclic_actor, receptor, pub-sub (PRs 6-8)').
