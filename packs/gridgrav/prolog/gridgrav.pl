...
% gridgrav_cols_to_grid_/3: transpose list of column-lists to list of row-lists.
% Cols is a list of W column-lists each of height H; Grid is H rows of W cells.
gridgrav_cols_to_grid_(Cols, H, Grid) :-
...
% gridgrav_settle_segment_/4: pack non-bg values to one end of a list segment.
...
gridgrav_settle_segment_(Seg, Bg, Dir, Settled) :-
...
% gridgrav_split_at_first_/4: split List at the first occurrence of BlockColor.
% Before is the prefix before BlockColor; After is the tail after it.
% Fails if BlockColor does not appear in List.
gridgrav_split_at_first_([BlockColor|After], BlockColor, [], After) :- !.
% Recurse while current head differs from BlockColor.
gridgrav_split_at_first_([H|T], BlockColor, [H|Before], After) :-
    gridgrav_split_at_first_(T, BlockColor, Before, After).

% gridgrav_segment_apply_/5: split at BlockColor walls; settle each segment; reassemble.
% Empty list base case.
gridgrav_segment_apply_([], _, _, _, []) :- !.
% Find next BlockColor, settle before it, recurse after it.
gridgrav_segment_apply_(Vals, Bg, BlockColor, Dir, Result) :-
    (gridgrav_split_at_first_(Vals, BlockColor, Before, After) ->
        gridgrav_settle_segment_(Before, Bg, Dir, SettledBefore),
        gridgrav_segment_apply_(After, Bg, BlockColor, Dir, SettledAfter),
        append(SettledBefore, [BlockColor|SettledAfter], Result)
    ;
% No BlockColor found: settle the whole remaining segment.
        gridgrav_settle_segment_(Vals, Bg, Dir, Result)
    ).

% gridgrav_set_col_/4: replace column Col in Grid with values from ColVals.
% Base case: empty grid and empty values list.
gridgrav_set_col_([], _, [], []) :- !.
% Recursive case: replace one cell per row using gridgrav_set_nth_.
gridgrav_set_col_([Row|Rows], Col, [V|Vs], [NewRow|Rest]) :-
    gridgrav_set_nth_(Row, Col, V, NewRow),
    gridgrav_set_col_(Rows, Col, Vs, Rest).

% gridgrav_set_nth_/4: replace element at 0-indexed position N in List with V.
% Base case: N=0, replace head.
gridgrav_set_nth_([_|T], 0, V, [V|T]) :- !.
% Recursive case: keep head, decrement index, recurse.
gridgrav_set_nth_([H|T], N, V, [H|Rest]) :-
    N1 is N - 1,
    gridgrav_set_nth_(T, N1, V, Rest).

% gridgrav_set_row_/4: replace the row at 0-indexed RowIdx in Grid with NewRow.
gridgrav_set_row_(Grid, RowIdx, NewRow, Result) :-
...
% gridgrav_count_diff_/3: count cell positions where two grids differ (base case).
gridgrav_count_diff_([], [], 0) :- !.
% Recursive case: sum per-row differences.
gridgrav_count_diff_([R1|T1], [R2|T2], Score) :-
    gridgrav_count_diff_(T1, T2, Score1),
    gridgrav_row_diff_count_(R1, R2, D),
    Score is Score1 + D.

% gridgrav_row_diff_count_/3: count positions where two rows differ (base case).
gridgrav_row_diff_count_([], [], 0) :- !.
% Equal cell: cut to avoid backtracking, do not increment.
gridgrav_row_diff_count_([V|T1], [V|T2], N) :- !,
    gridgrav_row_diff_count_(T1, T2, N).
% Different cell: increment counter.
gridgrav_row_diff_count_([_|T1], [_|T2], N) :-
    gridgrav_row_diff_count_(T1, T2, N1), N is N1 + 1.
...
% For each column index, extract all cell values and settle downward.
     findall(NewCol,
         (between(0, W1, C),
          ...
          gridgrav_settle_segment_(ColVals, Bg, down, NewCol)),
         Cols),
% Transpose settled column lists back into row-major grid.
     gridgrav_cols_to_grid_(Cols, H, Result)).
...
% For each column, extract cell values and settle upward.
     findall(NewCol,
         (between(0, W1, C),
          findall(V, (between(0, H1, R), nth0(R, Grid, Row), nth0(C, Row, V)), ColVals),
          gridgrav_settle_segment_(ColVals, Bg, up, NewCol)),
         Cols),
...
% Get row count H.
     length(Grid, H), H1 is H - 1,
% For each row, settle leftward using gridgrav_settle_segment_.
     findall(NewRow,
         (between(0, H1, R),
          nth0(R, Grid, Row),
          gridgrav_settle_segment_(Row, Bg, left, NewRow)),
         Result)).
...
% Get row count H.
     length(Grid, H), H1 is H - 1,
% For each row, settle rightward using gridgrav_settle_segment_.
     findall(NewRow,
         (between(0, H1, R),
          nth0(R, Grid, Row),
          gridgrav_settle_segment_(Row, Bg, right, NewRow)),
         Result)).

% gridgrav_fall(+Grid, +Bg, +Dir, -Result)
% Dispatch gravity in direction Dir: down, up, left, or right.
gridgrav_fall(Grid, Bg, down, Result) :- !, gridgrav_fall_down(Grid, Bg, Result).
% Fall up direction.
gridgrav_fall(Grid, Bg, up, Result) :- !, gridgrav_fall_up(Grid, Bg, Result).
% Fall left direction.
gridgrav_fall(Grid, Bg, left, Result) :- !, gridgrav_fall_left(Grid, Bg, Result).
% Fall right direction (no cut needed as last clause).
gridgrav_fall(Grid, Bg, right, Result) :- gridgrav_fall_right(Grid, Bg, Result).
...
% Replace the column in the grid.
    gridgrav_set_col_(Grid, Col, ColVals, Result).
...
gridgrav_blocked_fall(Grid, Bg, BlockColor, down, Result) :- !,
    (Grid = [] -> Result = [] ;
...
% For each column, apply segmented gravity stopping at BlockColor.
     findall(NewCol,
         (between(0, W1, C),
          findall(V, (between(0, H1, R), nth0(R, Grid, Row), nth0(C, Row, V)), ColVals),
          gridgrav_segment_apply_(ColVals, Bg, BlockColor, down, NewCol)),
         Cols),
     gridgrav_cols_to_grid_(Cols, H, Result)).
% Dir=up: same as down but using up direction for each segment.
gridgrav_blocked_fall(Grid, Bg, BlockColor, up, Result) :- !,
    (Grid = [] -> Result = [] ;
% Get column and row dimensions.
     Grid = [FR|_], length(FR, W), W1 is W - 1,
     length(Grid, H), H1 is H - 1,
% For each column, apply segmented gravity stopping at BlockColor.
     findall(NewCol,
         (between(0, W1, C),
          findall(V, (between(0, H1, R), nth0(R, Grid, Row), nth0(C, Row, V)), ColVals),
          gridgrav_segment_apply_(ColVals, Bg, BlockColor, up, NewCol)),
         Cols),
     gridgrav_cols_to_grid_(Cols, H, Result)).
% Dir=left: process row by row.
gridgrav_blocked_fall(Grid, Bg, BlockColor, left, Result) :- !,
    (Grid = [] -> Result = [] ;
% Get row count.
     length(Grid, H), H1 is H - 1,
% For each row, apply segmented gravity stopping at BlockColor.
     findall(NewRow,
         (between(0, H1, R),
          nth0(R, Grid, Row),
          gridgrav_segment_apply_(Row, Bg, BlockColor, left, NewRow)),
         Result)).
% Dir=right: process row by row (last clause, no cut needed).
gridgrav_blocked_fall(Grid, Bg, BlockColor, right, Result) :-
    (Grid = [] -> Result = [] ;
% Get row count.
     length(Grid, H), H1 is H - 1,
% For each row, apply segmented gravity stopping at BlockColor.
     findall(NewRow,
         (between(0, H1, R),
          nth0(R, Grid, Row),
          gridgrav_segment_apply_(Row, Bg, BlockColor, right, NewRow)),
         Result)).

% gridgrav_is_settled(+Grid, +Bg)
% Succeeds if gridgrav_fall_down(Grid, Bg, Grid): the grid is unchanged by downward gravity.
gridgrav_is_settled(Grid, Bg) :-
% Apply fall_down and unify result with original to test for no change.
    gridgrav_fall_down(Grid, Bg, Grid).

% gridgrav_gravity_score(+Grid, +Bg, -Score)
% Score is the number of cell positions that differ between Grid and gridgrav_fall_down result.
...
% Count positions where Grid and Result differ.
    gridgrav_count_diff_(Grid, Result, Score).
