...
test('AC-GOM-023: different vectors - constant move fails') :-
    ...
    \+ gom_constant_move(Pairs, _).

test('AC-GOM-024: single pair always constant') :-
    or00(O1), og22(O2),
    gom_constant_move([match(O1, O2)], V),
    V = mv(2, 2).
...
test('AC-GOM-025: no color changes - empty map') :-
    or00(O1), or01(O2),
    gom_infer_color_map([match(O1, O2)], Map),
    Map = [].
...
test('AC-GOM-028: no new colors - appeared is empty') :-
    or00(O1), ob11(O2),
    or01(O3), ob12(O4),
    gom_appeared([O1, O2], [O3, O4], Appeared),
    Appeared = [].

test('AC-GOM-029: one new color in Objs2') :-
    or00(O1), oy00(O2),
    gom_appeared([O1], [O1, O2], Appeared),
    Appeared = [O2].
...
test('AC-GOM-030: no lost colors - disappeared is empty') :-
    or00(O1), ob11(O2),
    or01(O3), ob12(O4),
    gom_disappeared([O1, O2], [O3, O4], Disappeared),
    Disappeared = [].

test('AC-GOM-031: one color lost from Objs1') :-
    or00(O1), oy00(O2),
    gom_disappeared([O1, O2], [O1], Disappeared),
    Disappeared = [O2].
...
test('AC-GOM-038: match nearest, color_diff finds all pairs changed') :-
    ...
    gom_match_nearest([R1, B1], [N1, N2], Pairs),
    gom_color_diff(Pairs, Same, Diff),
    Same = [],
    length(Diff, 2).
...
test('AC-GOM-039: match nearest then constant move') :-
    ...
    gom_match_nearest([R1, B1, G1], [R2, B2, G2], Pairs),
    gom_constant_move(Pairs, V),
    V = mv(0, 1).
...
test('AC-GOM-041: appeared and disappeared correct after color swap') :-
    ...
    gom_appeared([R, B], [B, G, Y], App),
    length(App, 2),
    memberchk(G, App), memberchk(Y, App),
    gom_disappeared([R, B], [B, G, Y], Dis),
    Dis = [R].

test('AC-GOM-042: unmatched_a and unmatched_b after partial color match') :-
    or00(R), ob11(B), og22(G),
    or01(R2),
    gom_match_color([R, B, G], [R2], Pairs),
    gom_unmatched_a(Pairs, [R, B, G], UA),
    length(UA, 2),
    memberchk(B, UA), memberchk(G, UA),
    gom_unmatched_b(Pairs, [R2], UB),
    UB = [].

test('AC-GOM-043: same structure check via match and structure') :-
    or00(R1), ob11(B1), og22(G1),
    or01(R2), ob12(B2), og23(G2),
    gom_same_structure([R1, B1, G1], [R2, B2, G2]),
    gom_count_change([R1, B1, G1], [R2, B2, G2], N),
    N =:= 0.
