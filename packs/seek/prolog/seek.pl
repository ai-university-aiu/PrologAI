...
seek_cols_with(Grid, Val, ColIdxs) :-
    ...
    include(seek_col_has_val_(Grid, Val), CIdxs, ColIdxs).

% seek_col_has_val_(+Grid, +Val, +CI): column CI contains Val in at least one row.
seek_col_has_val_(Grid, Val, CI) :-
...
% seek_interior_cells(+Grid, -Cells)
...
% seek_all_subs(+Grid, +Sub, -Cells)
% Cells is the list of R0-C0 pairs where Sub fits in Grid.
seek_all_subs(Grid, Sub, Cells) :-
    % Collect all positions where Sub fits.
    findall(R0-C0, seek_find_sub(Grid, Sub, R0, C0), Cells).

% seek_count_sub(+Grid, +Sub, -N)
...
seek_best_fit(Grid, Sub, R0, C0) :-
    ...
    findall(N-R-C,
        (between(0, MaxR, R), between(0, MaxC, C),
         seek_match_count(Grid, Sub, R, C, N)),
        Scored),
...
seek_find_d4(Grid1, Grid2, Name) :-
    ...
    once(seek_d4_pair_(Name, Grid1, Grid2)).
...
seek_d4_pair_(rotate90, G, G2) :- seek_transpose_(G, T), maplist(reverse, T, G2).
...
% seek_transpose_(+Grid, -Transposed): reflect Grid across the main diagonal.
seek_transpose_([], []) :- !.
seek_transpose_(Grid, Transposed) :-
...
seek_upscale(Grid, Factor, Scaled) :-
    ...
    maplist(seek_upscale_row_(Factor), Grid, TiledRows),
    % Repeat each tiled row Factor times vertically.
    seek_repeat_rows_(TiledRows, Factor, Scaled).

% seek_upscale_row_(+Factor, +Row, -TiledRow): repeat each cell Factor times.
seek_upscale_row_(Factor, Row, TiledRow) :-
...
% seek_repeat_rows_(+Rows, +Factor, -Repeated): repeat each row in Rows Factor times.
seek_repeat_rows_([], _Factor, []).
seek_repeat_rows_([Row|Rest], Factor, Result) :-
    % Build Factor copies of this row.
    findall(Row, between(1, Factor, _), Copies),
    % Append rest result to get full repeated tail.
    seek_repeat_rows_(Rest, Factor, RestResult),
    % Prepend the copies for this row.
    append(Copies, RestResult, Result).

% seek_find_scale(+Grid1, +Grid2, -Factor)
% Factor is the positive integer scale Factor such that seek_upscale(Grid1, Factor, Grid2) holds.
seek_find_scale(Grid1, Grid2, Factor) :-
    ...
    seek_upscale(Grid1, Factor, Grid2).
