...
:- module(spread, [
    ...
    spread_reachable/5,
    spread_path_dist/7,
    spread_path_exists/6,
    spread_ring/6,
    spread_zone/6,
    spread_dist_to_set/6,
    spread_farthest/5,
    spread_expand_once/5,
    spread_spread_color/5,
    spread_max_dist/5,
    spread_fill_dist/5,
    spread_voronoi/4
]).
...
% spread_dims_: get the row and column count of a grid.
spread_dims_(Grid, NR, NC) :-
% Count rows.
    length(Grid, NR),
% Get column count from the first row when grid is non-empty.
    (NR > 0 -> Grid = [FR|_], length(FR, NC) ; NC = 0).

% spread_cell_val_: retrieve the grid value at a cell coordinate R-C.
spread_cell_val_(Grid, R-C, V) :-
    nth0(R, Grid, Row),
    nth0(C, Row, V).

% spread_4candidates_: in-bounds 4-adjacent cells of (R,C) in a NR x NC grid.
spread_4candidates_(R, C, NR, NC, Nbrs) :-
...
% spread_bg4unvis_: in-bounds Bg 4-neighbors of (R,C) not already in Vis.
spread_bg4unvis_(R, C, NR, NC, Grid, Bg, Vis, Nbrs) :-
% Get all candidate 4-neighbors.
    spread_4candidates_(R, C, NR, NC, Cands),
...
% spread_make_bfs1_: create a single-source BFS queue entry.
spread_make_bfs1_(D, Cell, bfs1(Cell, D)).

% spread_bfs_single_: BFS loop for single (or multi) source distance computation.
...
spread_bfs_single_([], _, _, _, _, _, Acc, Acc) :- !.
spread_bfs_single_([bfs1(Cell, D)|Rest], Grid, Bg, NR, NC, Vis0, Acc0, Result) :-
% Unpack the current cell.
    Cell = CR-CC,
% Find unvisited Bg 4-neighbors.
    spread_bg4unvis_(CR, CC, NR, NC, Grid, Bg, Vis0, Nbrs),
% Distance to neighbors is D+1.
    D1 is D + 1,
% Build queue entries for neighbors.
    maplist(spread_make_bfs1_(D1), Nbrs, Tagged),
...
% Record the current cell with its distance.
    spread_bfs_single_(Queue1, Grid, Bg, NR, NC, Vis1, [Cell-D|Acc0], Result).

% spread_dist_map(+Grid, +R, +C, +Bg, -DistMap): BFS from (R,C) through cells with
...
spread_dist_map(Grid, R, C, Bg, DistMap) :-
    spread_cell_val_(Grid, R-C, Val),
    (Val =:= Bg ->
% Source is background: start BFS.
        spread_dims_(Grid, NR, NC),
        spread_bfs_single_([bfs1(R-C, 0)], Grid, Bg, NR, NC, [R-C], [], DistMap)
    ;
% Source is not background: no reachable cells.
        DistMap = []
    ).

% spread_dist_multi(+Grid, +Seeds, +Bg, -DistMap): multi-source BFS from all Seed
...
spread_dist_multi(Grid, Seeds, Bg, DistMap) :-
    spread_dims_(Grid, NR, NC),
% Filter seeds to those that are actually Bg.
    include([S]>>(spread_cell_val_(Grid, S, V), V =:= Bg), Seeds, BgSeeds),
% Build initial queue entries (all seeds at distance 0).
    maplist(spread_make_bfs1_(0), BgSeeds, InitQueue),
% Run BFS starting with all seeds as visited.
    spread_bfs_single_(InitQueue, Grid, Bg, NR, NC, BgSeeds, [], DistMap).

% spread_reachable(+Grid, +R, +C, +Bg, -Cells): all Bg cells reachable from (R,C)
% by 4-connected steps through Bg cells. Returns the sorted list of R-C cells.
spread_reachable(Grid, R, C, Bg, Cells) :-
    spread_dist_map(Grid, R, C, Bg, DistMap),
...
% spread_path_dist(+Grid, +R1, +C1, +R2, +C2, +Bg, -D): BFS shortest path distance
% from (R1,C1) to (R2,C2) through Bg cells. Fails if (R2,C2) is unreachable.
spread_path_dist(Grid, R1, C1, R2, C2, Bg, D) :-
    spread_dist_map(Grid, R1, C1, Bg, DistMap),
    member((R2-C2)-D, DistMap), !.

% spread_path_exists(+Grid, +R1, +C1, +R2, +C2, +Bg): succeed if (R2,C2) is
% reachable from (R1,C1) via 4-connected steps through Bg cells.
spread_path_exists(Grid, R1, C1, R2, C2, Bg) :-
    spread_path_dist(Grid, R1, C1, R2, C2, Bg, _).

% spread_ring(+Grid, +R, +C, +Bg, +D, -Cells): all Bg cells at exactly BFS distance
% D from (R,C). Cells is a sorted list.
spread_ring(Grid, R, C, Bg, D, Cells) :-
    spread_dist_map(Grid, R, C, Bg, DistMap),
...
% spread_zone(+Grid, +R, +C, +Bg, +MaxD, -Cells): all Bg cells at BFS distance at
% most MaxD from (R,C). Cells is a sorted list.
spread_zone(Grid, R, C, Bg, MaxD, Cells) :-
    spread_dist_map(Grid, R, C, Bg, DistMap),
...
% spread_dist_to_set(+Grid, +R, +C, +Targets, +Bg, -D): minimum BFS distance from
% (R,C) to any cell in Targets through Bg cells. Fails if no target is reachable.
spread_dist_to_set(Grid, R, C, Targets, Bg, D) :-
    spread_dist_map(Grid, R, C, Bg, DistMap),
...
% spread_farthest(+Grid, +R, +C, +Bg, -Cell): the Bg cell farthest from (R,C) by
% BFS distance. If multiple cells share the maximum distance the first in BFS
% order is returned.
spread_farthest(Grid, R, C, Bg, Cell) :-
    spread_dist_map(Grid, R, C, Bg, DistMap),
    DistMap \= [],
...
% spread_expand_once(+Grid, +Frontier, +Visited, +Bg, -NextFrontier): one BFS
...
spread_expand_once(Grid, Frontier, Visited, Bg, NextFrontier) :-
    spread_dims_(Grid, NR, NC),
...
% spread_color_cells_: find all cells with a given value in a grid.
spread_color_cells_(Grid, Color, Cells) :-
    spread_dims_(Grid, NR, NC),
...
% spread_set_grid_cell_: replace the value at (R,C) in Grid with Val.
spread_set_grid_cell_(Grid, R, C, Val, NewGrid) :-
...
% spread_apply_vals_: set all cells in a list to Val in Grid.
spread_apply_vals_(Grid, [], _, Grid) :- !.
spread_apply_vals_(Grid0, [CR-CC|Rest], Val, Out) :-
    spread_set_grid_cell_(Grid0, CR, CC, Val, Grid1),
    spread_apply_vals_(Grid1, Rest, Val, Out).

% spread_spread_color(+Grid, +Color, +Bg, +MaxD, -Out): spread Color cells outward
...
spread_spread_color(Grid, Color, Bg, MaxD, Out) :-
% Find all Color cells as the BFS seeds.
    spread_color_cells_(Grid, Color, Seeds),
    (Seeds = [] ->
...
% Run BFS directly from Color seeds without Bg filtering (seeds are non-Bg).
        spread_dims_(Grid, NR, NC),
        maplist(spread_make_bfs1_(0), Seeds, InitQueue),
% Vis initialised to Seeds so they are never revisited as Bg neighbors.
        spread_bfs_single_(InitQueue, Grid, Bg, NR, NC, Seeds, [], RawMap),
...
% Paint those cells with Color.
        spread_apply_vals_(Grid, ToColor, Color, Out)
    ).

% spread_max_dist(+Grid, +R, +C, +Bg, -MaxD): the maximum BFS distance from (R,C)
% to any reachable Bg cell. Returns 0 when (R,C) is the only reachable cell.
spread_max_dist(Grid, R, C, Bg, MaxD) :-
    spread_dist_map(Grid, R, C, Bg, DistMap),
    DistMap \= [],
    maplist([Pair, D]>>(Pair = _-D), DistMap, Dists),
    max_list(Dists, MaxD).

% spread_apply_dist_map_: set each cell in DistMap to its BFS distance value.
spread_apply_dist_map_(Grid, [], Grid) :- !.
spread_apply_dist_map_(Grid0, [Cell-D|Rest], Out) :-
    Cell = CR-CC,
    spread_set_grid_cell_(Grid0, CR, CC, D, Grid1),
    spread_apply_dist_map_(Grid1, Rest, Out).

% spread_fill_dist(+Grid, +R, +C, +Bg, -Out): overlay BFS distances onto the grid.
...
% unchanged.
spread_fill_dist(Grid, R, C, Bg, Out) :-
    spread_dist_map(Grid, R, C, Bg, DistMap),
    spread_apply_dist_map_(Grid, DistMap, Out).

% spread_make_bfs2_: create a Voronoi BFS queue entry with seed index.
spread_make_bfs2_(D, Seed, Cell, bfs2(Cell, D, Seed)).

% spread_bfs_voronoi_: BFS loop tracking which seed first discovered each cell.
% Queue entries: bfs2(Cell, D, SeedIdx). Result: list of Cell-SeedIdx pairs.
spread_bfs_voronoi_([], _, _, _, _, _, Acc, Acc) :- !.
spread_bfs_voronoi_([bfs2(Cell, D, Seed)|Rest], Grid, Bg, NR, NC, Vis0, Acc0, Result) :-
    Cell = CR-CC,
    spread_bg4unvis_(CR, CC, NR, NC, Grid, Bg, Vis0, Nbrs),
    D1 is D + 1,
    maplist(spread_make_bfs2_(D1, Seed), Nbrs, Tagged),
    append(Rest, Tagged, Queue1),
    append(Vis0, Nbrs, Vis1),
    spread_bfs_voronoi_(Queue1, Grid, Bg, NR, NC, Vis1, [Cell-Seed|Acc0], Result).

% spread_voronoi(+Grid, +Seeds, +Bg, -Assignment): Voronoi assignment of Bg cells to
...
% included.
spread_voronoi(Grid, Seeds, Bg, Assignment) :-
    spread_dims_(Grid, NR, NC),
...
% Run BFS with seed tracking.
    spread_bfs_voronoi_(InitQueue, Grid, Bg, NR, NC, BgSeeds, [], Assignment).
