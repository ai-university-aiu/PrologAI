:- use_module('../prolog/object_copy').
:- use_module(library(plunit)).

:- begin_tests(object_copy).
...
test(place_at_origin) :-
    dot(D),
    object_copy_place_at(D, 0, 0, D2),
    D2 == obj(r, [r(0,0)]).

test(place_at_move) :-
    dot(D),
    object_copy_place_at(D, 3, 5, D2),
    D2 == obj(r, [r(3,5)]).

test(place_at_bar) :-
    bar_h(B),
    object_copy_place_at(B, 2, 4, B2),
...
test(recolor_all_basic) :-
    dot(D), bar_h(B),
    object_copy_recolor_all(x, [D, B], Objs2),
    Objs2 == [obj(x,[r(0,0)]), obj(x,[r(0,0),r(0,1),r(0,2)])].

test(recolor_all_empty) :-
    object_copy_recolor_all(x, [], Objs2),
    Objs2 == [].
...
test(tile_row_basic) :-
    dot(D),
    object_copy_tile_row(D, 3, 2, Objs),
    Objs == [obj(r,[r(0,0)]), obj(r,[r(0,2)]), obj(r,[r(0,4)])].

test(tile_row_one) :-
    bar_h(B),
    object_copy_tile_row(B, 1, 10, Objs),
    length(Objs, 1).
...
test(tile_col_basic) :-
    dot(D),
    object_copy_tile_col(D, 3, 2, Objs),
    Objs == [obj(r,[r(0,0)]), obj(r,[r(2,0)]), obj(r,[r(4,0)])].

test(tile_col_one) :-
    dot(D),
    object_copy_tile_col(D, 1, 5, Objs),
    Objs == [obj(r,[r(0,0)])].
...
test(tile_grid_2x2) :-
    dot(D),
    object_copy_tile_grid(D, 2, 2, 3, 4, Objs),
    length(Objs, 4),
...
test(tile_grid_1x3) :-
    dot(D),
    object_copy_tile_grid(D, 1, 3, 2, 2, Objs),
    length(Objs, 3).
...
test(at_positions_basic) :-
    dot(D),
    object_copy_at_positions(D, [r(1,0), r(3,5)], Objs),
    Objs == [obj(r,[r(1,0)]), obj(r,[r(3,5)])].

test(at_positions_empty) :-
    dot(D),
    object_copy_at_positions(D, [], Objs),
    Objs == [].
...
test(align_top_basic) :-
    ...
    object_copy_align_top([A,B], Aligned),
...
test(align_left_basic) :-
    ...
    object_copy_align_left([A,B], Aligned),
...
test(spread_h_basic) :-
    dot(A), dot(B), dot(C),
    object_copy_spread_h([A, B, C], 0, 3, Objs),
    Objs == [obj(r,[r(0,0)]), obj(r,[r(0,3)]), obj(r,[r(0,6)])].

test(spread_h_offset) :-
    dot(A), dot(B),
    object_copy_spread_h([A, B], 2, 4, Objs),
...
test(spread_v_basic) :-
    dot(A), dot(B), dot(C),
    object_copy_spread_v([A, B, C], 0, 3, Objs),
    Objs == [obj(r,[r(0,0)]), obj(r,[r(3,0)]), obj(r,[r(6,0)])].

test(spread_v_offset) :-
    dot(A), dot(B),
    object_copy_spread_v([A, B], 1, 2, Objs),
...
test(place_at_from_nonorigin) :-
    dot_at22(D),  % at r(2,2)
    object_copy_place_at(D, 0, 1, D2),
    D2 == obj(r, [r(0,1)]).

test(tile_row_bar) :-
    bar_h(B),  % bbox W=3, so tiles at col 0, 4, 8 with step 4
    object_copy_tile_row(B, 2, 4, Objs),
    length(Objs, 2),
...
test(tile_col_bar) :-
    ...
    object_copy_tile_col(V, 2, 4, Objs),
    length(Objs, 2),
...
test(tile_grid_3x2) :-
    dot(D),
    object_copy_tile_grid(D, 3, 2, 2, 3, Objs),
    length(Objs, 6).
...
test(align_top_three) :-
    ...
    B = obj(b,[r(0,1)]),
    C = obj(g,[r(4,2)]),
    object_copy_align_top([A,B,C], Aligned),
