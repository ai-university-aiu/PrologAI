...
test(fill_region_all) :-
    g0(G), sq3(R), fill_fill_region(G, R, 5, Result),
    Result = [[5,5,5],[5,5,5],[5,5,5]].

test(fill_region_partial) :-
    g0(G), fill_fill_region(G, [r(0,0), r(1,1)], 3, Result),
    Result = [[3,0,0],[0,3,0],[0,0,0]].

test(fill_region_single) :-
    gc(G), fill_fill_region(G, [r(1,1)], 9, Result),
    Result = [[0,0,0],[0,9,0],[0,0,0]].

test(fill_region_empty_noop) :-
    g0(G), fill_fill_region(G, [], 5, Result),
...
test(fill_bbox_single_cell) :-
    g0(G), fill_fill_bbox(G, bbox(1,1,1,1), 7, Result),
    Result = [[0,0,0],[0,7,0],[0,0,0]].

test(fill_bbox_full_grid) :-
    g0(G), fill_fill_bbox(G, bbox(0,0,2,2), 9, Result),
    Result = [[9,9,9],[9,9,9],[9,9,9]].

test(fill_bbox_row_range) :-
    g0(G), fill_fill_bbox(G, bbox(0,1,1,2), 4, Result),
    Result = [[0,4,4],[0,4,4],[0,0,0]].

test(fill_bbox_corner) :-
    g0(G), fill_fill_bbox(G, bbox(2,2,2,2), 1, Result),
...
test(fill_row_0) :-
    g0(G), fill_fill_row(G, 0, 5, Result),
    Result = [[5,5,5],[0,0,0],[0,0,0]].

test(fill_row_last) :-
    g0(G), fill_fill_row(G, 2, 3, Result),
    Result = [[0,0,0],[0,0,0],[3,3,3]].

test(fill_row_middle) :-
    g0(G), fill_fill_row(G, 1, 9, Result),
...
test(fill_col_0) :-
    g0(G), fill_fill_col(G, 0, 5, Result),
    Result = [[5,0,0],[5,0,0],[5,0,0]].

test(fill_col_last) :-
    g0(G), fill_fill_col(G, 2, 7, Result),
    Result = [[0,0,7],[0,0,7],[0,0,7]].

test(fill_col_middle) :-
    g0(G), fill_fill_col(G, 1, 3, Result),
...
test(fill_cells_two) :-
    g0(G), fill_fill_cells(G, [r(0,0), r(2,2)], 1, Result),
    Result = [[1,0,0],[0,0,0],[0,0,1]].

test(fill_cells_empty) :-
    g0(G), fill_fill_cells(G, [], 5, Result),
    Result = [[0,0,0],[0,0,0],[0,0,0]].

test(fill_cells_one) :-
    g0(G), fill_fill_cells(G, [r(1,2)], 8, Result),
...
test(solid_rect_1x1) :-
    fill_solid_rect(1, 1, 5, G), G = [[5]].

test(solid_rect_2x3) :-
    fill_solid_rect(2, 3, 7, G), G = [[7,7,7],[7,7,7]].

test(solid_rect_zero_color) :-
    fill_solid_rect(2, 2, 0, G), G = [[0,0],[0,0]].
...
test(checker_1x1) :-
    fill_checkerboard(1, 1, 0, 1, G), G = [[0]].

test(checker_2x2) :-
    fill_checkerboard(2, 2, 0, 1, G),
    G = [[0,1],[1,0]].

test(checker_3x3) :-
    fill_checkerboard(3, 3, 0, 1, G),
    G = [[0,1,0],[1,0,1],[0,1,0]].

test(checker_colors) :-
    fill_checkerboard(2, 2, 3, 7, G),
    G = [[3,7],[7,3]].
...
test(hline_full_row) :-
    g0(G), fill_draw_hline(G, 1, 0, 2, 5, Result),
    Result = [[0,0,0],[5,5,5],[0,0,0]].

test(hline_partial) :-
    g0(G), fill_draw_hline(G, 0, 1, 2, 3, Result),
    Result = [[0,3,3],[0,0,0],[0,0,0]].

test(hline_single_cell) :-
    g0(G), fill_draw_hline(G, 2, 1, 1, 9, Result),
...
test(vline_full_col) :-
    g0(G), fill_draw_vline(G, 1, 0, 2, 5, Result),
    Result = [[0,5,0],[0,5,0],[0,5,0]].

test(vline_partial) :-
    g0(G), fill_draw_vline(G, 2, 0, 1, 3, Result),
    Result = [[0,0,3],[0,0,3],[0,0,0]].

test(vline_single_cell) :-
    g0(G), fill_draw_vline(G, 0, 1, 1, 7, Result),
...
test(diag_3x3) :-
    g0(G), fill_fill_main_diag(G, 1, Result),
    Result = [[1,0,0],[0,1,0],[0,0,1]].

test(diag_1x1) :-
    fill_fill_main_diag([[0]], 5, Result),
    Result = [[5]].

test(diag_over_existing) :-
    gc(G), fill_fill_main_diag(G, 9, Result),
