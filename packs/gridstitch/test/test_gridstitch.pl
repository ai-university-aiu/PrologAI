...
test(split_h_at_1, []) :-
    g3x2(G),
    gridstitch_split_h(G, 1, Top, Bottom),
    Top = [[a,b]],
    Bottom = [[c,d],[e,f]].

test(split_h_at_2, []) :-
    g4x4(G),
    gridstitch_split_h(G, 2, Top, Bottom),
    Top = [[a,b,c,d],[e,f,g,h]],
...
test(split_v_at_1, []) :-
    g2x3(G),
    gridstitch_split_v(G, 1, Left, Right),
    Left = [[a],[d]],
    Right = [[b,c],[e,f]].

test(split_v_at_2, []) :-
    g4x4(G),
    gridstitch_split_v(G, 2, Left, Right),
...
test(halves_h_4x4, []) :-
    g4x4(G),
    gridstitch_halves_h(G, Top, Bottom),
    Top = [[a,b,c,d],[e,f,g,h]],
    Bottom = [[i,j,k,l],[m,n,o,p]].
...
test(halves_h_reconstruct, []) :-
    g4x4(G),
    gridstitch_halves_h(G, Top, Bottom),
    gridstitch_concat_v(Top, Bottom, G).

% --- gridstitch_halves_v ---
test(halves_v_4x4, []) :-
    g4x4(G),
    gridstitch_halves_v(G, Left, Right),
    Left = [[a,b],[e,f],[i,j],[m,n]],
    Right = [[c,d],[g,h],[k,l],[o,p]].
...
test(halves_v_reconstruct, []) :-
    g4x4(G),
    gridstitch_halves_v(G, Left, Right),
    gridstitch_concat_h(Left, Right, G).

% --- gridstitch_quadrants ---
test(quadrants_4x4, []) :-
    g4x4(G),
    gridstitch_quadrants(G, TL, TR, BL, BR),
    g4x4_tl(TL),
    g4x4_tr(TR),
    g4x4_bl(BL),
    g4x4_br(BR).
...
test(quadrants_reconstruct, []) :-
    g4x4(G),
    gridstitch_quadrants(G, TL, TR, BL, BR),
    gridstitch_concat_h(TL, TR, Top),
    gridstitch_concat_h(BL, BR, Bottom),
    gridstitch_concat_v(Top, Bottom, G).

% --- gridstitch_tile_grid ---
test(tile_grid_2x2_layout, []) :-
    g2x2_a(A), g2x2_b(B), g2x2_b(C), g2x2_a(D),
    gridstitch_tile_grid([A, B, C, D], 2, 2, R),
    R = [[a,a,b,b],[a,a,b,b],[b,b,a,a],[b,b,a,a]].

test(tile_grid_1x3_layout, []) :-
    g2x2_a(A), g2x2_b(B), g2x2_a(C),
    gridstitch_tile_grid([A, B, C], 1, 3, R),
    R = [[a,a,b,b,a,a],[a,a,b,b,a,a]].

% --- gridstitch_repeat_h ---
test(repeat_h_twice, []) :-
    g2x2_a(G),
    gridstitch_repeat_h(G, 2, R),
    R = [[a,a,a,a],[a,a,a,a]].

test(repeat_h_three, []) :-
    g1x3(G),
    gridstitch_repeat_h(G, 3, R),
    R = [[p,q,r,p,q,r,p,q,r]].

test(repeat_h_once, []) :-
    g2x2_a(G),
    gridstitch_repeat_h(G, 1, G).

% --- gridstitch_repeat_v ---
test(repeat_v_twice, []) :-
    g2x2_b(G),
    gridstitch_repeat_v(G, 2, R),
    R = [[b,b],[b,b],[b,b],[b,b]].

test(repeat_v_once, []) :-
    g2x2_b(G),
    gridstitch_repeat_v(G, 1, G).

% --- gridstitch_add_border ---
test(add_border_1_cell, []) :-
    g2x2_x(G),
    gridstitch_add_border(G, 1, r, R),
    R = [[r,r,r,r],[r,x,x,r],[r,x,x,r],[r,r,r,r]].

test(add_border_2_cells, []) :-
    G = [[a]],
    gridstitch_add_border(G, 2, b, R),
    length(R, 5),
    R = [[b,b,b,b,b],[b,b,b,b,b],[b,b,a,b,b],[b,b,b,b,b],[b,b,b,b,b]].

test(add_border_preserves_interior, []) :-
    g3x3_r(G),
    gridstitch_add_border(G, 1, x, R),
    length(R, 5),
    R = [[x,x,x,x,x],[x,r,r,r,x],[x,r,r,r,x],[x,r,r,r,x],[x,x,x,x,x]].

% --- gridstitch_strip_border ---
test(strip_border_1_cell, []) :-
    R = [[r,r,r,r],[r,x,x,r],[r,x,x,r],[r,r,r,r]],
    gridstitch_strip_border(R, 1, G),
    G = [[x,x],[x,x]].

test(strip_border_is_inverse_of_add, []) :-
    g3x3_r(G),
    gridstitch_add_border(G, 1, x, Bordered),
    gridstitch_strip_border(Bordered, 1, G).

test(strip_border_2_cells, []) :-
    G = [[b,b,b,b,b],[b,b,b,b,b],[b,b,a,b,b],[b,b,b,b,b],[b,b,b,b,b]],
    gridstitch_strip_border(G, 2, Inner),
    Inner = [[a]].

% --- Combined ---
test(hstack_then_split_v_recovers, []) :-
    g2x2_a(A), g2x2_b(B),
    gridstitch_concat_h(A, B, AB),
    gridstitch_split_v(AB, 2, LA, RB),
    LA = [[a,a],[a,a]],
    RB = [[b,b],[b,b]].

test(vstack_then_split_h_recovers, []) :-
    g2x3(A), g2x3(B),
    gridstitch_concat_v(A, B, AB),
    gridstitch_split_h(AB, 2, TA, BB2),
    TA = A,
    BB2 = B.
...
test(tile_and_quadrant_consistent, []) :-
    ...
    gridstitch_tile_grid([TL, TR, BL, BR], 2, 2, Full),
    gridstitch_quadrants(Full, TL2, TR2, BL2, BR2),
    TL2 = TL, TR2 = TR, BL2 = BL, BR2 = BR.

test(concat_h_then_split_v_exact, []) :-
    g3x2(A), g3x2(B),
    gridstitch_concat_h(A, B, Full),
    gridstitch_split_v(Full, 2, LA, RB),
    LA = A, RB = B.

test(add_border_then_strip_roundtrip, []) :-
    g2x3(G),
    gridstitch_add_border(G, 2, x, Bordered),
    gridstitch_strip_border(Bordered, 2, G).

test(repeat_v_height_is_n_times, []) :-
    g2x2_b(G),
    gridstitch_repeat_v(G, 4, R),
    length(R, 8).

test(split_h_then_vstack_recovers, []) :-
    g4x4(G),
    gridstitch_split_h(G, 1, Top, Bottom),
    gridstitch_vstack([Top, Bottom], G).

test(split_v_then_hstack_recovers, []) :-
    g4x4(G),
    gridstitch_split_v(G, 1, Left, Right),
    gridstitch_hstack([Left, Right], G).
