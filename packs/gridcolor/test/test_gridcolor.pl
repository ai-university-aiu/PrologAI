...
test(count_color_present, []) :-
    g3x3_mixed(G),
    gridcolor_count(G, r, N),
    N =:= 4.

test(count_color_absent, []) :-
    g3x3_mixed(G),
    gridcolor_count(G, g, N),
    N =:= 0.

test(count_background, []) :-
    g3x3_mixed(G),
    gridcolor_count(G, x, N),
    N =:= 4.

% --- gridcolor_histogram ---
test(histogram_correct_pairs, []) :-
    G = [[r,r],[r,x]],
    gridcolor_histogram(G, Pairs),
    memberchk(r-3, Pairs),
    memberchk(x-1, Pairs).
...
% --- gridcolor_most_frequent ---
test(most_frequent_r, []) :-
    G = [[r,r,r],[r,x,x],[r,r,r]],
    gridcolor_most_frequent(G, r).

test(most_frequent_uniform, []) :-
    g3x3_all_r(G),
    gridcolor_most_frequent(G, r).

% --- gridcolor_least_frequent ---
test(least_frequent_b, []) :-
    g3x3_mixed(G),
    gridcolor_least_frequent(G, b).

test(least_frequent_single, []) :-
    g1x1_r(G),
    gridcolor_least_frequent(G, r).

% --- gridcolor_unique_colors ---
test(unique_colors_3, []) :-
    g3x3_mixed(G),
    gridcolor_unique_colors(G, Colors),
    Colors = [b, r, x].

test(unique_colors_1, []) :-
    g3x3_all_r(G),
    gridcolor_unique_colors(G, [r]).

test(unique_colors_equal, []) :-
    g2x4_equal(G),
    gridcolor_unique_colors(G, [r, x]).

% --- gridcolor_color_cells ---
test(color_cells_positions, []) :-
    G = [[r,x],[x,r]],
    gridcolor_color_cells(G, r, Cells),
    Cells = [0-0, 1-1].

test(color_cells_absent, []) :-
    g3x3_all_r(G),
    gridcolor_color_cells(G, x, []).

test(color_cells_all, []) :-
    G = [[a,a],[a,a]],
    gridcolor_color_cells(G, a, Cells),
    length(Cells, 4).

% --- gridcolor_contains ---
test(contains_true, []) :-
    g3x3_mixed(G),
    gridcolor_contains(G, b).

test(contains_false, [fail]) :-
    g3x3_all_r(G),
    gridcolor_contains(G, x).

% --- gridcolor_recolor ---
test(recolor_replaces_all, []) :-
    g2x2(G),
    gridcolor_recolor(G, a, r, R),
    R = [[r,b],[b,r]].

test(recolor_absent_color_unchanged, []) :-
    g2x2(G),
    gridcolor_recolor(G, z, r, G).

test(recolor_full_grid, []) :-
    g3x3_all_r(G),
    gridcolor_recolor(G, r, x, R),
    gridcolor_count(R, x, 9).

% --- gridcolor_color_map ---
test(color_map_two_swaps, []) :-
    G = [[r,g],[g,r]],
    gridcolor_color_map(G, [r-b, g-r], R),
    R = [[b,r],[r,b]].

test(color_map_partial_mapping, []) :-
    g2x3_map(G),
    gridcolor_color_map(G, [r-x], R),
    gridcolor_count(R, x, 2),
    gridcolor_count(R, r, 0).

test(color_map_empty_map, []) :-
    g2x2(G),
    gridcolor_color_map(G, [], G).

% --- gridcolor_threshold ---
test(threshold_select_r, []) :-
    g3x3_mixed(G),
    gridcolor_threshold(G, [r], sel, bg, R),
    gridcolor_count(R, sel, 4),
    gridcolor_count(R, bg, 5).

test(threshold_select_multiple, []) :-
    g3x3_mixed(G),
    gridcolor_threshold(G, [r, b], sel, bg, R),
    gridcolor_count(R, sel, 5).

test(threshold_empty_select, []) :-
    g3x3_all_r(G),
    gridcolor_threshold(G, [], sel, bg, R),
    gridcolor_count(R, bg, 9).

% --- gridcolor_dominant ---
test(dominant_non_bg, []) :-
    g3x3_rb(G),
    gridcolor_dominant(G, x, r).
...
test(fraction_absent_color, []) :-
    g3x3_all_r(G),
    gridcolor_fraction(G, x, F),
    F =:= 0.0.

% --- gridcolor_count_colors ---
test(count_colors_mixed, []) :-
    g3x3_mixed(G),
    gridcolor_count_colors(G, N),
    N =:= 3.

test(count_colors_uniform, []) :-
    g3x3_all_r(G),
    gridcolor_count_colors(G, 1).

% --- gridcolor_sorted_by_freq ---
test(sorted_by_freq_mixed, []) :-
    G = [[r,r,r],[r,x,b],[x,x,x]],
    gridcolor_sorted_by_freq(G, Colors),
    Colors = [r, x, b].

test(sorted_by_freq_uniform, []) :-
    g3x3_all_r(G),
    gridcolor_sorted_by_freq(G, [r]).
...
test(recolor_then_count, []) :-
    g3x3_mixed(G),
    gridcolor_recolor(G, x, r, R),
    gridcolor_count(R, x, 0),
    gridcolor_count(R, r, 8).

test(threshold_then_unique_colors, []) :-
    g3x3_mixed(G),
    gridcolor_threshold(G, [r, b], sel, bg, R),
    gridcolor_unique_colors(R, Colors),
    Colors = [bg, sel].

test(color_map_then_histogram, []) :-
    G = [[r,g],[g,r]],
    gridcolor_color_map(G, [r-x, g-x], R),
    gridcolor_histogram(R, [x-4]).

test(dominant_after_recolor, []) :-
    g3x3_mixed(G),
    gridcolor_recolor(G, b, r, R),
    gridcolor_dominant(R, x, r).
