:- module(gridmorph, [
    ...
    gmo_dilate1/3,
    gmo_erode1/3,
    gmo_boundary_inner/3,
    gmo_boundary_outer/3,
    gmo_gradient_cells/3,
    gmo_top_hat/4,
    gmo_bottom_hat/4,
    gmo_fill_holes/4,
    gmo_connected_border/4,
    gmo_remove_small/5
]).
...
gmo_dilate1_step_(Grid, FgColor, Result) :-
% Get dimensions.
    gmo_dims_(Grid, H, W),
% Build result: FgColor cells stay FgColor; non-FgColor adjacent to FgColor become FgColor.
    gmo_build_(H, W,
        [R, C, V]>>(gmo_cell_(Grid, R, C, GV),
                    (GV = FgColor -> V = FgColor
                    ; (once((gmo_nbr4_(R, C, NR, NC),
                              gmo_cell_(Grid, NR, NC, FgColor))) ->
                           V = FgColor
                       ;
                           V = GV))),
        Result).
...
gmo_erode1_step_(Grid, FgColor, BgColor, Result) :-
% Get dimensions.
    gmo_dims_(Grid, H, W),
...
% Iterate over cells to find first non-FgColor value.
    ...
    findall(V,
        (between(0, H1, R), between(0, W1, C),
         gmo_cell_(Grid, R, C, V), V \= FgColor),
        [BgColor|_]).

% Iterate N dilation steps.
gmo_dilate_n_(Grid, _, 0, Grid) :- !.
gmo_dilate_n_(Grid, FgColor, N, Result) :-
    N > 0,
    gmo_dilate1_step_(Grid, FgColor, G1),
    N1 is N - 1,
    gmo_dilate_n_(G1, FgColor, N1, Result).

% Iterate N erosion steps using discovered BgColor.
gmo_erode_n_(Grid, _, _, 0, Grid) :- !.
gmo_erode_n_(Grid, FgColor, BgColor, N, Result) :-
    N > 0,
    gmo_erode1_step_(Grid, FgColor, BgColor, G1),
    N1 is N - 1,
    gmo_erode_n_(G1, FgColor, BgColor, N1, Result).
...
% Find background color for erosion fill.
    (gmo_bg_color_(Grid, FgColor, BgColor) -> true ; BgColor = bg),
...
% Get border cells that are TargetColor.
    ...
    findall(R-C,
        ((R = 0 ; R = H1 ; C = 0 ; C = W1),
         between(0, H1, R), between(0, W1, C),
         gmo_cell_(Grid, R, C, TargetColor)),
        Seeds),
% BFS from Seeds through TargetColor cells.
    gmo_bfs_(Grid, Seeds, TargetColor, BlockColor, [], Cells).

% BFS over TargetColor cells starting from Seeds, avoiding BlockColor.
gmo_bfs_(_, [], _, _, Visited, Visited).
gmo_bfs_(Grid, [RC|Queue], TargetColor, BlockColor, Visited, Result) :-
% Skip already visited cells.
    (memberchk(RC, Visited) ->
        gmo_bfs_(Grid, Queue, TargetColor, BlockColor, Visited, Result)
    ;
        RC = R-C,
...
% Find unvisited TargetColor 4-neighbors.
        findall(NR-NC,
            (gmo_nbr4_(R, C, NR, NC),
             gmo_cell_(Grid, NR, NC, TargetColor),
             \+ memberchk(NR-NC, NewVisited)),
            NewNeighbors),
% Append new neighbors to the queue.
        append(Queue, NewNeighbors, NewQueue),
        gmo_bfs_(Grid, NewQueue, TargetColor, BlockColor, NewVisited, Result)).
...
% For each FgColor cell, find its connected component and mark small ones.
    gmo_small_cells_(Grid, FgColor, AllFg, MinSize, [], SmallCells),
...
gmo_small_cells_(_, _, [], _, Acc, Acc).
gmo_small_cells_(Grid, FgColor, [RC|Rest], MinSize, Acc, SmallCells) :-
% Skip cells already classified.
    (memberchk(RC, Acc) ->
        gmo_small_cells_(Grid, FgColor, Rest, MinSize, Acc, SmallCells)
    ;
        RC = R-C,
% BFS to find the full connected component.
        gmo_bfs_(Grid, [R-C], FgColor, x, [], Component),
% Check if the component is smaller than MinSize.
        (length(Component, Len), Len < MinSize ->
            append(Acc, Component, NewAcc)
        ;
            NewAcc = Acc),
        gmo_small_cells_(Grid, FgColor, Rest, MinSize, NewAcc, SmallCells)).
