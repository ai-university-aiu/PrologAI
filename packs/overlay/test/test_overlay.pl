...
test(diff_basic) :-
    A = [[1,2],[3,4]],
    B = [[1,0],[3,0]],
    ov_diff(A, B, 0, Result),
    Result = [[0,2],[0,4]].

test(diff_all_same) :-
    A = [[1,2],[3,4]],
    ov_diff(A, A, 0, Result),
    Result = [[0,0],[0,0]].
...
test(intersect_basic) :-
    A = [[1,2],[3,4]],
    B = [[1,0],[3,0]],
    ov_intersect(A, B, 0, Result),
    Result = [[1,0],[3,0]].

test(intersect_all_same) :-
    A = [[1,2],[3,4]],
    ov_intersect(A, A, 0, Result),
    Result = A.
...
test(mask_inv_basic) :-
    Grid = [[1,2,3],[4,5,6]],
    Mask = [[0,1,0],[1,0,1]],
    ov_mask_inv(Grid, Mask, 0, Result),
    Result = [[1,0,3],[0,5,0]].

test(mask_inv_complement_of_mask) :-
    Grid = [[1,2],[3,4]],
    Mask = [[1,0],[0,1]],
    ov_mask(Grid, Mask, 0, R1),
    ov_mask_inv(Grid, Mask, 0, R2),
    ov_or(R1, R2, Grid).
...
test(replace_basic) :-
    Grid = [[1,2,1],[2,1,2]],
    ov_replace(Grid, 1, 9, Result),
    Result = [[9,2,9],[2,9,2]].

test(replace_not_present) :-
    Grid = [[1,2],[3,4]],
    ov_replace(Grid, 5, 9, Result),
    Result = [[1,2],[3,4]].

test(replace_all_same) :-
    Grid = [[0,0],[0,0]],
    ov_replace(Grid, 0, 5, Result),
    Result = [[5,5],[5,5]].
