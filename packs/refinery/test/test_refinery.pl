/*  PrologAI — Refinery Unit Tests  (PR 55)
    ...
    Acceptance criteria:
        AC-RN-001: refinery_critique/4 returns empty critique when all criteria pass
        AC-RN-002: refinery_critique/4 returns issues for failing criteria
        AC-RN-003: refinery_score/3 returns 1.0 when all criteria pass
        AC-RN-004: refinery_score/3 returns 0.0 when no criteria pass
        AC-RN-005: refinery_score/3 returns partial score for mixed results
        AC-RN-006: refinery_optimize/5 stops when quality bar is met
        AC-RN-007: refinery_optimize/5 returns best found when MaxIter is exhausted
        AC-RN-008: refinery_learn/3 stores a lesson in the lesson database
        AC-RN-009: refinery_recall/2 retrieves lessons for a given problem type
        AC-RN-010: refinery_forget/1 clears lessons for a given problem type
*/
...
test(critique_all_pass, []) :-
    ...
    refinery_critique(Output, Criteria, 5, Critique),
...
% AC-RN-005: refinery_score/3 returns 0.5 for one passing and one failing criterion.
test(score_partial, []) :-
...
% AC-RN-006: refinery_optimize/5 stops when the quality bar is met.
...
% AC-RN-007: refinery_optimize/5 returns the best found when MaxIter is exhausted.
test(optimize_returns_best, []) :-
    ...
    refinery_optimize(GeneratorGoal, EvaluatorGoal, 1.0, 3, Best),
    % Best must have 3 elements (the longest produced within 3 iterations).
    length(Best, 3).

% AC-RN-008: refinery_learn/3 stores a lesson in the lesson database.
test(learn_stores_lesson,
     [cleanup(refinery_forget(test_problem_type))]) :-
    % Store a lesson.
    refinery_learn(test_problem_type, failed_with_empty, use_non_empty_generator),
    % Verify the lesson is in the database.
    refinery:refinery_lesson_db(test_problem_type,
                          failed_with_empty,
                          use_non_empty_generator).

% AC-RN-009: refinery_recall/2 retrieves lessons for a given problem type.
test(recall_retrieves,
     [setup(refinery_learn(recall_test, pattern_a, lesson_x)),
      cleanup(refinery_forget(recall_test))]) :-
    % Recall lessons for the test problem type.
    refinery_recall(recall_test, Lessons),
    % Must find the stored lesson.
    member(lesson(pattern_a, lesson_x), Lessons).

% AC-RN-010: refinery_forget/1 clears all lessons for a given problem type.
test(forget_clears,
     [setup((refinery_learn(forget_test, p1, l1),
             refinery_learn(forget_test, p2, l2))),
      cleanup(true)]) :-
    % Clear all lessons for the test type.
    refinery_forget(forget_test),
    % Recall should return empty.
    refinery_recall(forget_test, Lessons),
    Lessons = [].
