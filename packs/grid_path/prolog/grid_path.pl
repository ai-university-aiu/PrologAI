:- module(grid_path, [
    grid_path_shortest_path/7,
    grid_path_path_length/7,
    grid_path_reachable/6,
    grid_path_all_reachable/5,
    grid_path_distance_map/5,
    grid_path_nearest/6,
    grid_path_flood_n/6,
    grid_path_wavefront/6,
    grid_path_straight_h/4,
    grid_path_straight_v/4,
    grid_path_line_of_sight/6,
    grid_path_between_h/4,
    grid_path_between_v/4,
    grid_path_region_path/5
]).
...
% Grid dimensions.
grid_path_dims_(Grid, H, W) :-
    length(Grid, H),
    (H > 0 -> Grid = [FR|_], length(FR, W) ; W = 0).

% Cell value at (R, C).
grid_path_cell_(Grid, R, C, V) :-
...
grid_path_path_bfs_(_, _, _, _, TR, TC, [node(TR, TC, RevPath)|_], _, Path) :- !,
    reverse([TR-TC|RevPath], Path).
grid_path_path_bfs_(Grid, H, W, Obs, TR, TC, [node(R, C, RevPath)|Rest], Vis, Path) :-
% Skip already-visited cells.
    (memberchk(R-C, Vis) ->
        grid_path_path_bfs_(Grid, H, W, Obs, TR, TC, Rest, Vis, Path)
    ;
        Vis1 = [R-C|Vis],
% Expand 4-connected unvisited passable neighbors.
        findall(node(NR, NC, [R-C|RevPath]),
            (member(DR-DC, [(-1)-0, 1-0, 0-(-1), 0-1]),
             ...
             grid_path_cell_(Grid, NR, NC, V), V \= Obs),
            Nbrs),
        append(Rest, Nbrs, Q1),
        grid_path_path_bfs_(Grid, H, W, Obs, TR, TC, Q1, Vis1, Path)
    ).

% BFS collecting all cells reachable from the initial queue (non-Obs neighbors).
grid_path_reach_bfs_(_, _, _, _, [], Acc, Acc) :- !.
grid_path_reach_bfs_(Grid, H, W, Obs, [R-C|Q], Acc, Result) :-
    (memberchk(R-C, Acc) ->
        grid_path_reach_bfs_(Grid, H, W, Obs, Q, Acc, Result)
    ;
        Acc1 = [R-C|Acc],
        findall(NR-NC,
            (member(DR-DC, [(-1)-0, 1-0, 0-(-1), 0-1]),
            ...
            Nbrs),
        append(Q, Nbrs, Q1),
        grid_path_reach_bfs_(Grid, H, W, Obs, Q1, Acc1, Result)
    ).

% BFS collecting R-C-Dist triples for all reachable cells.
grid_path_dist_bfs_(_, _, _, _, [], Acc, Acc) :- !.
grid_path_dist_bfs_(Grid, H, W, Obs, [node(R, C, D)|Q], Acc, Map) :-
    (memberchk(R-C-_, Acc) ->
        grid_path_dist_bfs_(Grid, H, W, Obs, Q, Acc, Map)
    ;
        Acc1 = [R-C-D|Acc],
        D1 is D + 1,
        findall(node(NR, NC, D1),
            ...
            Nbrs),
        append(Q, Nbrs, Q1),
        grid_path_dist_bfs_(Grid, H, W, Obs, Q1, Acc1, Map)
    ).

% --- EXPORTED PREDICATES ---

% grid_path_shortest_path(+Grid, +R1, +C1, +R2, +C2, +ObsColor, -Path)
...
grid_path_shortest_path(Grid, R1, C1, R2, C2, Obs, Path) :-
    grid_path_dims_(Grid, H, W),
% Both endpoints must be passable.
    grid_path_cell_(Grid, R1, C1, V1), V1 \= Obs,
    grid_path_cell_(Grid, R2, C2, V2), V2 \= Obs,
    grid_path_path_bfs_(Grid, H, W, Obs, R2, C2, [node(R1, C1, [])], [], Path).

% grid_path_path_length(+Grid, +R1, +C1, +R2, +C2, +ObsColor, -N)
% N is the number of steps in the shortest 4-connected path (number of cells
% minus 1). Fails if no path exists.
grid_path_path_length(Grid, R1, C1, R2, C2, Obs, N) :-
    grid_path_shortest_path(Grid, R1, C1, R2, C2, Obs, Path),
    length(Path, L), N is L - 1.

% grid_path_reachable(+Grid, +R1, +C1, +R2, +C2, +ObsColor)
% Succeeds if (R2,C2) is 4-connected reachable from (R1,C1) avoiding ObsColor.
grid_path_reachable(Grid, R1, C1, R2, C2, Obs) :-
    grid_path_shortest_path(Grid, R1, C1, R2, C2, Obs, _).

% grid_path_all_reachable(+Grid, +R, +C, +ObsColor, -Cells)
...
grid_path_all_reachable(Grid, R, C, Obs, Cells) :-
    grid_path_dims_(Grid, H, W),
    grid_path_cell_(Grid, R, C, V), V \= Obs,
    grid_path_reach_bfs_(Grid, H, W, Obs, [R-C], [], Cells).

% grid_path_distance_map(+Grid, +R, +C, +ObsColor, -Map)
...
grid_path_distance_map(Grid, R, C, Obs, Map) :-
    grid_path_dims_(Grid, H, W),
    grid_path_cell_(Grid, R, C, V), V \= Obs,
    grid_path_dist_bfs_(Grid, H, W, Obs, [node(R, C, 0)], [], Map).

% grid_path_nearest(+Grid, +R, +C, +TargetColor, +ObsColor, -NR-NC)
...
grid_path_nearest(Grid, R, C, TargetColor, Obs, NR-NC) :-
    grid_path_distance_map(Grid, R, C, Obs, Map),
    findall(D-NR-NC, (member(NR-NC-D, Map), grid_path_cell_(Grid, NR, NC, TargetColor)), Keyed),
    Keyed = [_|_],
    msort(Keyed, [_-NR-NC|_]).

% grid_path_flood_n(+Grid, +R, +C, +N, +ObsColor, -Cells)
...
grid_path_flood_n(Grid, R, C, N, Obs, Cells) :-
    grid_path_distance_map(Grid, R, C, Obs, Map),
    findall(NR-NC, (member(NR-NC-D, Map), D =< N), Cells).

% grid_path_wavefront(+Grid, +R, +C, +N, +ObsColor, -Cells)
...
grid_path_wavefront(Grid, R, C, N, Obs, Cells) :-
    grid_path_distance_map(Grid, R, C, Obs, Map),
    findall(NR-NC, member(NR-NC-N, Map), Cells).

% grid_path_straight_h(+R, +C1, +C2, -Cells)
...
% grid_path_straight_v(+C, +R1, +R2, -Cells)
...
% grid_path_line_of_sight(+Grid, +R1, +C1, +R2, +C2, +ObsColor)
...
% grid_path_between_v(+R1, +R2, +C, -Cells)
...
% grid_path_region_path(+Grid, +Color1, +Color2, +ObsColor, -Path)
...
grid_path_region_path(Grid, Color1, Color2, Obs, Path) :-
    grid_path_dims_(Grid, H, W),
    H1 is H - 1, W1 is W - 1,
    findall(R-C,
        (between(0, H1, R), between(0, W1, C), grid_path_cell_(Grid, R, C, Color1)),
        Seeds1),
    findall(R-C,
        (between(0, H1, R), between(0, W1, C), grid_path_cell_(Grid, R, C, Color2)),
        Seeds2),
...
% Try all (Color1,Color2) pairs; collect paths and pick the shortest.
    findall(Len-P,
        (member(R1-C1, Seeds1),
         member(R2-C2, Seeds2),
         grid_path_shortest_path(Grid, R1, C1, R2, C2, Obs, P),
         length(P, Len)),
        Candidates),
    Candidates = [_|_],
