:- use_module('../prolog/object_morph').
:- begin_tests(object_morph).
...
test(interior4_2x2_empty) :-
    ...
    square_cells(2, Cells),
    object_morph_interior4(obj(c,Cells), obj(c,[])).

% object_morph_boundary8 tests.
...
test(boundary8_3x3_all_boundary) :-
    ...
    square_cells(3, Cells),
    object_morph_boundary8(obj(b,Cells), obj(b,B)),
    length(B, 8).
...
test(interior8_3x3_empty) :-
    ...
    square_cells(3, Cells),
    object_morph_interior8(obj(b,Cells), obj(b,I)),
...
test(interior8_2x2_empty) :-
    ...
    square_cells(2, Cells),
    object_morph_interior8(obj(c,Cells), obj(c,[])).

% object_morph_dilate4 tests.

test(dilate4_single_cell) :-
    % Single cell expands to a plus shape (5 cells).
    object_morph_dilate4(obj(red,[r(0,0)]), R),
    R == obj(red,[r(-1,0),r(0,-1),r(0,0),r(0,1),r(1,0)]).

test(dilate4_hline2) :-
    % Horizontal line [r(0,0),r(0,1)] expands.
    object_morph_dilate4(obj(a,[r(0,0),r(0,1)]), obj(a,D)),
...
test(dilate4_preserves_color) :-
    object_morph_dilate4(obj(blue,[r(1,1)]), R),
    R = obj(blue, _).

test(dilate4_contains_original) :-
    object_morph_dilate4(obj(x,[r(2,3)]), obj(x,D)),
    memberchk(r(2,3), D).

% object_morph_erode4 tests.

test(erode4_single_cell_empty) :-
    % Single cell: no interior4.
    object_morph_erode4(obj(red,[r(0,0)]), R),
    R == obj(red,[]).

test(erode4_hline_empty) :-
    % Horizontal line: no interior4.
    hline_cells(5, Cells),
    object_morph_erode4(obj(a,Cells), obj(a,[])).

test(erode4_3x3_one_cell) :-
    % 3x3 erodes to center only.
    square_cells(3, Cells),
    object_morph_erode4(obj(b,Cells), obj(b,E)),
    E == [r(1,1)].
...
% object_morph_dilate8 tests.

test(dilate8_single_cell) :-
    % Single cell: 8 neighbors + self = 9 cells (3x3 square at origin).
    object_morph_dilate8(obj(red,[r(0,0)]), R),
    R == obj(red,[r(-1,-1),r(-1,0),r(-1,1),r(0,-1),r(0,0),r(0,1),r(1,-1),r(1,0),r(1,1)]).

test(dilate8_count_single) :-
    object_morph_dilate8(obj(a,[r(5,5)]), obj(a,D)), length(D, 9).

test(dilate8_larger_than_dilate4) :-
    % 8-dilation produces more cells than 4-dilation.
    object_morph_dilate4(obj(x,[r(0,0)]), obj(x,D4)),
    object_morph_dilate8(obj(x,[r(0,0)]), obj(x,D8)),
    length(D4, L4), length(D8, L8), L8 > L4.

% object_morph_erode8 tests.

test(erode8_3x3_empty) :-
    % 3x3 square: center has all 8 neighbors in; interior8 = [r(1,1)].
    square_cells(3, Cells),
    object_morph_erode8(obj(b,Cells), obj(b,E)),
    E == [r(1,1)].

test(erode8_2x2_empty) :-
    square_cells(2, Cells),
    object_morph_erode8(obj(c,Cells), obj(c,[])).

test(erode8_subset_of_erode4) :-
    % erode8 cells are always a subset of erode4 cells (stricter requirement).
    square_cells(5, Cells),
    object_morph_erode4(obj(b,Cells), obj(b,E4)),
    object_morph_erode8(obj(b,Cells), obj(b,E8)),
    length(E4, L4), length(E8, L8), L8 =< L4.

% object_morph_dilate4_n tests.

test(dilate4_n0_identity) :-
    % N=0: identity.
    object_morph_dilate4_n(obj(red,[r(0,0)]), 0, R),
    R == obj(red,[r(0,0)]).

test(dilate4_n1_same_as_dilate4) :-
    % N=1: same as object_morph_dilate4.
    object_morph_dilate4(obj(a,[r(0,0)]), D1),
    object_morph_dilate4_n(obj(a,[r(0,0)]), 1, DN),
    D1 == DN.

test(dilate4_n2_count) :-
    % N=2 from single cell: diamond of radius 2 (13 cells).
    object_morph_dilate4_n(obj(x,[r(0,0)]), 2, obj(x,D)),
    % Cells with |R|+|C| <= 2.
    length(D, 13).

% object_morph_erode4_n tests.

test(erode4_n0_identity) :-
    % N=0: identity.
    square_cells(5, Cells),
    object_morph_erode4_n(obj(b,Cells), 0, R),
    R == obj(b,Cells).

test(erode4_n1_same_as_erode4) :-
    % N=1: same as object_morph_erode4.
    square_cells(4, Cells),
    object_morph_erode4(obj(b,Cells), E1),
    object_morph_erode4_n(obj(b,Cells), 1, EN),
    E1 == EN.

test(erode4_n2_from_5x5) :-
    % 5x5 eroded twice: first erode to 3x3 interior, then to single center.
    square_cells(5, Cells),
    object_morph_erode4_n(obj(b,Cells), 2, obj(b,E)),
    E == [r(2,2)].

% object_morph_open4 tests.

test(open4_3x3_identity) :-
    % A solid 3x3: erode to center, dilate back. Result = center's cross = 5 cells.
    square_cells(3, Cells),
    object_morph_open4(obj(b,Cells), obj(b,O)),
