...
:- module(object_morph, [
    object_morph_neighbors4/2,
    object_morph_neighbors8/2,
    object_morph_boundary4/2,
    object_morph_interior4/2,
    object_morph_boundary8/2,
    object_morph_interior8/2,
    object_morph_dilate4/2,
    object_morph_erode4/2,
    object_morph_dilate8/2,
    object_morph_erode8/2,
    object_morph_dilate4_n/3,
    object_morph_erode4_n/3,
    object_morph_open4/2,
    object_morph_close4/2
]).
% member/2 for cell-list membership testing.
:- use_module(library(lists), [member/2]).

% object_morph_neighbors4(+Cell, -Neighbors): 4-connected (up, down, left, right) neighbors of r(R,C).
% Always returns exactly 4 cells; coordinates may be negative or out of any grid bounds.
object_morph_neighbors4(r(R, C), [r(R1,C), r(R2,C), r(R,C1), r(R,C2)]) :-
% Compute row neighbors: one step up and one step down.
    R1 is R - 1, R2 is R + 1,
% Compute column neighbors: one step left and one step right.
    C1 is C - 1, C2 is C + 1.

% object_morph_neighbors8(+Cell, -Neighbors): 8-connected neighbors of r(R,C) (4-connected + diagonals).
% Always returns exactly 8 cells.
object_morph_neighbors8(r(R, C), [r(R1,C1),r(R1,C),r(R1,C2),
                         r(R, C1),          r(R, C2),
...
% object_morph_boundary4(+Obj, -Boundary): cells of Obj with at least one 4-connected neighbor outside Obj.
% The boundary is the outer ring of cells when viewed with 4-connectivity.
object_morph_boundary4(obj(Color, Cells), obj(Color, Boundary)) :-
...
% Keep cells for which not ALL 4-neighbors are also in S (= at least one is outside).
    findall(r(R,C),
            (member(r(R,C), S),
             object_morph_neighbors4(r(R,C), Nbrs),
             \+ forall(member(N, Nbrs), member(N, S))),
            Boundary).

% object_morph_interior4(+Obj, -Interior): cells of Obj where ALL 4-connected neighbors are also in Obj.
% The interior is the set of cells fully surrounded on all 4 sides by other cells of Obj.
object_morph_interior4(obj(Color, Cells), obj(Color, Interior)) :-
...
% object_morph_boundary8(+Obj, -Boundary): cells of Obj with at least one 8-connected neighbor outside Obj.
% Looser than boundary4: a cell is "interior" only if all 8 diagonal neighbors are also in Obj.
object_morph_boundary8(obj(Color, Cells), obj(Color, Boundary)) :-
% Sort for canonical order.
    sort(Cells, S),
% Keep cells for which at least one 8-neighbor is outside S.
    findall(r(R,C),
            (member(r(R,C), S),
             object_morph_neighbors8(r(R,C), Nbrs),
             \+ forall(member(N, Nbrs), member(N, S))),
            Boundary).

% object_morph_interior8(+Obj, -Interior): cells of Obj where ALL 8-connected neighbors are also in Obj.
% Stricter than interior4: requires all 8 diagonal neighbors to be present too.
object_morph_interior8(obj(Color, Cells), obj(Color, Interior)) :-
% Sort for canonical order.
    sort(Cells, S),
...
% object_morph_dilate4(+Obj, -Result): expand Obj by one 4-connected step.
...
object_morph_dilate4(obj(Color, Cells), obj(Color, Dilated)) :-
% Sort to canonical order.
    sort(Cells, S),
% Collect all 4-connected neighbors of every cell.
    findall(r(NR,NC),
            (member(r(R,C), S), object_morph_neighbors4(r(R,C), Nbrs), member(r(NR,NC), Nbrs)),
            NewCells),
% Union original cells with new neighbors; sort deduplicates.
    append(S, NewCells, All),
    sort(All, Dilated).

% object_morph_erode4(+Obj, -Result): shrink Obj by one 4-connected step.
% Removes all boundary4 cells; the result is the interior4 of Obj.
% Equivalent to object_morph_interior4: keeps only cells fully surrounded on all 4 sides.
object_morph_erode4(Obj, Eroded) :-
% Erosion = keep only interior4 cells.
    object_morph_interior4(Obj, Eroded).

% object_morph_dilate8(+Obj, -Result): expand Obj by one 8-connected step.
% Each existing cell contributes all 8 neighbors; includes diagonal expansion.
object_morph_dilate8(obj(Color, Cells), obj(Color, Dilated)) :-
% Sort to canonical order.
    sort(Cells, S),
% Collect all 8-connected neighbors of every cell.
    findall(r(NR,NC),
            (member(r(R,C), S), object_morph_neighbors8(r(R,C), Nbrs), member(r(NR,NC), Nbrs)),
            NewCells),
% Union original cells with new neighbors; sort deduplicates.
    append(S, NewCells, All),
    sort(All, Dilated).

% object_morph_erode8(+Obj, -Result): shrink Obj by one 8-connected step.
...
object_morph_erode8(Obj, Eroded) :-
% Erosion under 8-connectivity = keep only interior8 cells.
    object_morph_interior8(Obj, Eroded).

% object_morph_dilate4_n(+Obj, +N, -Result): apply object_morph_dilate4 exactly N times.
% N=0 returns Obj unchanged.
object_morph_dilate4_n(Obj, 0, Obj) :- !.
% Recursive case: dilate once then dilate the result N-1 more times.
object_morph_dilate4_n(Obj, N, Result) :-
    N > 0,
% One dilation step.
    object_morph_dilate4(Obj, Step),
% Continue for the remaining steps.
    N1 is N - 1,
    object_morph_dilate4_n(Step, N1, Result).

% object_morph_erode4_n(+Obj, +N, -Result): apply object_morph_erode4 exactly N times.
% N=0 returns Obj unchanged. Stops when the cell set becomes empty.
object_morph_erode4_n(Obj, 0, Obj) :- !.
% Recursive case: erode once then erode the result N-1 more times.
object_morph_erode4_n(obj(Color, Cells), N, Result) :-
    N > 0,
% One erosion step.
    object_morph_erode4(obj(Color, Cells), Step),
% Continue eroding; if Step has empty cells, further erosion gives empty cells.
    N1 is N - 1,
    object_morph_erode4_n(Step, N1, Result).

% object_morph_open4(+Obj, -Result): morphological opening = erode4 then dilate4.
% Removes small protrusions and thin connections while preserving bulk shapes.
object_morph_open4(Obj, Result) :-
% First erode to remove boundary cells.
    object_morph_erode4(Obj, Eroded),
% Then dilate to restore bulk size.
    object_morph_dilate4(Eroded, Result).

% object_morph_close4(+Obj, -Result): morphological closing = dilate4 then erode4.
% Fills small gaps and indentations while preserving bulk shapes.
object_morph_close4(Obj, Result) :-
% First dilate to fill gaps.
    object_morph_dilate4(Obj, Dilated),
% Then erode to restore bulk size.
    object_morph_erode4(Dilated, Result).
