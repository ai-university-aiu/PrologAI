...
test(added_colors_basic) :-
    red_dot(R), blue_dot(B), green_dot(G),
    sm_added_colors([R, B], [R, B, G], Added),
    Added == [g].

test(added_colors_none) :-
    red_dot(R), blue_dot(B),
    sm_added_colors([R, B], [R, B], Added),
    Added == [].

test(added_colors_all_new) :-
    red_dot(R), blue_dot(B),
    sm_added_colors([], [R, B], Added),
    Added == [b, r].
...
test(removed_colors_basic) :-
    red_dot(R), blue_dot(B), green_dot(G),
    sm_removed_colors([R, B, G], [R, B], Removed),
    Removed == [g].

test(removed_colors_none) :-
    red_dot(R), blue_dot(B),
    sm_removed_colors([R, B], [R, B], Removed),
    Removed == [].
...
test(added_forms_basic) :-
    red_dot(R), red_bar(RB),
    sm_added_forms([R], [R, RB], Added),
    length(Added, 1).

test(added_forms_none) :-
    red_dot(R), blue_dot(B),
    % same single-cell form
    sm_added_forms([R], [B], Added),
    Added == [].
...
test(removed_forms_basic) :-
    red_dot(R), red_bar(RB),
    sm_removed_forms([R, RB], [R], Removed),
    length(Removed, 1).
...
test(n_color_change_basic) :-
    ...
    sm_n_color_change([R, B], [R, G], N),
    N == 2.

test(n_color_change_none) :-
    red_dot(R), blue_dot(B),
    sm_n_color_change([R, B], [R, B], N),
    N == 0.

test(n_color_change_only_added) :-
    red_dot(R), blue_dot(B), green_dot(G),
    sm_n_color_change([R, B], [R, B, G], N),
    N == 1.
...
test(any_change_color) :-
    red_dot(R), blue_dot(B), green_dot(G),
    sm_any_change([R, B], [R, G]).

test(any_change_count) :-
    red_dot(R), blue_dot(B), green_dot(G),
    sm_any_change([R, B], [R, B, G]).
