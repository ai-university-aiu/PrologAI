...
grid_period_divides_(H, P) :-
% P divides H evenly.
    H mod P =:= 0.

% --- EXPORTED PREDICATES ---

% grid_period_is_row_periodic(+Grid, +P)
...
% grid_period_is_col_periodic(+Grid, +P)
...
% Check all column pairs separated by P.
    ...
    \+ (between(0, H1, R),
        ...
        C2 < W,
        grid_period_cell_(Grid, R, C, V),
        grid_period_cell_(Grid, R, C2, V2),
        V \= V2).

% grid_period_row_period(+Grid, -P)
% P is the smallest positive integer for which grid_period_is_row_periodic holds.
...
% grid_period_col_period(+Grid, -P)
% P is the smallest positive integer for which grid_period_is_col_periodic holds.
% Always found within 1..W.
grid_period_col_period(Grid, P) :-
    grid_period_dims_(Grid, _, W),
    between(1, W, P),
    grid_period_is_col_periodic(Grid, P),
    !.

% grid_period_tile_dims(+Grid, -PH, -PW)
% PH x PW is the smallest tile whose tiling (with wrap-around) reconstructs Grid.
% Equivalent to: PH = grid_period_row_period(Grid), PW = grid_period_col_period(Grid),
% but only reported when PH divides H and PW divides W (true tiling).
grid_period_tile_dims(Grid, PH, PW) :-
% Find the smallest periods for rows and columns.
    grid_period_row_period(Grid, PH),
    grid_period_col_period(Grid, PW).

% grid_period_extract_tile(+Grid, +TH, +TW, -Tile)
...
% Collect the first TH rows, each truncated to TW columns.
    findall(Row,
        (between(0, TH1, R),
         findall(V, (between(0, TW1, C), grid_period_cell_(Grid, R, C, V)), Row)),
        Tile).

% grid_period_tile_grid(+Tile, +H, +W, -Grid)
% Grid is a H x W grid formed by tiling Tile using wrap-around indexing:
% Grid[r][c] = Tile[r mod TH][c mod TW].
grid_period_tile_grid(Tile, H, W, Grid) :-
    grid_period_dims_(Tile, TH, TW),
    H1 is H - 1,
    W1 is W - 1,
    findall(Row,
        (between(0, H1, R),
         ...
         findall(V,
             (between(0, W1, C),
              TC is C mod TW,
              grid_period_cell_(Tile, TR, TC, V)),
             Row)),
        Grid).

% grid_period_extend_rows(+Grid, +TargetH, -Extended)
...
% grid_period_extend_cols(+Grid, +TargetW, -Extended)
...
% grid_period_extend(+Grid, +TargetH, +TargetW, -Extended)
% Extended is Grid tiled to fill a TargetH x TargetW area.
% Combines grid_period_extend_rows and grid_period_extend_cols in one pass.
grid_period_extend(Grid, TargetH, TargetW, Extended) :-
    grid_period_dims_(Grid, H, W),
    T1H is TargetH - 1,
    T1W is TargetW - 1,
...
% grid_period_autocorr_row(+Grid, +P, -Score)
...
% grid_period_autocorr_col(+Grid, +P, -Score)
...
% grid_period_shift_wrap_rows(+Grid, +N, -Shifted)
...
% grid_period_shift_wrap_cols(+Grid, +N, -Shifted)
