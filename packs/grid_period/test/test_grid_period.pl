...
% --- grid_period_row_period/2 ---

test(row_period_uniform) :-
% Uniform grid: row period = 1.
    g3x3_r(G),
    grid_period_row_period(G, P),
    P =:= 1.
...
% Single row grid: H=1, row period = 1 (vacuously).
    grid_period_row_period([[r,b,g]], P),
    P =:= 1.

% --- grid_period_col_period/2 ---

test(col_period_uniform) :-
% Uniform grid: col period = 1.
    g3x3_r(G),
    grid_period_col_period(G, P),
    P =:= 1.
...
% --- grid_period_tile_dims/3 ---

test(tile_dims_uniform) :-
% Uniform grid: tile is 1x1.
    g3x3_r(G),
    grid_period_tile_dims(G, PH, PW),
    PH =:= 1, PW =:= 1.
...
% --- grid_period_shift_wrap_rows/3 ---

test(shift_rows_zero) :-
% Shifting by 0 returns the same grid.
    g3x3_rows(G),
    grid_period_shift_wrap_rows(G, 0, S),
    S = G.
...
% Row 2 of shifted = row 1 = [b,b,b]
    g3x3_rows(G),
    grid_period_shift_wrap_rows(G, 1, S),
    S = [[g,g,g],[r,r,r],[b,b,b]].

test(shift_rows_by_h_identity) :-
% Shifting by H (height) returns the same grid.
    g3x3_rows(G),
    grid_period_shift_wrap_rows(G, 3, S),
    S = G.

% --- grid_period_shift_wrap_cols/3 ---

test(shift_cols_zero) :-
% Shifting cols by 0 returns same grid.
    g3x3_cols(G),
    grid_period_shift_wrap_cols(G, 0, S),
    S = G.
...
% Each row: [g,r,b]
    g3x3_cols(G),
    grid_period_shift_wrap_cols(G, 1, S),
    S = [[g,r,b],[g,r,b],[g,r,b]].

test(shift_cols_by_w_identity) :-
% Shifting cols by W returns same grid.
    g3x3_cols(G),
    grid_period_shift_wrap_cols(G, 3, S),
    S = G.
...
% Extend single-row [[r,b,g]] to 4 rows by row tiling.
    grid_period_extend_rows([[r,b,g]], 4, E),
    length(E, 4),
    E = [[r,b,g],[r,b,g],[r,b,g],[r,b,g]].

:- end_tests(grid_period).
