:- use_module('../prolog/grid_distance').
...
% The b cell at center has distance 0 to itself.
    g3x3(G),
    grid_distance_dist_map(G, b, DMap),
    nth0(1, DMap, Row),
    nth0(1, Row, D),
    D =:= 0.
...
% Corner (0,0): Chebyshev distance to center b (1,1) = max(1,1) = 1.
    g3x3(G),
    grid_distance_dist_map8(G, b, DMap8),
    nth0(0, DMap8, Row),
    nth0(0, Row, D),
    D =:= 1.
...
% From (0,0) in g3x3, the nearest b is at center (1,1).
    g3x3(G),
    grid_distance_nearest_coord(G, 0, 0, b, Coord),
    Coord = [1, 1].
...
% msort on dist-R-C pairs gives (0,0) first.
    g3x3_corners(G),
    grid_distance_nearest_coord(G, 1, 0, b, Coord),
    Coord = [0, 0].

test(nearest_coord_no_color, [fail]) :-
% No b cells: predicate should fail.
    g3x3_uniform(G),
    grid_distance_nearest_coord(G, 0, 0, b, _).
...
% Ring at distance 2 from center = the 4 corners (in a 3x3).
    g3x3(G),
    grid_distance_ring(G, 1, 1, 2, Coords),
    length(Coords, 4),
...
% Ring at distance 1 from corner (0,0) clips to 2 in-bounds cells.
    g3x3(G),
    grid_distance_ring(G, 0, 0, 1, Coords),
    length(Coords, 2),
    memberchk([0,1], Coords),
    memberchk([1,0], Coords).
...
% BFS from (0,0) with no walls in 3x3; (1,1) should be at distance 2.
    g3x3(G),
    grid_distance_bfs_flood(G, 0, 0, x, DistGrid),
    nth0(1, DistGrid, Row),
    nth0(1, Row, D),
    D =:= 2.
...
% BFS from (0,0) treating b as wall in g3x3: b at (1,1) gets dist 9999.
    g3x3(G),
    grid_distance_bfs_flood(G, 0, 0, b, DistGrid),
    nth0(1, DistGrid, Row),
    nth0(1, Row, D),
    D =:= 9999.
...
% Starting at a wall cell should fail.
    g3x3(G),
    grid_distance_bfs_flood(G, 1, 1, b, _).
...
% g3x3: b at (1,1) has dist 1 from nearest r. Palette=[yellow] -> becomes yellow.
    g3x3(G),
    grid_distance_recolor_by_dist(G, b, [yellow], NewG),
    nth0(1, NewG, Row),
    nth0(1, Row, V),
    V = yellow.
...
% N=2: center b at dist 2 is also eroded; all b become r.
    g5x5(G),
    grid_distance_shrink_n(G, b, 2, S),
...
% (1,1): dist to b = 2, dist to g = 2 -> equidist.
    g3x3_split(G),
    grid_distance_equidistant(G, b, g, EqG),
    nth0(1, EqG, Row),
    nth0(1, Row, V),
    V = equidist.
...
% ColorA = ColorB = b: every cell is equidistant (same map for both) -> all equidist.
    g3x3(G),
    grid_distance_equidistant(G, b, b, EqG),
...
% g3x3: b at center (1,1). Farthest cell is a corner at dist 2. First in row-major = (0,0).
    g3x3(G),
    grid_distance_max_dist(G, b, R, C),
    R =:= 0,
    C =:= 0.
...
% No b cells: no finite distances -> predicate fails.
    g3x3_uniform(G),
    grid_distance_max_dist(G, b, _, _).

:- end_tests(grid_distance).
