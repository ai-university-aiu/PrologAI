...
:- module(rotation, [
    % ro_rot90/2: rotate Grid 90 degrees clockwise.
    ro_rot90/2,
    % ro_rot180/2: rotate Grid 180 degrees.
    ro_rot180/2,
    % ro_rot270/2: rotate Grid 270 degrees clockwise (= 90 counter-clockwise).
    ro_rot270/2,
    % ro_rot_n/3: rotate Grid by N*90 degrees clockwise; N in 0..3.
    ro_rot_n/3,
    % ro_all/2: produce list of all four rotations [G0, G90, G180, G270].
    ro_all/2,
    % ro_canonical/2: lexicographically smallest of the four rotations (canonical form).
    ro_canonical/2,
    % ro_is_rot2/1: succeed if Grid is invariant under 180-degree rotation.
    ro_is_rot2/1,
    % ro_is_rot4/1: succeed if Grid is invariant under 90-degree rotation.
    ro_is_rot4/1,
    % ro_sym_order/2: smallest N in {1,2,4} such that Grid has N-fold rotational symmetry.
    ro_sym_order/2,
    % ro_rotate_cells/5: rotate a list of R-C cell coordinates by N*90 CW in an H x W box.
    ro_rotate_cells/5,
    % ro_spin2/3: overlay Grid with its 180-degree rotation; non-Bg wins.
    ro_spin2/3,
    % ro_spin4/3: overlay all four rotations of Grid; non-Bg wins.
    ro_spin4/3,
    % ro_match_rotation/3: find N in 0..3 such that ro_rot_n(GridA, N, GridB).
    ro_match_rotation/3,
    % ro_equiv_rotation/2: succeed if any rotation of GridA equals GridB.
    ro_equiv_rotation/2
]).
...
% ro_rot90(+Grid, -Out): rotate Grid 90 degrees clockwise.
...
% ro_rot180(+Grid, -Out): rotate Grid 180 degrees.
...
% ro_rot270(+Grid, -Out): rotate Grid 270 degrees clockwise (= 90 counter-clockwise).
% Out has W rows and H columns (dimensions are transposed).
...
% ro_rot_n(+Grid, +N, -Out): rotate Grid by N*90 degrees clockwise; N in 0..3.
% N=0: identity. N=1: 90 CW. N=2: 180. N=3: 270 CW.
% Uses cuts to prevent choicepoints when N is ground.
ro_rot_n(Grid, 0, Grid) :- !.
% N=1: delegate to ro_rot90.
ro_rot_n(Grid, 1, Out) :-
    !,
    ro_rot90(Grid, Out).
% N=2: delegate to ro_rot180.
ro_rot_n(Grid, 2, Out) :-
    !,
    ro_rot180(Grid, Out).
% N=3: delegate to ro_rot270.
ro_rot_n(Grid, 3, Out) :-
    ro_rot270(Grid, Out).

% ro_all(+Grid, -Rots): produce all four rotations as [G0, G90, G180, G270].
...
% ro_canonical(+Grid, -Canon): lexicographically smallest of the four rotations.
...
% ro_is_rot2(+Grid): succeed if Grid is invariant under 180-degree rotation.
% Equivalently: Grid == ro_rot180(Grid).
ro_is_rot2(Grid) :-
    % Compute 180-degree rotation.
    ro_rot180(Grid, Grid180),
    % Check structural equality with original.
    Grid == Grid180.

% ro_is_rot4(+Grid): succeed if Grid is invariant under 90-degree rotation.
% Equivalently: Grid == ro_rot90(Grid). Requires Grid to be square (H == W).
ro_is_rot4(Grid) :-
    % Compute 90-degree rotation.
    ro_rot90(Grid, Grid90),
    % Check structural equality with original.
    Grid == Grid90.

% ro_sym_order(+Grid, -N): smallest N in {1, 2, 4} such that Grid has N-fold symmetry.
...
% ro_rotate_cells(+Cells, +H, +W, +N, -Out): rotate a list of R-C pairs by N*90 CW.
...
% ro_apply_cell_(+N, +R, +C, +H1, +W1, -NR, -NC): apply rotation N to one cell.
% H1 = H-1, W1 = W-1 (precomputed for efficiency).
% N=0: identity.
ro_apply_cell_(0, R, C, _, _, R, C).
% N=1 (90 CW): new row = old col C; new col = H-1 - old row.
ro_apply_cell_(1, R, C, H1, _, C, NC) :-
    NC is H1 - R.
% N=2 (180): new row = H-1 - old row; new col = W-1 - old col.
ro_apply_cell_(2, R, C, H1, W1, NR, NC) :-
    NR is H1 - R,
    NC is W1 - C.
% N=3 (270 CW = 90 CCW): new row = W-1 - old col; new col = old row R.
ro_apply_cell_(3, R, C, _, W1, NR, R) :-
    NR is W1 - C.

% ro_overlay_bg_(+Bg, +VA, +VB, -V): overlay two cell values with background priority.
...
% ro_overlay_grids_(+A, +B, +Bg, -Out): cell-by-cell overlay of two same-size grids.
% Uses ro_overlay_bg_ for each cell. A takes priority over B.
ro_overlay_grids_([], [], _, []).
...
% ro_overlay_rows_(+RowA, +RowB, +Bg, -RowOut): overlay two rows cell by cell.
ro_overlay_rows_([], [], _, []).
...
% ro_spin2(+Grid, +Bg, -Out): overlay Grid with its 180-degree rotation.
...
ro_spin2(Grid, Bg, Out) :-
    ...
    ro_overlay_grids_(Grid, Grid180, Bg, Out).

% ro_spin4(+Grid, +Bg, -Out): overlay all four rotations of Grid.
...
ro_spin4(Grid, Bg, Out) :-
    ...
    ro_overlay_grids_(Grid, G90, Bg, Step1),
    % Overlay Step1 over G180.
    ro_overlay_grids_(Step1, G180, Bg, Step2),
    % Overlay Step2 over G270.
    ro_overlay_grids_(Step2, G270, Bg, Out).

% ro_match_rotation(+GridA, +GridB, -N): find N in 0..3 such that ro_rot_n(GridA,N) = GridB.
...
% ro_equiv_rotation(+GridA, +GridB): succeed if any rotation of GridA equals GridB.
% Convenience wrapper around ro_match_rotation/3 that discards the index.
