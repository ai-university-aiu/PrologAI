:- module(grid_morph, [
    ...
    grid_morph_open/4,
    grid_morph_close/4,
    grid_morph_dilate1/3,
    grid_morph_erode1/3,
    grid_morph_boundary_inner/3,
    grid_morph_boundary_outer/3,
    grid_morph_gradient_cells/3,
    grid_morph_top_hat/4,
    grid_morph_bottom_hat/4,
    grid_morph_fill_holes/4,
    grid_morph_connected_border/4,
    grid_morph_remove_small/5
]).
...
grid_morph_build_(H, W, Goal, Grid) :-
...
grid_morph_dilate1_step_(Grid, FgColor, Result) :-
% Get dimensions.
    grid_morph_dims_(Grid, H, W),
% Build result: FgColor cells stay FgColor; non-FgColor adjacent to FgColor become FgColor.
    grid_morph_build_(H, W,
        [R, C, V]>>(grid_morph_cell_(Grid, R, C, GV),
                    (GV = FgColor -> V = FgColor
                    ; (once((grid_morph_nbr4_(R, C, NR, NC),
                              grid_morph_cell_(Grid, NR, NC, FgColor))) ->
                           V = FgColor
                       ;
                           V = GV))),
        Result).
...
grid_morph_erode1_step_(Grid, FgColor, BgColor, Result) :-
% Get dimensions.
    grid_morph_dims_(Grid, H, W),
% Build result: FgColor cells with any non-FgColor neighbor become BgColor; others keep value.
    grid_morph_build_(H, W,
        [R, C, V]>>(grid_morph_cell_(Grid, R, C, GV),
                    (GV = FgColor ->
                        (once((grid_morph_nbr4_(R, C, NR, NC),
                               grid_morph_cell_(Grid, NR, NC, NV),
                               NV \= FgColor)) ->
                             V = BgColor
                        ;
                             V = FgColor)
...
grid_morph_bg_color_(Grid, FgColor, BgColor) :-
% Iterate over cells to find first non-FgColor value.
    grid_morph_dims_(Grid, H, W),
    H1 is H - 1, W1 is W - 1,
    findall(V,
        (between(0, H1, R), between(0, W1, C),
         grid_morph_cell_(Grid, R, C, V), V \= FgColor),
        [BgColor|_]).

% Iterate N dilation steps.
grid_morph_dilate_n_(Grid, _, 0, Grid) :- !.
grid_morph_dilate_n_(Grid, FgColor, N, Result) :-
    N > 0,
    grid_morph_dilate1_step_(Grid, FgColor, G1),
    N1 is N - 1,
    grid_morph_dilate_n_(G1, FgColor, N1, Result).

% Iterate N erosion steps using discovered BgColor.
grid_morph_erode_n_(Grid, _, _, 0, Grid) :- !.
grid_morph_erode_n_(Grid, FgColor, BgColor, N, Result) :-
    N > 0,
    grid_morph_erode1_step_(Grid, FgColor, BgColor, G1),
    N1 is N - 1,
    grid_morph_erode_n_(G1, FgColor, BgColor, N1, Result).

% --- MORPHOLOGICAL OPERATIONS ---

% grid_morph_dilate(+Grid, +FgColor, +N, -Result)
...
% grid_morph_erode(+Grid, +FgColor, +N, -Result)
...
% Find background color for erosion fill.
    (grid_morph_bg_color_(Grid, FgColor, BgColor) -> true ; BgColor = bg),
% Iterate N erosion steps.
    grid_morph_erode_n_(Grid, FgColor, BgColor, N, Result).

% grid_morph_dilate1(+Grid, +FgColor, -Result)
% Single dilation step: one-layer expansion of FgColor region.
grid_morph_dilate1(Grid, FgColor, Result) :-
% Apply a single dilation step.
    grid_morph_dilate1_step_(Grid, FgColor, Result).

% grid_morph_erode1(+Grid, +FgColor, -Result)
...
% grid_morph_open(+Grid, +FgColor, +N, -Result)
...
% Erode first to shrink.
    grid_morph_erode(Grid, FgColor, N, Eroded),
% Then dilate to restore approximate original size.
    grid_morph_dilate(Eroded, FgColor, N, Result).

% grid_morph_close(+Grid, +FgColor, +N, -Result)
...
% Dilate first to fill gaps.
    grid_morph_dilate(Grid, FgColor, N, Dilated),
% Then erode to restore approximate original size.
    grid_morph_erode(Dilated, FgColor, N, Result).

% --- BOUNDARY EXTRACTION ---

% grid_morph_boundary_inner(+Grid, +FgColor, -Cells)
...
% Collect FgColor cells with at least one non-FgColor neighbor.
    findall(R-C,
        (between(0, H1, R), between(0, W1, C),
         grid_morph_cell_(Grid, R, C, FgColor),
         once((grid_morph_nbr4_(R, C, NR, NC),
               grid_morph_cell_(Grid, NR, NC, NV),
               NV \= FgColor))),
        Cells).

% grid_morph_boundary_outer(+Grid, +FgColor, -Cells)
...
% grid_morph_gradient_cells(+Grid, +FgColor, -Cells)
...
% Get inner and outer boundaries.
    grid_morph_boundary_inner(Grid, FgColor, Inner),
    grid_morph_boundary_outer(Grid, FgColor, Outer),
% Combine inner and outer boundaries (may have no overlap by construction).
    append(Inner, Outer, Cells).

% --- DERIVED OPERATIONS ---

% grid_morph_top_hat(+Grid, +FgColor, +N, -Cells)
...
% Compute morphological opening.
    grid_morph_open(Grid, FgColor, N, Opened),
% Collect FgColor cells in Grid that became non-FgColor in Opened.
    ...
    findall(R-C,
        (between(0, H1, R), between(0, W1, C),
         grid_morph_cell_(Grid, R, C, FgColor),
         grid_morph_cell_(Opened, R, C, OV),
         OV \= FgColor),
        Cells).

% grid_morph_bottom_hat(+Grid, +FgColor, +N, -Cells)
...
% Compute morphological closing.
    grid_morph_close(Grid, FgColor, N, Closed),
% Collect FgColor cells in Closed that were non-FgColor in Grid.
    grid_morph_dims_(Grid, H, W),
    H1 is H - 1, W1 is W - 1,
    findall(R-C,
        (between(0, H1, R), between(0, W1, C),
         grid_morph_cell_(Closed, R, C, FgColor),
         grid_morph_cell_(Grid, R, C, GV),
         GV \= FgColor),
        Cells).

% grid_morph_fill_holes(+Grid, +FgColor, +BgColor, -Result)
...
% Find all BgColor cells reachable from the border (open background).
    grid_morph_connected_border(Grid, BgColor, FgColor, OpenBg),
...
% grid_morph_connected_border(+Grid, +TargetColor, +BlockColor, -Cells)
...
% Get border cells that are TargetColor.
    ...
    findall(R-C,
        ((R = 0 ; R = H1 ; C = 0 ; C = W1),
         between(0, H1, R), between(0, W1, C),
         grid_morph_cell_(Grid, R, C, TargetColor)),
        Seeds),
% BFS from Seeds through TargetColor cells.
    grid_morph_bfs_(Grid, Seeds, TargetColor, BlockColor, [], Cells).

% BFS over TargetColor cells starting from Seeds, avoiding BlockColor.
grid_morph_bfs_(_, [], _, _, Visited, Visited).
grid_morph_bfs_(Grid, [RC|Queue], TargetColor, BlockColor, Visited, Result) :-
% Skip already visited cells.
    (memberchk(RC, Visited) ->
        grid_morph_bfs_(Grid, Queue, TargetColor, BlockColor, Visited, Result)
    ;
        RC = R-C,
...
% Find unvisited TargetColor 4-neighbors.
        findall(NR-NC,
            (grid_morph_nbr4_(R, C, NR, NC),
             grid_morph_cell_(Grid, NR, NC, TargetColor),
             \+ memberchk(NR-NC, NewVisited)),
            NewNeighbors),
% Append new neighbors to the queue.
        append(Queue, NewNeighbors, NewQueue),
        grid_morph_bfs_(Grid, NewQueue, TargetColor, BlockColor, NewVisited, Result)).

% grid_morph_remove_small(+Grid, +FgColor, +BgColor, +MinSize, -Result)
...
% For each FgColor cell, find its connected component and mark small ones.
    grid_morph_small_cells_(Grid, FgColor, AllFg, MinSize, [], SmallCells),
...
grid_morph_small_cells_(_, _, [], _, Acc, Acc).
grid_morph_small_cells_(Grid, FgColor, [RC|Rest], MinSize, Acc, SmallCells) :-
% Skip cells already classified.
    (memberchk(RC, Acc) ->
        grid_morph_small_cells_(Grid, FgColor, Rest, MinSize, Acc, SmallCells)
    ;
        RC = R-C,
% BFS to find the full connected component.
        grid_morph_bfs_(Grid, [R-C], FgColor, x, [], Component),
% Check if the component is smaller than MinSize.
        (length(Component, Len), Len < MinSize ->
            append(Acc, Component, NewAcc)
        ;
            NewAcc = Acc),
        grid_morph_small_cells_(Grid, FgColor, Rest, MinSize, NewAcc, SmallCells)).
