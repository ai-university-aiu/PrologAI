/*  PrologAI — Causalontology Lore Test Suite  (WP-424)
    ...
    Run with the full library path:
        swipl $LIB -g "run_tests, halt" -t "halt(1)" packs/lore/test/test_lore.pl
*/
...
setup_lore :-
    lore:lore_reset,
    lore:lore_record([low_health, enemy_near], flee, good),
    lore:lore_record([low_health, enemy_near], fight, bad),
    lore:lore_record([low_health, enemy_near], flee, good),
    lore:lore_record([treasure_seen], grab, good).

% A situation seen at least twice is a theme; a one-off is not.
test(theme_detection) :-
    setup_lore,
    findall(S-C, lore:lore_theme(S, C), Themes),
...
test(lessons_recalled) :-
    setup_lore,
    findall(R-Res, lore:lore_lesson([low_health, enemy_near], R, Res), Lessons),
...
test(subset_match) :-
    ...
    lore:lore_advise([low_health, enemy_near, tired], R, _),
    assertion(R == flee).

% Advice picks the response that most often turned out good.
test(advise_best) :-
    setup_lore,
    lore:lore_advise([low_health, enemy_near], R, Conf),
    assertion(R == flee),
    assertion(Conf =:= 1.0).
...
test(responses_ranked) :-
    setup_lore,
    lore:lore_responses([low_health, enemy_near], Ranked),
