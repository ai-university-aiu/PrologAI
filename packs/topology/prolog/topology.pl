...
:- module(topology, [
    tp_component/4, tp_all_components/3,
    tp_component_count/3, tp_component_size/4,
    tp_largest_component/3, tp_reachable/5,
    tp_enclosed/4, tp_has_hole/4,
    tp_hole_count/4, tp_fill_holes/4,
    tp_border_components/3, tp_interior_components/3,
    tp_label_components/3, tp_same_component/5
]).
...
% tp_component(+Grid, +Val, +Seed, -Cells): find all cells of value Val
...
% BFS flood fill from Seed; only step through cells with value Val.
    ...
    tp_grow_(Grid, Val, [Seed], [], Acc),
    sort(Acc, Cells).

% tp_all_components(+Grid, +Val, -Components): all 4-connected components
...
% Collect all Val cells then partition into connected components.
    ...
    tp_split_cells_(Grid, Val, Sorted, Components).

% tp_component_count(+Grid, +Val, -N): count of 4-connected components.
tp_component_count(Grid, Val, N) :-
% Count components by measuring the resulting list.
    tp_all_components(Grid, Val, Comps),
    length(Comps, N).

% tp_component_size(+Grid, +Val, +Seed, -N): cell count of the 4-connected
...
% tp_largest_component(+Grid, +Val, -Cells): cells of the largest
...
% Tag each component with its size; pick the maximum.
    ...
    tp_max_key_(Tagged, _-Cells).

% tp_reachable(+Grid, +Val, +Src, +Dst, -Bool): Bool=1 iff there exists a
...
% Flood fill from Src then test if Dst is in the visited set.
    Src = RS-CS,
    nth0(RS, Grid, SRow), nth0(CS, SRow, Val),
    tp_grow_(Grid, Val, [Src], [], Acc),
    (memberchk(Dst, Acc) -> Bool = 1 ; Bool = 0).

% tp_enclosed(+Grid, +_Val, +Bg, -Cells): Bg cells that are completely
...
tp_enclosed(Grid, _Val, Bg, Enclosed) :-
% Flood fill from all border Bg cells through Bg; enclosed = unreachable Bg.
    ...
    tp_grow_(Grid, Bg, BorderBg, [], Reachable),
    findall(R-C, (
        between(0, H1, R), between(0, W1, C),
        nth0(R, Grid, Row), nth0(C, Row, Bg),
...
% tp_has_hole(+Grid, +Val, +Bg, -Bool): Bool=1 iff Val encloses at least
% one Bg region; Bool=0 otherwise. Val is passed through to tp_enclosed.
...
% tp_hole_count(+Grid, +Val, +Bg, -N): count of distinct enclosed Bg
% components (holes).
tp_hole_count(Grid, Val, Bg, N) :-
% Partition enclosed Bg cells into 4-connected groups and count.
    tp_enclosed(Grid, Val, Bg, Enclosed),
    tp_split_cells_plain_(Enclosed, Comps),
    length(Comps, N).

% tp_fill_holes(+Grid, +Val, +Bg, -Grid2): replace all enclosed Bg cells
...
% tp_border_components(+Grid, +Val, -Components): components of Val that
...
% Collect all components; keep those with at least one border cell.
    tp_all_components(Grid, Val, All),
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
    findall(C, (
        member(C, All),
...
% tp_interior_components(+Grid, +Val, -Components): components of Val that
...
% tp_label_components(+Grid, +Val, -LabelGrid): produce a same-size grid
...
% Build a label lookup from components then apply to each cell position.
    tp_all_components(Grid, Val, Comps),
    tp_build_label_map_(Comps, 1, LabelMap),
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
    findall(RowL, (
        between(0, H1, R),
...
% tp_same_component(+Grid, +Val, +P1, +P2, -Bool): Bool=1 iff P1 and P2
...
% Flood fill from P1 and check if P2 is reached.
    tp_reachable(Grid, Val, P1, P2, Bool).

% Private: BFS flood fill through Val cells.
% tp_grow_(+Grid, +Val, +Queue, +Visited, -Result).
tp_grow_(_, _, [], Acc, Acc).
tp_grow_(Grid, Val, [H|T], Acc, Result) :-
    (memberchk(H, Acc) ->
% Cell already visited: skip and continue.
        tp_grow_(Grid, Val, T, Acc, Result)
    ;
% New cell: find in-bounds Val-valued 4-neighbors, add to queue.
        ...
        findall(R2-C2, tp_val_nbr_(Grid, R, C, Val, TH, TW, R2-C2), Nbrs),
        append(Nbrs, T, Q1),
        tp_grow_(Grid, Val, Q1, [H|Acc], Result)
    ).

% Private: one valid 4-connected in-bounds Val-valued neighbor.
tp_val_nbr_(Grid, R, C, Val, H1, W1, NR-NC) :-
    (NR is R-1, NR >= 0, NC = C
...
tp_grow_plain_(_, [], Acc, Acc).
tp_grow_plain_(Allowed, [H|T], Acc, Result) :-
    (memberchk(H, Acc) ->
        tp_grow_plain_(Allowed, T, Acc, Result)
    ;
        H = R-C,
        findall(R2-C2, (
        ...
        append(Nbrs, T, Q1),
        tp_grow_plain_(Allowed, Q1, [H|Acc], Result)
    ).

% Private: partition a list of R-C pairs into 4-connected groups.
% tp_split_cells_(+Grid, +Val, +AllCells, -Components) uses Grid/Val BFS.
tp_split_cells_(_, _, [], []).
tp_split_cells_(Grid, Val, AllCells, [Comp|Rest]) :-
    AllCells = [Seed|_],
    tp_grow_(Grid, Val, [Seed], [], Acc),
    sort(Acc, Comp),
    subtract(AllCells, Comp, Remaining),
    tp_split_cells_(Grid, Val, Remaining, Rest).

% Private: partition a plain cell list into 4-connected groups.
tp_split_cells_plain_([], []).
tp_split_cells_plain_(All, [Comp|Rest]) :-
    All = [Seed|_],
    tp_grow_plain_(All, [Seed], [], Acc),
    sort(Acc, Comp),
    subtract(All, Comp, Remaining),
    tp_split_cells_plain_(Remaining, Rest).
...
tp_build_label_map_([], _, []).
tp_build_label_map_([Comp|Rest], N, Map) :-
    findall(R-C-N, member(R-C, Comp), Entries),
    N1 is N + 1,
    tp_build_label_map_(Rest, N1, RestMap),
