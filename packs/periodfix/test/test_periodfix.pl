...
test(tile_from_list_corrupted) :-
    l6_p3_v1(L), periodfix_tile_from_list(L, 3, T), T = [a,b,c].

% AC-PPF-010: tile with P=1 from [a,a,b,a] is [a] (majority is a, 3 vs 1).
test(tile_from_list_p1) :-
    periodfix_tile_from_list([a,a,b,a], 1, T), T = [a].

% --- periodfix_violations_list ---

% AC-PPF-011: no violations for exact period list.
test(violations_list_none) :-
    l6_p2(L), periodfix_tile_from_list(L, 2, T),
    periodfix_violations_list(L, 2, T, V), V = [].

% AC-PPF-012: one violation for l6_p3_v1 with P=3 and tile [a,b,c].
test(violations_list_one) :-
    l6_p3_v1(L), periodfix_tile_from_list(L, 3, T),
    periodfix_violations_list(L, 3, T, V), V = [viol(4, x, b)].

% AC-PPF-013: one violation in l4_p2_v2 at index 3.
test(violations_list_l4) :-
    l4_p2_v2(L), periodfix_tile_from_list(L, 2, T),
    periodfix_violations_list(L, 2, T, V), V = [viol(3, c, b)].

% --- periodfix_repair_list ---

% AC-PPF-014: repairing l6_p3_v1 with P=3 yields [a,b,c,a,b,c].
test(repair_list) :-
    l6_p3_v1(L), periodfix_tile_from_list(L, 3, T),
    periodfix_repair_list(L, 3, T, R), R = [a,b,c,a,b,c].

% AC-PPF-015: repairing l4_p2_v2 with P=2 yields [a,b,a,b].
test(repair_list_l4) :-
    l4_p2_v2(L), periodfix_tile_from_list(L, 2, T),
    periodfix_repair_list(L, 2, T, R), R = [a,b,a,b].

% --- periodfix_best_period_list ---
...
test(tile_from_arc_row) :-
    periodfix_tile_from_list([1,3,1,3,1,3,3,3,1], 2, T), T = [1,3].
