...
% dn_count_: count how many times V appears in List.
dn_count_([], _, 0).
% Matching head: add one to the count of the tail.
dn_count_([V|T], V, N) :- !, dn_count_(T, V, N1), N is N1 + 1.
% Non-matching head: skip it.
dn_count_([_|T], V, N) :- dn_count_(T, V, N).

% dn_norm_cells_: normalize obj cells to origin (min row = 0, min col = 0).
dn_norm_cells_(obj(_, Cells), Norm) :-
...
% dn_block_dims(+Grid, +N, -BI, -BJ): count complete NxN blocks in Grid.
% BI = H // N (rows of blocks); BJ = W // N (cols of blocks). Uses integer division.
dn_block_dims(Grid, N, BI, BJ) :-
...
% dn_block_cells(+N, +I, +J, -Cells): r(R,C) pairs covering block (I,J) in NxN partition.
% Block (I,J) spans rows [I*N .. I*N+N-1] and cols [J*N .. J*N+N-1].
dn_block_cells(N, I, J, Cells) :-
...
% dn_block_color(+Grid, +N, +I, +J, -Color): unique color of block (I,J).
% Fails when the block contains two or more distinct values.
dn_block_color(Grid, N, I, J, Color) :-
...
% dn_block_majority(+Grid, +N, +I, +J, -Color): most frequent value in block (I,J).
% When two values tie for the highest count, the one with larger standard order wins.
dn_block_majority(Grid, N, I, J, Color) :-
% Get the cell coordinates for block (I,J).
    dn_block_cells(N, I, J, Cells),
...
% Count occurrences of each distinct value.
    findall(Count-V, (member(V, Unique), dn_count_(Vals, V, Count)), Pairs),
% msort keeps duplicates; ascending order puts highest count last.
    msort(Pairs, Sorted),
% last/2 retrieves the highest-count (or tie-breaking) value.
    last(Sorted, _-Color).

% dn_is_blocky(+Grid, +N): succeed if every complete NxN block in Grid is uniform.
...
% dn_block_color must succeed for every (I,J) pair.
    forall((between(0, Bi1, I), between(0, Bj1, J)),
           dn_block_color(Grid, N, I, J, _)).

% dn_shrink(+Grid, +N, -Small): downscale Grid by N using majority vote per block.
% Small[I][J] = most frequent color in block (I,J). Never fails for valid N.
dn_shrink(Grid, N, Small) :-
...
% Outer findall produces one row per block row; inner findall collects one color per block col.
    findall(Row,
            (between(0, Bi1, I),
             findall(Color, (between(0, Bj1, J), dn_block_majority(Grid, N, I, J, Color)), Row)),
            Small).

% dn_shrink_strict(+Grid, +N, -Small): downscale Grid by N; fail if any block is mixed.
% Identical to dn_shrink but each block must be uniform (dn_block_color, not majority).
dn_shrink_strict(Grid, N, Small) :-
...
% Build Small using unique-color blocks; no ambiguity since all blocks are uniform.
    findall(Row,
            (between(0, Bi1, I),
             findall(Color, (between(0, Bj1, J), dn_block_color(Grid, N, I, J, Color)), Row)),
            Small).

% dn_find_scale(+Grid, -N): find the smallest N >= 2 such that Grid is N-blocky.
% Tries divisors of H and W in ascending order; cuts at the first match.
dn_find_scale(Grid, N) :-
% Get grid dimensions.
...
% dn_obj_shrink(+Obj, +N, -Small): downscale an obj term by dividing all coords by N.
% Each cell r(R,C) maps to r(R//N, C//N); sort/2 deduplicates overlapping results.
dn_obj_shrink(obj(Color, Cells), N, obj(Color, Small)) :-
...
% dn_scale_factor(+Obj1, +Obj2, -N): smallest N >= 2 where shrinking Obj2 by N yields Obj1.
% Both objects are normalized to origin before comparison. Tries N from 2 to 30.
dn_scale_factor(Obj1, Obj2, N) :-
% Normalize Obj1 cells to origin for canonical comparison.
    dn_norm_cells_(Obj1, Norm1),
% Try each candidate N.
    between(2, 30, N),
% Shrink Obj2 by N.
    dn_obj_shrink(Obj2, N, Shrunken),
% Normalize the shrunken result to origin.
    dn_norm_cells_(Shrunken, Norm2),
% Succeed when both normalized cell sets are identical; cut for determinism.
    Norm1 == Norm2, !.

% dn_uniform_blocks(+Grid, +N, -Pairs): I-J pairs for all uniform NxN blocks.
% A uniform block has exactly one distinct color.
dn_uniform_blocks(Grid, N, Pairs) :-
% Determine block grid dimensions.
    dn_block_dims(Grid, N, BI, BJ),
    Bi1 is BI - 1, Bj1 is BJ - 1,
% Collect (I,J) pairs where dn_block_color succeeds (uniform).
    findall(I-J, (between(0, Bi1, I), between(0, Bj1, J), dn_block_color(Grid, N, I, J, _)), Pairs).

% dn_mixed_blocks(+Grid, +N, -Pairs): I-J pairs for all non-uniform NxN blocks.
% A mixed block contains two or more distinct colors.
dn_mixed_blocks(Grid, N, Pairs) :-
% Determine block grid dimensions.
    dn_block_dims(Grid, N, BI, BJ),
    Bi1 is BI - 1, Bj1 is BJ - 1,
% Collect (I,J) pairs where dn_block_color fails (more than one color).
    findall(I-J, (between(0, Bi1, I), between(0, Bj1, J), \+ dn_block_color(Grid, N, I, J, _)), Pairs).

% dn_block_grid(+Grid, +N, +I, +J, -Patch): extract block (I,J) as an NxN sub-grid.
% Patch is a list of N rows, each a list of N values, in row-major order.
dn_block_grid(Grid, N, I, J, Patch) :-
...
% dn_block_val(+Grid, +N, +I, +J, -Vals): flat list of all values in block (I,J).
% Values appear in row-major order: row I*N first, then row I*N+1, and so on.
dn_block_val(Grid, N, I, J, Vals) :-
% First and last row of the block.
    R0 is I * N, R1 is R0 + N - 1,
% First and last column of the block.
    C0 is J * N, C1 is C0 + N - 1,
