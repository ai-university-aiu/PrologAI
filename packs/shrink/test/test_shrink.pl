...
test(block_dims_4x4_n2) :-
    g4x4b2(G), dn_block_dims(G, 2, BI, BJ), BI =:= 2, BJ =:= 2.

test(block_dims_6x6_n2) :-
    g6x6b2(G), dn_block_dims(G, 2, BI, BJ), BI =:= 3, BJ =:= 3.

test(block_dims_6x6_n3) :-
    g6x6b2(G), dn_block_dims(G, 3, BI, BJ), BI =:= 2, BJ =:= 2.

test(block_dims_4x4_n4) :-
    g4x4b2(G), dn_block_dims(G, 4, BI, BJ), BI =:= 1, BJ =:= 1.

% dn_block_cells tests.

test(block_cells_n2_00) :-
    dn_block_cells(2, 0, 0, Cells),
    sort(Cells, S),
    S == [r(0,0),r(0,1),r(1,0),r(1,1)].

test(block_cells_n2_01) :-
    dn_block_cells(2, 0, 1, Cells),
    sort(Cells, S),
    S == [r(0,2),r(0,3),r(1,2),r(1,3)].

test(block_cells_n2_11) :-
    dn_block_cells(2, 1, 1, Cells),
    sort(Cells, S),
    S == [r(2,2),r(2,3),r(3,2),r(3,3)].

test(block_cells_n3_00) :-
    dn_block_cells(3, 0, 0, Cells), length(Cells, 9).

% dn_block_color tests.

test(block_color_4x4_00) :-
    g4x4b2(G), dn_block_color(G, 2, 0, 0, C), C == 1.

test(block_color_4x4_01) :-
    g4x4b2(G), dn_block_color(G, 2, 0, 1, C), C == 2.

test(block_color_4x4_10) :-
    g4x4b2(G), dn_block_color(G, 2, 1, 0, C), C == 3.

test(block_color_4x4_11) :-
    g4x4b2(G), dn_block_color(G, 2, 1, 1, C), C == 4.

test(block_color_mixed_fails, [fail]) :-
    g4x4b2m(G), dn_block_color(G, 2, 0, 0, _).

test(block_color_6x6_12) :-
    g6x6b2(G), dn_block_color(G, 2, 1, 2, C), C == f.

% dn_block_majority tests.

test(block_majority_uniform) :-
    g4x4b2(G), dn_block_majority(G, 2, 0, 0, C), C == 1.

test(block_majority_mixed) :-
    % block (0,0) of g2x2mix has [1,2,1,1] = 1 appears 3 times.
    g2x2mix(G), dn_block_majority(G, 2, 0, 0, C), C == 1.

test(block_majority_tie) :-
    % Grid where block (0,0) has 2 ones and 2 twos: tie goes to 2 (larger in standard order).
    dn_block_majority([[1,2],[2,1]], 2, 0, 0, C), C == 2.

test(block_majority_6x6) :-
    g6x6b2(G), dn_block_majority(G, 2, 2, 2, C), C == i.

% dn_is_blocky tests.

test(is_blocky_4x4_n2) :-
    g4x4b2(G), dn_is_blocky(G, 2).

test(is_blocky_6x6_n2) :-
    g6x6b2(G), dn_is_blocky(G, 2).

test(is_blocky_fails_mixed, [fail]) :-
    g4x4b2m(G), dn_is_blocky(G, 2).

test(is_blocky_fails_2x2mix, [fail]) :-
    g2x2mix(G), dn_is_blocky(G, 2).

% dn_shrink tests.

test(shrink_4x4_n2) :-
    g4x4b2(G), dn_shrink(G, 2, S), S == [[1,2],[3,4]].

test(shrink_6x6_n2) :-
    g6x6b2(G), dn_shrink(G, 2, S), S == [[a,b,c],[d,e,f],[g,h,i]].

test(shrink_majority_vote) :-
    % Grid with one mixed block: majority vote used for block (0,0).
    dn_shrink([[1,2,3,3],[2,1,3,3],[5,5,6,6],[5,5,6,6]], 2, S),
    % Block (0,0) has [1,2,2,1] -> tie -> larger wins; blocks (0,1),(1,0),(1,1) are uniform.
    S = [[_,3],[5,6]].

test(shrink_6x6_n3) :-
    g6x6b3(G), dn_shrink(G, 3, S), S == [[p,q],[r,s]].

% dn_shrink_strict tests.

test(shrink_strict_4x4) :-
    g4x4b2(G), dn_shrink_strict(G, 2, S), S == [[1,2],[3,4]].

test(shrink_strict_6x6) :-
    g6x6b2(G), dn_shrink_strict(G, 2, S), S == [[a,b,c],[d,e,f],[g,h,i]].

test(shrink_strict_fails_mixed, [fail]) :-
    g4x4b2m(G), dn_shrink_strict(G, 2, _).

% dn_find_scale tests.

test(find_scale_4x4) :-
    g4x4b2(G), dn_find_scale(G, N), N =:= 2.

test(find_scale_6x6) :-
    g6x6b2(G), dn_find_scale(G, N), N =:= 2.

test(find_scale_not_blocky, [fail]) :-
    dn_find_scale([[1,2],[3,4]], _).

test(find_scale_n3) :-
    % 6x6 grid where each 3x3 block is uniform.
    G = [[r,r,r,g,g,g],[r,r,r,g,g,g],[r,r,r,g,g,g],[b,b,b,w,w,w],[b,b,b,w,w,w],[b,b,b,w,w,w]],
    dn_find_scale(G, N), N =:= 3.

% dn_obj_shrink tests.

test(obj_shrink_2x2_block) :-
    % 2x2 block at origin all maps to r(0,0).
    dn_obj_shrink(obj(red,[r(0,0),r(0,1),r(1,0),r(1,1)]), 2, R),
    R == obj(red,[r(0,0)]).

test(obj_shrink_offset_block) :-
    % 2x2 block at rows 2-3, cols 4-5.
    dn_obj_shrink(obj(blue,[r(2,4),r(2,5),r(3,4),r(3,5)]), 2, R),
    R == obj(blue,[r(1,2)]).

test(obj_shrink_l_shape) :-
    % L-shape scaled by 2: cells at r(0,0) r(0,1) r(0,2) r(0,3) r(2,0) r(2,1).
    dn_obj_shrink(obj(x,[r(0,0),r(0,1),r(0,2),r(0,3),r(2,0),r(2,1)]), 2, R),
...
test(obj_shrink_n1) :-
    ...
    dn_obj_shrink(obj(a,[r(3,5),r(3,6)]), 1, R),
    R == obj(a,[r(3,5),r(3,6)]).

% dn_scale_factor tests.
...
test(scale_factor_dot) :-
    ...
    Obj2 = obj(x,Block),
    dn_scale_factor(Obj1, Obj2, N), N =:= 3.
...
test(scale_factor_hline) :-
    ...
    dn_scale_factor(Obj1, Obj2, N), N =:= 2.
...
% dn_uniform_blocks tests.

test(uniform_blocks_all) :-
    g4x4b2(G), dn_uniform_blocks(G, 2, Pairs),
    sort(Pairs, S),
    S == [0-0,0-1,1-0,1-1].

test(uniform_blocks_partial) :-
    g4x4b2m(G), dn_uniform_blocks(G, 2, Pairs),
    sort(Pairs, S),
    % Blocks (0,1),(1,0),(1,1) are uniform; (0,0) is mixed.
    S == [0-1,1-0,1-1].

test(uniform_blocks_none) :-
    dn_uniform_blocks([[1,2],[3,4]], 2, Pairs), Pairs == [].

% dn_mixed_blocks tests.

test(mixed_blocks_none) :-
    g4x4b2(G), dn_mixed_blocks(G, 2, Pairs), Pairs == [].

test(mixed_blocks_one) :-
    g4x4b2m(G), dn_mixed_blocks(G, 2, Pairs),
    sort(Pairs, S), S == [0-0].

test(mixed_blocks_all) :-
    % 4x4 grid where every 2x2 block has two colors.
    G = [[1,2,1,2],[2,1,2,1],[1,2,1,2],[2,1,2,1]],
    dn_mixed_blocks(G, 2, Pairs), sort(Pairs, S),
    S == [0-0,0-1,1-0,1-1].

% dn_block_grid tests.

test(block_grid_00) :-
    g4x4b2(G), dn_block_grid(G, 2, 0, 0, P),
    P == [[1,1],[1,1]].

test(block_grid_11) :-
    g4x4b2(G), dn_block_grid(G, 2, 1, 1, P),
    P == [[4,4],[4,4]].

test(block_grid_6x6_22) :-
    g6x6b2(G), dn_block_grid(G, 2, 2, 2, P),
    P == [[i,i],[i,i]].

% dn_block_val tests.

test(block_val_uniform) :-
    g4x4b2(G), dn_block_val(G, 2, 0, 0, Vals), Vals == [1,1,1,1].

test(block_val_mixed) :-
    g2x2mix(G), dn_block_val(G, 2, 0, 0, Vals), Vals == [1,2,1,1].

test(block_val_count) :-
    g6x6b2(G), dn_block_val(G, 2, 1, 1, Vals), length(Vals, 4).
