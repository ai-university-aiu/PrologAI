% State the fact: this pack is named safety_governor.
name(safety_governor).
% State the fact: this is version 1.0.0.
version('1.0.0').
% State the fact: the title carries the work-package number.
title('PrologAI Causalontology Safety Governor — an immutable constitution of forbidden action patterns, a pre-action veto with a stated reason, and an append-only veto log (WP-414)').
