...
:- module(vec2, [
    ...
    vec2_cross/5,
    vec2_parallel/4,
    vec2_translate_region/4
]).
% Import higher-order utilities for list transformation.
:- use_module(library(apply), [maplist/3]).

% vec2_offset(+R1, +C1, +R2, +C2, -DR, -DC): displacement vector from (R1,C1)
...
% vec2_add(+R, +C, +DR, +DC, -NR, -NC): apply vector (DR,DC) to point (R,C).
% NR = R + DR, NC = C + DC.
...
% vec2_scale(+DR, +DC, +K, -SDR, -SDC): scale vector (DR,DC) by integer K.
% SDR = K * DR, SDC = K * DC.
...
% vec2_neg(+DR, +DC, -NDR, -NDC): negate a vector. NDR = -DR, NDC = -DC.
...
% vec2_len_sq(+DR, +DC, -LSq): squared Euclidean length of a vector.
...
% vec2_manhattan(+DR, +DC, -M): L1 (Manhattan) magnitude of a vector.
% M = |DR| + |DC|.
vec2_manhattan(DR, DC, M) :-
% Sum the absolute values of both components.
    M is abs(DR) + abs(DC).

% vec2_chebyshev(+DR, +DC, -D): L-infinity (Chebyshev) magnitude of a vector.
...
% vec2_rot90_cw(+DR, +DC, -RDR, -RDC): rotate vector 90 degrees clockwise.
...
% vec2_rot90_ccw(+DR, +DC, -RDR, -RDC): rotate vector 90 degrees counter-clockwise.
% In row-column grid coordinates: RDR = -DC, RDC = DR.
...
% vec2_rot180(+DR, +DC, -RDR, -RDC): rotate vector 180 degrees.
% Equivalent to vec2_neg: RDR = -DR, RDC = -DC.
...
% vec2_dot(+DR1, +DC1, +DR2, +DC2, -D): dot product of two 2D vectors.
% D = DR1 * DR2 + DC1 * DC2.
...
% vec2_cross(+DR1, +DC1, +DR2, +DC2, -C): z-component of 2D cross product.
% C = DR1 * DC2 - DC1 * DR2.
...
% vec2_parallel(+DR1, +DC1, +DR2, +DC2): succeed if two vectors are parallel.
...
% vec2_translate_cell_: translate one R-C cell by a displacement (DR,DC).
vec2_translate_cell_(DR, DC, R-C, NR-NC) :-
% Add the displacement to the cell coordinates.
    NR is R + DR,
% Translate the column.
    NC is C + DC.

% vec2_translate_region(+Cells, +DR, +DC, -NewCells): translate a list of R-C
...
% Apply the translation to every cell in the list.
    maplist(vec2_translate_cell_(DR, DC), Cells, NewCells).
