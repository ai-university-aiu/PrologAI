:- module(griddelta, [
    griddelta_diff_cells/3,
    griddelta_diff_count/3,
    griddelta_same_cells/3,
    griddelta_changed_pairs/3,
    griddelta_color_changes/4,
    griddelta_changed_colors/3,
    griddelta_apply_delta/4,
    griddelta_diff_rows/3,
    griddelta_diff_cols/3,
    griddelta_compatible/2,
    griddelta_agree_region/6,
    griddelta_overlay/4,
    griddelta_invert_delta/3,
    griddelta_is_identity/2
]).
...
griddelta_build_(H, W, Goal, Grid) :-
...
% Both grids must have same H and W.
    griddelta_dims_(G1, H, W),
    griddelta_dims_(G2, H, W).
...
% Collect cells where values differ.
    findall(R-C,
        (between(0, H1, R),
         between(0, W1, C),
         griddelta_cell_(G1, R, C, V1),
         griddelta_cell_(G2, R, C, V2),
         V1 \= V2),
        Cells).
...
% Collect cells where values agree.
    findall(R-C,
        (between(0, H1, R),
         between(0, W1, C),
         griddelta_cell_(G1, R, C, V),
         griddelta_cell_(G2, R, C, V)),
        Cells).
...
% griddelta_color_changes(+G1, +G2, +From, -Count)
...
griddelta_apply_delta(G1, G2, G3, Result) :-
    griddelta_dims_(G1, H, W),
    griddelta_dims_(G2, H, W),
    griddelta_dims_(G3, H, W),
% Build result by applying delta to G3.
    griddelta_build_(H, W,
        [R, C, V]>>(griddelta_cell_(G1, R, C, V1),
                    griddelta_cell_(G2, R, C, V2),
                    griddelta_cell_(G3, R, C, V3),
...
% griddelta_agree_region(+G1, +G2, +RowMin, +RowMax, +ColMin, +ColMax)
...
% griddelta_overlay(+G1, +G2, +Marker, -Result)
...
% Build result: use G2 where non-Marker, else G1.
    griddelta_build_(H, W,
        [R, C, V]>>(griddelta_cell_(G2, R, C, V2),
                    griddelta_cell_(G1, R, C, V1),
                    (V2 \= Marker -> V = V2 ; V = V1)),
        Result).

% griddelta_invert_delta(+G1, +G2, -Inverted)
