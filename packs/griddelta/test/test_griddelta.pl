...
test(diff_cells_none, []) :-
    ...
    gdt_diff_cells(G1, G2, []).

test(diff_cells_one, []) :-
    g3x3a(G1), g3x3b(G2),
    % Only center (1,1) changed
    gdt_diff_cells(G1, G2, [1-1]).
...
test(diff_count_one, []) :-
    g3x3a(G1), g3x3b(G2),
    gdt_diff_count(G1, G2, 1).

test(diff_count_all, []) :-
    g3x3r(G1), g3x3e(G2),
    gdt_diff_count(G1, G2, 9).
...
test(same_cells_none, []) :-
    g3x3r(G1), g3x3e(G2),
    % No cell agrees
    gdt_same_cells(G1, G2, []).

% --- gdt_changed_pairs ---
test(changed_pairs_one, []) :-
    g3x3a(G1), g3x3b(G2),
    % One change: (1,1) from x to b
    gdt_changed_pairs(G1, G2, [1-1-(x->b)]).
...
test(color_changes_r_count, []) :-
    ...
    gdt_color_changes(G1, G2, r, 9).

test(color_changes_x_count, []) :-
    g3x3a(G1), g3x3b(G2),
    % 1 x cell (center) changed to b
    gdt_color_changes(G1, G2, x, 1).

test(color_changes_none, []) :-
    g3x3a(G1), g3x3a_copy(G2),
    % No changes from r
    gdt_color_changes(G1, G2, r, 0).

% --- gdt_changed_colors ---
test(changed_colors_one, []) :-
    g3x3a(G1), g3x3b(G2),
    % Center changed to b
    gdt_changed_colors(G1, G2, [b]).
...
test(diff_rows_one, []) :-
    ...
    gdt_diff_rows(G1, G2, [1]).

test(diff_rows_all, []) :-
    g3x3r(G1), g3x3e(G2),
    % All 3 rows differ
    gdt_diff_rows(G1, G2, [0, 1, 2]).

% --- gdt_diff_cols ---
test(diff_cols_one, []) :-
    g3x3a(G1), g3x3b(G2),
    % Only column 1 has a difference
    gdt_diff_cols(G1, G2, [1]).

test(diff_cols_all, []) :-
    g3x3r(G1), g3x3e(G2),
    % All 3 columns differ
    gdt_diff_cols(G1, G2, [0, 1, 2]).

% --- gdt_agree_region ---
test(agree_region_all, []) :-
    g3x3a(G1), g3x3a_copy(G2),
    gdt_agree_region(G1, G2, 0, 2, 0, 2).

test(agree_region_top_row, []) :-
    g3x3a(G1), g3x3b(G2),
    % Row 0 is unchanged
    gdt_agree_region(G1, G2, 0, 0, 0, 2).

test(agree_region_fails_center, []) :-
    g3x3a(G1), g3x3b(G2),
    % Center row includes (1,1) which changed
    \+ gdt_agree_region(G1, G2, 1, 1, 1, 1).
...
test(overlay_non_transparent, []) :-
    ...
    gdt_overlay(Base, Overlay, r, Result),
...
test(diff_count_plus_same_count_equals_total, []) :-
    g3x3a(G1), g3x3d(G2),
    gdt_diff_count(G1, G2, D),
    gdt_same_cells(G1, G2, Same),
    length(Same, S),
    Total is D + S,
    Total =:= 9.
...
test(diff_rows_two_changes, []) :-
    ...
    gdt_diff_rows(G1, G2, [0]).

test(changed_pairs_none, []) :-
    g3x3a(G1), g3x3a_copy(G2),
    gdt_changed_pairs(G1, G2, []).
