:- module(grid_crop, [
    grid_crop_bbox/3,
    grid_crop_crop_bbox/3,
    grid_crop_trim/3,
    grid_crop_trim_rows/3,
    grid_crop_trim_cols/3,
    grid_crop_crop/6,
    grid_crop_pad_to/5,
    grid_crop_center_in/5,
    grid_crop_add_border/4,
    grid_crop_remove_border/4,
    grid_crop_expand_down/4,
    grid_crop_expand_right/4,
    grid_crop_content_h/3,
    grid_crop_content_w/3
]).
...
grid_crop_crop_(Grid, R0, C0, R1, C1, Cropped) :-
...
% grid_crop_crop_bbox(+Grid, +BgColor, -Cropped)
...
% grid_crop_trim(+Grid, +BgColor, -Trimmed)
% Trimmed is Grid with all outer uniform-BgColor rows and columns removed.
% Equivalent to grid_crop_crop_bbox/3.
grid_crop_trim(Grid, BgColor, Trimmed) :-
% Bounding box gives the trim region.
    grid_crop_bbox(Grid, BgColor, R0-C0-R1-C1),
% Crop to content region.
    grid_crop_crop_(Grid, R0, C0, R1, C1, Trimmed).

% grid_crop_trim_rows(+Grid, +BgColor, -Trimmed)
...
% Row bounds from bounding box.
    grid_crop_bbox(Grid, BgColor, R0-_-R1-_),
% Keep full column range.
    grid_crop_dims_(Grid, _, W), W1 is W - 1,
% Crop to trimmed row range.
    grid_crop_crop_(Grid, R0, 0, R1, W1, Trimmed).

% grid_crop_trim_cols(+Grid, +BgColor, -Trimmed)
% Trimmed is Grid with leading and trailing uniform-BgColor columns removed.
% Row count is unchanged.
grid_crop_trim_cols(Grid, BgColor, Trimmed) :-
% Column bounds from bounding box.
    grid_crop_bbox(Grid, BgColor, _-C0-_-C1),
% Keep full row range.
    grid_crop_dims_(Grid, H, _), H1 is H - 1,
% Crop to trimmed column range.
    grid_crop_crop_(Grid, 0, C0, H1, C1, Trimmed).

% grid_crop_crop(+Grid, +R0, +C0, +R1, +C1, -Cropped)
% Cropped is the sub-grid at rows R0..R1 and columns C0..C1 (0-indexed, inclusive).
grid_crop_crop(Grid, R0, C0, R1, C1, Cropped) :-
% Delegate to private helper.
    grid_crop_crop_(Grid, R0, C0, R1, C1, Cropped).

% grid_crop_pad_to(+Grid, +BgColor, +TargetH, +TargetW, -Padded)
...
% grid_crop_center_in(+Grid, +BgColor, +TargetH, +TargetW, -Centered)
...
% grid_crop_expand_right(+Grid, +N, +Color, -Expanded)
