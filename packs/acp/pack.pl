% State the fact: name(acp).
name(acp).
% State the fact: version('1.0.0').
version('1.0.0').
% State the fact: title('PrologAI ACP Gateway — Agent Communication Protocol REST endpoint (PR 47)').
title('PrologAI ACP Gateway — Agent Communication Protocol REST endpoint (PR 47)').
