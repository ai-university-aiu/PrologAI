/*  PrologAI — ACP Gateway  (Specification PR 47)
    ...
    Predicates:
        pai_acp_start/1          — +Port: start the ACP HTTP listener
        pai_acp_stop/0           — stop the ACP HTTP listener
        pai_acp_run/4            — +Task, +Mode, -RunId, -Artifact
        pai_acp_status/2         — +RunId, -Status
        pai_acp_cancel/1         — +RunId: cancel a pending or running run
        pai_acp_agent_description/1  — -Desc: the ACP agent description term
*/

% Declare this file as the 'acp' module and list its exported predicates.
:- module(acp, [
    % Supply 'pai_acp_start/1' as the next argument to the expression above.
    pai_acp_start/1,
    % Supply 'pai_acp_stop/0' as the next argument to the expression above.
    pai_acp_stop/0,
    % Supply 'pai_acp_run/4' as the next argument to the expression above.
    pai_acp_run/4,
    % Supply 'pai_acp_status/2' as the next argument to the expression above.
    pai_acp_status/2,
    % Supply 'pai_acp_cancel/1' as the next argument to the expression above.
    pai_acp_cancel/1,
    % Supply 'pai_acp_agent_description/1' as the next argument to the expression above.
    pai_acp_agent_description/1
...
% Declare 'acp_run_record/4' as dynamic: RunId, Task, Status, Artifact.
:- dynamic acp_run_record/4.
% Declare 'acp_active_port/1' as dynamic.
:- dynamic acp_active_port/1.
% Declare 'acp_capability/1' as dynamic: registered capability atoms.
:- dynamic acp_capability/1.

% Register the core PrologAI capabilities by default.
:- assertz(acp_capability(lattice_query)).
% Register the actor management capability.
:- assertz(acp_capability(actor_list)).
% Register the assessment capability.
:- assertz(acp_capability(assess_all)).
% Register the reasoning capability.
:- assertz(acp_capability(reasoning)).

% ---------------------------------------------------------------------------
% pai_acp_start/1 — start the ACP HTTP listener on Port
% ---------------------------------------------------------------------------
...
pai_acp_start(Port) :-
    ...
    ;   pai_acp_stop,
        ...
        http_handler(root(runs), acp_handle_runs, [prefix]),
        % Register the well-known agent description endpoint.
        http_handler(root('.well-known'('agent.json')), acp_handle_agent_json, []),
        % Start the HTTP server on the given port.
        http_server(http_dispatch, [port(Port)]),
        % Remove any stale port record.
        retractall(acp_active_port(_)),
        % Record the active port.
        assertz(acp_active_port(Port))
    ).
...
% ---------------------------------------------------------------------------
% pai_acp_run/4 — create and execute an ACP run synchronously
%
...
pai_acp_run(Task, Mode, RunId, Artifact) :-
    ...
    ;   retract(acp_run_record(RunId, Task, created, none)),
        ...
        assertz(acp_run_record(RunId, Task, failed, veto(constitutional_gate))),
...
acp_execute_run(RunId, Task, _Mode, Artifact) :-
...
acp_dispatch(task(Skill, Input), Result) :-
    ...
    ;   throw(error(unknown_acp_skill(Skill), acp_dispatch/2))
    ).

% Define a clause for 'acp dispatch': handle an atom task body (raw input).
acp_dispatch(Input, Result) :-
...
% ---------------------------------------------------------------------------
% pai_acp_status/2 — query the status of an existing run
% ---------------------------------------------------------------------------

% Define a clause for 'pai acp status': look up the current status of a run.
pai_acp_status(RunId, Status) :-
    % Look up the run record.
    ( acp_run_record(RunId, _, Status, _)
...
pai_acp_agent_description(description(
    ...
    findall(C, acp_capability(C), Caps).

% ---------------------------------------------------------------------------
% acp_constitutional_check/1 — gate the task through the constitutional module
% ---------------------------------------------------------------------------
...
acp_route(get, Request) :-
    ...
    ;   findall(Id, acp_run_record(Id, _, _, _), Ids),
...
acp_handle_agent_json(_Request) :-
