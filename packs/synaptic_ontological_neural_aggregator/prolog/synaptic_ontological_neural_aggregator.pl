/*  PrologAI — SONA: Synaptic Ontological Neural Aggregator  (PR 11)
    ...
    synaptic_ontological_neural_aggregator_absorb/1    — submit a trajectory for clustering and integration;
    ...
    synaptic_ontological_neural_aggregator_retrieve/2  — retrieve the K most relevant past trajectories for a
                       given situation pattern (cued recall / pattern
                       completion).

    synaptic_ontological_neural_aggregator_metrics/1   — return a snapshot of SONA's operational metrics.

    synaptic_ontological_neural_aggregator_crystallize/1 — compress the ReasoningBank: forget the
    ...
    outcomes differ, both are stored separately so synaptic_ontological_neural_aggregator_retrieve can return
...
:- module(synaptic_ontological_neural_aggregator, [
    % Continue the multi-line expression started above.
    synaptic_ontological_neural_aggregator_absorb/1,         % +Trajectory
    % Continue the multi-line expression started above.
    synaptic_ontological_neural_aggregator_retrieve/2,       % +SituationPattern, -Trajectories  (K defaults to 5)
    % Continue the multi-line expression started above.
    synaptic_ontological_neural_aggregator_retrieve/3,       % +SituationPattern, +K, -Trajectories
    % Continue the multi-line expression started above.
    synaptic_ontological_neural_aggregator_metrics/1,        % -Metrics
    % Continue the multi-line expression started above.
    synaptic_ontological_neural_aggregator_crystallize/1     % +Options
...
% Declare 'synaptic_ontological_neural_aggregator_trajectory_entry/6' as dynamic — its facts may be added or removed at runtime.
:- dynamic synaptic_ontological_neural_aggregator_trajectory_entry/6.
%  synaptic_ontological_neural_aggregator_trajectory_entry(+EntryId, +SituationId, +ActionSequence,
%                        +Outcome, +Reward, +Timestamp)

% Declare 'synaptic_ontological_neural_aggregator_importance/2.       % EntryId, Importance (0.0–1.0)' as dynamic — its facts may be added or removed at runtime.
:- dynamic synaptic_ontological_neural_aggregator_importance/2.       % EntryId, Importance (0.0–1.0)
% Declare 'synaptic_ontological_neural_aggregator_retrieval_count/2.  % EntryId, Count' as dynamic — its facts may be added or removed at runtime.
:- dynamic synaptic_ontological_neural_aggregator_retrieval_count/2.  % EntryId, Count
% Declare 'synaptic_ontological_neural_aggregator_consolidation_cycle/1' as dynamic — its facts may be added or removed at runtime.
:- dynamic synaptic_ontological_neural_aggregator_consolidation_cycle/1.
% State the fact: sona consolidation cycle(0).
synaptic_ontological_neural_aggregator_consolidation_cycle(0).

% Declare 'synaptic_ontological_neural_aggregator_last_crystallize_time/1' as dynamic — its facts may be added or removed at runtime.
:- dynamic synaptic_ontological_neural_aggregator_last_crystallize_time/1.
% State the fact: sona last crystallize time(0.0).
synaptic_ontological_neural_aggregator_last_crystallize_time(0.0).

% Declare 'synaptic_ontological_neural_aggregator_trajectory_id_counter/1' as dynamic — its facts may be added or removed at runtime.
:- dynamic synaptic_ontological_neural_aggregator_trajectory_id_counter/1.
% State the fact: sona trajectory id counter(0).
synaptic_ontological_neural_aggregator_trajectory_id_counter(0).
...
synaptic_ontological_neural_aggregator_next_entry_id(Id) :-
    % Remove a single matching fact or rule from the runtime knowledge base.
    retract(synaptic_ontological_neural_aggregator_trajectory_id_counter(N)),
    % Evaluate the arithmetic expression 'N + 1' and bind the result to 'N1'.
    N1 is N + 1,
    % Add a new fact or rule to the runtime knowledge base.
    assertz(synaptic_ontological_neural_aggregator_trajectory_id_counter(N1)),
    % Check that 'Id' is unifiable with 'N1'.
    Id = N1.

% ---------------------------------------------------------------------------
% synaptic_ontological_neural_aggregator_trajectory_fingerprint/2 — hash-project a trajectory's situation+actions
% ---------------------------------------------------------------------------

% Define a clause for 'trajectory fingerprint': succeed when the following conditions hold.
synaptic_ontological_neural_aggregator_trajectory_fingerprint(trajectory(SitId, Actions, _, _, _), Vec) :-
...
% ---------------------------------------------------------------------------
% synaptic_ontological_neural_aggregator_absorb/1
% ---------------------------------------------------------------------------

% Define a clause for 'sona absorb': succeed when the following conditions hold.
synaptic_ontological_neural_aggregator_absorb(Trajectory) :-
    ...
    synaptic_ontological_neural_aggregator_trajectory_fingerprint(Trajectory, NewVec),
    % State a fact for 'nb getval' with the arguments listed below.
    ...
    findall(Eid-EVec-EOut, (
        % Continue the multi-line expression started above.
        synaptic_ontological_neural_aggregator_trajectory_entry(Eid, ESitId, EActions, EOut, _, _),
        % Continue the multi-line expression started above.
        synaptic_ontological_neural_aggregator_trajectory_fingerprint(
            % Continue the multi-line expression started above.
            trajectory(ESitId, EActions, EOut, _, _), EVec)
    % Continue the multi-line expression started above.
    ), Existing),
    % Pattern-separation check: is there a near-duplicate?
    % Execute: ( synaptic_ontological_neural_aggregator_near_duplicate(NewVec, Outcome, SepThresh, Existing).
    ( synaptic_ontological_neural_aggregator_near_duplicate(NewVec, Outcome, SepThresh, Existing)
    ...
    ;   % Store as new entry (may be a separated near-duplicate with diff outcome)
        % Continue the multi-line expression started above.
        synaptic_ontological_neural_aggregator_next_entry_id(Eid),
        % Continue the multi-line expression started above.
        assertz(synaptic_ontological_neural_aggregator_trajectory_entry(Eid, SitId, Actions, Outcome, Reward, Timestamp)),
        % EWC++ importance: initialise from reward magnitude
        % Continue the multi-line expression started above.
        BaseImportance is min(1.0, max(0.0, abs(Reward))),
        % Continue the multi-line expression started above.
        assertz(synaptic_ontological_neural_aggregator_importance(Eid, BaseImportance)),
        % Continue the multi-line expression started above.
        assertz(synaptic_ontological_neural_aggregator_retrieval_count(Eid, 0)),
...
%  synaptic_ontological_neural_aggregator_near_duplicate(+NewVec, +NewOutcome, +Threshold, +Existing) succeeds
...
synaptic_ontological_neural_aggregator_near_duplicate(NewVec, NewOutcome, Threshold, Existing) :-
...
% ---------------------------------------------------------------------------
% synaptic_ontological_neural_aggregator_retrieve/2 and synaptic_ontological_neural_aggregator_retrieve/3
% ---------------------------------------------------------------------------

% Define a clause for 'sona retrieve': succeed when the following conditions hold.
synaptic_ontological_neural_aggregator_retrieve(SituationPattern, Trajectories) :-
    % State the fact: sona retrieve(SituationPattern, 5, Trajectories).
    synaptic_ontological_neural_aggregator_retrieve(SituationPattern, 5, Trajectories).

% Define a clause for 'sona retrieve': succeed when the following conditions hold.
synaptic_ontological_neural_aggregator_retrieve(SituationPattern, K, Trajectories) :-
    % State a fact for 'nb getval' with the arguments listed below.
    nb_getval(sona_vec_dim, Dim),
    % State a fact for 'term to atom' with the arguments listed below.
    ...
    findall(Score-T, (
        % Continue the multi-line expression started above.
        synaptic_ontological_neural_aggregator_trajectory_entry(Eid, SitId, Actions, Outcome, Reward, Ts),
        % Continue the multi-line expression started above.
        synaptic_ontological_neural_aggregator_trajectory_fingerprint(
            % Continue the multi-line expression started above.
        ...
        % Continue the multi-line expression started above.
        ( synaptic_ontological_neural_aggregator_importance(Eid, Imp) -> true ; Imp = 0.0 ),
        % Continue the multi-line expression started above.
    ...
    synaptic_ontological_neural_aggregator_take_k(K, Desc, TopK),
    ...
    maplist([_-trajectory(SitId, Actions, Outcome, _, _)]>>(
        % Continue the multi-line expression started above.
        ( synaptic_ontological_neural_aggregator_trajectory_entry(Eid2, SitId, Actions, Outcome, _, _),
          % Continue the multi-line expression started above.
          retract(synaptic_ontological_neural_aggregator_retrieval_count(Eid2, C))
        % If the condition above succeeded, perform the following action.
        -> C1 is C + 1,
           % Continue the multi-line expression started above.
           assertz(synaptic_ontological_neural_aggregator_retrieval_count(Eid2, C1)),
           % Continue the multi-line expression started above.
           synaptic_ontological_neural_aggregator_update_importance(Eid2)
        % Otherwise (else branch), perform the following action.
        ;  true )
    % Continue the multi-line expression started above.
    ), TopK),
...
synaptic_ontological_neural_aggregator_take_k(K, List, Result) :-
...
synaptic_ontological_neural_aggregator_update_importance(Eid) :-
    % Execute: ( synaptic_ontological_neural_aggregator_retrieval_count(Eid, C),.
    ( synaptic_ontological_neural_aggregator_retrieval_count(Eid, C),
      % Continue the multi-line expression started above.
      synaptic_ontological_neural_aggregator_importance(Eid, OldImp)
    % If the condition above succeeded, perform the following action.
    ->  Boost is min(1.0, OldImp + C * 0.01),
        % Continue the multi-line expression started above.
        retract(synaptic_ontological_neural_aggregator_importance(Eid, OldImp)),
        % Continue the multi-line expression started above.
        assertz(synaptic_ontological_neural_aggregator_importance(Eid, Boost))
    % Otherwise (else branch), perform the following action.
    ;   true
    % Close the expression opened above.
    ).
...
% ---------------------------------------------------------------------------
% synaptic_ontological_neural_aggregator_metrics/1
% ---------------------------------------------------------------------------

% Define a clause for 'sona metrics': succeed when the following conditions hold.
synaptic_ontological_neural_aggregator_metrics(Metrics) :-
    % Aggregate solutions using 'count' and bind the result to a single value.
    aggregate_all(count, synaptic_ontological_neural_aggregator_trajectory_entry(_, _, _, _, _, _), TrajCount),
    % Check that '( synaptic_ontological_neural_aggregator_consolidation_cycle(CC) -> true ; CC' is unifiable with '0 )'.
    ( synaptic_ontological_neural_aggregator_consolidation_cycle(CC) -> true ; CC = 0 ),
    % Check that '( synaptic_ontological_neural_aggregator_last_crystallize_time(LCT) -> true ; LCT' is unifiable with '0.0 )'.
    ( synaptic_ontological_neural_aggregator_last_crystallize_time(LCT) -> true ; LCT = 0.0 ),
    ...
    ->  aggregate_all(max(Ts),
                      % Continue the multi-line expression started above.
                      synaptic_ontological_neural_aggregator_trajectory_entry(_, _, _, _, _, Ts),
    ...
    % Check that 'Metrics' is unifiable with 'synaptic_ontological_neural_aggregator_metrics{'.
    Metrics = synaptic_ontological_neural_aggregator_metrics{
...
synaptic_ontological_neural_aggregator_crystallize(Options) :-
    % State a fact for 'option value' with the arguments listed below.
    synaptic_ontological_neural_aggregator_option_value(min_trajectory_count(MinN), Options, 0),
    % State a fact for 'option value' with the arguments listed below.
    synaptic_ontological_neural_aggregator_option_value(compression_target(Frac),   Options, 0.5),
    % State a fact for 'option value' with the arguments listed below.
    synaptic_ontological_neural_aggregator_option_value(domain_filter(Domain),      Options, any),
    % Count current entries
    % Aggregate solutions using 'count' and bind the result to a single value.
    aggregate_all(count, synaptic_ontological_neural_aggregator_trajectory_entry(_, _, _, _, _, _), Total),
    ...
    ;   % Determine how many to evict (low-importance, not guarded)
        ...
        % Continue the multi-line expression started above.
        synaptic_ontological_neural_aggregator_evict_lowest(EvictCount, Domain),
        % Inscribe crystallized_pattern node_facts for survivors
        % Continue the multi-line expression started above.
        synaptic_ontological_neural_aggregator_inscribe_crystallized(Domain),
        % Bump consolidation cycle counter
        % Continue the multi-line expression started above.
        retract(synaptic_ontological_neural_aggregator_consolidation_cycle(C)),
        % Continue the multi-line expression started above.
        C1 is C + 1,
        % Continue the multi-line expression started above.
        assertz(synaptic_ontological_neural_aggregator_consolidation_cycle(C1)),
        % Continue the multi-line expression started above.
        get_time(T),
        % Continue the multi-line expression started above.
        retractall(synaptic_ontological_neural_aggregator_last_crystallize_time(_)),
        % Continue the multi-line expression started above.
        assertz(synaptic_ontological_neural_aggregator_last_crystallize_time(T))
    % Close the expression opened above.
    ).

% Define a clause for 'option value': succeed when the following conditions hold.
synaptic_ontological_neural_aggregator_option_value(Term, Options, _Default) :-
    % State a fact for 'memberchk' with the arguments listed below.
    memberchk(Term, Options), !.
% Define a clause for 'option value': succeed when the following conditions hold.
synaptic_ontological_neural_aggregator_option_value(Term, _Options, Default) :-
    % Execute: Term =.. [_|[Default|_]], !..
    Term =.. [_|[Default|_]], !.
% State the fact: option value(_, _, _).
synaptic_ontological_neural_aggregator_option_value(_, _, _).

% Define a clause for 'evict lowest': succeed when the following conditions hold.
synaptic_ontological_neural_aggregator_evict_lowest(N, Domain) :-
    % State a fact for 'nb getval' with the arguments listed below.
    ...
    findall(Imp-Eid, (
        % Continue the multi-line expression started above.
        synaptic_ontological_neural_aggregator_trajectory_entry(Eid, SitId, _, _, _, _),
        % Continue the multi-line expression started above.
        ( Domain == any
        % If the condition above succeeded, perform the following action.
        -> true
    ...
    maplist([_-Eid]>>(
        % Continue the multi-line expression started above.
        retractall(synaptic_ontological_neural_aggregator_trajectory_entry(Eid, _, _, _, _, _)),
        % Continue the multi-line expression started above.
        retractall(synaptic_ontological_neural_aggregator_importance(Eid, _)),
        % Continue the multi-line expression started above.
        retractall(synaptic_ontological_neural_aggregator_retrieval_count(Eid, _))
    % Continue the multi-line expression started above.
    ), ToEvict).
...
synaptic_ontological_neural_aggregator_inscribe_crystallized(Domain) :-
    ...
    findall(SitId-Actions-Outcome, (
        % Continue the multi-line expression started above.
        synaptic_ontological_neural_aggregator_trajectory_entry(_, SitId, Actions, Outcome, _, _),
        % Continue the multi-line expression started above.
        ( Domain == any
        % If the condition above succeeded, perform the following action.
        -> true
