...
:- module(signal, [
    ...
    signal_argmin/2,
    signal_diffs/2,
    signal_slope_signs/2,
    signal_is_increasing/1,
    signal_is_decreasing/1,
    signal_is_nondecreasing/1,
    signal_is_nonincreasing/1,
    signal_threshold/4,
    signal_row_profile/3,
    signal_col_profile/3,
    signal_cross_profile/5
]).
...
% signal_peaks(+List, -PeakIdxs): PeakIdxs is the sorted list of 0-based indices
...
% signal_valleys(+List, -ValleyIdxs): ValleyIdxs is the sorted list of 0-based
...
signal_valleys(List, ValleyIdxs) :-
% Compute the range of valid interior indices (1 through length-2).
    length(List, N),
    N2 is N - 2,
    findall(I, (
...
% signal_argmax(+List, -Idx): Idx is the 0-based index of the maximum value in
% List. On ties the first (leftmost) occurrence wins. Fails if List is empty.
signal_argmax(List, Idx) :-
% Find the maximum value first, then locate its first index.
    max_list(List, Max),
    nth0(Idx, List, Max),
    !.

% signal_argmin(+List, -Idx): Idx is the 0-based index of the minimum value in
% List. On ties the first (leftmost) occurrence wins. Fails if List is empty.
signal_argmin(List, Idx) :-
% Find the minimum value first, then locate its first index.
    min_list(List, Min),
    nth0(Idx, List, Min),
    !.

% signal_diffs(+List, -Diffs): Diffs is the list of consecutive element-wise
% differences: Diffs[i] = List[i+1] - List[i]. Length of Diffs is
% length(List) - 1. Returns [] for lists of length 0 or 1.
signal_diffs([], []) :- !.
signal_diffs([_], []) :- !.
signal_diffs([A, B | Rest], [D | Ds]) :-
% Compute the difference for the current pair.
    D is B - A,
    signal_diffs([B | Rest], Ds).

% signal_sign_: compute the sign of an integer: 1 for positive, -1 for negative, 0 for zero.
signal_sign_(D, S) :-
    (D > 0 -> S = 1 ; D < 0 -> S = -1 ; S = 0).

% signal_slope_signs(+List, -Signs): Signs is the list of slope signs for
% consecutive element pairs. Signs[i] is 1 if List[i+1] > List[i],
% -1 if List[i+1] < List[i], and 0 if they are equal.
signal_slope_signs(List, Signs) :-
% Compute differences and then map each to its sign.
    signal_diffs(List, Diffs),
    maplist(signal_sign_, Diffs, Signs).

% signal_is_increasing(+List): succeed if List is strictly increasing.
% Each element must be strictly less than the next. Vacuously true for
% lists of length 0 or 1.
signal_is_increasing([]) :- !.
signal_is_increasing([_]) :- !.
signal_is_increasing([A, B | Rest]) :-
    A < B,
    signal_is_increasing([B | Rest]).

% signal_is_decreasing(+List): succeed if List is strictly decreasing.
% Each element must be strictly greater than the next.
signal_is_decreasing([]) :- !.
signal_is_decreasing([_]) :- !.
signal_is_decreasing([A, B | Rest]) :-
    A > B,
    signal_is_decreasing([B | Rest]).

% signal_is_nondecreasing(+List): succeed if List is non-strictly increasing.
% Each element must be less than or equal to the next.
signal_is_nondecreasing([]) :- !.
signal_is_nondecreasing([_]) :- !.
signal_is_nondecreasing([A, B | Rest]) :-
    A =< B,
    signal_is_nondecreasing([B | Rest]).

% signal_is_nonincreasing(+List): succeed if List is non-strictly decreasing.
% Each element must be greater than or equal to the next.
signal_is_nonincreasing([]) :- !.
signal_is_nonincreasing([_]) :- !.
signal_is_nonincreasing([A, B | Rest]) :-
    A >= B,
    signal_is_nonincreasing([B | Rest]).

% signal_threshold(+List, +T, -AboveIdxs, -BelowOrEqIdxs): AboveIdxs is the
% sorted list of 0-based indices where List[i] > T; BelowOrEqIdxs is the
% sorted list of indices where List[i] =< T.
signal_threshold(List, T, AboveIdxs, BelowOrEqIdxs) :-
    length(List, N),
    N1 is N - 1,
...
% signal_row_profile(+Grid, +R, -Values): Values is the list of cell values in
% row R (0-based) of Grid.
signal_row_profile(Grid, R, Values) :-
    nth0(R, Grid, Values).

% signal_col_profile(+Grid, +C, -Values): Values is the list of cell values in
% column C (0-based) of Grid, from row 0 to the last row.
signal_col_profile(Grid, C, Values) :-
% Extract the Cth element from each row.
    maplist([Row, V]>>(nth0(C, Row, V)), Grid, Values).

% signal_cross_profile(+Grid, +R, +C, -RowVals, -ColVals): RowVals is the list
...
signal_cross_profile(Grid, R, C, RowVals, ColVals) :-
    signal_row_profile(Grid, R, RowVals),
    signal_col_profile(Grid, C, ColVals).
