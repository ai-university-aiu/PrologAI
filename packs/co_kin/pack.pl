% State the fact: this pack is named co_kin.
name(co_kin).
% State the fact: this is version 1.0.0.
version('1.0.0').
% State the fact: the title carries the work-package number.
title('PrologAI Causalontology Theory of Mind — model another agent: infer its goal from the targets its moves approach, predict its next step, and represent a false belief that differs from the true state (WP-418)').
