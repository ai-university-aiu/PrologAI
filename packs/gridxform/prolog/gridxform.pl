:- module(gridxform, [
    gridxform_rotate90/2,
    gridxform_rotate180/2,
    gridxform_rotate270/2,
    gridxform_flip_h/2,
    gridxform_flip_v/2,
    gridxform_transpose/2,
    gridxform_flip_d2/2,
    gridxform_crop/6,
    gridxform_crop_content/3,
    gridxform_pad/7,
    gridxform_scale/3,
    gridxform_tile/4,
    gridxform_d4_group/2,
    gridxform_normalize/2
]).
...
gridxform_build_(NewH, NewW, Goal, Grid) :-
...
% gridxform_rotate90(+Grid, -Result)
...
% Build W x H result; map each (R,C) to original row H1-C, col R.
    gridxform_build_(W, H,
        [R, C, V]>>(OR is H1 - C, gridxform_cell_(Grid, OR, R, V)),
        Result).

% gridxform_rotate180(+Grid, -Result)
...
% Build same-size result; flip both row and column indices.
    gridxform_build_(H, W,
        [R, C, V]>>(OR is H1 - R, OC is W1 - C, gridxform_cell_(Grid, OR, OC, V)),
        Result).

% gridxform_rotate270(+Grid, -Result)
...
% Build W x H result; map each (R,C) to original row C, col W1-R.
    gridxform_build_(W, H,
        [R, C, V]>>(OC is W1 - R, gridxform_cell_(Grid, C, OC, V)),
        Result).

% --- REFLECTIONS ---

% gridxform_flip_h(+Grid, -Result)
...
% Build same-size result; mirror each column index.
    gridxform_build_(H, W,
        [R, C, V]>>(OC is W1 - C, gridxform_cell_(Grid, R, OC, V)),
        Result).

% gridxform_flip_v(+Grid, -Result)
...
% Build same-size result; mirror each row index.
    gridxform_build_(H, W,
        [R, C, V]>>(OR is H1 - R, gridxform_cell_(Grid, OR, C, V)),
        Result).

% gridxform_transpose(+Grid, -Result)
...
% Build W x H result; swap row and column in the lookup.
    gridxform_build_(W, H,
        [R, C, V]>>gridxform_cell_(Grid, C, R, V),
        Result).

% gridxform_flip_d2(+Grid, -Result)
...
% Build W x H result; map each (R,C) to original row H1-C, col W1-R.
    gridxform_build_(W, H,
        [R, C, V]>>(OR is H1 - C, OC is W1 - R, gridxform_cell_(Grid, OR, OC, V)),
        Result).

% --- CROP AND PAD ---

% gridxform_crop(+Grid, +R0, +C0, +R1, +C1, -Result)
...
% Build output; each (R,C) maps to original (R0+R, C0+C).
    gridxform_build_(NewH, NewW,
        [R, C, V]>>(OR is R0 + R, OC is C0 + C, gridxform_cell_(Grid, OR, OC, V)),
        Result).

% gridxform_crop_content(+Grid, +BgColor, -Result)
...
% Collect all (R,C) positions whose cell value differs from BgColor.
    findall(R-C,
        (between(0, H1, R), between(0, W1, C),
         gridxform_cell_(Grid, R, C, V), V \= BgColor),
        Cells),
...
% Crop to the tight bounding box.
        gridxform_crop(Grid, MinR, MinC, MaxR, MaxC, Result)
    ).

% gridxform_pad(+Grid, +Top, +Bot, +Left, +Right, +BgColor, -Result)
...
% Build scaled grid: divide (R,C) by N to find originating cell.
    gridxform_build_(NewH, NewW,
        [R, C, V]>>(OR is R // N, OC is C // N, gridxform_cell_(Grid, OR, OC, V)),
        Result).

% gridxform_tile(+Grid, +H, +W, -Result)
% Tile Grid to fill an H x W output. Grid is the repeating unit.
% result[R][C] = original[R mod TileH][C mod TileW].
gridxform_tile(Grid, H, W, Result) :-
% Get tile dimensions.
    gridxform_dims_(Grid, TH, TW),
% Build tiled output using modular indexing into the tile.
    gridxform_build_(H, W,
        [R, C, V]>>(OR is R mod TH, OC is C mod TW, gridxform_cell_(Grid, OR, OC, V)),
        Result).

% --- D4 SYMMETRY GROUP ---

% gridxform_d4_group(+Grid, -Transforms)
