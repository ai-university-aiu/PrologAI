...
% --- contrast_profile_diff ---

test('AC-CA-005: profile_diff returns empty for same profile') :-
    g_11(G), contrast_feature_profile(G, P),
    contrast_profile_diff(P, P, Diff),
    Diff = [].

test('AC-CA-006: profile_diff finds changed feature') :-
    g_11(G1), g_22(G2),
    contrast_feature_profile(G1, P1), contrast_feature_profile(G2, P2),
    contrast_profile_diff(P1, P2, Diff),
    Diff \= [].

% --- contrast_pairwise_delta ---

test('AC-CA-007: pairwise_delta for single swap pair') :-
    swap_pair_a(P),
    contrast_pairwise_delta([P], Deltas),
...
test('AC-CA-009: pairwise_delta for two pairs returns two deltas') :-
    ...
    contrast_pairwise_delta([A, B], Deltas),
    length(Deltas, 2).
...
test('AC-CA-023: rank_features returns list for non-empty pairs') :-
    swap_pair_a(A), swap_pair_b(B),
    contrast_rank_features([A, B], Ranked),
    is_list(Ranked).
...
test('AC-CA-028: separates returns score between 0 and 1') :-
    swap_pair_a(A), swap_pair_b(B),
    contrast_separates([A, B], feat(dims, 2-2), [A, B], Score),
    Score >= 0.0, Score =< 1.0.

test('AC-CA-029: separates returns 0 when feature not in any pair') :-
    swap_pair_a(A),
    contrast_separates([A], feat(nonexistent, xyz), [A], Score),
    Score =:= 0.0.

% --- contrast_minimal_features ---

test('AC-CA-030: minimal_features returns list') :-
    swap_pair_a(A), swap_pair_b(B),
    contrast_minimal_features([A, B], Feats),
    is_list(Feats).
...
test('AC-CA-032: pairwise_delta records correct changed cell') :-
    ...
    swap_pair_a(P),
    contrast_pairwise_delta([P], [delta(1, Changes)]),
...
test('AC-CA-034: stable_features and unstable_features are disjoint') :-
    swap_pair_a(A), swap_pair_b(B),
    contrast_stable_features([A, B], Stable),
    contrast_unstable_features([A, B], Unstable),
...
test('AC-CA-040: pairwise_delta indexes start at 1') :-
    ...
    contrast_pairwise_delta([A, S], [delta(1,_), delta(2,_)]).
...
test('AC-CA-042: correlated_features result contains feat terms') :-
    swap_pair_a(A), swap_pair_b(B),
    contrast_correlated_features([A, B], any, Corr),
...
test('AC-CA-043: context_gate from two swap pairs is a feature') :-
    swap_pair_a(A), swap_pair_b(B),
    contrast_context_gate([A, B], feat(Name, _)),
    atom(Name).
...
test('AC-CA-044: full contrastive pipeline on two symmetric pairs') :-
    ...
    length(Deltas, 3),
    contrast_stable_features(Pairs, Stable),
    member(feat(dims, 2-2), Stable),
    contrast_covarying_features(Pairs, _Covar),
