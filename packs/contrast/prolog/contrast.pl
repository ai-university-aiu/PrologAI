...
% contrast_grid_dims_/3: get grid dimensions.
contrast_grid_dims_(Grid, Rows, Cols) :-
    length(Grid, Rows),
    (Grid = [Row|_] -> length(Row, Cols) ; Cols = 0).

% contrast_grid_colors_/3: sorted non-bg colors in a grid.
contrast_grid_colors_(Grid, BgColor, Colors) :-
    findall(V, (member(Row, Grid), member(V, Row), V \= BgColor), All),
    sort(All, Colors).

% contrast_cell_changes_/3: list of chg(R,C,OldV,NewV) for every changed cell.
contrast_cell_changes_(InGrid, OutGrid, Changes) :-
...
% contrast_dominant_color_/2: most frequent non-0 color in a grid.
contrast_dominant_color_(Grid, Color) :-
    findall(V, (member(Row, Grid), member(V, Row), V \= 0), All),
    msort(All, Sorted),
    contrast_run_length_(Sorted, Runs),
    msort(Runs, SRuns),
    last(SRuns, _-Color).

% contrast_run_length_/2: compute N-Color pairs for color frequency.
contrast_run_length_([], []).
contrast_run_length_([H|T], [N-H|Rest]) :-
    include(=(H), [H|T], Same),
    subtract([H|T], Same, Others),
    length(Same, N),
    contrast_run_length_(Others, Rest).

% contrast_apply_hyp_/3: apply a hypothesis (color map) to InGrid; succeed if result = OutGrid.
contrast_apply_hyp_(InGrid, Map, OutGrid) :-
    length(InGrid, Rows), Rows1 is Rows - 1,
    numlist(0, Rows1, RowIdxs),
...
% contrast_pairwise_delta(+Pairs, -Deltas)
...
contrast_pairwise_delta(Pairs, Deltas) :-
    length(Pairs, N),
    numlist(1, N, Idxs),
    maplist(contrast_delta_for_pair_, Idxs, Pairs, Deltas).

contrast_delta_for_pair_(Idx, pair(In, Out), delta(Idx, Changes)) :-
    contrast_cell_changes_(In, Out, Changes).

% contrast_covarying_features(+Pairs, -Features)
...
contrast_covarying_features([], []).
contrast_covarying_features(Pairs, Features) :-
    % Classify pairs as changing (output differs from input) or stable.
    include(contrast_pair_changes_, Pairs, ChangingPairs),
    subtract(Pairs, ChangingPairs, StablePairs),
    (ChangingPairs = [] -> Features = [] ;
        % Get feature profiles for all changing inputs.
        maplist(contrast_input_profile_, ChangingPairs, ChangingProfiles),
        % Find features present in ALL changing pairs.
        contrast_common_features_(ChangingProfiles, CommonChanging),
        % Subtract features also present in stable pairs.
        (StablePairs = [] ->
            Features = CommonChanging
        ;
            maplist(contrast_input_profile_, StablePairs, StableProfiles),
            contrast_union_features_(StableProfiles, StableFeats),
            subtract(CommonChanging, StableFeats, Features)
        )
    ).

% contrast_pair_changes_/1: succeed if the pair has at least one changed cell.
contrast_pair_changes_(pair(In, Out)) :-
    contrast_cell_changes_(In, Out, Changes),
    Changes \= [].

% contrast_input_profile_/2: get feature profile of the input grid.
contrast_input_profile_(pair(In, _), Profile) :-
    contrast_feature_profile(In, Profile).

% contrast_common_features_/2: features present in ALL profiles.
contrast_common_features_([], []).
contrast_common_features_([P|Rest], Common) :-
    (Rest = [] ->
        Common = P
    ;
        contrast_common_features_(Rest, RestCommon),
        include(contrast_in_profile_(P), RestCommon, Common)
    ).

% contrast_in_profile_/2: succeed if feat F is in Profile.
contrast_in_profile_(Profile, F) :- member(F, Profile).

% contrast_union_features_/2: all unique features across all profiles.
contrast_union_features_(Profiles, Union) :-
    findall(F, (member(P, Profiles), member(F, P)), All),
    sort(All, Union).

% contrast_context_gate(+Pairs, -GateFeature)
...
% Returns the first feature for which contrast_separates/4 succeeds.
contrast_context_gate(Pairs, GateFeature) :-
    contrast_rank_features(Pairs, Ranked),
...
% contrast_discriminating_pair(+Pairs, +Hyp1, +Hyp2, -Pair)
...
contrast_discriminating_pair([P|Rest], Hyp1, Hyp2, DiscPair) :-
    P = pair(In, Out),
    contrast_cell_changes_(In, Out, Changes),
    (contrast_explains_(Changes, Hyp1, In) \= contrast_explains_(Changes, Hyp2, In) ->
        DiscPair = P
    ;
        contrast_discriminating_pair(Rest, Hyp1, Hyp2, DiscPair)
    ).

% contrast_explains_/3: succeed if applying Map to In explains the Changes.
contrast_explains_(Changes, Map, In) :-
    findall(ok,
...
% contrast_correlated_features(+Pairs, +OutputChange, -Features)
...
contrast_correlated_features(Pairs, OutputChange, Features) :-
    include(contrast_has_output_change_(OutputChange), Pairs, Matching),
    (Matching = [] ->
        Features = []
    ;
        maplist(contrast_input_profile_, Matching, Profiles),
        contrast_common_features_(Profiles, Features)
    ).

% contrast_has_output_change_/2: succeed if the pair contains the given change pattern.
contrast_has_output_change_(OutputChange, pair(In, Out)) :-
    contrast_cell_changes_(In, Out, Changes),
...
% contrast_change_count(+Pairs, +Feature, -N)
% N is the number of pairs where Feature appears in the input profile AND
% the output differs from the input.
contrast_change_count(Pairs, Feature, N) :-
    include(contrast_pair_changes_, Pairs, ChangingPairs),
    include(contrast_has_feature_(Feature), ChangingPairs, Matching),
    length(Matching, N).

% contrast_has_feature_/2: succeed if Feature appears in the input profile.
contrast_has_feature_(Feature, pair(In, _)) :-
    contrast_feature_profile(In, Profile),
    member(Feature, Profile).

% contrast_common_context(+Pairs, +OutputChange, -Context)
...
contrast_common_context(Pairs, OutputChange, Context) :-
    contrast_correlated_features(Pairs, OutputChange, Context).

% contrast_separates(+Pairs, +Hyp, +PositivePairs, -Result)
...
contrast_separates(Pairs, Feature, PositivePairs, Result) :-
    include(contrast_has_feature_(Feature), Pairs, WithFeature),
    include(contrast_has_feature_(Feature), PositivePairs, TP),
    subtract(WithFeature, PositivePairs, FP),
    length(TP, TPN), length(FP, FPN),
    (TPN + FPN =:= 0 -> Result = 0.0 ; Result is TPN / (TPN + FPN)).

% contrast_minimal_features(+Pairs, -Features)
...
% contrast_feature_profile(+Grid, -Profile)
...
contrast_feature_profile(Grid, Profile) :-
    contrast_grid_dims_(Grid, Rows, Cols),
    contrast_grid_colors_(Grid, 0, Colors),
    length(Colors, ColorCount),
    (Colors = [] -> DomColor = 0 ;
        (contrast_dominant_color_(Grid, DomColor) -> true ; Colors = [DomColor|_])),
    sort([feat(dims, Rows-Cols),
          feat(color_count, ColorCount),
          feat(dominant_color, DomColor)],
         Profile).

% contrast_profile_diff(+Profile1, +Profile2, -Changed)
...
contrast_profile_diff(Profile1, Profile2, Changed) :-
    include(contrast_not_in_profile2_(Profile2), Profile1, Changed).

% contrast_not_in_profile2_/2: helper for include.
contrast_not_in_profile2_(Profile2, feat(Name, Value)) :-
    \+ member(feat(Name, Value), Profile2).

% contrast_stable_features(+Pairs, -Features)
...
contrast_stable_features([], []).
contrast_stable_features(Pairs, Features) :-
    maplist(contrast_input_profile_, Pairs, Profiles),
    contrast_common_features_(Profiles, Features).

% contrast_unstable_features(+Pairs, -Features)
...
contrast_unstable_features([], []).
contrast_unstable_features(Pairs, Features) :-
    maplist(contrast_input_profile_, Pairs, Profiles),
    contrast_union_features_(Profiles, AllFeats),
    contrast_common_features_(Profiles, Stable),
    subtract(AllFeats, Stable, Features).

% contrast_rank_features(+Pairs, -RankedFeatures)
...
contrast_rank_features([], []).
contrast_rank_features(Pairs, RankedFeatures) :-
    include(contrast_pair_changes_, Pairs, Changing),
    maplist(contrast_input_profile_, Pairs, AllProfiles),
    contrast_union_features_(AllProfiles, AllFeats),
    findall(Score-F,
        (member(F, AllFeats),
         contrast_change_count(Pairs, F, N),
