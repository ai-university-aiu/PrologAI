...
test(bbox_sq) :-
    sq_obj(O), objsym_bbox(O, 1, 1, 2, 2).

test(bbox_hbar) :-
    hbar_obj(O), objsym_bbox(O, 0, 0, 0, 2).

test(bbox_vbar) :-
    vbar_obj(O), objsym_bbox(O, 0, 0, 2, 0).

test(bbox_plus) :-
    plus_obj(O), objsym_bbox(O, 0, 0, 2, 2).

test(bbox_dot) :-
    dot_obj(O), objsym_bbox(O, 3, 5, 3, 5).

% objsym_normalize tests.

test(normalize_sq) :-
    sq_obj(O), objsym_normalize(O, obj(s, Cells)),
    msort(Cells, S),
    msort([r(0,0), r(0,1), r(1,0), r(1,1)], Expected),
    S == Expected.

test(normalize_hbar) :-
    hbar_obj(O), objsym_normalize(O, obj(h, Cells)),
    msort(Cells, S),
    msort([r(0,0), r(0,1), r(0,2)], Expected),
    S == Expected.

test(normalize_dot) :-
    dot_obj(O), objsym_normalize(O, obj(d, Cells)),
    Cells == [r(0,0)].

test(normalize_color_preserved) :-
    l_obj(O), objsym_normalize(O, obj(Color, _)),
    Color == l.

% objsym_translate tests.

test(translate_shift) :-
    hbar_obj(O), objsym_translate(O, 2, 3, obj(h, Cells)),
    msort(Cells, S),
    msort([r(2,3), r(2,4), r(2,5)], Expected),
    S == Expected.

test(translate_negative) :-
    sq_obj(O), objsym_translate(O, -1, -1, obj(s, Cells)),
    msort(Cells, S),
    msort([r(0,0), r(0,1), r(1,0), r(1,1)], Expected),
    S == Expected.

test(translate_zero) :-
    vbar_obj(O), objsym_translate(O, 0, 0, O).

% objsym_reflect_h tests.

test(reflect_h_hbar) :-
    hbar_obj(O), objsym_reflect_h(O, obj(h, Cells)),
    msort(Cells, S),
...
test(reflect_h_sq) :-
    sq_obj(O), objsym_reflect_h(O, obj(s, Cells)),
    msort(Cells, S),
...
test(reflect_h_color_preserved) :-
    plus_obj(O), objsym_reflect_h(O, obj(Color, _)),
    Color == p.

% objsym_reflect_v tests.

test(reflect_v_vbar) :-
    vbar_obj(O), objsym_reflect_v(O, obj(v, Cells)),
    msort(Cells, S),
...
test(reflect_v_sq) :-
    sq_obj(O), objsym_reflect_v(O, obj(s, Cells)),
    msort(Cells, S),
...
% objsym_rotate180 tests.

test(rotate180_sq) :-
    sq_obj(O), objsym_rotate180(O, obj(s, Cells)),
    msort(Cells, S),
...
test(rotate180_l) :-
    l_obj(O), objsym_rotate180(O, obj(l, Cells)),
    msort(Cells, S),
...
test(rotate180_hbar) :-
    hbar_obj(O), objsym_rotate180(O, obj(h, Cells)),
    msort(Cells, S),
...
% objsym_rotate90 tests.

test(rotate90_single_cell) :-
    dot_obj(O), objsym_normalize(O, N), objsym_rotate90(N, obj(d, Cells)),
    Cells == [r(0,0)].

test(rotate90_hbar_becomes_vbar) :-
    hbar_obj(O), objsym_rotate90(O, obj(h, Cells)),
    msort(Cells, S),
...
test(rotate90_vbar_becomes_hbar) :-
    vbar_obj(O), objsym_rotate90(O, obj(v, Cells)),
    msort(Cells, S),
...
test(rotate90_plus_invariant) :-
    plus_obj(O), objsym_normalize(O, N), objsym_rotate90(N, obj(p, Cells)),
    msort(Cells, S),
    % Plus is rot90 symmetric; rotated cells should match normalized original.
    N = obj(_, NCells), msort(NCells, S0),
    S == S0.

% objsym_is_hsymm tests.

test(is_hsymm_hbar) :-
    hbar_obj(O), objsym_is_hsymm(O).

test(is_hsymm_sq) :-
    sq_obj(O), objsym_is_hsymm(O).

test(is_hsymm_plus) :-
    plus_obj(O), objsym_is_hsymm(O).

test(is_hsymm_diamond) :-
    diamond_obj(O), objsym_is_hsymm(O).

test(not_hsymm_l) :-
    l_obj(O), \+ objsym_is_hsymm(O).

test(not_hsymm_z) :-
    z_obj(O), \+ objsym_is_hsymm(O).

% objsym_is_vsymm tests.

test(is_vsymm_vbar) :-
    vbar_obj(O), objsym_is_vsymm(O).

test(is_vsymm_sq) :-
    sq_obj(O), objsym_is_vsymm(O).

test(is_vsymm_plus) :-
    plus_obj(O), objsym_is_vsymm(O).

test(not_vsymm_t) :-
    t_obj(O), \+ objsym_is_vsymm(O).

test(not_vsymm_l) :-
    l_obj(O), \+ objsym_is_vsymm(O).

% objsym_is_rot180 tests.

test(is_rot180_sq) :-
    sq_obj(O), objsym_is_rot180(O).

test(is_rot180_plus) :-
    plus_obj(O), objsym_is_rot180(O).

test(is_rot180_z) :-
    z_obj(O), objsym_is_rot180(O).

test(is_rot180_hbar) :-
    hbar_obj(O), objsym_is_rot180(O).

test(not_rot180_l) :-
    l_obj(O), \+ objsym_is_rot180(O).

test(not_rot180_t) :-
    t_obj(O), \+ objsym_is_rot180(O).

% objsym_is_rot90 tests.

test(is_rot90_plus) :-
    plus_obj(O), objsym_is_rot90(O).

test(is_rot90_sq) :-
    sq_obj(O), objsym_is_rot90(O).

test(is_rot90_dot) :-
    dot_obj(O), objsym_is_rot90(O).

test(is_rot90_diamond) :-
    diamond_obj(O), objsym_is_rot90(O).

test(not_rot90_hbar) :-
    hbar_obj(O), \+ objsym_is_rot90(O).

test(not_rot90_l) :-
    l_obj(O), \+ objsym_is_rot90(O).

% objsym_has_symmetry tests.

test(has_symmetry_plus) :-
    plus_obj(O), objsym_has_symmetry(O).

test(has_symmetry_z) :-
    z_obj(O), objsym_has_symmetry(O).

test(has_symmetry_t) :-
    t_obj(O), objsym_has_symmetry(O).

test(not_has_symmetry_l) :-
    l_obj(O), \+ objsym_has_symmetry(O).

% objsym_symmetries tests.

test(symmetries_plus_all) :-
    plus_obj(O), objsym_symmetries(O, List),
    msort(List, S),
    S == [h, rot180, rot90, v].

test(symmetries_z_rot180_only) :-
    z_obj(O), objsym_symmetries(O, List),
    List == [rot180].

test(symmetries_t_h_only) :-
    t_obj(O), objsym_symmetries(O, List),
    List == [h].

test(symmetries_l_empty) :-
    l_obj(O), objsym_symmetries(O, List),
    List == [].

test(symmetries_sq_all) :-
    sq_obj(O), objsym_symmetries(O, List),
    msort(List, S),
    S == [h, rot180, rot90, v].

% objsym_equivalent tests.
...
test(equivalent_hbar_vbar) :-
    ...
    objsym_equivalent(O1, O2).

test(equivalent_self) :-
    l_obj(O), objsym_equivalent(O, O).

test(not_equivalent_l_z) :-
    l_obj(O1), z_obj(O2), \+ objsym_equivalent(O1, O2).

test(equivalent_l_rotated) :-
    % An L rotated 90 degrees is equivalent to the original L.
    l_obj(O), objsym_normalize(O, N), objsym_rotate90(N, R), objsym_normalize(R, O2),
    objsym_equivalent(O, O2).

test(not_equivalent_plus_l) :-
    plus_obj(O1), l_obj(O2), \+ objsym_equivalent(O1, O2).
