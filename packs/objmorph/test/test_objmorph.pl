...
test(boundary4_2x2_all_boundary) :-
    ...
    om_boundary4(obj(c,Cells), obj(c,B)),
    length(B, 4).
...
test(interior4_2x2_empty) :-
    ...
    square_cells(2, Cells),
    om_interior4(obj(c,Cells), obj(c,[])).
...
test(boundary8_3x3_all_boundary) :-
    ...
    square_cells(3, Cells),
    om_boundary8(obj(b,Cells), obj(b,B)),
    length(B, 8).
...
test(interior8_3x3_empty) :-
    ...
    square_cells(3, Cells),
    om_interior8(obj(b,Cells), obj(b,I)),
...
test(dilate4_preserves_color) :-
    om_dilate4(obj(blue,[r(1,1)]), R),
    R = obj(blue, _).

test(dilate4_contains_original) :-
    om_dilate4(obj(x,[r(2,3)]), obj(x,D)),
    memberchk(r(2,3), D).
...
test(dilate4_n0_identity) :-
    % N=0: identity.
    om_dilate4_n(obj(red,[r(0,0)]), 0, R),
    R == obj(red,[r(0,0)]).

test(dilate4_n1_same_as_dilate4) :-
    % N=1: same as om_dilate4.
    om_dilate4(obj(a,[r(0,0)]), D1),
    om_dilate4_n(obj(a,[r(0,0)]), 1, DN),
    D1 == DN.

test(dilate4_n2_count) :-
    % N=2 from single cell: diamond of radius 2 (13 cells).
    om_dilate4_n(obj(x,[r(0,0)]), 2, obj(x,D)),
    % Cells with |R|+|C| <= 2.
    length(D, 13).
...
test(erode4_n0_identity) :-
    % N=0: identity.
    square_cells(5, Cells),
    om_erode4_n(obj(b,Cells), 0, R),
    R == obj(b,Cells).

test(erode4_n1_same_as_erode4) :-
    % N=1: same as om_erode4.
    square_cells(4, Cells),
    om_erode4(obj(b,Cells), E1),
    om_erode4_n(obj(b,Cells), 1, EN),
    E1 == EN.

test(erode4_n2_from_5x5) :-
    % 5x5 eroded twice: first erode to 3x3 interior, then to single center.
    square_cells(5, Cells),
    om_erode4_n(obj(b,Cells), 2, obj(b,E)),
    E == [r(2,2)].
