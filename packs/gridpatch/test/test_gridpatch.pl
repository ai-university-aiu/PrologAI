...
% AC-GPT-008: overlay with no transparent cells = same as gpt_place.
test(overlay_no_transparency) :-
    base4(G), p2x2(P),
    gpt_overlay(G, P, 1, 1, z, R1),
    gpt_place(G, P, 1, 1, R2),
    R1 = R2.
...
test(w_2x2) :- p2x2(P), gpt_w(P, W), W =:= 2.

% AC-GPT-027: width of 4x4 checker.
test(w_4x4) :- checker(G), gpt_w(G, W), W =:= 4.

:- end_tests(gpt_w).

:- begin_tests(gpt_size).

% AC-GPT-028: size of 2x2 patch.
test(size_2x2) :- p2x2(P), gpt_size(P, H, W), H =:= 2, W =:= 2.

% AC-GPT-029: size of 4x4 checker.
test(size_4x4) :- checker(G), gpt_size(G, H, W), H =:= 4, W =:= 4.
...
test(extract_then_place) :-
    checker(G),
    gpt_extract(G, 0, 0, 1, 1, Patch),
    base4(Blank),
    gpt_place(Blank, Patch, 0, 0, R),
...
test(overlay_then_extract) :-
    base4(G), p2x2(P),
    gpt_overlay(G, P, 1, 1, z, R),
    gpt_extract(R, 1, 1, 2, 2, Q),
    Q = [[x,x],[x,x]].
...
test(inpaint_then_place) :-
    checker(G),
    gpt_extract(G, 0, 0, 1, 1, Patch),
    gpt_inpaint(G, 0, 0, 1, 1, b, Blank),
    gpt_place(Blank, Patch, 0, 0, Restored),
    nth0(0, Restored, R0), nth0(0, R0, x).

% AC-GPT-042: scatter then count.
test(scatter_then_count) :-
    base4(G), p1x1(P),
    gpt_scatter(G, P, [0-0, 1-1, 2-2, 3-3], R),
    gpt_count(R, [[m]], N), N =:= 4.

% AC-GPT-043: replace_first reduces the match count.
test(replace_first_reduces_count) :-
    checker(G),
    gpt_count(G, [[x,b],[b,x]], N0),
    gpt_replace_first(G, [[x,b],[b,x]], [[b,b],[b,b]], R),
    gpt_count(R, [[x,b],[b,x]], N1),
    N1 < N0.

% AC-GPT-044: gpt_size and tile_fill agree on dimensions.
test(size_tile_agree) :-
    Patch = [[a,b],[c,d]],
    gpt_tile_fill(Patch, 6, 6, G),
    gpt_size(G, H, W), H =:= 6, W =:= 6.
