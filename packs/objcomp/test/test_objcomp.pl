...
test(touches_ab) :-
    a_obj(A), b_obj(B), oc_touches(A, B).

test(touches_ad) :-
    a_obj(A), d_obj(D), oc_touches(A, D).

test(touches_bf) :-
    b_obj(B), f_obj(F), oc_touches(B, F).

test(touches_cf) :-
    c_obj(C), f_obj(F), oc_touches(C, F).

test(not_touches_ac) :-
    a_obj(A), c_obj(C), \+ oc_touches(A, C).

test(not_touches_ae) :-
    a_obj(A), e_obj(E), \+ oc_touches(A, E).

test(not_touches_bc) :-
    b_obj(B), c_obj(C), \+ oc_touches(B, C).

test(touches_symmetric) :-
    a_obj(A), b_obj(B),
    oc_touches(A, B), oc_touches(B, A).

% oc_touching_pairs tests.
...
test(touching_pairs_empty) :-
    e_obj(E), oc_touching_pairs([E], Pairs), Pairs == [].
...
test(isolated_empty_when_all_touch) :-
    a_obj(A), b_obj(B),
    oc_isolated([A, B], Isolated),
    Isolated == [].

test(isolated_all_when_single) :-
    a_obj(A),
    oc_isolated([A], Isolated),
    Isolated == [A].
...
test(num_components_2) :-
    ...
    objs_abcdef(Objs),
    oc_num_components(Objs, N), N == 2.

test(components_sizes) :-
    objs_abcdef(Objs),
    oc_components(Objs, Comps),
    findall(N, (member(C, Comps), length(C, N)), Sizes),
    sort(Sizes, Sorted),
    Sorted == [1, 5].

test(num_components_single_obj) :-
    a_obj(A), oc_num_components([A], N), N == 1.

test(num_components_no_touching) :-
    a_obj(A), e_obj(E),
    oc_num_components([A, E], N), N == 2.

% oc_largest_component tests.

test(largest_component_5) :-
    a_obj(A), b_obj(B), c_obj(C), d_obj(D), f_obj(F),
    objs_abcdef(Objs),
    oc_largest_component(Objs, Comp),
    sort(Comp, Sorted),
    sort([A,B,C,D,F], Expected),
    Sorted == Expected.

test(largest_component_single_list) :-
    a_obj(A), oc_largest_component([A], Comp),
    Comp == [A].

% oc_smallest_component tests.

test(smallest_component_e) :-
    e_obj(E), objs_abcdef(Objs),
    oc_smallest_component(Objs, Comp),
    Comp == [E].

% oc_singleton_components tests.
...
test(singletons_empty_when_all_touching) :-
    a_obj(A), b_obj(B),
    oc_singleton_components([A, B], Singles),
    Singles == [].

% oc_shared_components tests.
...
test(shared_empty_all_isolated) :-
    a_obj(A), e_obj(E),
    oc_shared_components([A, E], Groups),
    Groups == [].
...
test(max_degree_abcdef) :-
    ...
    objs_abcdef(Objs),
    oc_max_degree(Objs, MaxD),
    MaxD == 2.

test(max_degree_single_obj) :-
    a_obj(A), oc_max_degree([A], MaxD), MaxD == 0.

% oc_sort_by_degree tests.
