...
% objcomp_touches tests.

test(touches_ab) :-
    a_obj(A), b_obj(B), objcomp_touches(A, B).

test(touches_ad) :-
    a_obj(A), d_obj(D), objcomp_touches(A, D).

test(touches_bf) :-
    b_obj(B), f_obj(F), objcomp_touches(B, F).

test(touches_cf) :-
    c_obj(C), f_obj(F), objcomp_touches(C, F).

test(not_touches_ac) :-
    a_obj(A), c_obj(C), \+ objcomp_touches(A, C).

test(not_touches_ae) :-
    a_obj(A), e_obj(E), \+ objcomp_touches(A, E).

test(not_touches_bc) :-
    b_obj(B), c_obj(C), \+ objcomp_touches(B, C).

test(touches_symmetric) :-
    a_obj(A), b_obj(B),
    objcomp_touches(A, B), objcomp_touches(B, A).

% objcomp_touching_pairs tests.
...
test(touching_pairs_empty) :-
    e_obj(E), objcomp_touching_pairs([E], Pairs), Pairs == [].

% objcomp_adj_list tests.
...
% objcomp_degree tests.

test(degree_a) :-
    % A touches B and D -> degree 2.
    a_obj(A), objs_abcdef(Objs),
    objcomp_degree(A, Objs, D), D == 2.

test(degree_e_zero) :-
    e_obj(E), objs_abcdef(Objs),
    objcomp_degree(E, Objs, D), D == 0.

test(degree_f) :-
    % F touches B and C -> degree 2.
    f_obj(F), objs_abcdef(Objs),
    objcomp_degree(F, Objs, D), D == 2.

test(degree_c) :-
    % C touches only F -> degree 1.
    c_obj(C), objs_abcdef(Objs),
    objcomp_degree(C, Objs, D), D == 1.

% objcomp_isolated tests.
...
test(connected_from_c) :-
    ...
    a_obj(A), b_obj(B), c_obj(C), d_obj(D), f_obj(F),
    objs_abcdef(Objs),
    objcomp_connected(C, Objs, Comp),
    sort(Comp, Sorted),
    sort([A,B,C,D,F], Expected),
    Sorted == Expected.

% objcomp_components tests.

test(num_components_2) :-
    % Two components: {A,B,C,D,F} and {E}.
    objs_abcdef(Objs),
    objcomp_num_components(Objs, N), N == 2.

test(components_sizes) :-
    objs_abcdef(Objs),
    objcomp_components(Objs, Comps),
    findall(N, (member(C, Comps), length(C, N)), Sizes),
    sort(Sizes, Sorted),
    Sorted == [1, 5].

test(num_components_single_obj) :-
    a_obj(A), objcomp_num_components([A], N), N == 1.

test(num_components_no_touching) :-
    a_obj(A), e_obj(E),
    objcomp_num_components([A, E], N), N == 2.

% objcomp_largest_component tests.

test(largest_component_5) :-
    a_obj(A), b_obj(B), c_obj(C), d_obj(D), f_obj(F),
    objs_abcdef(Objs),
    objcomp_largest_component(Objs, Comp),
    sort(Comp, Sorted),
    sort([A,B,C,D,F], Expected),
    Sorted == Expected.

test(largest_component_single_list) :-
    a_obj(A), objcomp_largest_component([A], Comp),
    Comp == [A].

% objcomp_smallest_component tests.

test(smallest_component_e) :-
    e_obj(E), objs_abcdef(Objs),
    objcomp_smallest_component(Objs, Comp),
    Comp == [E].

% objcomp_singleton_components tests.
...
test(singletons_empty_when_all_touching) :-
    a_obj(A), b_obj(B),
    objcomp_singleton_components([A, B], Singles),
    Singles == [].

% objcomp_shared_components tests.
...
test(shared_empty_all_isolated) :-
    a_obj(A), e_obj(E),
    objcomp_shared_components([A, E], Groups),
    Groups == [].

% objcomp_max_degree tests.

test(max_degree_abcdef) :-
    % Max degree: A has 2 (B,D), B has 2 (A,F), F has 2 (B,C). Max = 2.
    objs_abcdef(Objs),
    objcomp_max_degree(Objs, MaxD),
    MaxD == 2.

test(max_degree_single_obj) :-
    a_obj(A), objcomp_max_degree([A], MaxD), MaxD == 0.

% objcomp_sort_by_degree tests.
