...
test(left_yes) :-
    block_a(A),
    block_c(C),
    relation_left_of(A, C).

test(left_no, [fail]) :-
    block_a(A),
    block_c(C),
    relation_left_of(C, A).
...
test(right_yes) :-
    block_a(A),
    block_c(C),
    relation_right_of(C, A).

test(right_no, [fail]) :-
    block_a(A),
    block_c(C),
    relation_right_of(A, C).
...
test(adjacent_yes) :-
    block_a(A),
    block_adj(Adj),
    relation_adjacent(A, Adj).

test(adjacent_symmetric) :-
    block_a(A),
    block_adj(Adj),
    relation_adjacent(Adj, A).
...
test(contained_yes) :-
    inner(I),
    outer(O),
    relation_contained_bbox(I, O).

test(contained_self) :-
    block_a(A),
    relation_contained_bbox(A, A).

test(contained_no, [fail]) :-
    block_a(A),
    block_c(C),
    relation_contained_bbox(A, C).
...
test(same_col_yes) :-
    bar_v(V),
    A = [r(5,2)],
    relation_same_col(V, A).

test(same_col_no, [fail]) :-
    block_a(A),
    block_c(C),
    relation_same_col(A, C).
