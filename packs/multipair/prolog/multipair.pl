...
% mp_grid_colors_/3: sorted non-bg colors in a grid.
mp_grid_colors_(Grid, BgColor, Colors) :-
    findall(V, (member(Row, Grid), member(V, Row), V \= BgColor), All),
    sort(All, Colors).

% mp_pair_input_colors_/3: colors in the input grid of a pair.
mp_pair_input_colors_(pair(In, _), BgColor, Colors) :-
    mp_grid_colors_(In, BgColor, Colors).

% mp_pair_output_colors_/3: colors in the output grid of a pair.
mp_pair_output_colors_(pair(_, Out), BgColor, Colors) :-
    mp_grid_colors_(Out, BgColor, Colors).

% mp_color_in_pair_/3: succeed if Color appears in the input of a pair.
mp_color_in_pair_(Color, BgColor, pair(In, _)) :-
    mp_grid_colors_(In, BgColor, Colors),
    member(Color, Colors).

% mp_count_pairs_with_color_/4: count pairs that have Color in their input.
mp_count_pairs_with_color_(Pairs, BgColor, Color, N) :-
    include(mp_color_in_pair_(Color, BgColor), Pairs, Matching),
    length(Matching, N).

% mp_grid_cell_count_/3: count cells of a specific color in a grid.
mp_grid_cell_count_(Grid, Color, N) :-
    findall(_, (member(Row, Grid), member(Color, Row)), Cells),
    length(Cells, N).

% --- PUBLIC PREDICATES ---

% mp_track_objects(+Pairs, -TrackedObjects)
...
mp_track_objects(Pairs, TrackedObjects) :-
    mp_all_input_objects(Pairs, AllObjects),
...
mp_all_input_objects(Pairs, AllObjects) :-
    ...
    findall(po(Idx, Color),
        (member(Idx, Idxs),
         ...
         mp_grid_colors_(In, 0, Colors),
         member(Color, Colors)),
        AllObjects).

% mp_invariant_objects(+TrackedObjects, -Objects)
...
% mp_role_objects(+TrackedObjects, -ContentObjs, -MarkerObjs)
...
mp_role_objects(TrackedObjects, ContentObjs, MarkerObjs) :-
    mp_invariant_objects(TrackedObjects, ContentObjs),
    subtract(TrackedObjects, ContentObjs, MarkerObjs).

% mp_cross_pair_match(+Pairs, -MatchMatrix)
...
mp_cross_pair_match(Pairs, MatchMatrix) :-
    ...
    findall(match(I, J, Common),
        (member(I, Idxs), member(J, Idxs), I < J,
         ...
         mp_grid_colors_(InI, 0, CI),
         mp_grid_colors_(InJ, 0, CJ),
         findall(C, (member(C, CI), member(C, CJ)), Common0),
         sort(Common0, Common)),
        MatchMatrix).

% mp_color_frequency(+Pairs, +Color, -Frequency)
% Frequency is the number of training pair inputs that contain Color.
mp_color_frequency(Pairs, Color, Frequency) :-
    mp_count_pairs_with_color_(Pairs, 0, Color, Frequency).

% mp_universal_colors(+Pairs, -Colors)
% Colors is the list of non-bg colors that appear in EVERY training pair input.
mp_universal_colors([], []).
mp_universal_colors(Pairs, Colors) :-
    Pairs \= [],
    findall(CL, (member(P, Pairs), mp_pair_input_colors_(P, 0, CL)), ColorLists),
    mp_list_intersection_(ColorLists, Colors).

% mp_list_intersection_/2: intersection of a list of lists.
mp_list_intersection_([], []).
mp_list_intersection_([L], L).
mp_list_intersection_([L|Rest], Intersection) :-
    Rest \= [],
    mp_list_intersection_(Rest, RestIntersection),
    findall(X, (member(X, L), member(X, RestIntersection)), Raw),
    sort(Raw, Intersection).

% mp_variable_colors(+Pairs, -Colors)
...
mp_variable_colors(Pairs, Colors) :-
    ...
    mp_list_intersection_(ColorLists, Universal),
    subtract(AllColors, Universal, Colors).

% mp_disappeared_objects(+Pairs, -Colors)
% Colors is the sorted list of colors that appear in every input but no output.
mp_disappeared_objects(Pairs, Colors) :-
    mp_universal_colors(Pairs, Universal),
    findall(C,
        (member(C, Universal),
         \+ (member(pair(_, Out), Pairs),
             mp_grid_colors_(Out, 0, OutColors),
             member(C, OutColors))),
        Raw),
    sort(Raw, Colors).

% mp_appeared_objects(+Pairs, -Colors)
% Colors is the sorted list of colors that appear in outputs but in NO input.
mp_appeared_objects(Pairs, Colors) :-
    findall(C, (member(P, Pairs), mp_pair_output_colors_(P, 0, OC), member(C, OC)), OutRaw),
    sort(OutRaw, AllOutColors),
    findall(C, (member(P, Pairs), mp_pair_input_colors_(P, 0, IC), member(C, IC)), InRaw),
    sort(InRaw, AllInColors),
    subtract(AllOutColors, AllInColors, Colors).

% mp_stable_color_objects(+Pairs, -Colors)
...
mp_stable_color_objects(Pairs, Colors) :-
    mp_universal_colors(Pairs, Universal),
    include([Color]>>(mp_stable_count_(Pairs, Color)), Universal, Colors).

% mp_stable_count_/2: succeed if Color has same cell count in all inputs.
mp_stable_count_(Pairs, Color) :-
    findall(N, (member(pair(In, _), Pairs),
                mp_grid_cell_count_(In, Color, N)), Counts),
    sort(Counts, Sorted),
    Sorted = [_].

% mp_modal_object_count(+Pairs, -Count)
...
mp_modal_object_count(Pairs, Count) :-
    ...
    (Counts = [] -> Count = 0 ;
        msort(Counts, Sorted),
        mp_mode_(Sorted, Count)
    ).

% mp_mode_/2: find the most frequent element in a sorted list.
mp_mode_(Sorted, Mode) :-
...
% mp_singleton_color(+Pairs, -Color)
