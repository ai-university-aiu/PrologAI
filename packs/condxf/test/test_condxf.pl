...
test(split_color_basic) :-
    ...
    condxf_split_color([R, B, RB], r, Match, NoMatch),
    Match == [R, RB],
    NoMatch == [B].

test(split_color_none_match) :-
    red_dot(R), blue_dot(B),
    condxf_split_color([R, B], g, Match, NoMatch),
    Match == [],
    NoMatch == [R, B].

test(split_color_all_match) :-
    red_dot(R), red_bar(RB),
    condxf_split_color([R, RB], r, Match, NoMatch),
    Match == [R, RB],
    NoMatch == [].
...
test(swap_colors_basic) :-
    red_dot(R), blue_dot(B),
    condxf_swap_colors([R, B], r, b, [obj(b,[r(0,0)]), obj(r,[r(0,1)])]).

test(swap_colors_no_effect_other) :-
    green_dot(G),
    condxf_swap_colors([G], r, b, [G]).

test(swap_colors_single) :-
    red_dot(R),
    condxf_swap_colors([R], r, g, [obj(g,[r(0,0)])]).
...
test(move_color_basic) :-
    red_dot(R), blue_dot(B),
    condxf_move_color([R, B], r, 1, 1, Out),
    Out == [obj(r,[r(1,1)]), B].
...
test(recolor_unique_size_basic) :-
    ...
    condxf_recolor_unique_size([R, B, L], g, Out),
...
test(split_size_gt) :-
    red_dot(R), red_bar(RB),
    condxf_split_size([R, RB], 1, gt, Match, NoMatch),
    Match == [RB],
    NoMatch == [R].

test(split_size_eq) :-
    red_dot(R), blue_dot(B), red_bar(RB),
    condxf_split_size([R, B, RB], 1, eq, Match, NoMatch),
    Match == [R, B],
    NoMatch == [RB].

test(split_size_le) :-
    red_dot(R), lshape(L), red_bar(RB),
    condxf_split_size([R, L, RB], 3, le, Match, NoMatch),
    Match == [R, L, RB],
    NoMatch == [].
...
test(recolor_matching_all) :-
    red_dot(R), red_bar(RB),
    condxf_recolor_matching([R, RB], r, g, Out),
    Out == [obj(g,[r(0,0)]), obj(g,[r(0,0),r(0,1),r(0,2)])].

test(shift_color_all_match) :-
    red_dot(R), red_bar(RB),
    condxf_shift_color([R, RB], r, 1, 0, Out),
...
test(recolor_unique_size_no_unique) :-
    ...
    red_dot(R), blue_dot(B),
    \+ condxf_recolor_unique_size([R, B], g, _).

:- end_tests(condxf).

:- begin_tests(condxf_infer_gate).
...
test(infer_gate_finds_a_gate) :-
    ...
    condxf_infer_gate([PA, PB, PC, PD], [], gate_color(3)).
...
test(infer_gate_returns_gate_color_term) :-
    ...
    condxf_infer_gate([PA, PB, PC, PD], [], Gate),
    Gate = gate_color(_).
