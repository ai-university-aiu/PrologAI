...
% Shifting cols by 1 twice = shifting by 2.
    g3x3_cols(G),
    gper_shift_wrap_cols(G, 1, S1),
    gper_shift_wrap_cols(S1, 1, S2),
    gper_shift_wrap_cols(G, 2, S3),
    S2 = S3.

test(tile_grid_1x1_to_2x2) :-
% Tile [[b]] to 2x2 gives [[b,b],[b,b]].
    gper_tile_grid([[b]], 2, 2, G),
    G = [[b,b],[b,b]].

test(row_period_two_identical_rows) :-
% Two identical rows: row period 1.
    gper_row_period([[r,b],[r,b]], P),
    P =:= 1.
