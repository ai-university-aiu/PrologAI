...
:- module(extrema, [
    extrema_max_val/2, extrema_min_val/2,
    extrema_max_cells/2, extrema_min_cells/2,
    extrema_row_argmax/2, extrema_col_argmax/2,
    extrema_row_argmin/2, extrema_col_argmin/2,
    extrema_local_max4/2, extrema_local_min4/2,
    extrema_above/3, extrema_below/3,
    extrema_range/2, extrema_nonzero/2
]).
...
% extrema_max_val(+Grid, -Val): Val is the maximum value across all cells in Grid.
extrema_max_val(Grid, Val) :-
% Collect all cell values, then find the maximum.
    extrema_flat_vals_(Grid, Vals),
    max_list(Vals, Val).

% extrema_min_val(+Grid, -Val): Val is the minimum value across all cells in Grid.
extrema_min_val(Grid, Val) :-
% Collect all cell values, then find the minimum.
    extrema_flat_vals_(Grid, Vals),
    min_list(Vals, Val).

% extrema_max_cells(+Grid, -Cells): Cells is the sorted list of R-C positions whose
% value equals the grid maximum.
extrema_max_cells(Grid, Cells) :-
...
% extrema_min_cells(+Grid, -Cells): Cells is the sorted list of R-C positions whose
% value equals the grid minimum.
extrema_min_cells(Grid, Cells) :-
...
% extrema_row_argmax(+Grid, -R): R is the 0-based index of the row with the maximum
% sum. On ties, the first (lowest-index) row wins.
extrema_row_argmax(Grid, R) :-
% Collect sum-index pairs for each row, then pick the index with maximum sum.
    ...
    extrema_argmax_idx_(Pairs, R).

% extrema_col_argmax(+Grid, -C): C is the 0-based index of the column with the maximum
% sum. On ties, the first (lowest-index) column wins.
extrema_col_argmax(Grid, C) :-
% Collect sum-index pairs for each column, then pick the index with maximum sum.
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
    findall(S-I, (between(0, W1, I), extrema_col_sum_(Grid, I, S)), Pairs),
    extrema_argmax_idx_(Pairs, C).

% extrema_row_argmin(+Grid, -R): R is the 0-based index of the row with the minimum
% sum. On ties, the first row wins.
extrema_row_argmin(Grid, R) :-
% Collect sum-index pairs for each row, then pick the index with minimum sum.
    length(Grid, H), H1 is H - 1,
    findall(S-I, (between(0, H1, I), nth0(I, Grid, Row), sum_list(Row, S)), Pairs),
    extrema_argmin_idx_(Pairs, R).

% extrema_col_argmin(+Grid, -C): C is the 0-based index of the column with the minimum
% sum. On ties, the first column wins.
extrema_col_argmin(Grid, C) :-
% Collect sum-index pairs for each column, then pick the index with minimum sum.
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
    findall(S-I, (between(0, W1, I), extrema_col_sum_(Grid, I, S)), Pairs),
    extrema_argmin_idx_(Pairs, C).

% extrema_local_max4(+Grid, -Cells): Cells is the sorted list of R-C positions that
...
% For each cell, test that all in-bounds 4-neighbors are strictly less.
    ...
    findall(R-C, (
        ...
        extrema_all_nbr4_lt_(Grid, H, W, R, C, V)
    ), Cells).

% extrema_local_min4(+Grid, -Cells): Cells is the sorted list of R-C positions that
% are strictly less than every in-bounds 4-connected neighbor.
extrema_local_min4(Grid, Cells) :-
% For each cell, test that all in-bounds 4-neighbors are strictly greater.
    ...
    findall(R-C, (
        between(0, H1, R), between(0, W1, C),
        nth0(R, Grid, Row), nth0(C, Row, V),
        extrema_all_nbr4_gt_(Grid, H, W, R, C, V)
    ), Cells).

% extrema_above(+Grid, +Thresh, -Cells): Cells is the sorted list of R-C positions
% whose value is strictly greater than Thresh.
extrema_above(Grid, Thresh, Cells) :-
...
% extrema_below(+Grid, +Thresh, -Cells): Cells is the sorted list of R-C positions
% whose value is strictly less than Thresh.
extrema_below(Grid, Thresh, Cells) :-
...
% extrema_range(+Grid, -Range): Range is the maximum value minus the minimum value
% across all cells in Grid.
extrema_range(Grid, Range) :-
% Compute max and min of all cell values, then subtract.
    extrema_flat_vals_(Grid, Vals),
    max_list(Vals, Mx), min_list(Vals, Mn),
    Range is Mx - Mn.

% extrema_nonzero(+Grid, -Cells): Cells is the sorted list of R-C positions whose
% value is not arithmetically equal to 0.
extrema_nonzero(Grid, Cells) :-
...
extrema_argmax_idx_([S-I | Rest], Best) :-
    extrema_argmax_acc_(Rest, S, I, Best).

% Private: accumulator for extrema_argmax_idx_.
extrema_argmax_acc_([], _, I, I).
extrema_argmax_acc_([S2-I2 | Rest], BestS, BestI, Final) :-
% Update best if new sum is strictly greater; otherwise keep current best.
    (S2 > BestS ->
        extrema_argmax_acc_(Rest, S2, I2, Final)
    ;
        extrema_argmax_acc_(Rest, BestS, BestI, Final)
    ).

% Private: find the index I with the minimum sum S from a S-I pair list.
% Ties are broken by first occurrence (lowest I).
extrema_argmin_idx_([S-I | Rest], Best) :-
    extrema_argmin_acc_(Rest, S, I, Best).

% Private: accumulator for extrema_argmin_idx_.
extrema_argmin_acc_([], _, I, I).
extrema_argmin_acc_([S2-I2 | Rest], BestS, BestI, Final) :-
% Update best if new sum is strictly less; otherwise keep current best.
    (S2 < BestS ->
        extrema_argmin_acc_(Rest, S2, I2, Final)
    ;
        extrema_argmin_acc_(Rest, BestS, BestI, Final)
    ).
...
% Check each of the four orthogonal directions with bounds guards.
    ...
    extrema_check_nbr_lt_(Grid, H, W, RU, C, V),
    extrema_check_nbr_lt_(Grid, H, W, RD, C, V),
    extrema_check_nbr_lt_(Grid, H, W, R, CL, V),
    extrema_check_nbr_lt_(Grid, H, W, R, CR, V).

% Private: succeed iff all in-bounds 4-neighbors of R-C have value strictly > V.
extrema_all_nbr4_gt_(Grid, H, W, R, C, V) :-
% Check each of the four orthogonal directions with bounds guards.
    RU is R - 1, RD is R + 1, CL is C - 1, CR is C + 1,
    extrema_check_nbr_gt_(Grid, H, W, RU, C, V),
    extrema_check_nbr_gt_(Grid, H, W, RD, C, V),
    extrema_check_nbr_gt_(Grid, H, W, R, CL, V),
    extrema_check_nbr_gt_(Grid, H, W, R, CR, V).

% Private: if NR-NC is in bounds, require that its value < V; out-of-bounds succeeds.
extrema_check_nbr_lt_(Grid, H, W, NR, NC, V) :-
    (NR >= 0, NR < H, NC >= 0, NC < W ->
...
% Private: if NR-NC is in bounds, require that its value > V; out-of-bounds succeeds.
extrema_check_nbr_gt_(Grid, H, W, NR, NC, V) :-
    (NR >= 0, NR < H, NC >= 0, NC < W ->
        nth0(NR, Grid, NRow), nth0(NC, NRow, NV), NV > V
    ;
        true
