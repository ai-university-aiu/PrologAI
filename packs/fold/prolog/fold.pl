...
:- module(fold, [
    % fold_split_h/4: split Grid into Top (rows 0..R) and Bottom (rows R+1..H-1).
    fold_split_h/4,
    % fold_split_v/4: split Grid into Left (cols 0..C) and Right (cols C+1..W-1).
    fold_split_v/4,
    % fold_overlay/4: Out[R][C] = A[R][C] if A[R][C] != Bg, else B[R][C].
    fold_overlay/4,
    % fold_fold_h/4: fold bottom half up over top; crease after row R.
    fold_fold_h/4,
    % fold_fold_v/4: fold right half left over left; crease after col C.
    fold_fold_v/4,
    % fold_unfold_h/2: create symmetric full grid by stacking Half with reversed Half.
    fold_unfold_h/2,
    % fold_unfold_v/2: create symmetric full grid by joining Half with its L-R mirror.
    fold_unfold_v/2,
    % fold_sym_h/2: succeed if Grid is symmetric around row R.
    fold_sym_h/2,
    % fold_sym_v/2: succeed if Grid is symmetric around column C.
    fold_sym_v/2,
    % fold_find_fold_h/2: find row R around which Grid is horizontally symmetric.
    fold_find_fold_h/2,
    % fold_find_fold_v/2: find col C around which Grid is vertically symmetric.
    fold_find_fold_v/2,
    % fold_mark_row/3: R is the unique row where every cell equals V.
    fold_mark_row/3,
    % fold_mark_col/3: C is the unique column where every cell equals V.
    fold_mark_col/3,
    % fold_fold_both/5: fold at row R then fold the result at column C.
    fold_fold_both/5
]).
...
% fold_split_v(+Grid, +C, -Left, -Right): split Grid vertically at column C.
...
fold_split_v(Grid, C, Left, Right) :-
    % Column count for each left sub-row.
    C1 is C + 1,
    % Apply the row split to every row in parallel.
    maplist(fold_split_row_(C1), Grid, Left, Right).

% fold_split_row_(+C1, +Row, -LeftRow, -RightRow): split Row at column C (0-indexed).
% LeftRow has C1 = C+1 elements; RightRow has the remaining elements.
fold_split_row_(C1, Row, LeftRow, RightRow) :-
    % Bind LeftRow to exactly C1 elements.
    length(LeftRow, C1),
    % Append splits Row at that prefix boundary.
    append(LeftRow, RightRow, Row).

% fold_overlay(+A, +B, +Bg, -Out): cell-by-cell overlay of two same-size grids.
...
fold_overlay(A, B, Bg, Out) :-
    % Apply row-level overlay across both grids simultaneously.
    maplist(fold_overlay_row_(Bg), A, B, Out).

% fold_overlay_row_(+Bg, +RowA, +RowB, -RowOut): overlay two rows.
fold_overlay_row_(Bg, RowA, RowB, RowOut) :-
    % Apply cell-level overlay across both rows simultaneously.
    maplist(fold_overlay_cell_(Bg), RowA, RowB, RowOut).

% fold_overlay_cell_(+Bg, +VA, +VB, -V): A-cell wins unless it equals background.
fold_overlay_cell_(Bg, VA, _VB, VA) :-
    % Non-background A-cell takes priority.
    VA \= Bg,
    % Cut prevents the fallback clause from executing.
    !.
% fold_overlay_cell_ fallback: A is background so B-cell is used.
fold_overlay_cell_(_Bg, _VA, VB, VB).

% fold_fold_h(+Grid, +R, +Bg, -Out): fold the bottom half of Grid upward.
% Crease is after row R; Left half = rows 0..R; Bottom half = rows R+1..H-1.
% Row at position I in Out is fold_overlay(Top[I], Bottom[R-I], Bg).
...
fold_fold_h(Grid, R, Bg, Out) :-
    ...
    findall(OutRow, (
        ...
        ( K >= 0, K < HB ->
            ...
            fold_overlay_row_(Bg, TopRow, BotRow, OutRow)
        ;
            OutRow = TopRow
        )
    ), Out).

% fold_fold_v(+Grid, +C, +Bg, -Out): fold the right half of Grid leftward.
...
fold_fold_v(Grid, C, Bg, Out) :-
    ...
    maplist(fold_fold_row_v_(WL, WR, C, Bg), Left, Right, Out).

% fold_fold_row_v_(+WL, +WR, +C, +Bg, +LRow, +RRow, -OutRow): fold one row.
fold_fold_row_v_(WL, WR, C, Bg, LRow, RRow, OutRow) :-
    ...
    findall(V, (
        ...
        ( K >= 0, K < WR ->
            nth0(K, RRow, RV),
            fold_overlay_cell_(Bg, LV, RV, V)
        ;
            V = LV
        )
    ), OutRow).

% fold_unfold_h(+Half, -Grid): create a symmetric full grid from a top half.
...
% fold_unfold_v(+Half, -Grid): create a symmetric full grid from a left half.
...
fold_unfold_v(Half, Grid) :-
    ...
    maplist(fold_mirror_row_, Half, Grid).

% fold_mirror_row_(+LeftRow, -FullRow): join LeftRow with its reverse.
fold_mirror_row_(LeftRow, FullRow) :-
...
% fold_sym_h(+Grid, +R): succeed if Grid is symmetric under folding with crease after row R.
...
% This matches fold_fold_h semantics exactly: Bottom[K] = Grid[R+1+K] = Grid[2R+1-I] for K=R-I.
fold_sym_h(Grid, R) :-
...
% fold_sym_v(+Grid, +C): succeed if Grid is symmetric under folding with crease after col C.
% Mirror formula: col J reflects to col (2*C + 1 - J). This matches fold_fold_v semantics:
...
% fold_find_fold_h(+Grid, -R): find a row R around which Grid is symmetric.
% Tries R = 0, 1, ..., H-1 and returns the first R that satisfies fold_sym_h.
...
% fold_find_fold_v(+Grid, -C): find a column C around which Grid is symmetric.
% Tries C = 0, 1, ..., W-1 and returns the first C that satisfies fold_sym_v.
...
% fold_mark_row(+Grid, +V, -R): R is the first row where every cell equals V.
% Useful for identifying fold-marker rows (e.g., an all-separator row).
fold_mark_row(Grid, V, R) :-
...
% fold_mark_col(+Grid, +V, -C): C is the first column where every cell equals V.
fold_mark_col(Grid, V, C) :-
    % Get dimensions.
    ( Grid = [FR|_] -> length(FR, W) ; W = 0 ),
    % Maximum column index.
    W1 is W - 1,
...
% fold_fold_both(+Grid, +R, +C, +Bg, -Out): fold at row R then fold the result at column C.
% First applies fold_fold_h to fold the bottom half up (keeping width unchanged).
% Then applies fold_fold_v to fold the right half left on the horizontal result.
% The final output has height R+1 and width C+1.
fold_fold_both(Grid, R, C, Bg, Out) :-
    % First fold: bottom half up over top, result has R+1 rows.
    fold_fold_h(Grid, R, Bg, FoldedH),
    % Second fold: right half over left, result has C+1 cols.
    fold_fold_v(FoldedH, C, Bg, Out).
