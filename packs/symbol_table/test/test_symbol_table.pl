...
:- use_module('../prolog/symbol_table.pl').

:- begin_tests(symbol_table).
...
% --- symbol_table_color_feature ---

test('AC-ST-001: symbol_table_color_feature extracts color(r) from red object') :-
    red1(R), symbol_table_color_feature(R, F), F = color(r).

test('AC-ST-002: symbol_table_color_feature extracts color(b) from blue object') :-
    blue1(B), symbol_table_color_feature(B, F), F = color(b).

test('AC-ST-003: symbol_table_color_feature extracts color(g) from green object') :-
    green1(G), symbol_table_color_feature(G, F), F = color(g).

% --- symbol_table_size_feature ---

test('AC-ST-004: symbol_table_size_feature extracts size(1) from single-cell object') :-
    red1(R), symbol_table_size_feature(R, F), F = size(1).

test('AC-ST-005: symbol_table_size_feature extracts size(2) from two-cell object') :-
    red2(R), symbol_table_size_feature(R, F), F = size(2).

test('AC-ST-006: symbol_table_size_feature extracts size(3) from three-cell object') :-
    blue3(B), symbol_table_size_feature(B, F), F = size(3).

% --- symbol_table_hole_count ---

test('AC-ST-007: symbol_table_hole_count returns 0 for solid single cell') :-
    red1(R), symbol_table_hole_count(R, N), N =:= 0.

test('AC-ST-008: symbol_table_hole_count returns 0 for solid two-cell object') :-
    red2(R), symbol_table_hole_count(R, N), N =:= 0.

test('AC-ST-009: symbol_table_hole_count returns positive for L-shape with missing corner') :-
    hollow(H), symbol_table_hole_count(H, N), N > 0.

test('AC-ST-010: symbol_table_hole_count for 1x1 solid returns 0') :-
    O = ob(r, [r(5,5)], r0(5,5,5,5)),
    symbol_table_hole_count(O, N), N =:= 0.

% --- symbol_table_lookup ---

test('AC-ST-011: symbol_table_lookup finds first matching entry') :-
    Table = [sym(color(r), b), sym(color(b), g)],
    symbol_table_lookup(Table, color(r), V),
    V = b.

test('AC-ST-012: symbol_table_lookup finds second entry') :-
    Table = [sym(color(r), b), sym(color(b), g)],
    symbol_table_lookup(Table, color(b), V),
    V = g.

test('AC-ST-013: symbol_table_lookup fails for unknown feature') :-
    Table = [sym(color(r), b)],
    \+ symbol_table_lookup(Table, color(g), _).

% --- symbol_table_entry_consistent ---

test('AC-ST-014: entry consistent with empty table') :-
    symbol_table_entry_consistent(sym(color(r), b), []).

test('AC-ST-015: entry consistent when table maps same F to same V') :-
    symbol_table_entry_consistent(sym(color(r), b), [sym(color(r), b)]).

test('AC-ST-016: entry inconsistent when table maps F to different V') :-
    \+ symbol_table_entry_consistent(sym(color(r), b), [sym(color(r), g)]).

test('AC-ST-017: entry consistent when table maps different F') :-
    symbol_table_entry_consistent(sym(color(r), b), [sym(color(g), y)]).

% --- symbol_table_position_feature ---

test('AC-ST-018: position feature top-left quadrant') :-
    O = ob(r, [r(0,0)], r0(0,0,0,0)),
    symbol_table_position_feature(O, dims(10,10), pos(tl)).

test('AC-ST-019: position feature top-right quadrant') :-
    O = ob(r, [r(0,8)], r0(0,8,0,8)),
    symbol_table_position_feature(O, dims(10,10), pos(tr)).

test('AC-ST-020: position feature bottom-left quadrant') :-
    O = ob(r, [r(8,0)], r0(8,0,8,0)),
    symbol_table_position_feature(O, dims(10,10), pos(bl)).

test('AC-ST-021: position feature bottom-right quadrant') :-
    O = ob(r, [r(8,8)], r0(8,8,8,8)),
    symbol_table_position_feature(O, dims(10,10), pos(br)).

% --- symbol_table_is_symbol ---
...
test('AC-ST-025: candidate symbols picks smallest quartile') :-
    ...
    symbol_table_candidate_symbols([O1, O2, O3, O4], Cands),
...
test('AC-ST-027: single object always a candidate') :-
    red1(R), symbol_table_candidate_symbols([R], Cands), Cands = [R].

% --- symbol_table_apply_table ---

test('AC-ST-028: apply color table - red becomes blue') :-
    Table = [sym(color(r), b)],
    InObjs = [ob(r,[r(0,0)],r0(0,0,0,0))],
    symbol_table_apply_table(Table, InObjs, Map),
...
test('AC-ST-030: apply table to empty objs gives empty map') :-
    Table = [sym(color(r), b)],
    symbol_table_apply_table(Table, [], Map),
    Map = [].

test('AC-ST-031: empty table gives empty map') :-
    InObjs = [ob(r,[r(0,0)],r0(0,0,0,0))],
    symbol_table_apply_table([], InObjs, Map),
    Map = [].
...
% --- symbol_table_identify_symbols ---

test('AC-ST-033: identify symbols from color-covarying pair') :-
    pair1(P),
    symbol_table_identify_symbols([P], Syms),
    Syms \= [].
