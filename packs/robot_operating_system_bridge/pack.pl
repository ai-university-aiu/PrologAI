% State the fact: name(robot_operating_system_bridge).
name(robot_operating_system_bridge).
% State the fact: version('1.0.0').
version('1.0.0').
% State the fact: title('PrologAI ROS 2 Bridge — robot embodiment via the Mind-Body pattern (PR 46)').
title('PrologAI ROS 2 Bridge — robot embodiment via the Mind-Body pattern (PR 46)').
