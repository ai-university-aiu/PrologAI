...
% canvas_dims_(+Grid, -H, -W): height and width of a grid (list of rows).
canvas_dims_(Grid, H, W) :-
% Count rows.
    length(Grid, H),
...
% canvas_paint_cells_(+Grid, +Cells, +Color, -Grid2): single-pass painting.
...
canvas_paint_cells_(Grid, Cells, Color, Grid2) :-
% Rebuild each row.
    findall(Row2,
...
% canvas_erase_cells_(+Grid, +Cells, +Bg, -Grid2): single-pass erasing.
% Cells in Cells are set to Bg; all other cells are unchanged.
canvas_erase_cells_(Grid, Cells, Bg, Grid2) :-
% Rebuild each row.
    findall(Row2,
% Enumerate rows by index.
        (nth0(R, Grid, Row),
...
% canvas_blank(+H, +W, +Bg, -Grid): create an H x W grid filled with Bg.
% Grid is represented as a list of H lists each of length W.
canvas_blank(H, W, Bg, Grid) :-
...
% canvas_size(+Grid, -H, -W): return the height H and width W of a grid.
canvas_size(Grid, H, W) :-
% Delegate to private helper.
    canvas_dims_(Grid, H, W).

% canvas_paint(+Grid, +Obj, -Grid2): paint obj(Color, Cells) onto Grid.
% All cells listed in Obj are set to Color; other cells are unchanged.
canvas_paint(Grid, obj(Color, Cells), Grid2) :-
% Single-pass update for all cells of this object.
    canvas_paint_cells_(Grid, Cells, Color, Grid2).

% canvas_paint_all(+Grid, +Objs, -Grid2): paint every obj in Objs onto Grid.
% Objects are painted in list order; later objects overwrite earlier ones.
canvas_paint_all(Grid, [], Grid).
canvas_paint_all(Grid, [Obj|Rest], Grid2) :-
% Paint the first object.
    canvas_paint(Grid, Obj, Grid1),
% Continue with remaining objects.
    canvas_paint_all(Grid1, Rest, Grid2).

% canvas_paint_at(+Grid, +Obj, +DR, +DC, -Grid2): translate Obj by (DR,DC) then paint.
% Each cell r(R,C) in Obj is painted at r(R+DR, C+DC). No bounds checking.
canvas_paint_at(Grid, obj(Color, Cells), DR, DC, Grid2) :-
...
% Paint the shifted cells.
    canvas_paint_cells_(Grid, Cells2, Color, Grid2).

% canvas_paint_clip(+Grid, +Obj, -Grid2): paint Obj, silently skipping out-of-bounds cells.
% Only cells with 0 =< R < H and 0 =< C < W are painted.
canvas_paint_clip(Grid, obj(Color, Cells), Grid2) :-
...
% Paint the clipped cell set.
    canvas_paint_cells_(Grid, ClipCells, Color, Grid2).

% canvas_paint_bg(+Grid, +Obj, +Bg, -Grid2): paint Obj only onto cells currently equal to Bg.
% Cells in Obj whose current grid value is not Bg are left unchanged.
canvas_paint_bg(Grid, obj(Color, Cells), Bg, Grid2) :-
...
% Paint only the bg-valued cells.
    canvas_paint_cells_(Grid, BgCells, Color, Grid2).

% canvas_erase(+Grid, +Obj, +Bg, -Grid2): fill all cells of Obj with Bg in Grid.
canvas_erase(Grid, obj(_, Cells), Bg, Grid2) :-
% Single-pass erase for all cells of this object.
    canvas_erase_cells_(Grid, Cells, Bg, Grid2).

% canvas_extract(+Grid, +Color, -Obj): extract all Color cells from Grid as obj(Color, Cells).
% Cells is a list of r(R,C) pairs in row-major order. Obj is empty if Color absent.
canvas_extract(Grid, Color, obj(Color, Cells)) :-
...
% canvas_extract_all(+Grid, +Bg, -Objs): extract all non-Bg objects from Grid.
...
canvas_extract_all(Grid, Bg, Objs) :-
...
% canvas_render(+H, +W, +Bg, +Objs, -Grid): create a blank H x W canvas and paint Objs.
% Convenience predicate combining canvas_blank/4 and canvas_paint_all/3.
canvas_render(H, W, Bg, Objs, Grid) :-
% Create blank canvas.
    canvas_blank(H, W, Bg, Blank),
% Paint all objects onto it.
    canvas_paint_all(Blank, Objs, Grid).

% canvas_move(+Grid, +Obj, +DR, +DC, +Bg, -Grid2): erase Obj then repaint at offset (DR,DC).
% The object is removed from its original position and placed at (R+DR, C+DC).
canvas_move(Grid, Obj, DR, DC, Bg, Grid2) :-
% Erase the object from its current position.
    canvas_erase(Grid, Obj, Bg, Grid1),
% Repaint at the offset position (no clipping).
    canvas_paint_at(Grid1, Obj, DR, DC, Grid2).

% canvas_stamp(+Obj, +Bg, -Patch): extract Obj as a tight bounding-box grid.
...
canvas_stamp(obj(Color, Cells), Bg, Patch) :-
...
% Build blank patch and paint the normalised cells.
    canvas_blank(H, W, Bg, Blank),
    canvas_paint_cells_(Blank, NormCells, Color, Patch).

% canvas_blit(+Canvas, +Patch, +R0, +C0, -Grid2): paste Patch onto Canvas at top-left (R0,C0).
% Each cell Patch[r][c] overwrites Canvas[R0+r][C0+c]. No Bg transparency.
canvas_blit(Canvas, Patch, R0, C0, Grid2) :-
% Get patch dimensions for bounds checking.
    canvas_dims_(Patch, PH, PW),
    PH1 is PH - 1,
    PW1 is PW - 1,
