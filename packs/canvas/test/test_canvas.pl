...
% cv_paint_all/3 tests
test(paint_all_two_objs) :-
    cv_blank(3, 3, 0, G0),
    dot(Dot), hbar(Hbar),
    cv_paint_all(G0, [Dot, Hbar], G1),
    nth0(0, G1, [r,0,0]),
    nth0(1, G1, [b,b,b]).
...
% cv_paint_clip/3 tests
test(paint_clip_all_in_bounds) :-
    cv_blank(3, 3, 0, G0),
    dot(Dot),
    cv_paint_clip(G0, Dot, G1),
    cv_paint(G0, Dot, G2),
    G1 = G2.
...
% cv_extract_all/3 tests
test(extract_all_two_colors) :-
    cv_blank(3, 3, 0, G0),
    dot(Dot), hbar(Hbar),
    cv_paint_all(G0, [Dot, Hbar], G1),
    cv_extract_all(G1, 0, Objs),
    length(Objs, 2),
    member(obj(r, _), Objs),
    member(obj(b, _), Objs).
...
test(render_round_trip) :-
    ...
    dot(Dot), vbar(Vbar),
    cv_render(3, 3, 0, [Dot, Vbar], G),
    cv_extract_all(G, 0, Objs),
...
test(stamp_single_cell) :-
    dot(Dot),
    cv_stamp(Dot, 0, Patch),
    Patch = [[r]].

test(stamp_hbar) :-
    hbar(Hbar),
    cv_stamp(Hbar, 0, Patch),
