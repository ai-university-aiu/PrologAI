:- use_module('../prolog/rule_inference').
:- use_module(library(plunit)).

:- begin_tests(rule_inference).
...
test(infer_recolor_basic) :-
    ...
    rule_inference_infer_recolor([R, B], [G, B], r, g).

test(infer_recolor_single) :-
    red_dot(R),
    rule_inference_infer_recolor([R], [obj(g,[r(0,0)])], r, g).

test(infer_recolor_fails_no_change) :-
    red_dot(R), blue_dot(B),
    \+ rule_inference_infer_recolor([R, B], [R, B], _, _).

test(infer_recolor_fails_two_changed) :-
    red_dot(R), blue_dot(B), green_dot(G),
    % both r and b changed
    \+ rule_inference_infer_recolor([R, B], [G, obj(p,[r(0,1)])], _, _).

% --- rule_inference_infer_recolor_all/3 ---

test(infer_recolor_all_basic) :-
    red_dot(R), blue_dot(B),
    rule_inference_infer_recolor_all([R, B], [obj(g,[r(0,0)]), obj(g,[r(0,1)])], g).

test(infer_recolor_all_single) :-
    red_dot(R),
    rule_inference_infer_recolor_all([R], [obj(g,[r(0,0)])], g).

test(infer_recolor_all_not_uniform) :-
    % After has two distinct colors — not recolor_all
    red_dot(R), blue_dot(B),
    \+ rule_inference_infer_recolor_all([R], [R, B], _).

% --- rule_inference_infer_color_map/3 ---

test(infer_color_map_single_change) :-
    red_dot(R),
    rule_inference_infer_color_map([R], [obj(g,[r(0,0)])], Map),
    memberchk(r-g, Map).

test(infer_color_map_swap) :-
    red_dot(R), blue_dot(B),
    rule_inference_infer_color_map([R, B], [obj(b,[r(0,0)]), obj(r,[r(0,1)])], Map),
    memberchk(r-b, Map),
    memberchk(b-r, Map).

test(infer_color_map_no_change) :-
    red_dot(R), blue_dot(B),
    rule_inference_infer_color_map([R, B], [R, B], []).

% --- rule_inference_infer_keep_color/3 ---

test(infer_keep_color_basic) :-
    red_dot(R), blue_dot(B), red_bar(RB),
    rule_inference_infer_keep_color([R, B, RB], [R, RB], r).

test(infer_keep_color_one_object) :-
    red_dot(R), blue_dot(B), green_dot(G),
    rule_inference_infer_keep_color([R, B, G], [B], b).

test(infer_keep_color_fails_no_filter) :-
    red_dot(R), blue_dot(B),
    % no objects removed
    \+ rule_inference_infer_keep_color([R, B], [R, B], _).

% --- rule_inference_infer_remove_color/3 ---

test(infer_remove_color_basic) :-
    red_dot(R), blue_dot(B), red_bar(RB),
    rule_inference_infer_remove_color([R, B, RB], [B], r).

test(infer_remove_color_single) :-
    red_dot(R), blue_dot(B),
    rule_inference_infer_remove_color([R, B], [B], r).

test(infer_remove_color_fails_no_removal) :-
    red_dot(R), blue_dot(B),
    \+ rule_inference_infer_remove_color([R, B], [R, B], _).

% --- rule_inference_infer_shift/4 ---

test(infer_shift_basic) :-
    red_dot(R),
    rule_inference_infer_shift([R], [obj(r,[r(2,3)])], 2, 3).

test(infer_shift_zero) :-
    red_dot(R), blue_dot(B),
    rule_inference_infer_shift([R, B], [R, B], 0, 0).

test(infer_shift_negative) :-
    lshape_shifted(LS),
    lshape(L),
    % lshape_shifted is lshape + r(2,3)
    rule_inference_infer_shift([LS], [L], -2, -3).

test(infer_shift_multi_obj) :-
    red_dot(R), blue_dot(B),
    rule_inference_infer_shift([R, B], [obj(r,[r(1,1)]), obj(b,[r(1,2)])], 1, 1).

% --- rule_inference_infer_to_origin/2 ---

test(infer_to_origin_basic) :-
    Shifted = obj(r, [r(3,4)]),
    rule_inference_infer_to_origin([Shifted], [obj(r,[r(0,0)])]).

test(infer_to_origin_scene) :-
    A = obj(r, [r(2,1)]), B = obj(b, [r(2,3)]),
    rule_inference_infer_to_origin([A, B], [obj(r,[r(0,0)]), obj(b,[r(0,2)])]).

test(infer_to_origin_already) :-
    red_dot(R), blue_dot(B),
    rule_inference_infer_to_origin([R, B], [R, B]).
...
test(consistent_recolor_two_pairs) :-
    ...
    rule_inference_consistent_recolor(r, g, [P1, P2]).
...
test(consistent_remove_color_basic) :-
    P1 = [obj(r,[r(0,0)]), obj(b,[r(0,1)])] - [obj(b,[r(0,1)])],
    rule_inference_consistent_remove_color(r, [P1]).
...
test(all_same_colors_empty_pairs) :-
    rule_inference_all_same_colors([]).

test(infer_recolor_multiple_same_color) :-
    % Two red objects become green
    R1 = obj(r,[r(0,0)]), R2 = obj(r,[r(1,0)]),
    rule_inference_infer_recolor([R1, R2], [obj(g,[r(0,0)]), obj(g,[r(1,0)])], r, g).

:- end_tests(rule_inference).
