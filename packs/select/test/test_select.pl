...
test(largest_single) :-
    r1(A), select_largest([A], L), L = [r(0,0)].

test(largest_two) :-
    r1(A), r3(B), select_largest([A, B], L), length(L, 3).

test(largest_three) :-
    r1(A), r3(B), r5(C), select_largest([A, B, C], L), length(L, 5).

test(largest_first) :-
    % When largest is first, still returns correctly.
    r5(A), r3(B), select_largest([A, B], L), length(L, 5).
...
test(smallest_single) :-
    r1(A), select_smallest([A], S), length(S, 1).

test(smallest_two) :-
    r1(A), r3(B), select_smallest([A, B], S), length(S, 1).

test(smallest_three) :-
    r1(A), r3(B), r5(C), select_smallest([A, B, C], S), length(S, 1).

test(smallest_last) :-
    r5(A), r3(B), r1(C), select_smallest([A, B, C], S), length(S, 1).
...
test(filter_area_none) :-
    r1(A), r3(B), select_filter_area([A, B], 9, F), F = [].

test(filter_area_one) :-
    r1(A), r3(B), r5(C), select_filter_area([A, B, C], 3, F), F = [B].

test(filter_area_all) :-
    r3(A), r3b(B), select_filter_area([A, B], 3, F), length(F, 2).

test(filter_area_single_cell) :-
    r1(A), r3(B), select_filter_area([A, B], 1, F), F = [A].
...
test(min_keep_all) :-
    r1(A), r3(B), r5(C), select_filter_area_min([A, B, C], 1, F), length(F, 3).

test(min_keep_two) :-
    r1(A), r3(B), r5(C), select_filter_area_min([A, B, C], 3, F), length(F, 2).

test(min_keep_one) :-
    r1(A), r3(B), r5(C), select_filter_area_min([A, B, C], 5, F), length(F, 1).

test(min_keep_none) :-
    r1(A), r3(B), select_filter_area_min([A, B], 10, F), F = [].
...
test(max_keep_all) :-
    r1(A), r3(B), r5(C), select_filter_area_max([A, B, C], 5, F), length(F, 3).

test(max_keep_two) :-
    r1(A), r3(B), r5(C), select_filter_area_max([A, B, C], 3, F), length(F, 2).

test(max_keep_one) :-
    r1(A), r3(B), r5(C), select_filter_area_max([A, B, C], 1, F), length(F, 1).

test(max_keep_none) :-
    r3(A), r5(B), select_filter_area_max([A, B], 0, F), F = [].
...
test(touches_top_row) :-
    border_r(R), select_touches_border(R, 5, 5).

test(touches_corner) :-
    corner_r(R), select_touches_border(R, 5, 5).

test(does_not_touch) :-
    interior_r(R), \+ select_touches_border(R, 5, 5).

test(single_cell_interior) :-
    \+ select_touches_border([r(2,2)], 5, 5).

test(single_cell_border_r0) :-
    select_touches_border([r(0,2)], 5, 5).

test(single_cell_border_c0) :-
    select_touches_border([r(2,0)], 5, 5).
...
test(filter_border_empty) :-
    select_filter_border([], 5, 5, F), F = [].

test(filter_border_one) :-
    border_r(A), interior_r(B),
    select_filter_border([A, B], 5, 5, F), F = [A].

test(filter_border_none_touch) :-
    interior_r(A), select_filter_border([A], 5, 5, F), F = [].

test(filter_border_all_touch) :-
    border_r(A), corner_r(B),
    select_filter_border([A, B], 5, 5, F), length(F, 2).
...
test(filter_interior_empty) :-
    select_filter_interior([], 5, 5, F), F = [].

test(filter_interior_one) :-
    border_r(A), interior_r(B),
    select_filter_interior([A, B], 5, 5, F), F = [B].

test(filter_interior_none) :-
    border_r(A), corner_r(B),
    select_filter_interior([A, B], 5, 5, F), F = [].

test(filter_interior_all) :-
    interior_r(A), select_filter_interior([A], 5, 5, F), F = [A].
...
test(below_empty) :-
    select_below_row([], 2, F), F = [].

test(below_keeps_at_row) :-
    % Cell at row 2, below_row(2) means all cells >= 2.
    select_below_row([[r(2,0)]], 2, F), length(F, 1).

test(below_excludes_above) :-
    select_below_row([[r(1,0)]], 2, F), F = [].

test(below_mixed) :-
    A = [r(0,0)], B = [r(3,0)],
    select_below_row([A, B], 2, F), F = [B].
...
test(left_empty) :-
    select_left_of_col([], 3, F), F = [].

test(left_keeps_col0) :-
    select_left_of_col([[r(0,0), r(1,0)]], 2, F), length(F, 1).

test(left_excludes_at_col) :-
    select_left_of_col([[r(0,2)]], 2, F), F = [].

test(left_mixed) :-
    A = [r(0,0)], B = [r(0,3)],
    select_left_of_col([A, B], 2, F), F = [A].
...
test(right_empty) :-
    select_right_of_col([], 2, F), F = [].

test(right_keeps_at_col) :-
    select_right_of_col([[r(0,2)]], 2, F), length(F, 1).

test(right_excludes_left) :-
    select_right_of_col([[r(0,1)]], 2, F), F = [].

test(right_mixed) :-
    A = [r(0,0)], B = [r(0,3)],
    select_right_of_col([A, B], 2, F), F = [B].
...
test(unique_empty) :-
    select_unique_area([], U), U = [].

test(unique_all_same) :-
    r3(A), r3b(B), select_unique_area([A, B], U), U = [].

test(unique_one) :-
    r1(A), r3(B), r3b(C),
    select_unique_area([A, B, C], U), U = [A].

test(unique_all_different) :-
    r1(A), r3(B), r5(C),
    select_unique_area([A, B, C], U), length(U, 3).
