...
test(coverage_pairs_all) :-
    pair_recolor(P1), pair_recolor2(P2),
    rk_coverage_pairs(recolor(r,g), [P1, P2], Covered),
    Covered == [P1, P2].

test(coverage_pairs_none) :-
    pair_recolor(P1),
    rk_coverage_pairs(recolor(r,b), [P1], []).

test(coverage_pairs_partial) :-
    pair_recolor(P1), pair_wrong(P2),
    rk_coverage_pairs(recolor(r,g), [P1, P2], [P1]).

% --- rk_coverage/3 ---

test(coverage_full) :-
    pair_recolor(P1), pair_recolor2(P2),
    rk_coverage(recolor(r,g), [P1, P2], 2).

test(coverage_zero) :-
    pair_recolor(P1),
    rk_coverage(recolor(r,b), [P1], 0).

test(coverage_partial) :-
    pair_recolor(P1), pair_wrong(P2),
    rk_coverage(recolor(r,g), [P1, P2], 1).

% --- rk_consistent/2 ---

test(consistent_yes) :-
    pair_recolor(P1), pair_recolor2(P2),
    rk_consistent(recolor(r,g), [P1, P2]).
...
test(consistent_empty) :-
    rk_consistent(recolor(r,g), []).

% --- rk_inconsistent_pairs/3 ---

test(inconsistent_pairs_none) :-
    pair_recolor(P1), pair_recolor2(P2),
    rk_inconsistent_pairs(recolor(r,g), [P1, P2], []).

test(inconsistent_pairs_one) :-
    pair_recolor(P1), pair_wrong(P2),
    rk_inconsistent_pairs(recolor(r,g), [P1, P2], [P2]).

% --- rk_rule_score/3 ---

test(rule_score_alias) :-
    pair_recolor(P1), pair_recolor2(P2),
    rk_rule_score(recolor(r,g), [P1, P2], 2).
...
test(rank_candidates_basic) :-
    pair_recolor(P1), pair_recolor2(P2),
    Pairs = [P1, P2],
    rk_rank_candidates([recolor(r,g), recolor(r,b)], Pairs, CovRules),
...
test(best_rule_basic) :-
    pair_recolor(P1), pair_recolor2(P2),
    rk_best_rule([recolor(r,g), recolor(r,b)], [P1, P2], recolor(r,g)).

test(best_rule_only_one) :-
    pair_recolor(P),
    rk_best_rule([recolor(r,g)], [P], recolor(r,g)).
...
test(all_consistent_some) :-
    pair_recolor(P1), pair_recolor2(P2),
    Pairs = [P1, P2],
    rk_all_consistent([recolor(r,g), recolor(r,b)], Pairs, [recolor(r,g)]).

test(all_consistent_none) :-
    pair_wrong(P),
    rk_all_consistent([recolor(r,g)], [P], []).

test(all_consistent_empty_pairs) :-
    rk_all_consistent([recolor(r,g), recolor(r,b)], [], Consistent),
...
test(filter_min_coverage_basic) :-
    ...
    Pairs = [P1, P2, P3],
    rk_filter_min_coverage([recolor(r,g), recolor(r,b)], Pairs, 2, [recolor(r,g)]).

test(filter_min_coverage_zero) :-
    pair_recolor(P),
    rk_filter_min_coverage([recolor(r,b)], [P], 0, [recolor(r,b)]).

% --- rk_perfect_rules/3 ---

test(perfect_rules_yes) :-
    pair_recolor(P1), pair_recolor2(P2),
    rk_perfect_rules([recolor(r,g), recolor(r,b)], [P1, P2], [recolor(r,g)]).

test(perfect_rules_none) :-
    pair_recolor(P1), pair_wrong(P2),
    rk_perfect_rules([recolor(r,g)], [P1, P2], []).

% --- rk_coverage_ratio/3 ---

test(coverage_ratio_basic) :-
    pair_recolor(P1), pair_recolor2(P2),
    rk_coverage_ratio(recolor(r,g), [P1, P2], Num/Den),
    Num == 2, Den == 2.
...
test(top_n_rules_basic) :-
    ...
    Pairs = [P1, P2, P3],
    Candidates = [recolor(r,g), shift(1,0), recolor(r,b)],
    rk_top_n_rules(Candidates, Pairs, 2, TopN),
    length(TopN, 2),
    TopN = [2-recolor(r,g) | _].

test(top_n_rules_all) :-
    pair_recolor(P),
    rk_top_n_rules([recolor(r,g)], [P], 5, TopN),
...
test(coverage_two_obj_pair) :-
    pair2_recolor(P),
    rk_coverage(recolor(r,g), [P], 1).

test(coverage_two_obj_shift) :-
    pair2_shift(P),
    rk_coverage(shift(1,0), [P], 1).
