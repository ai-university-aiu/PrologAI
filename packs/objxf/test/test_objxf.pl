...
:- begin_tests(objxf_bbox).

test(dot)    :- dot(O), objxf_bbox(O, 0, 0, 0, 0).
test(hline)  :- hline(O), objxf_bbox(O, 0, 0, 0, 1).
test(vline)  :- vline(O), objxf_bbox(O, 0, 0, 1, 0).
test(sq2x2)  :- sq2x2(O), objxf_bbox(O, 0, 0, 1, 1).
test(offset) :- offset(O), objxf_bbox(O, 2, 3, 3, 4).

:- end_tests(objxf_bbox).

:- begin_tests(objxf_size).

test(dot_1x1)      :- dot(O),    objxf_size(O, 1, 1).
test(hline_1x2)    :- hline(O),  objxf_size(O, 1, 2).
test(vline_2x1)    :- vline(O),  objxf_size(O, 2, 1).
test(sq2x2_2x2)    :- sq2x2(O),  objxf_size(O, 2, 2).
test(sq3x3_3x3)    :- sq3x3(O),  objxf_size(O, 3, 3).
test(offset_2x2)   :- offset(O), objxf_size(O, 2, 2).

:- end_tests(objxf_size).

:- begin_tests(objxf_translate).

test(translate_positive) :-
    dot(D), objxf_translate(D, 3, 5, obj(red, [r(3,5)])).

test(translate_negative) :-
    offset(O), objxf_translate(O, -2, -3, Moved),
...
test(translate_zero) :-
    hline(H), objxf_translate(H, 0, 0, H).

test(translate_preserves_color) :-
    corner(C), objxf_translate(C, 1, 1, obj(yellow, _)).

:- end_tests(objxf_translate).

:- begin_tests(objxf_to_origin).

test(already_at_origin) :-
    dot(D), objxf_to_origin(D, obj(red, [r(0,0)])).

test(offset_to_origin) :-
    offset(O), objxf_to_origin(O, Norm),
    Norm = obj(cyan, Cells),
    sort(Cells, Sorted),
    sort([r(0,0), r(0,1), r(1,0)], Expected),
    Sorted = Expected.
...
test(vline_to_origin) :-
    vline(V), objxf_to_origin(V, obj(green, [r(0,0), r(1,0)])).

test(color_preserved) :-
    offset(O), objxf_to_origin(O, obj(cyan, _)).

:- end_tests(objxf_to_origin).

:- begin_tests(objxf_recolor).

test(recolor_dot) :-
    dot(D), objxf_recolor(D, purple, obj(purple, [r(0,0)])).

test(recolor_sq2x2) :-
    sq2x2(S), objxf_recolor(S, orange, obj(orange, _)),
    objxf_recolor(S, orange, obj(_, Cells)),
    Cells = [r(0,0), r(0,1), r(1,0), r(1,1)].

test(recolor_preserves_cells) :-
    corner(C),
    objxf_recolor(C, new_color, obj(_, Cells)),
    corner(obj(_, Cells)).

:- end_tests(objxf_recolor).

:- begin_tests(objxf_rot90).
...
test(hline_to_vline) :-
    hline(H), objxf_rot90(H, obj(blue, Rotated)),
...
:- end_tests(objxf_rot270).

:- begin_tests(objxf_reflect_h).

% hline [r(0,0),r(0,1)]: bbox R0=0,R1=0. NR = 0+0-R = 0. Unchanged.
test(hline_reflect_h_same) :-
    hline(H), objxf_reflect_h(H, obj(blue, Reflected)),
...
test(vline_reflect_h_same) :-
    vline(V), objxf_reflect_h(V, obj(green, Reflected)),
...
test(corner_reflect_h) :-
    corner(C), objxf_reflect_h(C, obj(yellow, Reflected)),
    sort(Reflected, S),
...
test(reflect_v_twice_identity) :-
    corner(C),
    objxf_reflect_v(C, Once), objxf_reflect_v(Once, Twice),
    Twice = obj(yellow, TwiceCells),
    corner(obj(yellow, OrigCells)),
    sort(TwiceCells, TS), sort(OrigCells, OS), TS = OS.

:- end_tests(objxf_reflect_v).

:- begin_tests(objxf_merge).

% Merge two non-overlapping objects.
test(merge_non_overlapping) :-
    hline(H), vline(V),
    objxf_merge(H, V, obj(blue, Merged)),
    sort(Merged, S),
...
test(intersect_hline_vline) :-
    hline(H), vline(V),
    objxf_intersect(H, V, obj(blue, Intersection)),
    sort(Intersection, S), sort([r(0,0)], E), S = E.

% intersect with self = self.
test(intersect_self) :-
    corner(C),
    objxf_intersect(C, C, obj(yellow, Intersection)),
    corner(obj(yellow, OrigCells)),
...
test(inclusion_exclusion) :-
    left_half(L), right_half(R),
    objxf_merge(L, R, obj(_, Merged)), length(Merged, NM),
    left_half(obj(_, LC)), length(LC, NL),
    right_half(obj(_, RC)), length(RC, NR),
    objxf_intersect(L, R, obj(_, Inter)), length(Inter, NI),
    NM =:= NL + NR - NI.

:- end_tests(objxf_intersect).

:- begin_tests(objxf_scale_up).

% dot scaled by 2: r(0,0) -> r(0,0),r(0,1),r(1,0),r(1,1).
test(dot_scale_2) :-
    dot(D), objxf_scale_up(D, 2, obj(red, Scaled)),
    sort(Scaled, S),
    sort([r(0,0),r(0,1),r(1,0),r(1,1)], E), S = E.

% scale by 1 = identity.
test(scale_1_identity) :-
    corner(C), objxf_scale_up(C, 1, obj(yellow, Scaled)),
    corner(obj(yellow, OrigCells)),
...
test(hline_scale_2) :-
    hline(H), objxf_scale_up(H, 2, obj(blue, Scaled)),
    length(Scaled, 8).

% scale preserves color.
test(scale_preserves_color) :-
    vline(V), objxf_scale_up(V, 3, obj(green, _)).

% cell count after scaling = original count * Factor * Factor.
test(cell_count_after_scale) :-
    sq2x2(S), objxf_scale_up(S, 3, obj(_, Scaled)),
    length(Scaled, N),
    N =:= 4 * 3 * 3.

:- end_tests(objxf_scale_up).
