...
% field unless explicitly replacing it (objxf_recolor). Transformations include:
...
% objxf_bbox(+Obj, -R0, -C0, -R1, -C1): bounding box of Obj's cells.
% R0=min row, C0=min col, R1=max row, C1=max col.
objxf_bbox(obj(_, Cells), R0, C0, R1, C1) :-
...
% objxf_size(+Obj, -H, -W): bounding box dimensions.
% H = height (number of rows), W = width (number of columns).
objxf_size(Obj, H, W) :-
% Compute bounding box then derive H and W from extents.
    objxf_bbox(Obj, R0, C0, R1, C1),
    H is R1 - R0 + 1,
    W is C1 - C0 + 1.

% objxf_translate(+Obj, +DR, +DC, -Result): shift all cells by (DR, DC).
% DR is the row delta; DC is the column delta. May be negative.
objxf_translate(obj(Color, Cells), DR, DC, obj(Color, Shifted)) :-
% Add DR to every row and DC to every column in each cell.
    findall(r(NR, NC),
            (member(r(R, C), Cells), NR is R + DR, NC is C + DC),
            Shifted).

% objxf_to_origin(+Obj, -Result): shift so the topmost row = 0 and leftmost col = 0.
% The object's bounding box is anchored at r(0,0) in the result.
objxf_to_origin(Obj, Result) :-
% Find the current minimum row and column.
    objxf_bbox(Obj, R0, C0, _, _),
% Translate by negated minimums to bring the anchor to the origin.
    objxf_translate(Obj, -R0, -C0, Result).

% objxf_recolor(+Obj, +NewColor, -Result): replace the color field with NewColor.
% Cells are unchanged.
objxf_recolor(obj(_, Cells), NewColor, obj(NewColor, Cells)).

% objxf_rot90(+Obj, -Result): rotate Obj 90 degrees clockwise within its bounding box.
% Each cell r(R,C) -> r(R0 + Lc, C0 + (H-1) - Lr) where Lr = R-R0, Lc = C-C0.
% After rotation the bounding box is W x H (dimensions swap).
objxf_rot90(obj(Color, Cells), obj(Color, Rotated)) :-
...
% objxf_rot180(+Obj, -Result): rotate Obj 180 degrees within its bounding box.
% Each cell r(R,C) -> r(R0+R1-R, C0+C1-C). Bounding box dimensions unchanged.
objxf_rot180(obj(Color, Cells), obj(Color, Rotated)) :-
...
% objxf_rot270(+Obj, -Result): rotate Obj 270 degrees clockwise (= 90 degrees CCW).
% Each cell r(R,C) -> r(R0+(W-1)-Lc, C0+Lr) where Lr = R-R0, Lc = C-C0.
% After rotation the bounding box is W x H (dimensions swap).
objxf_rot270(obj(Color, Cells), obj(Color, Rotated)) :-
...
% objxf_reflect_h(+Obj, -Result): reflect horizontally (flip upside-down) within the bounding box.
% Each cell r(R,C) -> r(R0+H-1-(R-R0), C) = r(R0+R1-R, C). Columns unchanged.
objxf_reflect_h(obj(Color, Cells), obj(Color, Reflected)) :-
...
% objxf_reflect_v(+Obj, -Result): reflect vertically (flip left-right) within the bounding box.
% Each cell r(R,C) -> r(R, C0+W-1-(C-C0)) = r(R, C0+C1-C). Rows unchanged.
objxf_reflect_v(obj(Color, Cells), obj(Color, Reflected)) :-
