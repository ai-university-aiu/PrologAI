...
:- begin_tests(lk_by_position).

test(empty_both) :-
    lk_by_position([], [], []).

test(empty_first) :-
    red1(R), lk_by_position([], [R], []).
...
test(single_pair) :-
    red1(R), blue1(B),
    lk_by_position([R], [B], [R-B]).

test(two_pairs) :-
    red1(R), blue1(B), green1(G), red2(R2),
    lk_by_position([R, G], [B, R2], [R-B, G-R2]).

test(truncates_at_shorter) :-
    % Three in first, two in second: only two links.
    red1(R), blue1(B), green1(G), red2(R2), blue2(B2),
    lk_by_position([R, B, G], [R2, B2], [R-R2, B-B2]).

:- end_tests(lk_by_position).

:- begin_tests(lk_by_nearest).
...
test(two_sources) :-
    ...
    lk_by_nearest([R, B], [G, B2], [R-G, B-B2]).

:- end_tests(lk_by_nearest).

:- begin_tests(lk_by_color).
...
:- end_tests(lk_by_form).

:- begin_tests(lk_source).
...
test(multiple) :-
    red1(R), blue1(B), green1(G), red2(R2),
    lk_source([R-B, G-R2], [R, G]).

:- end_tests(lk_source).

:- begin_tests(lk_target).
...
test(multiple) :-
    red1(R), blue1(B), green1(G), red2(R2),
    lk_target([R-B, G-R2], [B, R2]).

:- end_tests(lk_target).

:- begin_tests(lk_invert).
...
test(multiple) :-
    red1(R), blue1(B), green1(G), red2(R2),
    lk_invert([R-B, G-R2], [B-R, R2-G]).

:- end_tests(lk_invert).

:- begin_tests(lk_count).
...
test(three) :-
    red1(R), blue1(B), green1(G), red2(R2), blue2(B2),
    lk_count([R-B, G-R2, B-B2], 3).

:- end_tests(lk_count).

:- begin_tests(lk_apply_color).
...
test(single_recolor) :-
    ...
    lk_apply_color([A-B], [obj(blue,[r(0,0)])]).

test(two_links) :-
    A = obj(red,[r(0,0)]),   B = obj(blue,[r(1,0)]),
    C = obj(green,[r(2,0)]), D = obj(yellow,[r(3,0)]),
    lk_apply_color([A-B, C-D], [obj(blue,[r(0,0)]), obj(yellow,[r(2,0)])]).

:- end_tests(lk_apply_color).

:- begin_tests(lk_apply_cells).
...
test(single_replace_cells) :-
    ...
    lk_apply_cells([A-B], [obj(red,[r(1,0),r(1,1)])]).

test(two_links) :-
    A = obj(red,[r(0,0)]),   B = obj(blue,[r(5,5)]),
    C = obj(green,[r(1,0)]), D = obj(yellow,[r(6,6)]),
    lk_apply_cells([A-B, C-D], [obj(red,[r(5,5)]), obj(green,[r(6,6)])]).

:- end_tests(lk_apply_cells).

:- begin_tests(lk_filter_same_color).

test(all_same) :-
    R1 = obj(red,[r(0,0)]), R2 = obj(red,[r(1,0)]),
    lk_filter_same_color([R1-R2], [R1-R2]).

test(none_same) :-
    R = obj(red,[r(0,0)]), B = obj(blue,[r(1,0)]),
    lk_filter_same_color([R-B], []).

test(mixed) :-
    R1 = obj(red,[r(0,0)]), R2 = obj(red,[r(1,0)]),
    B = obj(blue,[r(2,0)]),
    lk_filter_same_color([R1-R2, R1-B], [R1-R2]).

test(empty) :-
    lk_filter_same_color([], []).

:- end_tests(lk_filter_same_color).

:- begin_tests(lk_filter_diff_color).

test(all_diff) :-
    R = obj(red,[r(0,0)]), B = obj(blue,[r(1,0)]),
    lk_filter_diff_color([R-B], [R-B]).

test(none_diff) :-
    R1 = obj(red,[r(0,0)]), R2 = obj(red,[r(1,0)]),
    lk_filter_diff_color([R1-R2], []).

test(mixed) :-
    R1 = obj(red,[r(0,0)]), R2 = obj(red,[r(1,0)]),
    B = obj(blue,[r(2,0)]),
    lk_filter_diff_color([R1-R2, R1-B], [R1-B]).

:- end_tests(lk_filter_diff_color).

:- begin_tests(lk_unlinked).
