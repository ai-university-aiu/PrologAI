:- use_module('../prolog/object_match').
:- use_module(library(plunit)).

:- begin_tests(object_match).
...
test(by_color_basic) :-
    ...
    object_match_by_color([R, B], [RB, BB], Pairs),
    member(R-RB, Pairs),
    member(B-BB, Pairs).
...
test(by_color_no_match) :-
    red_dot(R),
    blue_bar(BB),
    object_match_by_color([R], [BB], Pairs),
    Pairs == [].

test(by_color_all_same_color) :-
    red_dot(R1), red_bar(R2), red_dot_far(R3),
    object_match_by_color([R1], [R2, R3], Pairs),
    length(Pairs, 2).
...
test(by_form_matching) :-
    ...
    object_match_by_form([L], [LF], Pairs),
    Pairs == [L-LF].

test(by_form_no_match) :-
    red_dot(R), red_bar(RB),
    object_match_by_form([R], [RB], Pairs),
    Pairs == [].
...
test(by_nearest_basic) :-
    ...
    object_match_by_nearest([R, RF], [B, R11], Pairs),
    Pairs == [R-R11, RF-B].

test(by_nearest_single) :-
    red_dot(R),
    blue_dot22(B),
    object_match_by_nearest([R], [B], Pairs),
    Pairs == [R-B].
...
test(by_nearest_two) :-
    ...
    object_match_by_nearest([R1, R2], [B1, B2], Pairs),
    Pairs == [R1-B1, R2-B2].

test(by_nearest_empty_list2) :-
    red_dot(R),
    object_match_by_nearest([R], [], Pairs),
    Pairs == [].
...
test(unmatched_basic) :-
    red_dot(R), blue_dot(B), green_dot(G),
    Pairs = [R-B],
    object_match_unmatched(Pairs, [R, G], [B, G], Um1, Um2),
    Um1 == [G],
    Um2 == [G].
...
test(unmatched_all_matched) :-
    red_dot(R), blue_dot(B),
    Pairs = [R-B],
    object_match_unmatched(Pairs, [R], [B], Um1, Um2),
    Um1 == [],
    Um2 == [].

test(unmatched_empty_pairs) :-
    red_dot(R), blue_dot(B),
    object_match_unmatched([], [R], [B], Um1, Um2),
    Um1 == [R],
    Um2 == [B].
...
test(filter_changed_basic) :-
    ...
    object_match_filter_changed_color([R-B, R-G], Changed),
    length(Changed, 2).

test(filter_changed_none) :-
    red_dot(R1), red_dot_far(R2),
    object_match_filter_changed_color([R1-R2], Changed),
    Changed == [].

test(filter_changed_mixed) :-
    red_dot(R), blue_dot(B), red_dot_far(R2),
    object_match_filter_changed_color([R-B, R-R2], Changed),
    Changed == [R-B].
...
test(filter_same_basic) :-
    ...
    object_match_filter_same_color([R-R2, R-B], Same),
    Same == [R-R2].

test(filter_same_empty) :-
    red_dot(R), blue_dot(B),
    object_match_filter_same_color([R-B], Same),
    Same == [].
...
test(color_deltas_basic) :-
    red_dot(R), blue_dot(B), green_dot(G),
    object_match_color_deltas([R-B, R-G], Deltas),
    Deltas == [r-b, r-g].

test(color_deltas_empty) :-
    object_match_color_deltas([], Deltas),
    Deltas == [].
...
test(pos_deltas_basic) :-
    ...
    object_match_pos_deltas([R-B], Deltas),
    Deltas == [dr(2,2)].
...
test(size_deltas_basic) :-
    ...
    object_match_size_deltas([R-RB], Deltas),
    Deltas == [2].

test(size_deltas_negative) :-
    red_bar(RB), red_dot(R),  % size 3 -> 1, delta = -2
    object_match_size_deltas([RB-R], Deltas),
    Deltas == [-2].

test(size_deltas_zero) :-
    red_dot(R), blue_dot(B),
    object_match_size_deltas([R-B], Deltas),
    Deltas == [0].
...
test(all_same_color_delta_true) :-
    ...
    % Both pairs: r -> b
    object_match_all_same_color_delta([R1-B1, R2-B2]).

test(all_same_color_delta_false) :-
    red_dot(R), blue_dot(B), green_dot(G),
    % Pairs: r->b and r->g — not same delta
    \+ object_match_all_same_color_delta([R-B, R-G]).
...
test(all_same_pos_delta_true) :-
    ...
    object_match_all_same_pos_delta([R1-R2]).
...
test(all_same_pos_delta_two_equal) :-
    ...
    \+ object_match_all_same_pos_delta([R1-R2, B1-B2]).
...
test(zip_basic) :-
    red_dot(R), blue_dot(B),
    red_bar(RB), blue_bar(BB),
    object_match_zip([R, B], [RB, BB], Pairs),
    Pairs == [R-RB, B-BB].

test(zip_single) :-
    red_dot(R), blue_dot(B),
    object_match_zip([R], [B], Pairs),
    Pairs == [R-B].

test(zip_empty) :-
    object_match_zip([], [], Pairs),
    Pairs == [].

test(zip_three) :-
    red_dot(R), blue_dot(B), green_dot(G),
    red_bar(RB), blue_bar(BB), yellow_bar(YB),
    object_match_zip([R, B, G], [RB, BB, YB], Pairs),
    length(Pairs, 3),
    nth0(0, Pairs, R-RB),
    nth0(1, Pairs, B-BB),
    nth0(2, Pairs, G-YB).

:- end_tests(object_match).
