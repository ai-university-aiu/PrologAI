...
:- module(naggr, [
    naggr_sum4/2, naggr_sum8/2,
    naggr_max4/2, naggr_max8/2,
    naggr_min4/2, naggr_min8/2,
    naggr_mean4/2, naggr_mean8/2,
    naggr_range4/2, naggr_range8/2,
    naggr_spread4/2, naggr_spread8/2,
    naggr_diff4/2, naggr_diff8/2
]).
...
% naggr_sum4(+Grid, -SumGrid): SumGrid[R][C] is the sum of in-bounds 4-neighbor values.
% Returns 0 for cells with no in-bounds 4-neighbors (e.g., a 1x1 grid).
naggr_sum4(Grid, SumGrid) :-
% Compute grid dimensions and build the output grid row by row via nested findall.
    ...
    findall(Row, (between(0, H1, R),
        findall(S, (between(0, W1, C),
            naggr_nbr4_vals_(Grid, H, W, R, C, Vals),
            sum_list(Vals, S)), Row)), SumGrid).

% naggr_sum8(+Grid, -SumGrid): SumGrid[R][C] is the sum of in-bounds 8-neighbor values.
% Returns 0 for cells with no in-bounds 8-neighbors.
naggr_sum8(Grid, SumGrid) :-
% Same structure as naggr_sum4 using the 8-connected neighborhood helper.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
    findall(Row, (between(0, H1, R),
        findall(S, (between(0, W1, C),
            naggr_nbr8_vals_(Grid, H, W, R, C, Vals),
            sum_list(Vals, S)), Row)), SumGrid).

% naggr_max4(+Grid, -MaxGrid): MaxGrid[R][C] is the maximum of in-bounds 4-neighbor values.
% Returns 0 for cells with no in-bounds neighbors.
naggr_max4(Grid, MaxGrid) :-
...
% naggr_max8(+Grid, -MaxGrid): MaxGrid[R][C] is the maximum of in-bounds 8-neighbor values.
% Returns 0 for cells with no in-bounds neighbors.
naggr_max8(Grid, MaxGrid) :-
...
% naggr_min4(+Grid, -MinGrid): MinGrid[R][C] is the minimum of in-bounds 4-neighbor values.
% Returns 0 for cells with no in-bounds neighbors.
naggr_min4(Grid, MinGrid) :-
% Use 0 as sentinel for empty neighborhoods.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
    findall(Row, (between(0, H1, R),
...
% naggr_min8(+Grid, -MinGrid): MinGrid[R][C] is the minimum of in-bounds 8-neighbor values.
% Returns 0 for cells with no in-bounds neighbors.
naggr_min8(Grid, MinGrid) :-
% Use 0 as sentinel for empty neighborhoods.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
    findall(Row, (between(0, H1, R),
...
% naggr_mean4(+Grid, -MeanGrid): MeanGrid[R][C] is floor(sum/count) of 4-neighbor values.
% Returns 0 for cells with no in-bounds neighbors.
naggr_mean4(Grid, MeanGrid) :-
...
% naggr_mean8(+Grid, -MeanGrid): MeanGrid[R][C] is floor(sum/count) of 8-neighbor values.
% Returns 0 for cells with no in-bounds neighbors.
naggr_mean8(Grid, MeanGrid) :-
% Use integer floor division; 0 sentinel for empty neighborhoods.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
    findall(Row, (between(0, H1, R),
...
% naggr_range4(+Grid, -RangeGrid): RangeGrid[R][C] is max - min of 4-neighbor values.
% Returns 0 when fewer than 2 in-bounds neighbors exist.
naggr_range4(Grid, RangeGrid) :-
...
% naggr_range8(+Grid, -RangeGrid): RangeGrid[R][C] is max - min of 8-neighbor values.
% Returns 0 when fewer than 2 in-bounds neighbors exist.
naggr_range8(Grid, RangeGrid) :-
% Need at least 2 values to define a range; 0 for 0 or 1 neighbor.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
    findall(Row, (between(0, H1, R),
...
% naggr_spread4(+Grid, -SpreadGrid): SpreadGrid[R][C] is the count of distinct values
% among in-bounds 4-neighbors. Returns 0 when no in-bounds neighbors exist.
naggr_spread4(Grid, SpreadGrid) :-
...
% naggr_spread8(+Grid, -SpreadGrid): SpreadGrid[R][C] is the count of distinct values
% among in-bounds 8-neighbors. Returns 0 when no in-bounds neighbors exist.
naggr_spread8(Grid, SpreadGrid) :-
...
% naggr_diff4(+Grid, -DiffGrid): DiffGrid[R][C] is the count of in-bounds 4-neighbors
% whose value differs from the cell's own value.
naggr_diff4(Grid, DiffGrid) :-
...
% naggr_diff8(+Grid, -DiffGrid): DiffGrid[R][C] is the count of in-bounds 8-neighbors
% whose value differs from the cell's own value.
naggr_diff8(Grid, DiffGrid) :-
% Count same-valued neighbors via inner findall, then subtract from total.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
    findall(Row, (between(0, H1, R),
