...
:- begin_tests(griddiag).

% --- griddiag_trace ---
test(trace_3x3_diag, []) :-
    g3x3_diag(G),
    griddiag_trace(G, Vals),
    Vals = [r, r, r].

test(trace_3x3_all_r, []) :-
    g3x3_all_r(G),
    griddiag_trace(G, [r, r, r]).

test(trace_4x4, []) :-
    g4x4(G),
    % Main diagonal: (0,0)=a,(1,1)=f,(2,2)=k,(3,3)=p
    griddiag_trace(G, [a, f, k, p]).

test(trace_2x3, []) :-
    g2x3(G),
    % Main diagonal: (0,0)=r,(1,1)=r (length = min(2,3) = 2)
    griddiag_trace(G, [r, r]).

% --- griddiag_main_diag ---
test(main_diag_k0, []) :-
    g4x4(G),
    griddiag_main_diag(G, 0, [a, f, k, p]).

test(main_diag_k1, []) :-
    g4x4(G),
    % K=1: (0,1)=b,(1,2)=g,(2,3)=l
    griddiag_main_diag(G, 1, [b, g, l]).

test(main_diag_km1, []) :-
    g4x4(G),
    % K=-1: (1,0)=e,(2,1)=j,(3,2)=o
    griddiag_main_diag(G, -1, [e, j, o]).

test(main_diag_corner_k3, []) :-
    g4x4(G),
    % K=3: only cell (0,3)=d
    griddiag_main_diag(G, 3, [d]).

test(main_diag_corner_km3, []) :-
    g4x4(G),
    % K=-3: only cell (3,0)=m
    griddiag_main_diag(G, -3, [m]).

% --- griddiag_anti_diag ---
test(anti_diag_k0, []) :-
    g4x4(G),
    % K=0: only (0,0)=a
    griddiag_anti_diag(G, 0, [a]).

test(anti_diag_k3, []) :-
    g4x4(G),
    % K=3: (0,3)=d,(1,2)=g,(2,1)=j,(3,0)=m (C ascending: 0,1,2,3 -> R=3,2,1,0)
    % Actually C in [max(0,3-3),min(3,3)] = [0,3] → C=0,1,2,3, R=3,2,1,0
    griddiag_anti_diag(G, 3, [m, j, g, d]).

test(anti_diag_k1, []) :-
    g4x4(G),
    % K=1: C in [0,1], R=1,0 → (1,0)=e,(0,1)=b
    griddiag_anti_diag(G, 1, [e, b]).

test(anti_diag_3x3, []) :-
    g3x3_anti(G),
    % K=2: (0,2)=r,(1,1)=r,(2,0)=r -> anti-diagonal of r's
    griddiag_anti_diag(G, 2, [r, r, r]).

% --- griddiag_all_main_diags ---
test(all_main_diags_count_3x3, []) :-
    g3x3_all_r(G),
    griddiag_all_main_diags(G, Diags),
...
% --- griddiag_all_anti_diags ---
test(all_anti_diags_count_3x3, []) :-
    g3x3_all_r(G),
    griddiag_all_anti_diags(G, Diags),
...
test(main_count_k0_all_r, []) :-
    g3x3_diag(G),
    griddiag_main_count(G, 0, r, 3).

test(main_count_k1_diag, []) :-
    g3x3_diag(G),
    % K=1: (0,1)=x,(1,2)=x -> 0 r's
    griddiag_main_count(G, 1, r, 0).
...
test(anti_count_k2_anti_diag, []) :-
    ...
    griddiag_anti_count(G, 2, r, 3).

test(anti_count_k0, []) :-
    g3x3_diag(G),
    % K=0: only (0,0)=r -> 1 r
    griddiag_anti_count(G, 0, r, 1).

% --- griddiag_main_uniform ---
test(main_uniform_k0_diag_yes, []) :-
    g3x3_diag(G),
    griddiag_main_uniform(G, 0, yes).
...
% --- griddiag_uniform_anti_diags ---
test(uniform_anti_diags_all_r, []) :-
    g3x3_all_r(G),
    griddiag_uniform_anti_diags(G, Ks),
...
% --- griddiag_diag_length ---
test(diag_length_k0_square, []) :-
    g3x3_all_r(G),
    griddiag_diag_length(G, 0, 3).

test(diag_length_k1, []) :-
    g3x3_all_r(G),
    griddiag_diag_length(G, 1, 2).

test(diag_length_k2, []) :-
    g3x3_all_r(G),
    griddiag_diag_length(G, 2, 1).

test(diag_length_4x4_k0, []) :-
    g4x4(G),
    griddiag_diag_length(G, 0, 4).

test(diag_length_2x3_k0, []) :-
    g2x3(G),
    griddiag_diag_length(G, 0, 2).

% --- Combined tests ---
test(trace_is_main_diag_0, []) :-
    g4x4(G),
    griddiag_trace(G, T),
    griddiag_main_diag(G, 0, T).
...
test(set_and_retrieve, []) :-
    g3x3_all_r(G),
    griddiag_set_main_diag(G, 0, x, R),
    griddiag_main_diag(R, 0, [x, x, x]).
