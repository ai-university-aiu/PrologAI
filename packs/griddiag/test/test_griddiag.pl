...
test(main_diag_k0, []) :-
    g4x4(G),
    gdi_main_diag(G, 0, [a, f, k, p]).

test(main_diag_k1, []) :-
    g4x4(G),
    % K=1: (0,1)=b,(1,2)=g,(2,3)=l
    gdi_main_diag(G, 1, [b, g, l]).

test(main_diag_km1, []) :-
    g4x4(G),
    % K=-1: (1,0)=e,(2,1)=j,(3,2)=o
    gdi_main_diag(G, -1, [e, j, o]).
...
test(main_count_k0_all_r, []) :-
    g3x3_diag(G),
    gdi_main_count(G, 0, r, 3).

test(main_count_k1_diag, []) :-
    g3x3_diag(G),
    % K=1: (0,1)=x,(1,2)=x -> 0 r's
    gdi_main_count(G, 1, r, 0).
...
test(anti_count_k2_anti_diag, []) :-
    ...
    gdi_anti_count(G, 2, r, 3).

test(anti_count_k0, []) :-
    g3x3_diag(G),
    % K=0: only (0,0)=r -> 1 r
    gdi_anti_count(G, 0, r, 1).

% --- gdi_main_uniform ---
test(main_uniform_k0_diag_yes, []) :-
    g3x3_diag(G),
    gdi_main_uniform(G, 0, yes).
...
% --- gdi_uniform_anti_diags ---
test(uniform_anti_diags_all_r, []) :-
    g3x3_all_r(G),
    gdi_uniform_anti_diags(G, Ks),
...
test(diag_length_k0_square, []) :-
    g3x3_all_r(G),
    gdi_diag_length(G, 0, 3).

test(diag_length_k1, []) :-
    g3x3_all_r(G),
    gdi_diag_length(G, 1, 2).

test(diag_length_k2, []) :-
    g3x3_all_r(G),
    gdi_diag_length(G, 2, 1).

test(diag_length_4x4_k0, []) :-
    g4x4(G),
    gdi_diag_length(G, 0, 4).

test(diag_length_2x3_k0, []) :-
    g2x3(G),
    gdi_diag_length(G, 0, 2).
...
test(set_and_retrieve, []) :-
    g3x3_all_r(G),
    gdi_set_main_diag(G, 0, x, R),
    gdi_main_diag(R, 0, [x, x, x]).
