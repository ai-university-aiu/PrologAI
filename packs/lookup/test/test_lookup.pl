...
test(get_first) :-
    sample_map(M), lookup_get(a, M, V), V =:= 1.

test(get_middle) :-
    sample_map(M), lookup_get(b, M, V), V =:= 2.

test(get_last) :-
    sample_map(M), lookup_get(c, M, V), V =:= 3.

test(get_integer_key) :-
    lookup_get(0, [0-red, 1-blue], V), V = red.
...
test(has_key_present) :-
    sample_map(M), lookup_has_key(a, M).

test(has_key_absent) :-
    sample_map(M), \+ lookup_has_key(z, M).
...
test(put_new_key) :-
    sample_map(M), lookup_put(d, 4, M, M2),
    lookup_get(d, M2, V), V =:= 4.

test(put_existing_key) :-
    sample_map(M), lookup_put(b, 99, M, M2),
    lookup_get(b, M2, V), V =:= 99.

test(put_first_key) :-
    lookup_put(x, 7, [], M), M = [x-7].
...
test(delete_existing) :-
    sample_map(M), lookup_delete(b, M, M2),
    \+ lookup_has_key(b, M2),
    lookup_has_key(a, M2), lookup_has_key(c, M2).

test(delete_absent) :-
    % Deleting a key that doesn't exist: map unchanged.
    sample_map(M), lookup_delete(z, M, M2),
    M2 = M.

test(delete_from_empty) :-
    lookup_delete(a, [], M2), M2 = [].
...
test(keys_basic) :-
    sample_map(M), lookup_keys(M, Ks), Ks = [a, b, c].
...
test(values_basic) :-
    sample_map(M), lookup_values(M, Vs), Vs = [1, 2, 3].
...
test(map_values_double) :-
    M = [a-2, b-4, c-6],
    lookup_map_values(M, lookup_half_, NewM),
    NewM = [a-1, b-2, c-3].

% Helper predicate for test.
lookup_half_(V, H) :- H is V // 2.
...
test(row_0) :-
    color_grid(G), lookup_grid_row(G, 0, R), R = [0,1,0].

test(row_1) :-
    color_grid(G), lookup_grid_row(G, 1, R), R = [2,0,1].
...
test(col_0) :-
    color_grid(G), lookup_grid_col(G, 0, C), C = [0,2,0].

test(col_1) :-
    color_grid(G), lookup_grid_col(G, 1, C), C = [1,0,2].
...
test(cell_0_1) :-
    color_grid(G), lookup_grid_cell(G, 0, 1, V), V =:= 1.

test(cell_1_0) :-
    color_grid(G), lookup_grid_cell(G, 1, 0, V), V =:= 2.
...
test(color_positions_basic) :-
    ...
    lookup_get(1, Map, Pos1), msort(Pos1, SP1), SP1 = [r(0,1), r(1,2)],
    lookup_get(2, Map, Pos2), msort(Pos2, SP2), SP2 = [r(1,0), r(2,1)].

test(color_positions_empty) :-
    % All background: empty map.
    lookup_color_positions([[0,0],[0,0]], 0, Map),
    Map = [].
...
test(position_color_basic) :-
    ...
    length(Map, L), L =:= 4,
    lookup_get(r(0,1), Map, V01), V01 =:= 1,
    lookup_get(r(1,0), Map, V10), V10 =:= 2.

test(position_color_empty) :-
    lookup_position_color([[0,0]], 0, Map), Map = [].
...
test(invert_basic) :-
    sample_map(M), lookup_invert(M, Inv),
    Inv = [1-a, 2-b, 3-c].

test(invert_twice) :-
    sample_map(M), lookup_invert(M, Inv), lookup_invert(Inv, M2),
    M2 = M.
