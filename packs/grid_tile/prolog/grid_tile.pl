:- module(grid_tile, [
    grid_tile_h_period/2,
    grid_tile_w_period/2,
    grid_tile_tile_size/3,
    grid_tile_is_tiling/3,
    grid_tile_extract_tile/4,
    grid_tile_tile_to_grid/4,
    grid_tile_row_is_periodic/3,
    grid_tile_col_is_periodic/3,
    grid_tile_tile_count_h/3,
    grid_tile_tile_count_w/3,
    grid_tile_matches_tile/4,
    grid_tile_tile_offset/5,
    grid_tile_all_tiles/4,
    grid_tile_crop_to_tile/2
]).
...
grid_tile_cols_have_period_(Grid, P) :-
    (Grid = [FR|_] -> length(FR, W) ; W = 0),
    W1 is W - 1,
    \+ (between(0, W1, C),
        C2 is C mod P,
        grid_tile_col_(Grid, C, Col), grid_tile_col_(Grid, C2, Col2),
        Col \= Col2).

% Extract a rectangular sub-grid rows [R0..R1], cols [C0..C1] inclusive.
grid_tile_subgrid_(Grid, R0, R1, C0, C1, Sub) :-
    findall(Row,
        (between(R0, R1, R),
         nth0(R, Grid, GRow),
...
grid_tile_w_period(Grid, P) :-
    grid_tile_dims_(Grid, _, W),
    between(1, W, P),
    0 is W mod P,
    grid_tile_cols_have_period_(Grid, P),
    !.

% grid_tile_tile_size(+Grid, -HP, -WP)
...
grid_tile_tile_size(Grid, HP, WP) :-
    grid_tile_h_period(Grid, HP),
    grid_tile_w_period(Grid, WP).

% grid_tile_is_tiling(+Grid, +TH, +TW)
...
grid_tile_is_tiling(Grid, TH, TW) :-
    ...
    0 is H mod TH,
    0 is W mod TW,
    grid_tile_rows_have_period_(Grid, TH),
    grid_tile_cols_have_period_(Grid, TW).

% grid_tile_extract_tile(+Grid, +TH, +TW, -Tile)
...
grid_tile_extract_tile(Grid, TH, TW, Tile) :-
    ...
    grid_tile_subgrid_(Grid, 0, TH1, 0, TW1, Tile).

% grid_tile_tile_to_grid(+Tile, +H, +W, -Grid)
...
% grid_tile_row_is_periodic(+Row, +P, +Len)
...
% grid_tile_col_is_periodic(+Grid, +C, +P)
...
% grid_tile_tile_offset(+TH, +TW, +R, +C, -Offset)
% Offset is TileR-TileC: the position within the tile for grid cell (R, C).
% TileR = R mod TH, TileC = C mod TW.
grid_tile_tile_offset(TH, TW, R, C, TileR-TileC) :-
    TileR is R mod TH,
    TileC is C mod TW.

% grid_tile_all_tiles(+Grid, +TH, +TW, -Tiles)
...
grid_tile_all_tiles(Grid, TH, TW, Tiles) :-
    ...
    findall(Tile,
        (between(0, NH1, TR),
         ...
         grid_tile_subgrid_(Grid, R0, R1, C0, C1, Tile)),
        Tiles).

% grid_tile_crop_to_tile(+Grid, -Tile)
% Tile is the minimal tile of Grid: extract the top-left sub-grid of size
% h_period x w_period. Combines grid_tile_tile_size and grid_tile_extract_tile.
grid_tile_crop_to_tile(Grid, Tile) :-
% Find minimal periods.
    grid_tile_h_period(Grid, HP),
    grid_tile_w_period(Grid, WP),
% Extract the top-left HP x WP region.
    grid_tile_extract_tile(Grid, HP, WP, Tile).
