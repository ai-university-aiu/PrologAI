/*  PrologAI — Causalontology Repair  (WP-415, Layer 390)
    ...
    (co_core, co_verify, co_ward), but nothing knew how to recover once a wrong
    ...
    Predicates:
      md_reset/0            -- forget all repair knowledge
      md_inverse_add/2      -- +Action, +InverseAction
      md_neutralizer_add/2  -- +Effect, +Action
      md_reroute_add/2      -- +Goal, +AlternateGoal
      md_inverse/2          -- ?Action, ?InverseAction
      md_neutralizer/2      -- ?Effect, ?Action
      md_reroute/2          -- ?Goal, ?AlternateGoal
      md_classify/2         -- +Disturbance, -Kind   (reversible|side_effect|blocked|unknown)
      md_compensate/3       -- +Disturbance, -Plan, -Grade
      md_can_compensate/1   -- +Disturbance          (a real repair exists?)
*/
...
:- module(co_mend, [
    % md_reset/0: forget all repair knowledge.
    md_reset/0,
    % md_inverse_add/2: teach that one action reverses another.
    md_inverse_add/2,
    % md_neutralizer_add/2: teach that an action cancels an effect.
    md_neutralizer_add/2,
    % md_reroute_add/2: teach an alternate goal for a blocked one.
    md_reroute_add/2,
    % md_inverse/2: query inverse actions.
    md_inverse/2,
    % md_neutralizer/2: query neutralizing actions.
    md_neutralizer/2,
    % md_reroute/2: query alternate goals.
    md_reroute/2,
    % md_classify/2: name the kind of a disturbance.
    md_classify/2,
    % md_compensate/3: the best available repair plan and its grade.
    md_compensate/3,
    % md_can_compensate/1: whether a real repair exists.
    md_can_compensate/1
]).
...
% md_reset/0: forget every taught repair relation.
md_reset :-
...
% md_inverse_add/2: teach that InverseAction reverses Action.
md_inverse_add(Action, InverseAction) :-
    % Store the relation unless it is already known.
    ( inverse(Action, InverseAction) -> true ; assertz(inverse(Action, InverseAction)) ).

% md_neutralizer_add/2: teach that Action cancels Effect.
md_neutralizer_add(Effect, Action) :-
    % Store the relation unless it is already known.
    ( neutralizer(Effect, Action) -> true ; assertz(neutralizer(Effect, Action)) ).

% md_reroute_add/2: teach that AlternateGoal can replace a blocked Goal.
md_reroute_add(Goal, AlternateGoal) :-
    % Store the relation unless it is already known.
    ( reroute(Goal, AlternateGoal) -> true ; assertz(reroute(Goal, AlternateGoal)) ).

% md_inverse/2: expose the inverse relations.
md_inverse(Action, InverseAction) :-
    % Read the stored inverse.
    inverse(Action, InverseAction).

% md_neutralizer/2: expose the neutralizer relations.
md_neutralizer(Effect, Action) :-
    % Read the stored neutralizer.
    neutralizer(Effect, Action).

% md_reroute/2: expose the reroute relations.
md_reroute(Goal, AlternateGoal) :-
    % Read the stored reroute.
    reroute(Goal, AlternateGoal).

% md_classify/2: name the kind of a disturbance by the repair it can accept.
% A disturbance with an inverse for its action is reversible.
md_classify(disturbance(Action, _), reversible) :-
    inverse(Action, _), !.
% Otherwise a disturbance whose effect has a neutralizer is a side_effect.
md_classify(disturbance(_, Effect), side_effect) :-
    neutralizer(Effect, _), !.
% A blocked goal with an alternate is a blocked disturbance.
md_classify(blocked(Goal), blocked) :-
    reroute(Goal, _), !.
% Anything else is unknown — no repair is known.
md_classify(_, unknown).

% md_compensate/3: the preferred repair plan and its grade for a disturbance.
% First preference: undo the effect with an inverse action.
md_compensate(disturbance(Action, Effect), Plan, Grade) :-
    ( inverse(Action, Inv)
...
md_compensate(blocked(Goal), Plan, Grade) :-
    ( reroute(Goal, Alt)
      -> Plan = reroute(Alt), Grade = reroute
    % Otherwise accept that the goal cannot be reached.
    ; Plan = accept, Grade = none ).

% md_can_compensate/1: a real repair exists when the grade is not none.
md_can_compensate(Disturbance) :-
    % Compute the best plan and check its grade.
    md_compensate(Disturbance, _, Grade),
    Grade \== none.
