:- use_module('../prolog/grid_segment').
...
:- begin_tests(grid_segment).

% --- grid_segment_is_sep_row ---
test(is_sep_row_yes, []) :-
    g_row1sep(G), grid_segment_is_sep_row(G, 1, s).

test(is_sep_row_no, []) :-
    g3x3(G), \+ grid_segment_is_sep_row(G, 0, _).

test(is_sep_row_binds_color, []) :-
    g_row1sep(G), grid_segment_is_sep_row(G, 1, C), C = s.

% --- grid_segment_is_sep_col ---
test(is_sep_col_yes, []) :-
    g_cols02sep(G), grid_segment_is_sep_col(G, 0, s).

test(is_sep_col_no, []) :-
    g3x3(G), \+ grid_segment_is_sep_col(G, 0, _).

test(is_sep_col_binds_color, []) :-
    g_cols02sep(G), grid_segment_is_sep_col(G, 2, C), C = s.

% --- grid_segment_sep_rows ---
test(sep_rows_one, []) :-
    g_row1sep(G), grid_segment_sep_rows(G, s, [1]).

test(sep_rows_none, []) :-
    g3x3(G), grid_segment_sep_rows(G, s, []).

test(sep_rows_two, []) :-
    g_rows02sep(G), grid_segment_sep_rows(G, s, [0,2]).

% --- grid_segment_sep_cols ---
test(sep_cols_two, []) :-
    g_cols02sep(G), grid_segment_sep_cols(G, s, [0,2]).

test(sep_cols_none, []) :-
    g3x3(G), grid_segment_sep_cols(G, s, []).

test(sep_cols_one_1x1, []) :-
    g1x1_sep(G), grid_segment_sep_cols(G, s, [0]).

% --- grid_segment_split_h ---
test(split_h_two_segs, []) :-
    g_row1sep(G), grid_segment_split_h(G, s, [[[a,b,c]],[[d,e,f]]]).

test(split_h_no_sep, []) :-
    g3x3(G), grid_segment_split_h(G, s, [G]).

test(split_h_border_sep, []) :-
    % Sep at top and bottom: one segment in the middle
    g_border_sep(G), grid_segment_split_h(G, s, [[[a,b,c],[d,e,f]]]).

test(split_h_consecutive_sep, []) :-
    % Two consecutive sep rows at top: one segment at bottom
    g_consec_sep(G), grid_segment_split_h(G, s, [[[a,b,c]]]).

% --- grid_segment_split_v ---
test(split_v_two_segs, []) :-
    g_cols02sep(G), grid_segment_split_v(G, s, [[[a],[b],[c]]]).

test(split_v_no_sep, []) :-
    g3x3(G), grid_segment_split_v(G, s, [G]).

test(split_v_border_cols, []) :-
    % Sep at left and right: one segment in the middle
    g_border_cols(G), grid_segment_split_v(G, s, [[[a,b],[c,d],[e,f]]]).

% --- grid_segment_segment_count_h ---
test(segment_count_h_two, []) :-
    g_row1sep(G), grid_segment_segment_count_h(G, s, 2).

test(segment_count_h_one, []) :-
    g3x3(G), grid_segment_segment_count_h(G, s, 1).

% --- grid_segment_segment_count_v ---
test(segment_count_v_one, []) :-
    % g_cols02sep has sep at 0 and 2, content in middle col: 1 segment
    g_cols02sep(G), grid_segment_segment_count_v(G, s, 1).

test(segment_count_v_one_nosep, []) :-
    g3x3(G), grid_segment_segment_count_v(G, s, 1).
...
test(panels_four, []) :-
    ...
    g_both_sep(G), grid_segment_panels(G, s, Panels), length(Panels, 4).

test(panels_two, []) :-
    % g_row1sep: only horizontal sep → 2 row segs, each 1 col seg
    g_row1sep(G), grid_segment_panels(G, s, Panels), length(Panels, 2).

test(panels_one, []) :-
    g3x3(G), grid_segment_panels(G, s, [G]).

% --- grid_segment_panel_count ---
test(panel_count_four, []) :-
    g_both_sep(G), grid_segment_panel_count(G, s, 4).

test(panel_count_two, []) :-
    g_row1sep(G), grid_segment_panel_count(G, s, 2).

test(panel_count_one, []) :-
    g3x3(G), grid_segment_panel_count(G, s, 1).

% --- grid_segment_trim_h ---
test(trim_h_removes_borders, []) :-
    g_border_sep(G), grid_segment_trim_h(G, s, [[a,b,c],[d,e,f]]).

test(trim_h_noop, []) :-
    g3x3(G), grid_segment_trim_h(G, s, G).

test(trim_h_consecutive, []) :-
    % Two sep rows at top removed, content at bottom kept
    g_consec_sep(G), grid_segment_trim_h(G, s, [[a,b,c]]).

% --- grid_segment_trim_v ---
test(trim_v_removes_borders, []) :-
    g_border_cols(G), grid_segment_trim_v(G, s, [[a,b],[c,d],[e,f]]).

test(trim_v_noop, []) :-
    g3x3(G), grid_segment_trim_v(G, s, G).
...
test(trim_both, []) :-
    ...
    g_both_sep(G), grid_segment_trim(G, s, G).

test(trim_noop, []) :-
    g3x3(G), grid_segment_trim(G, s, G).

% --- grid_segment_sep_color ---
test(sep_color_row, []) :-
    % g_row1sep has only s as separator color
    g_row1sep(G), grid_segment_sep_color(G, s).

test(sep_color_col, []) :-
    % g_cols02sep has only s as separator color
    g_cols02sep(G), grid_segment_sep_color(G, s).

% --- Combined tests ---
test(combined_split_count, []) :-
    % segment_count_h equals length of split_h result
    g_three_segs(G),
    grid_segment_split_h(G, s, Segs),
    grid_segment_segment_count_h(G, s, N),
    length(Segs, N).

test(combined_panel_count_matches, []) :-
    % panel_count equals length of panels
    g_both_sep(G),
    grid_segment_panels(G, s, Panels),
    grid_segment_panel_count(G, s, N),
    length(Panels, N).

test(combined_three_segs, []) :-
    % g_three_segs has 3 segments
    g_three_segs(G), grid_segment_segment_count_h(G, s, 3).

test(combined_trim_then_count, []) :-
    % Trimming border seps then counting gives same content
    g_border_sep(G),
    grid_segment_trim_h(G, s, Trimmed),
    length(Trimmed, 2).

test(sep_rows_border, []) :-
    % g_border_sep has sep at rows 0 and 3
    g_border_sep(G), grid_segment_sep_rows(G, s, [0,3]).

test(split_h_three_segments, []) :-
    % g_three_segs: sep at rows 1 and 3, giving 3 segments
    g_three_segs(G), grid_segment_split_h(G, s, Segs), length(Segs, 3).

:- end_tests(grid_segment).
