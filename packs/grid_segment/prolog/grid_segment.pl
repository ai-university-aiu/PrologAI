:- module(grid_segment, [
    grid_segment_is_sep_row/3,
    grid_segment_is_sep_col/3,
    grid_segment_sep_rows/3,
    grid_segment_sep_cols/3,
    grid_segment_split_h/3,
    grid_segment_split_v/3,
    grid_segment_segment_count_h/3,
    grid_segment_segment_count_v/3,
    grid_segment_panels/3,
    grid_segment_panel_count/3,
    grid_segment_trim_h/3,
    grid_segment_trim_v/3,
    grid_segment_trim/3,
    grid_segment_sep_color/2
]).
...
grid_segment_uniform_([], _).
grid_segment_uniform_([Color|Rest], Color) :-
% Every element must equal the head color.
    grid_segment_uniform_(Rest, Color).

% Extract column C from Grid as a top-to-bottom list.
grid_segment_col_(Grid, C, Col) :-
...
grid_segment_transpose_([], []).
grid_segment_transpose_(Grid, Trans) :-
% Build one column-as-row for each column index.
    Grid = [FR|_], length(FR, W),
    W1 is W - 1,
    findall(Col, (between(0, W1, C), grid_segment_col_(Grid, C, Col)), Trans).
...
grid_segment_split_rows_([], _, Acc, Segs) :-
% Flush the last segment when the input is exhausted.
    (Acc = [] -> Segs = [] ; reverse(Acc, S), Segs = [S]).
grid_segment_split_rows_([Row|Rest], Color, Acc, Segs) :-
    (Row = [Color|_], grid_segment_uniform_(Row, Color) ->
        % This row is a separator.
        (Acc = [] ->
            % No accumulated segment yet: just skip.
            grid_segment_split_rows_(Rest, Color, [], Segs)
        ;
            % End current segment and start fresh.
            reverse(Acc, S),
            grid_segment_split_rows_(Rest, Color, [], RestSegs),
            Segs = [S|RestSegs]
        )
    ;
        % This row is content: accumulate it.
        grid_segment_split_rows_(Rest, Color, [Row|Acc], Segs)
    ).

% Drop leading uniform-Color rows from a list of rows.
grid_segment_drop_sep_([], _, []).
grid_segment_drop_sep_([Row|Rest], Color, Result) :-
    (Row = [Color|_], grid_segment_uniform_(Row, Color) ->
% This row is a separator: skip it.
        grid_segment_drop_sep_(Rest, Color, Result)
    ;
...
% grid_segment_is_sep_row(+Grid, +R, ?Color)
...
% grid_segment_is_sep_col(+Grid, +C, ?Color)
...
% grid_segment_sep_rows(+Grid, +Color, -Rows)
...
% Collect every R (bounded) where the row is a separator.
    findall(R, (between(0, H1, R), grid_segment_is_sep_row(Grid, R, Color)), Rows).

% grid_segment_sep_cols(+Grid, +Color, -Cols)
...
% Collect every C (bounded) where the column is a separator.
    findall(C, (between(0, W1, C), grid_segment_is_sep_col(Grid, C, Color)), Cols).

% grid_segment_split_h(+Grid, +Color, -Segments)
...
% Delegate to the accumulator-based helper.
    grid_segment_split_rows_(Grid, Color, [], Segments).

% grid_segment_split_v(+Grid, +Color, -Segments)
...
% Split the transposed grid horizontally.
    grid_segment_split_h(Trans, Color, TSegs),
% Transpose each segment back to column orientation.
    maplist(grid_segment_transpose_, TSegs, Segments).

% grid_segment_segment_count_h(+Grid, +Color, -N)
% N is the number of horizontal segments (groups of rows between separator rows).
grid_segment_segment_count_h(Grid, Color, N) :-
    grid_segment_split_h(Grid, Color, Segs),
    length(Segs, N).

% grid_segment_segment_count_v(+Grid, +Color, -N)
% N is the number of vertical segments (groups of columns between separator cols).
grid_segment_segment_count_v(Grid, Color, N) :-
    grid_segment_split_v(Grid, Color, Segs),
    length(Segs, N).

% grid_segment_panels(+Grid, +Color, -Panels)
...
% Trim leading and trailing separator rows (originally columns).
    grid_segment_trim_h(Trans, Color, TrimmedTrans),
% Transpose back to restore row orientation.
    grid_segment_transpose_(TrimmedTrans, Trimmed).

% grid_segment_trim(+Grid, +Color, -Trimmed)
% Trimmed is Grid with all border separator rows and columns of Color removed.
grid_segment_trim(Grid, Color, Trimmed) :-
% Trim separator rows from top and bottom.
    grid_segment_trim_h(Grid, Color, TrimH),
% Trim separator columns from left and right.
    grid_segment_trim_v(TrimH, Color, Trimmed).

% grid_segment_sep_color(+Grid, -Color)
...
% Collect all colors used in separator rows.
    length(Grid, H), H1 is H - 1,
    findall(C, (between(0, H1, R), grid_segment_is_sep_row(Grid, R, C)), RowColors),
% Collect all colors used in separator columns.
    (Grid = [FR|_] -> length(FR, W) ; W = 0),
    W1 is W - 1,
    findall(C, (between(0, W1, Col), grid_segment_is_sep_col(Grid, Col, C)), ColColors),
