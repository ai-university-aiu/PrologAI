% State the fact: name(agent_network_protocol).
name(agent_network_protocol).
% State the fact: version('1.0.0').
version('1.0.0').
% State the fact: title('PrologAI ANP Gateway — Agent Network Protocol decentralized identity and peer discovery (PR 48)').
title('PrologAI ANP Gateway — Agent Network Protocol decentralized identity and peer discovery (PR 48)').
