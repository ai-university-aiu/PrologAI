/*  PrologAI — Causalontology Object Relations Test Suite  (WP-408)
    ...
    Run with the full library path:
        swipl $LIB -g "run_tests, halt" -t "halt(1)" packs/co_rel/test/test_co_rel.pl
*/
...
% Open the test block for co_rel.
:- begin_tests(co_rel).
...
test(larger_than_all_others) :-
    ...
    assertion(cr_larger(A, B)),
    % More than the pusher.
    assertion(cr_larger(A, C)),
    % And more than the follower.
    assertion(cr_larger(A, D)).
