...
% ERC
    tmp_file(pai_rsa_priv, PrivPEMFile),
% ERC
    tmp_file(pai_rsa_pub, PubPEMFile),
% ERC
...
% ERC
    ;   throw(error(
% ERC
            lattice_crypto_error(pqc_provider_unavailable,
% ERC
...
% ERC
    tmp_file(pai_pqc_priv, PrivPEMFile),
% ERC
    tmp_file(pai_pqc_pub, PubPEMFile),
% ERC
...
% ERC
lc_encrypt_rsa(PubKey, Plaintext, Bundle, TagHex) :-
% ERC
    lc_random_bytes_atom(32, AesKeyAtom),
% ERC
    lc_random_bytes_atom(12, IVAtom),
% ERC
...
% ERC
    lc_atom_to_hex(WrappedKeyAtom, WrappedKeyHex),
% ERC
    lc_atom_to_hex(IVAtom, IVHex),
% ERC
    lc_atom_to_hex(CipherAtom, CipherHex),
% ERC
...
% ERC
% lc_decrypt_rsa(+PrivKey, +Bundle, +TagHex, -Plaintext)
% ERC
lc_decrypt_rsa(PrivKey,
% ERC
               rsa_bundle(WrappedKeyHex, IVHex, CipherHex),
% ERC
               TagHex, Plaintext) :-
% ERC
    lc_hex_to_atom(WrappedKeyHex, WrappedKeyAtom),
% ERC
    rsa_private_decrypt(PrivKey, WrappedKeyAtom, AesKeyAtom, [padding(pkcs1_oaep)]),
% ERC
    lc_hex_to_atom(IVHex, IVAtom),
% ERC
    lc_hex_to_atom(CipherHex, CipherAtom),
% ERC
    hex_bytes(TagHex, TagBytes),
% ERC
...
% ERC
% lc_encrypt_ecdh(+Curve, +RecipPub, +Plaintext, -Bundle, -Tag)
% ECDH hybrid encrypt:
...
% ERC
lc_encrypt_ecdh(Curve, RecipPub, Plaintext, Bundle, TagHex) :-
% ERC
    crypto_name_curve(Curve, CurveHandle),
% ERC
    crypto_curve_generator(CurveHandle, Gen),
% ERC
    lc_random_scalar(EphPriv),
% ERC
...
% ERC
    lc_derive_aes_key_atom(SharedX, AesKeyAtom),
% ERC
    lc_random_bytes_atom(12, IVAtom),
% ERC
    atom_string(Plaintext, PlainStr),
...
% ERC
% lc_decrypt_ecdh(+Curve, +RecipPriv, +Bundle, +TagHex, -Plaintext)
% ERC
lc_decrypt_ecdh(Curve, RecipPriv,
% ERC
                ecdh_bundle(EphPubX, EphPubY, IVHex, CipherHex),
% ERC
                TagHex, Plaintext) :-
...
% ERC
% lc_encrypt_pqc(+Algorithm, +RecipPubKeyFile, +Plaintext, -Bundle, -Tag)
% PQC hybrid encrypt using ML-KEM key encapsulation.
% Requires OQS provider; throws lattice_crypto_error if absent.
% ERC
...
% ERC
    tmp_file(pai_pqc_secret, SecretFile),
% ERC
    process_create(path(openssl),
% ERC
                   [pkeyutl, '-encap',
...
% ERC
% lc_decrypt_pqc(+RecipPrivKeyFile, +Bundle, +TagHex, -Plaintext)
% ERC
lc_decrypt_pqc(RecipPrivKeyFile,
% ERC
...
% ERC
    tmp_file(pai_pqc_decap_in, EncapInFile),
% ERC
    tmp_file(pai_pqc_decap_secret, SecretOutFile),
% ERC
    lc_write_bytes_to_file(EncapInFile, EncapBytes),
% ERC
    process_create(path(openssl),
% ERC
                   [pkeyutl, '-decap',
...
% ERC
% lc_require_pqc_provider(+Caller)
...
% ERC
% lc_atom_to_hex(+BinaryAtom, -HexAtom)
...
% ERC
% lc_hex_to_atom(+HexAtom, -BinaryAtom)
...
% ERC
% lc_write_bytes_to_file(+File, +ByteList)
