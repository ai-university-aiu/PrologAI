% State the fact: name(lattice_crypto).
name(lattice_crypto).
% State the fact: version('1.0.0').
version('1.0.0').
% State the fact: title('PrologAI Lattice Cryptographic Privacy Layer — RSA, ECDH, and PQC hybrid encryption (PR 49)').
title('PrologAI Lattice Cryptographic Privacy Layer — RSA, ECDH, and PQC hybrid encryption (PR 49)').
