:- module(gridhist, [
    gridhist_row_hist/3,
    gridhist_col_hist/3,
    gridhist_all_row_hists/2,
    gridhist_all_col_hists/2,
    gridhist_modal_row/3,
    gridhist_modal_col/3,
    gridhist_row_count/4,
    gridhist_col_count/4,
    gridhist_row_entropy/3,
    gridhist_col_entropy/3,
    gridhist_max_row/3,
    gridhist_min_row/3,
    gridhist_rows_with/4,
    gridhist_cols_with/4
]).
...
gridhist_modal_(Hist, BestColor) :-
...
% Build histogram from the list.
    gridhist_list_hist_(Row, Hist).

% gridhist_col_hist(+Grid, +C, -Hist)
...
% Extract column C as a top-to-bottom list.
    gridhist_col_(Grid, C, Col),
% Build histogram from the list.
    gridhist_list_hist_(Col, Hist).

% gridhist_all_row_hists(+Grid, -AllHists)
...
% Collect one histogram per row.
    findall(Hist, (between(0, H1, R), gridhist_row_hist(Grid, R, Hist)), AllHists).

% gridhist_all_col_hists(+Grid, -AllHists)
...
% Collect one histogram per column.
    findall(Hist, (between(0, W1, C), gridhist_col_hist(Grid, C, Hist)), AllHists).

% gridhist_modal_row(+Grid, +R, -Color)
% Color is the most frequent color in row R. Ties broken by last-in-sort order.
gridhist_modal_row(Grid, R, Color) :-
% Get the row histogram.
    gridhist_row_hist(Grid, R, Hist),
% Find the color with the highest count.
    gridhist_modal_(Hist, Color).

% gridhist_modal_col(+Grid, +C, -Color)
...
% gridhist_row_count(+Grid, +R, +Color, -Count)
...
% gridhist_col_count(+Grid, +C, +Color, -Count)
...
% gridhist_max_row(+Grid, +Color, -Row)
...
% Count Color in every row.
    length(Grid, H), H1 is H - 1,
    findall(N-R, (between(0, H1, R), gridhist_row_count(Grid, R, Color, N), N > 0), Pairs),
...
% gridhist_min_row(+Grid, +Color, -Row)
...
% gridhist_rows_with(+Grid, +Color, +N, -Rows)
% Rows is the list of row indices where Color appears at least N times.
gridhist_rows_with(Grid, Color, N, Rows) :-
% Compute the valid row index range.
    length(Grid, H), H1 is H - 1,
% Collect every R where the count of Color is >= N.
    findall(R, (between(0, H1, R), gridhist_row_count(Grid, R, Color, C), C >= N), Rows).

% gridhist_cols_with(+Grid, +Color, +N, -Cols)
...
% Collect every C where the count of Color is >= N.
    findall(C, (between(0, W1, C), gridhist_col_count(Grid, C, Color, Cnt), Cnt >= N), Cols).
