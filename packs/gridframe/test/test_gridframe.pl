...
test(cell_depth_center_5x5, []) :-
    ...
    gridframe_cell_depth(G, 2, 2, 2).

% --- gridframe_frame cell counts ---
...
test(max_depth_3x3, []) :-
    g3x3(G),
    gridframe_max_depth(G, 1).

test(max_depth_5x5, []) :-
    g5x5(G),
    gridframe_max_depth(G, 2).
...
test(max_depth_2x4, []) :-
    g2x4(G),
    % min(2,4)=2; (2-1)//2=0
    gridframe_max_depth(G, 0).

% --- gridframe_frame_count ---
test(frame_count_3x3, []) :-
    g3x3(G),
    gridframe_frame_count(G, 2).

test(frame_count_5x5, []) :-
    g5x5(G),
    gridframe_frame_count(G, 3).

test(frame_count_1x1, []) :-
    g1x1(G),
    gridframe_frame_count(G, 1).

% --- gridframe_peel ---
test(peel_3x3_size, []) :-
    g3x3_rx(G),
    gridframe_peel(G, Inner),
    % 3x3 peeled = 1x1
    length(Inner, 1),
    Inner = [[x]].
...
test(frame_uniform_0_3x3_no, []) :-
    ...
    gridframe_frame_uniform(G, 0, no).

% --- gridframe_frame_colors ---
...
test(frame_colors_2_onion, []) :-
    g5x5_onion(G),
    gridframe_frame_colors(G, 2, Colors),
    Colors = [b].
...
test(fill_frames_5x5, []) :-
    g5x5(G),
    gridframe_fill_frames(G, [r, x, b], Result),
...
% --- gridframe_onion_layers ---
test(onion_layers_rx, []) :-
    g3x3_rx(G),
    gridframe_onion_layers(G, Layers),
    Layers = [0-r, 1-x].
