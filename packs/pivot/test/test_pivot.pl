...
% pv_centroid of a 2x2 block of cells returns the floor average.
test(centroid_block) :-
    % Cells forming a 2x2 block starting at (0,0).
    pv_centroid([0-0, 0-1, 1-0, 1-1], PR-PC),
    % Floor average of rows: (0+0+1+1)//4 = 0; columns: same.
    PR =:= 0, PC =:= 0.

% pv_centroid of a column gives the middle row.
test(centroid_column) :-
    % Three cells in column 2, rows 0, 2, 4.
    pv_centroid([0-2, 2-2, 4-2], PR-PC),
    % Row average: (0+2+4)//3 = 2; col average: (2+2+2)//3 = 2.
    PR =:= 2, PC =:= 2.

% pv_centroid of a single cell is the cell itself.
...
% pv_to_rel computes correct offsets for a cell above and left of pivot.
test(to_rel_above_left) :-
    % Pivot at (2,2), cell at (0,0): offsets are -2, -2.
    pv_to_rel(2-2, 0-0, DR-DC),
    DR =:= -2, DC =:= -2.

% pv_to_rel for a cell at the pivot itself returns 0-0.
test(to_rel_same) :-
    % Pivot and cell at (3,4).
    pv_to_rel(3-4, 3-4, DR-DC),
    DR =:= 0, DC =:= 0.

% pv_to_rel for a cell below and right of pivot.
test(to_rel_below_right) :-
    % Pivot at (1,1), cell at (3,4): offsets are 2, 3.
    pv_to_rel(1-1, 3-4, DR-DC),
    DR =:= 2, DC =:= 3.

% --- pv_from_rel/3 ---

% pv_from_rel restores the original cell from offset 0-0.
test(from_rel_identity) :-
    % Pivot at (2,3), zero offset gives the pivot itself.
    pv_from_rel(2-3, 0-0, R-C),
    R =:= 2, C =:= 3.

% pv_from_rel converts a negative offset to a cell above the pivot.
test(from_rel_negative) :-
    % Pivot at (4,4), offset -2-(-1) gives cell at (2,3).
    pv_from_rel(4-4, (-2)-(-1), R-C),
    R =:= 2, C =:= 3.

% pv_to_rel and pv_from_rel are inverses.
test(from_rel_round_trip) :-
    % Convert cell to relative then back should give original.
    pv_to_rel(2-3, 5-7, DR-DC),
    pv_from_rel(2-3, DR-DC, R-C),
    R =:= 5, C =:= 7.
...
test(rotate_cw_right_to_down) :-
    ...
    pv_rotate_cell_cw(2-2, 2-3, R2-C2),
    R2 =:= 3, C2 =:= 2.
...
test(rotate_cw_up_to_right) :-
    ...
    pv_rotate_cell_cw(2-2, 1-2, R2-C2),
    R2 =:= 2, C2 =:= 3.
...
test(rotate_cw_pivot_unchanged) :-
    ...
    pv_rotate_cell_cw(3-3, 3-3, R2-C2),
    R2 =:= 3, C2 =:= 3.

% --- pv_rotate_cells_cw/3 ---

% pv_rotate_cells_cw rotates all four cardinal neighbors 90 degrees CW.
...
% pv_rotate_cells_cw on an empty list returns empty.
test(rotate_cells_cw_empty) :-
    % No cells to rotate.
    pv_rotate_cells_cw(0-0, [], Cells2),
    Cells2 = [].

% pv_rotate_cells_cw: four CW rotations return to original.
test(rotate_cells_cw_4x) :-
    % One cell offset (1,0) below pivot.
    pv_rotate_cells_cw(0-0, [1-0], C1),
    pv_rotate_cells_cw(0-0, C1, C2),
    pv_rotate_cells_cw(0-0, C2, C3),
    pv_rotate_cells_cw(0-0, C3, C4),
...
% pv_rotate_cells_180 twice returns the original.
test(rotate_180_double) :-
    % Two 180-degree rotations restore the original.
    pv_rotate_cells_180(3-3, [1-1, 5-5], Step1),
    pv_rotate_cells_180(3-3, Step1, Step2),
    Step2 = [1-1, 5-5].

% pv_rotate_cells_180 of the pivot cell is itself.
test(rotate_180_pivot) :-
    % The pivot rotated 180 around itself stays put.
    pv_rotate_cells_180(2-2, [2-2], [R2-C2]),
    R2 =:= 2, C2 =:= 2.
...
test(rotate_ccw_cw_cancel) :-
    ...
    pv_rotate_cells_cw(1-1, [1-3], Step1),
    pv_rotate_cells_ccw(1-1, Step1, Step2),
    Step2 = [1-3].
...
test(reflect_diag_swap_offsets) :-
    ...
    pv_reflect_cells_diag(0-0, [1-3], [R2-C2]),
    R2 =:= 3, C2 =:= 1.

% Diagonal reflection is its own inverse.
test(reflect_diag_involution) :-
    % Two diagonal reflections = identity.
    pv_reflect_cells_diag(2-2, [0-4], Step1),
    pv_reflect_cells_diag(2-2, Step1, Step2),
    Step2 = [0-4].
...
test(reflect_diag_on_axis) :-
    ...
    pv_reflect_cells_diag(1-1, [3-3], [R2-C2]),
    R2 =:= 3, C2 =:= 3.
...
test(reflect_antidiag_negate_swap) :-
    ...
    pv_reflect_cells_antidiag(1-1, [0-3], [R2-C2]),
    R2 =:= -1, C2 =:= 2.

% Anti-diagonal reflection is its own inverse.
test(reflect_antidiag_involution) :-
    % Two anti-diagonal reflections = identity.
    pv_reflect_cells_antidiag(0-0, [2-1], Step1),
    pv_reflect_cells_antidiag(0-0, Step1, Step2),
    Step2 = [2-1].
...
test(reflect_antidiag_on_axis) :-
    ...
    pv_reflect_cells_antidiag(1-1, [0-2], [R2-C2]),
    R2 =:= 0, C2 =:= 2.

% --- pv_orbit/3 ---

% pv_orbit of a cell at an offset of (1,0) from pivot has 4 elements.
test(orbit_cardinal) :-
    ...
    pv_orbit(0-0, 1-0, Orbit),
    % Expect the 4 cardinal positions (reflections coincide with rotations here).
    length(Orbit, 4).

% pv_orbit of the pivot cell itself has only 1 element.
test(orbit_pivot_single) :-
    % The pivot maps to itself under all 8 D4 transforms.
    pv_orbit(2-2, 2-2, Orbit),
    Orbit = [2-2].

% pv_orbit of a diagonal cell has 4 distinct positions.
test(orbit_diagonal) :-
    ...
    pv_orbit(0-0, 1-1, Orbit),
    length(Orbit, 4).

% --- pv_sym_closure/3 ---

% pv_sym_closure of a single cell equals its orbit.
...
% pv_sym_closure of a D4-symmetric set is the set itself.
test(sym_closure_already_symmetric) :-
    % The 4 cardinal neighbors form a D4-symmetric set.
    pv_sym_closure(0-0, [1-0, 0-1, (-1)-0, 0-(-1)], Closure),
    % Closure = same 4 cells.
    length(Closure, 4).

% pv_sym_closure of an empty list is empty.
...
% pv_stamp_at silently ignores out-of-bounds offsets.
test(stamp_at_oob) :-
    % Offset (0,-1) from pivot (0,0) gives column -1, which is out of bounds.
    pv_stamp_at([[0,0],[0,0]], [0-(-1)], 9, 0-0, Result),
