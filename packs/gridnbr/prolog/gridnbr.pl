:- module(gridnbr, [
    ...
    gridnbr_count4/5,
    gridnbr_count8/5,
    gridnbr_count4_grid/3,
    gridnbr_count8_grid/3,
    gridnbr_any4/4,
    gridnbr_all4/4,
    gridnbr_mark_border/3,
    gridnbr_mark_isolated/3,
    gridnbr_expand_color/3,
    gridnbr_shrink_color/3,
    gridnbr_majority_nbr4/4,
    gridnbr_conway_step/4
]).
...
gridnbr_nbrs_(Grid, R, C, DeltaPred, Vals) :-
% Get grid dimensions.
    gridnbr_dims_(Grid, H, W),
% For each delta, check bounds and read cell value.
    findall(V,
        (call(DeltaPred, [DR, DC]),
         R2 is R + DR,
         C2 is C + DC,
         gridnbr_in_bounds_(R2, C2, H, W),
         gridnbr_cell_(Grid, R2, C2, V)),
        Vals).

% Count occurrences of Value in a list.
gridnbr_count_val_([], _, 0).
gridnbr_count_val_([H|T], V, N) :-
% Increment if head matches value.
    gridnbr_count_val_(T, V, N0),
    (H = V -> N is N0 + 1 ; N = N0).

% Find the most frequent value in a list; fail on empty list.
gridnbr_majority_val_(Vals, Maj) :-
% Require non-empty.
    Vals \= [],
% Get distinct values.
...
% Collect values from four cardinal directions.
    gridnbr_nbrs_(Grid, R, C, gridnbr_delta4_, Vals).

% gridnbr_nbr8(+Grid, +R, +C, -Vals)
...
% Collect values from all eight directions.
    gridnbr_nbrs_(Grid, R, C, gridnbr_delta8_, Vals).

% gridnbr_count4(+Grid, +R, +C, +Color, -N)
% N is the count of 4-connected neighbors of (R,C) whose value equals Color.
gridnbr_count4(Grid, R, C, Color, N) :-
% Get 4-neighbor values then count matches.
    gridnbr_nbr4(Grid, R, C, Vals),
    gridnbr_count_val_(Vals, Color, N).

% gridnbr_count8(+Grid, +R, +C, +Color, -N)
...
% gridnbr_count4_grid(+Grid, +Color, -CountGrid)
...
% gridnbr_count8_grid(+Grid, +Color, -CountGrid)
% CountGrid is a raw grid where each cell value is the count of Color
...
% gridnbr_mark_border(+Grid, +Bg, -MarkedGrid)
    ...
    findall(Row,
        (between(0, H1, R),
         findall(NV,
             (between(0, W1, C),
              gridnbr_cell_(Grid, R, C, V),
              (V \= Bg, gridnbr_any4(Grid, R, C, Bg)
              -> NV = border
              ;  NV = V)),
             Row)),
        MarkedGrid).

% gridnbr_mark_isolated(+Grid, +Bg, -MarkedGrid)
    ...
    findall(Row,
        (between(0, H1, R),
         findall(NV,
             (between(0, W1, C),
              gridnbr_cell_(Grid, R, C, V),
              (V \= Bg, \+ gridnbr_any4(Grid, R, C, V)
              -> NV = isolated
              ;  NV = V)),
             Row)),
        MarkedGrid).

% gridnbr_expand_color(+Grid, +Color, -Expanded)
    ...
    findall(Row,
        (between(0, H1, R),
         findall(NV,
             (between(0, W1, C),
              gridnbr_cell_(Grid, R, C, V),
              (V = Bg, gridnbr_any4(Grid, R, C, Color)
              -> NV = Color
              ;  NV = V)),
             Row)),
        Expanded).

% gridnbr_shrink_color(+Grid, +Color, -Shrunk)
...
% gridnbr_majority_nbr4(+Grid, +R, +C, -Maj)
...
% gridnbr_conway_step(+Grid, +Color, +N, -NewGrid)
...
% Find background.
    ...
    findall(NegNV-BgC,
        (member(BgC, Colors),
         gridnbr_count_val_(Vals, BgC, NV),
         NegNV is -NV),
        Keyed),
    msort(Keyed, [_-Bg | _]),
        ...
        (between(0, H1, R),
         findall(NVal,
             (between(0, W1, C),
              gridnbr_cell_(Grid, R, C, V),
              (V = Color,
               gridnbr_count4(Grid, R, C, Color, CN),
               CN =:= N
              -> NVal = Bg
              ;  NVal = V)),
             Row)),
