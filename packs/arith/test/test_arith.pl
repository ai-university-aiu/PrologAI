...
test(add_basic) :-
    ...
    ar_cell_add([[1,2],[3,4]], [[5,6],[7,8]], G3),
    G3 = [[6,8],[10,12]].

test(add_zeros) :-
    % Adding zeros leaves the grid unchanged.
    ar_cell_add([[1,2],[3,4]], [[0,0],[0,0]], G3),
    G3 = [[1,2],[3,4]].

test(add_symmetric) :-
    % Addition is symmetric: A+B = B+A.
    ar_cell_add([[1,2]], [[3,4]], G1),
    ar_cell_add([[3,4]], [[1,2]], G2),
    G1 = G2.
...
test(mul_basic) :-
    ...
    ar_cell_mul([[1,2],[3,4]], [[2,3],[4,5]], G3),
    G3 = [[2,6],[12,20]].

test(mul_by_ones) :-
    % Multiplying by 1s leaves the grid unchanged.
    ar_cell_mul([[3,4],[5,6]], [[1,1],[1,1]], G3),
    G3 = [[3,4],[5,6]].

test(mul_by_zeros) :-
    % Multiplying by 0s gives all zeros.
    ar_cell_mul([[7,8],[9,10]], [[0,0],[0,0]], G3),
    G3 = [[0,0],[0,0]].
...
test(mod_basic) :-
    ...
    ar_cell_mod([[5,6],[7,8]], 3, G2),
    G2 = [[2,0],[1,2]].

test(mod_one) :-
    % Any value mod 1 = 0.
    ar_cell_mod([[4,9],[2,7]], 1, G2),
    G2 = [[0,0],[0,0]].

test(mod_large) :-
    % Modulo larger than all values: unchanged.
    ar_cell_mod([[1,2],[3,4]], 10, G2),
    G2 = [[1,2],[3,4]].
...
test(row_sum_basic) :-
    ...
    ar_row_sum([[1,2,3],[4,5,6],[7,8,9]], 1, Sum),
    Sum =:= 15.

test(row_sum_zeros) :-
    % Sum of a row of zeros is 0.
    ar_row_sum([[0,0,0],[1,2,3]], 0, Sum),
    Sum =:= 0.
...
test(col_sum_basic) :-
    ...
    ar_col_sum([[1,2,3],[4,5,6],[7,8,9]], 2, Sum),
    Sum =:= 18.

test(col_sum_zeros) :-
    % Column 1 is all zeros.
    ar_col_sum([[1,0,3],[4,0,6]], 1, Sum),
    Sum =:= 0.
...
test(row_sums_basic) :-
    ...
    ar_row_sums([[1,2],[3,4],[5,6]], Sums),
    Sums = [3,7,11].

test(row_sums_single_row) :-
    % Single-row grid.
    ar_row_sums([[4,5,6]], Sums),
    Sums = [15].
...
test(col_sums_basic) :-
    ...
    ar_col_sums([[1,2,3],[4,5,6]], Sums),
    Sums = [5,7,9].

test(col_sums_single_col) :-
    % Single-column grid.
    ar_col_sums([[2],[3],[4]], Sums),
    Sums = [9].
...
test(max_basic) :-
    ...
    ar_cell_max([[1,5],[3,2]], Max),
    Max =:= 5.

test(max_uniform) :-
    % All same: max equals that value.
    ar_cell_max([[7,7],[7,7]], Max),
    Max =:= 7.

test(max_negative) :-
    % Maximum of negative values.
    ar_cell_max([[-3,-1],[-5,-2]], Max),
    Max =:= -1.
...
test(min_basic) :-
    ...
    ar_cell_min([[4,1],[3,2]], Min),
    Min =:= 1.

test(min_uniform) :-
    % All same: min equals that value.
    ar_cell_min([[5,5],[5,5]], Min),
    Min =:= 5.
...
test(abs_diff_basic) :-
    ...
    ar_cell_abs_diff([[1,5],[3,2]], [[4,2],[1,7]], G3),
    G3 = [[3,3],[2,5]].

test(abs_diff_same) :-
    % Absolute difference of a grid with itself is all zeros.
    ar_cell_abs_diff([[3,4],[5,6]], [[3,4],[5,6]], G3),
    G3 = [[0,0],[0,0]].

test(abs_diff_symmetric) :-
    % |A - B| = |B - A|.
    ar_cell_abs_diff([[5,1]], [[2,4]], G1),
    ar_cell_abs_diff([[2,4]], [[5,1]], G2),
    G1 = G2.
