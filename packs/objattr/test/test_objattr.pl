...
test(cell_counts_by_color_basic) :-
    ...
    oa_cell_counts_by_color([R, RB, B], Pairs),
    member(b-1, Pairs),
    member(r-4, Pairs).
...
test(cell_counts_by_color_same) :-
    ...
    % both size 1
    oa_cell_counts_by_color([D, B], Pairs),
    member(b-1, Pairs),
    member(r-1, Pairs).
...
test(n_objs_of_color_basic) :-
    red_dot(R), red_bar(RB), blue_dot(B),
    oa_n_objs_of_color([R, RB, B], r, N),
    N == 2.

test(n_objs_of_color_zero) :-
    red_dot(R),
    oa_n_objs_of_color([R], b, N),
    N == 0.
...
test(dominant_color_basic) :-
    ...
    oa_dominant_color([S, B, G], C),
    C == r.

test(dominant_color_single) :-
    red_dot(R),
    oa_dominant_color([R], C),
    C == r.
...
test(unique_color_basic) :-
    ...
    oa_unique_color([R, B, RB], C),
    C == b.

test(unique_color_all_unique) :-
    red_dot(R), blue_dot(B), green_dot(G),
    % all appear once; oa_unique_color/2 returns first alphabetically
    oa_unique_color([R, B, G], _).

test(unique_color_none) :-
    red_dot(R), red_bar(RB),
    \+ oa_unique_color([R, RB], _).
...
test(majority_size_basic) :-
    ...
    oa_majority_size([D1, D2, B, S], N),
    N == 1.

test(majority_size_single) :-
    red_bar(B),
    oa_majority_size([B], N),
    N == 3.
...
test(all_same_color_true) :-
    red_dot(D), red_bar(B), red_sq(S),
    oa_all_same_color([D, B, S]).

test(all_same_color_false) :-
    red_dot(D), blue_dot(B),
    \+ oa_all_same_color([D, B]).
...
test(all_same_size_true) :-
    ...
    oa_all_same_size([D, BD, G]).

test(all_same_size_false) :-
    red_dot(D), red_bar(B),
    \+ oa_all_same_size([D, B]).
...
test(all_same_form_true) :-
    ...
    oa_all_same_form([L, LF]).

test(all_same_form_false) :-
    lshape(L), red_bar(B),
    \+ oa_all_same_form([L, B]).
