...
% bound_bbox(+Grid, +Color, -R0, -C0, -R1, -C1): tight bounding box of Color in Grid.
...
bound_bbox(Grid, Color, R0, C0, R1, C1) :-
...
% bound_bbox_h(+R0, +R1, -H): height of the inclusive bounding box [R0, R1].
bound_bbox_h(R0, R1, H) :-
% Add 1 because both endpoints are inclusive.
    H is R1 - R0 + 1.

% bound_bbox_w(+C0, +C1, -W): width of the inclusive bounding box [C0, C1].
bound_bbox_w(C0, C1, W) :-
% Add 1 because both endpoints are inclusive.
    W is C1 - C0 + 1.

% bound_crop_bbox(+Grid, +R0, +C0, +R1, +C1, -Sub): crop Grid to [R0..R1, C0..C1].
% All bounds are 0-based and inclusive. Returns the sub-grid as a list of rows.
bound_crop_bbox(Grid, R0, C0, R1, C1, Sub) :-
...
% Extract the column slice [C0, C1] inclusive.
        bound_slice_row_(C0, C1, Row, SubRow)
    ), Sub).

% bound_slice_row_(+C0, +C1, +Row, -Sub): extract elements at columns [C0, C1] inclusive.
% Skips the first C0 elements then takes C1-C0+1 elements using length/append.
bound_slice_row_(C0, C1, Row, Sub) :-
...
% bound_crop_color(+Grid, +Color, -Sub): crop to the tight bounding box of Color.
% Combines bound_bbox and bound_crop_bbox. Fails if Color is absent.
bound_crop_color(Grid, Color, Sub) :-
% Find the tight bounding box of Color.
    bound_bbox(Grid, Color, R0, C0, R1, C1),
% Crop the grid to that box.
    bound_crop_bbox(Grid, R0, C0, R1, C1, Sub).

% bound_trim(+Grid, +Bg, -Trimmed): remove all all-Bg border rows and columns.
% Finds the tight bounding box of all non-Bg cells and returns that sub-grid.
% Returns [[]] for a grid where every cell is Bg.
bound_trim(Grid, Bg, Trimmed) :-
% Get grid dimensions.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0), W1 is W - 1,
...
% Compute the bounding box of non-Bg cells.
        ...
        min_list(Cs, C0), max_list(Cs, C1),
% Crop to that box.
        bound_crop_bbox(Grid, R0, C0, R1, C1, Trimmed)
    ).

% bound_pad(+Grid, +N, +Bg, -Padded): add N background cells on all four sides.
% The result has H+2*N rows and W+2*N columns where H and W are the original dims.
bound_pad(Grid, N, Bg, Padded) :-
...
% Pad each original row with N background cells on each side.
    maplist(bound_pad_row_(N, Bg), Grid, PaddedRows),
% Prepend the top padding.
    append(Padding, PaddedRows, Tmp),
% Append the bottom padding.
    append(Tmp, Padding, Padded).

% bound_pad_row_(+N, +Bg, +Row, -Padded): add N Bg elements on each side of Row.
bound_pad_row_(N, Bg, Row, Padded) :-
...
% bound_place(+Canvas, +Patch, +R0, +C0, +Bg, -New): place Patch at (R0, C0) in Canvas.
...
bound_place(Canvas, Patch, R0, C0, _Bg, New) :-
...
% bound_center(+Grid, +Color, -CR, -CC): integer center of Color's bounding box.
% CR = (R0 + R1) // 2, CC = (C0 + C1) // 2.
bound_center(Grid, Color, CR, CC) :-
...
% bound_bbox_contains(+R0, +C0, +R1, +C1, +R, +C): succeed if (R,C) is in [R0..R1, C0..C1].
bound_bbox_contains(R0, C0, R1, C1, R, C) :-
% Test all four inclusive bounds.
    R >= R0, R =< R1, C >= C0, C =< C1.

% bound_bbox_overlap(+R0A, +C0A, +R1A, +C1A, +R0B, +C0B, +R1B, +C1B): two boxes overlap.
% Fails if the boxes are disjoint.
% Overlap iff: R1A >= R0B, R0A =< R1B, C1A >= C0B, C0A =< C1B.
bound_bbox_overlap(R0A, C0A, R1A, C1A, R0B, C0B, R1B, C1B) :-
% Row ranges must overlap.
    R1A >= R0B, R0A =< R1B,
% Column ranges must overlap.
    C1A >= C0B, C0A =< C1B.

% bound_bbox_union(+R0A,+C0A,+R1A,+C1A,+R0B,+C0B,+R1B,+C1B,-R0,-C0,-R1,-C1):
...
% bound_expand(+R0, +C0, +R1, +C1, +N, -ER0, -EC0, -ER1, -EC1): expand bbox by N.
