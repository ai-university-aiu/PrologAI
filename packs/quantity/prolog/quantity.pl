...
:- module(quantity, [
    quantity_histogram/2,
    quantity_group_by_color/2,
    quantity_group_by_size/2,
    quantity_group_by_shape/2,
    quantity_most_frequent_color/2,
    quantity_least_frequent_color/2,
    quantity_unique_shapes/2,
    quantity_count_where/3,
    quantity_all_same_color/1,
    quantity_all_same_size/1,
    quantity_all_same_shape/1,
    quantity_colors_match/2,
    quantity_shapes_match/2,
    quantity_sizes_match/2,
    quantity_exactly_n/3,
    quantity_at_least_n/3,
    quantity_max_color_count/2,
    quantity_min_color_count/2
]).
...
% HISTOGRAM
% quantity_histogram(+Objects, -Pairs): list of Color-Count pairs for all object colors.
quantity_histogram(Objects, Pairs) :-
...
% color_count_pair(+Objects, +Color, -Color-Count): helper for quantity_histogram.
...
% GROUPING
% quantity_group_by_color(+Objects, -Groups): list of Color-[Objects] pairs.
quantity_group_by_color(Objects, Groups) :-
% Build Color-Obj pairs, sort by color key.
    maplist(object_to_color_pair, Objects, Pairs),
% Sort by key so group_pairs_by_key works.
    msort(Pairs, Sorted),
% Group all objects under their color key.
    group_pairs_by_key(Sorted, Groups).

% object_to_color_pair(+Obj, -Color-Obj): build a keyed pair for grouping.
object_to_color_pair(Obj, Color-Obj) :-
% Extract the color as the grouping key.
    scene_obj_color(Obj, Color).

% quantity_group_by_size(+Objects, -Groups): list of Size-[Objects] pairs.
quantity_group_by_size(Objects, Groups) :-
% Build Size-Obj pairs.
    maplist(object_to_size_pair, Objects, Pairs),
% Sort by integer key for correct grouping.
    msort(Pairs, Sorted),
% Group all objects sharing the same size.
    group_pairs_by_key(Sorted, Groups).

% object_to_size_pair(+Obj, -Size-Obj): build a size-keyed pair for grouping.
object_to_size_pair(Obj, Size-Obj) :-
% Compute cell count as the grouping key.
    scene_obj_size(Obj, Size).

% quantity_group_by_shape(+Objects, -Groups): list of Shape-[Objects] pairs.
quantity_group_by_shape(Objects, Groups) :-
% Build Shape-Obj pairs where Shape is the normalized cell set.
    maplist(object_to_shape_pair, Objects, Pairs),
% Sort by term order (shapes are lists of r/2 terms, comparable).
    msort(Pairs, Sorted),
% Group all objects sharing the same normalized shape.
    group_pairs_by_key(Sorted, Groups).

% object_to_shape_pair(+Obj, -Shape-Obj): build a shape-keyed pair for grouping.
object_to_shape_pair(Obj, Shape-Obj) :-
% Normalize the cell set as the grouping key.
    scene_obj_shape(Obj, Shape).


% FREQUENCY ANALYSIS
% quantity_most_frequent_color(+Objects, -Color): color occurring in the most objects.
quantity_most_frequent_color(Objects, Color) :-
% Build the histogram of Color-Count pairs.
    quantity_histogram(Objects, Histogram),
% Select the pair with the maximum count.
    max_member(_-Color, Histogram).

% quantity_least_frequent_color(+Objects, -Color): color occurring in the fewest objects.
quantity_least_frequent_color(Objects, Color) :-
% Build the histogram of Color-Count pairs.
    quantity_histogram(Objects, Histogram),
% Select the pair with the minimum count.
    min_member(_-Color, Histogram).

% quantity_max_color_count(+Objects, -Max): the maximum count among all colors.
quantity_max_color_count(Objects, Max) :-
...
% quantity_min_color_count(+Objects, -Min): the minimum count among all colors.
quantity_min_color_count(Objects, Min) :-
% Build Color-Count histogram.
    quantity_histogram(Objects, Histogram),
% Extract just the counts.
    pairs_keys(Histogram, Counts),
...
% UNIQUE SHAPES
% quantity_unique_shapes(+Objects, -Shapes): sorted list of distinct normalized shapes.
quantity_unique_shapes(Objects, Shapes) :-
...
% quantity_count_where(+Objects, +Goal, -Count): count objects for which call(Goal, Obj) succeeds.
quantity_count_where(Objects, Goal, Count) :-
...
% UNIFORMITY TESTS
% quantity_all_same_color(+Objects): succeeds if all objects have the same color.
quantity_all_same_color([]).
% Base: empty list trivially satisfies the condition.
quantity_all_same_color([_]).
% Single-element list: trivially true.
quantity_all_same_color([H|T]) :-
...
% quantity_all_same_size(+Objects): succeeds if all objects have the same cell count.
quantity_all_same_size([]).
% Empty list: trivially true.
quantity_all_same_size([_]).
% Single element: trivially true.
quantity_all_same_size([H|T]) :-
...
% quantity_all_same_shape(+Objects): succeeds if all objects have the same normalized shape.
quantity_all_same_shape([]).
% Empty list: trivially true.
quantity_all_same_shape([_]).
% Single element: trivially true.
quantity_all_same_shape([H|T]) :-
...
% MULTISET MATCHING
% quantity_colors_match(+Objects1, +Objects2): same multiset of colors in both lists.
quantity_colors_match(Objs1, Objs2) :-
...
% quantity_shapes_match(+Objects1, +Objects2): same multiset of normalized shapes.
quantity_shapes_match(Objs1, Objs2) :-
...
% quantity_sizes_match(+Objects1, +Objects2): same multiset of sizes.
quantity_sizes_match(Objs1, Objs2) :-
...
% THRESHOLD COUNTING
% quantity_exactly_n(+Objects, +Pred, +N): exactly N objects satisfy Pred.
quantity_exactly_n(Objects, Pred, N) :-
% Count matching objects.
    quantity_count_where(Objects, Pred, Count),
% Assert equality with the target count.
    Count =:= N.

% quantity_at_least_n(+Objects, +Pred, +N): at least N objects satisfy Pred.
quantity_at_least_n(Objects, Pred, N) :-
% Count matching objects.
    quantity_count_where(Objects, Pred, Count),
