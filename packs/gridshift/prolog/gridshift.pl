:- module(gridshift, [
    gridshift_shift_right/4,
    gridshift_shift_left/4,
    gridshift_shift_down/4,
    gridshift_shift_up/4,
    gridshift_roll_right/3,
    gridshift_roll_left/3,
    gridshift_roll_down/3,
    gridshift_roll_up/3,
    gridshift_roll_row/4,
    gridshift_roll_col/4,
    gridshift_shift_color/6,
    gridshift_offset/5,
    gridshift_shift_row/5,
    gridshift_shift_col/5
]).
...
% gridshift_dims_(+Grid, -H, -W): grid dimensions.
gridshift_dims_(Grid, H, W) :-
% Count rows for height.
    length(Grid, H),
% Use first row for width.
    Grid = [Row|_], length(Row, W).

% gridshift_bg_row_(+W, +BgColor, -Row): create a row of W BgColor values.
gridshift_bg_row_(W, BgColor, Row) :-
% Generate W copies of BgColor using between/3.
    findall(BgColor, between(1, W, _), Row).

% gridshift_roll_list_(+List, +N, -Rolled): cyclically roll List right by N positions.
% The last N elements move to the front.
gridshift_roll_list_(List, N, Rolled) :-
...
% gridshift_transpose_(+Grid, -T): transpose a rectangular grid.
gridshift_transpose_([], []) :- !.
gridshift_transpose_([[]|_], []) :- !.
gridshift_transpose_(Grid, [Heads|Tails]) :-
% Split each row into its first element (head) and the rest (tail).
    gridshift_heads_tails_(Grid, Heads, RestRows),
    gridshift_transpose_(RestRows, Tails).

% gridshift_heads_tails_(+Rows, -Heads, -Tails): split rows into head elements and tail lists.
gridshift_heads_tails_([], [], []).
gridshift_heads_tails_([[H|T]|Rest], [H|Hs], [T|Ts]) :-
% Recurse over remaining rows.
    gridshift_heads_tails_(Rest, Hs, Ts).

% gridshift_shift_row_r_(+Row, +N, +BgColor, -Shifted): shift one row right by N, fill left with BgColor.
gridshift_shift_row_r_(Row, N, BgColor, Shifted) :-
...
% gridshift_shift_row_l_(+Row, +N, +BgColor, -Shifted): shift one row left by N, fill right with BgColor.
gridshift_shift_row_l_(Row, N, BgColor, Shifted) :-
% Clamp N to row width.
    length(Row, W), N2 is min(N, W),
    KeepLen is W - N2,
...
% Apply row shift to every row.
    findall(NewRow,
        (member(GRow, Grid),
         gridshift_shift_row_r_(GRow, N, BgColor, NewRow)),
        Result).
...
% Apply left row shift to every row.
    findall(NewRow,
        (member(GRow, Grid),
         gridshift_shift_row_l_(GRow, N, BgColor, NewRow)),
        Result).
...
% Build a single bg row.
    gridshift_bg_row_(W, BgColor, BgRow),
...
% Apply cyclic row roll to every row.
    findall(NewRow,
        (member(GRow, Grid),
         gridshift_roll_list_(GRow, N, NewRow)),
        Result).
...
% Roll left by N = roll right by (W - N mod W).
    findall(NewRow,
        (member(GRow, Grid),
         ...
         gridshift_roll_list_(GRow, RN, NewRow)),
        Result).
...
% Roll the list of rows cyclically.
    gridshift_roll_list_(Grid, N, Result).
...
% Roll up by N = roll down by (H - N mod H).
    ...
    gridshift_roll_list_(Grid, RN, Result).
...
% Transpose to treat columns as rows, roll the target row, transpose back.
    gridshift_transpose_(Grid, TGrid),
    gridshift_roll_row(TGrid, C, N, TRolled),
    gridshift_transpose_(TRolled, Result).
...
% Apply shift only to the target row.
         (RI =:= R ->
             (N >= 0 ->
                 gridshift_shift_row_r_(GRow, N, BgColor, NewRow)
             ;   N2 is -N,
                 gridshift_shift_row_l_(GRow, N2, BgColor, NewRow)
             )
         ;   NewRow = GRow
         )),
        Result).
...
% Transpose to treat column C as a row, shift it, then transpose back.
    gridshift_transpose_(Grid, TGrid),
    gridshift_shift_row(TGrid, C, N, BgColor, TShifted),
    gridshift_transpose_(TShifted, Result).
