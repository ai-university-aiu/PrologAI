/*  PrologAI — Causalontology Efficiency Governor Test Suite  (WP-399)
    ...
    Run with the full library path:
        swipl $LIB -g "run_tests, halt" -t "halt(1)" packs/efficiency_governor/test/test_efficiency_governor.pl
*/

% Declare this file as a test module.
:- module(test_efficiency_governor, []).
...
:- begin_tests(efficiency_governor).
...
test(match_human_full_score, [true(S =:= 1.0)]) :-
    ...
    efficiency_governor_level_score(10, 10, S).
...
test(double_actions_quarter, [true(S =:= 0.25)]) :-
    ...
    efficiency_governor_level_score(10, 20, S).
...
test(counter_and_budget) :-
    ...
    efficiency_governor_set_baseline(level1, 2),
    % Spend ten actions (exactly the cap of five times two).
    forall(between(1, 10, _), efficiency_governor_count(level1)),
    % The count is ten.
    efficiency_governor_actions(level1, 10),
    % Ten is within the cap of ten.
    efficiency_governor_within_budget(level1),
    % One more action breaks the cap.
    efficiency_governor_count(level1),
...
test(env_weighted_mean, [true(E =:= 0.875)]) :-
    ...
    efficiency_governor_env_score([w(1, 0.5), w(3, 1.0)], E).
...
test(total_percent, [true(T =:= 75.0)]) :-
    ...
    efficiency_governor_total_score([0.5, 1.0], T).

% Close the test unit.
:- end_tests(efficiency_governor).
