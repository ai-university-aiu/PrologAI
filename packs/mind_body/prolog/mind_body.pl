/*  PrologAI — Mind-Body Interface  (Specification Section 3.6, PR 10)
    ...
    mind_body_dispatch_command/2 — send a command from the mind to an enrolled body;
...
:- module(mind_body, [
    ...
    % Continue the multi-line expression started above.
    mind_body_dispatch_command/2,    % +Address, +Command
    % Continue the multi-line expression started above.
...
relay_percept(Address, Signal) :-
    ...
    mind_body_channel(Signal, Channel),
...
mind_body_channel(perception_signal(_, _, _),    'channel://perceiver').
% State the fact: percept channel(interoceptive_signal(_, _, _), 'channel://motivation').
mind_body_channel(interoceptive_signal(_, _, _), 'channel://motivation').
% State the fact: percept channel(proprioceptive_signal(_, _, _, _), 'channel://regulation').
mind_body_channel(proprioceptive_signal(_, _, _, _), 'channel://regulation').

% ---------------------------------------------------------------------------
% mind_body_dispatch_command/2
%
...
mind_body_dispatch_command(Address, Command) :-
