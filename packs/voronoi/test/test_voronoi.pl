...
% vn_non_bg_cells tests.

test(non_bg_cells_g1) :-
    g1(G), vn_non_bg_cells(G, 0, Cells),
    Cells == [r(0,0), r(2,2)].

test(non_bg_cells_g4_all) :-
    g4(G), vn_non_bg_cells(G, 0, Cells),
    length(Cells, 9).

test(non_bg_cells_g3_empty) :-
    g3(G), vn_non_bg_cells(G, 0, Cells),
    Cells == [].

test(non_bg_cells_g5) :-
    g5(G), vn_non_bg_cells(G, 0, Cells),
    Cells == [r(0,0)].

% vn_non_bg_colors tests.

test(non_bg_colors_g1) :-
    g1(G), vn_non_bg_colors(G, 0, Colors),
    Colors == [1, 2].

test(non_bg_colors_g2) :-
    g2(G), vn_non_bg_colors(G, 0, Colors),
    Colors == [a, b].

test(non_bg_colors_g3_empty) :-
    g3(G), vn_non_bg_colors(G, 0, Colors),
    Colors == [].

test(non_bg_colors_g4_one) :-
    g4(G), vn_non_bg_colors(G, 0, Colors),
    Colors == [1].

% vn_nearest_dist tests.
...
test(nearest_dist_g1_00) :-
    ...
    g1(G), vn_nearest_dist(G, 0, 0, 0, D), D == 0.
...
test(expand_n_0) :-
    % N=0: no expansion.
    g6(G), vn_expand_n(G, 0, 1, 0, Cells),
    Cells == [].

test(expand_n_1_g6) :-
    % N=1: same as expand1.
    g6(G), vn_expand_n(G, 0, 1, 1, Cells),
    vn_expand1(G, 0, 1, E1),
    Cells == E1.

test(expand_n_2_g6) :-
    % N=2: within 2 Manhattan steps of r(1,1) in Bg = all 8 Bg cells.
    g6(G), vn_expand_n(G, 0, 1, 2, Cells),
    length(Cells, 8).

test(expand_n_1_g2) :-
    % g2 = [[a,0,0,0,b]]. N=1 from a: only r(0,1).
    g2(G), vn_expand_n(G, 0, a, 1, Cells),
    Cells == [r(0,1)].

test(expand_n_2_g2) :-
    % N=2 from a: r(0,1) and r(0,2) (both within dist 2 of col 0).
    g2(G), vn_expand_n(G, 0, a, 2, Cells),
    sort(Cells, S),
    S == [r(0,1),r(0,2)].
