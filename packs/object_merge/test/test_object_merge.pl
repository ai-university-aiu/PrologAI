:- use_module('../prolog/object_merge').
:- use_module(library(plunit)).

:- begin_tests(object_merge).
...
test(union_disjoint) :-
    dot(D), dot_right(DR),
    object_merge_union_cells(D, DR, x, U),
    U = obj(x, Cells),
    msort(Cells, SC),
    SC == [r(0,0),r(0,1)].
...
test(union_overlap) :-
    ...
    object_merge_union_cells(B, B2, x, U),
    U = obj(x, Cells),
    length(Cells, 5).

test(union_self) :-
    dot(D),
    object_merge_union_cells(D, D, x, U),
    U = obj(x, [r(0,0)]).
...
test(intersect_overlap) :-
    ...
    object_merge_intersect_cells(B, B2, x, I),
    I = obj(x, [r(0,2)]).

test(intersect_disjoint) :-
    dot(D), dot_right(DR),
    object_merge_intersect_cells(D, DR, x, I),
    I = obj(x, []).

test(intersect_sq) :-
    sq(S), sq2(S2),
    % overlap at r(1,1) only
    object_merge_intersect_cells(S, S2, x, I),
    I = obj(x, [r(1,1)]).

% --- object_merge_diff_cells/4 ---

test(difference_partial_overlap) :-
    bar_h(B), bar_h2(B2),
    % B \ B2 = r(0,0), r(0,1)
    object_merge_diff_cells(B, B2, x, D),
    D = obj(x, Cells),
    msort(Cells, SC),
    SC == [r(0,0),r(0,1)].

test(difference_disjoint) :-
    dot(D1), dot_right(D2),
    object_merge_diff_cells(D1, D2, x, Diff),
    Diff = obj(x, [r(0,0)]).

test(difference_same) :-
    dot(D),
    object_merge_diff_cells(D, D, x, Diff),
    Diff = obj(x, []).

% --- object_merge_sym_diff_cells/4 ---

test(symmetry_transform_diff_partial) :-
    bar_h(B), bar_h2(B2),
    % B \ B2 = {0,1}, B2 \ B = {3,4}; sym diff = {r(0,0),r(0,1),r(0,3),r(0,4)}
    object_merge_sym_diff_cells(B, B2, x, S),
    S = obj(x, Cells),
    length(Cells, 4).

test(symmetry_transform_diff_disjoint) :-
    dot(D1), dot_right(D2),
    object_merge_sym_diff_cells(D1, D2, x, S),
    S = obj(x, Cells),
    length(Cells, 2).

test(symmetry_transform_diff_same) :-
    dot(D),
    object_merge_sym_diff_cells(D, D, x, S),
    S = obj(x, []).
...
test(concat_h_gap0) :-
    ...
    object_merge_concat_h(D, DB, 0, obj(r, Cells)),
    msort(Cells, SC),
    SC == [r(0,0),r(1,1)].
...
test(concat_h_gap1) :-
    ...
    object_merge_concat_h(D, DR, 1, obj(r, Cells)),
    msort(Cells, SC),
    SC == [r(0,0),r(0,2)].
...
test(concat_h_bars) :-
    ...
    object_merge_concat_h(B1, B2, 0, obj(r, Cells)),
    length(Cells, 6),
    member(r(0,3), Cells),
    member(r(0,5), Cells).
...
test(concat_v_gap0) :-
    ...
    object_merge_concat_v(D, DR, 0, obj(r, Cells)),
    msort(Cells, SC),
    SC == [r(0,0),r(1,1)].
...
test(concat_v_bars) :-
    ...
    object_merge_concat_v(V1, V2, 0, obj(g, Cells)),
    length(Cells, 6),
    member(r(3,0), Cells),
    member(r(5,0), Cells).
...
test(merge_list_basic) :-
    ...
    object_merge_merge_list([D, DR, DB], x, M),
    M = obj(x, Cells),
    length(Cells, 3).
...
test(merge_list_with_overlap) :-
    ...
    object_merge_merge_list([D, B], x, M),
    M = obj(x, Cells),
    length(Cells, 3).

test(merge_list_single) :-
    dot(D),
    object_merge_merge_list([D], x, M),
    M = obj(x, [r(0,0)]).
...
test(subtract_list_basic) :-
    ...
    object_merge_subtract_list(B, [D], x, R),
    R = obj(x, Cells),
    length(Cells, 2),
    \+ member(r(0,0), Cells).
...
test(subtract_list_empty) :-
    bar_h(B),
    object_merge_subtract_list(B, [], x, R),
    R = obj(x, Cells),
    length(Cells, 3).
...
test(expand_bbox_sq) :-
    ...
    object_merge_expand_bbox(S, Expanded),
...
test(expand_bbox_lshape) :-
    ...
    object_merge_expand_bbox(L, Expanded),
...
test(hollow_bbox_3x3) :-
    ...
    Obj = obj(r, [r(0,0),r(0,1),r(0,2),
                  r(1,0),r(1,1),r(1,2),
                  r(2,0),r(2,1),r(2,2)]),
    object_merge_hollow_bbox(Obj, H),
    H = obj(r, Frame),
...
test(split_cc8_connected) :-
    ...
    object_merge_split_cc8(D, Parts),
    length(Parts, 2).
...
test(split_cc8_two_blobs) :-
    ...
    object_merge_split_cc8(TB, Parts),
    length(Parts, 2).
...
test(n_components4_one) :-
    bar_h(B),
    object_merge_n_components4(B, N),
    N == 1.

test(n_components4_two) :-
    disconnected(D),
    object_merge_n_components4(D, N),
    N == 2.

test(n_components4_sq) :-
    sq(S),
    object_merge_n_components4(S, N),
    N == 1.
...
test(union_three_separate) :-
    dot(D), dot_right(DR), dot_below(DB),
    object_merge_union_cells(D, DR, x, U1),
    object_merge_union_cells(U1, DB, x, U2),
    U2 = obj(x, Cells),
    length(Cells, 3).
