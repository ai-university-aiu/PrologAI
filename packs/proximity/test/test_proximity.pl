...
% Two candidates equidistant from ref; first one wins.
    ...
    proximity_nearest([A, B], Ref, A).

:- end_tests(proximity_nearest).

:- begin_tests(proximity_farthest).

test(single_candidate) :-
    o_red(R), o_blue(B),
    proximity_farthest([B], R, B).

test(two_candidates) :-
% Blue(4,4) is farther from red(0,0) than green(2,2): dist 8 vs 4.
    o_red(Ref), o_green(G), o_blue(B),
    proximity_farthest([G, B], Ref, B).
...
% Two equidistant candidates: first wins.
    ...
    proximity_farthest([A, B], Ref, A).

:- end_tests(proximity_farthest).

:- begin_tests(proximity_sort_by_dist).

test(already_sorted) :-
% Yellow(0,2) dist 2, green(2,2) dist 4, blue(4,4) dist 8 from red(0,0).
    o_red(Ref), o_yellow(Y), o_green(G), o_blue(B),
    proximity_sort_by_dist([Y,G,B], Ref, [Y,G,B]).

test(reverse_order) :-
% Blue, green, yellow from red: should sort to yellow, green, blue.
    o_red(Ref), o_yellow(Y), o_green(G), o_blue(B),
    proximity_sort_by_dist([B,G,Y], Ref, [Y,G,B]).

test(single_element) :-
    o_red(R), o_blue(B),
    proximity_sort_by_dist([B], R, [B]).

test(empty) :-
    o_red(R),
    proximity_sort_by_dist([], R, []).

:- end_tests(proximity_sort_by_dist).

:- begin_tests(proximity_within_dist).

test(all_within) :-
% Yellow(0,2) dist 2, green(2,2) dist 4 from red(0,0): all within 4.
    o_red(Ref), o_yellow(Y), o_green(G),
    proximity_within_dist([Y,G], Ref, 4, [Y,G]).

test(none_within) :-
% Blue(4,4) dist 8 from red: none within 4.
    o_red(Ref), o_blue(B),
    proximity_within_dist([B], Ref, 4, []).

test(partial) :-
% Yellow(dist 2) within 3, green(dist 4) not within 3.
    o_red(Ref), o_yellow(Y), o_green(G),
    proximity_within_dist([Y,G], Ref, 3, [Y]).

test(exact_boundary) :-
% Yellow at dist 2: within 2 (boundary inclusive).
    o_red(Ref), o_yellow(Y),
    proximity_within_dist([Y], Ref, 2, [Y]).

:- end_tests(proximity_within_dist).

:- begin_tests(proximity_beyond_dist).

test(all_beyond) :-
% Blue(dist 8) and green(dist 4) from red: both beyond 3.
    o_red(Ref), o_blue(B), o_green(G),
    proximity_beyond_dist([B,G], Ref, 3, [B,G]).

test(none_beyond) :-
% Yellow(dist 2) is not beyond 4.
    o_red(Ref), o_yellow(Y),
    proximity_beyond_dist([Y], Ref, 4, []).

test(exact_not_beyond) :-
% Yellow at dist 2: not beyond 2 (strict inequality).
    o_red(Ref), o_yellow(Y),
    proximity_beyond_dist([Y], Ref, 2, []).

test(partial) :-
% Yellow(dist 2) not beyond 3, blue(dist 8) is beyond 3.
    o_red(Ref), o_yellow(Y), o_blue(B),
    proximity_beyond_dist([Y,B], Ref, 3, [B]).

:- end_tests(proximity_beyond_dist).

:- begin_tests(proximity_closest_pair).

test(two_objects) :-
% Red(0,0) and green(2,2): centroid dist = 4.
    o_red(R), o_green(G),
    proximity_closest_pair([R,G], R-G).
...
% Min dist is 2 (tie between red-yellow and yellow-green). First pair by index order wins.
    ...
    proximity_closest_pair([R,Y,G], R-Y).

test(order_independent_result) :-
% Green(2,2) and blue(4,4): dist = 4. Only pair, so it is the closest.
    o_green(G), o_blue(B),
    proximity_closest_pair([G,B], G-B).

:- end_tests(proximity_closest_pair).

:- begin_tests(proximity_farthest_pair).

test(two_objects) :-
% Red(0,0) and blue(4,4): centroid dist = 8.
    o_red(R), o_blue(B),
    proximity_farthest_pair([R,B], R-B).
...
% Max is red-blue at 8.
    ...
    proximity_farthest_pair([R,G,B], R-B).

:- end_tests(proximity_farthest_pair).

:- begin_tests(proximity_touching_objs).

test(one_touching) :-
% Orange at r(1,2) touches red 2x2 via r(1,1)-r(1,2).
    o_red(Ref), o_orange(Org), o_blue(B),
    proximity_touching_objs([Org, B], Ref, [Org]).

test(none_touching) :-
% Blue and green are not touching red.
    o_red(Ref), o_blue(B), o_green(G),
    proximity_touching_objs([B,G], Ref, []).

test(all_touching) :-
% Yellow at r(0,2) touches red r(0,1); orange at r(1,2) touches red r(1,1).
    o_red(Ref), o_yellow(Y), o_orange(Org),
    proximity_touching_objs([Y, Org], Ref, [Y, Org]).

test(empty_list) :-
    o_red(Ref),
    proximity_touching_objs([], Ref, []).

:- end_tests(proximity_touching_objs).

:- begin_tests(proximity_non_touching_objs).

test(one_non_touching) :-
% Blue does not touch red; orange does.
    o_red(Ref), o_orange(Org), o_blue(B),
    proximity_non_touching_objs([Org, B], Ref, [B]).

test(all_non_touching) :-
    o_red(Ref), o_blue(B), o_green(G),
    proximity_non_touching_objs([B,G], Ref, [B,G]).

test(empty_result) :-
    o_red(Ref), o_yellow(Y), o_orange(Org),
    proximity_non_touching_objs([Y, Org], Ref, []).

:- end_tests(proximity_non_touching_objs).

:- begin_tests(proximity_dist_rank).

test(single_object) :-
    o_red(Ref), o_blue(B),
    proximity_dist_rank([B], Ref, [8-B]).

test(three_objects_sorted) :-
% Yellow(dist 2), green(dist 4), blue(dist 8) from red.
    o_red(Ref), o_yellow(Y), o_green(G), o_blue(B),
    proximity_dist_rank([B,Y,G], Ref, [2-Y, 4-G, 8-B]).

test(empty) :-
    o_red(Ref),
    proximity_dist_rank([], Ref, []).
...
% Two objects equidistant: appear in original list order.
    ...
    A = obj(a,[r(0,2)]),
    B = obj(b,[r(2,0)]),
    proximity_dist_rank([A,B], Ref, [2-A, 2-B]).

:- end_tests(proximity_dist_rank).
