...
test(centroid_single) :-
    ag_centroid(obj(1, [r(4,6)]), r(4,6)).

test(centroid_hpair) :-
    ag_centroid(obj(2, [r(0,0), r(0,2)]), r(0,1)).

test(centroid_square) :-
    ag_centroid(obj(3, [r(0,0), r(0,1), r(1,0), r(1,1)]), r(0,0)).

% ag_offset/3 tests.
test(offset_right) :-
    ag_offset(obj(1, [r(0,0)]), obj(2, [r(0,3)]), r(0,3)).

test(offset_down) :-
    ag_offset(obj(1, [r(0,2)]), obj(2, [r(4,2)]), r(4,0)).

test(offset_diagonal) :-
    ag_offset(obj(1, [r(1,1)]), obj(2, [r(3,4)]), r(2,3)).

% ag_row_order/2 tests.
test(row_order_above) :-
    ag_row_order(obj(1, [r(0,0)]), obj(2, [r(3,0)])).

test(row_order_equal) :-
    ag_row_order(obj(1, [r(2,0)]), obj(2, [r(2,5)])).

test(row_order_fail, [fail]) :-
    ag_row_order(obj(1, [r(5,0)]), obj(2, [r(1,0)])).

% ag_col_order/2 tests.
test(col_order_left) :-
    ag_col_order(obj(1, [r(0,0)]), obj(2, [r(0,4)])).

test(col_order_equal) :-
    ag_col_order(obj(1, [r(0,3)]), obj(2, [r(5,3)])).

test(col_order_fail, [fail]) :-
    ag_col_order(obj(1, [r(0,7)]), obj(2, [r(0,2)])).

% ag_row_aligned/2 tests.
test(row_aligned_same) :-
    ag_row_aligned(obj(1, [r(2,0)]), obj(2, [r(2,5)])).

test(row_aligned_pair) :-
    ag_row_aligned(obj(1, [r(0,0), r(0,1)]), obj(2, [r(0,3), r(0,4)])).

test(row_aligned_fail, [fail]) :-
    ag_row_aligned(obj(1, [r(0,0)]), obj(2, [r(1,0)])).

% ag_col_aligned/2 tests.
test(col_aligned_same) :-
    ag_col_aligned(obj(1, [r(0,3)]), obj(2, [r(4,3)])).

test(col_aligned_pair) :-
    ag_col_aligned(obj(1, [r(0,0), r(1,0)]), obj(2, [r(3,0), r(4,0)])).

test(col_aligned_fail, [fail]) :-
    ag_col_aligned(obj(1, [r(0,0)]), obj(2, [r(0,1)])).

% ag_sort_by_row/2 tests.
test(sort_row_basic) :-
...
test(sort_row_single) :-
    ag_sort_by_row([obj(5,[r(1,1)])], Sorted),
    Sorted = [obj(5,[r(1,1)])].

% ag_sort_by_col/2 tests.
test(sort_col_basic) :-
...
test(sort_col_single) :-
    ag_sort_by_col([obj(7,[r(3,2)])], Sorted),
    Sorted = [obj(7,[r(3,2)])].
...
test(nearest_single) :-
    Ref = obj(1, [r(3,3)]),
    ag_nearest(Ref, [obj(2,[r(1,1)])], Nearest),
    Nearest = obj(2,[r(1,1)]).
