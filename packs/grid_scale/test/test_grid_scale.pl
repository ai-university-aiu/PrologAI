...
% When all blocks are uniform, block_majority = downsample.
    g4x4_up2(G),
    grid_scale_block_majority(G, 2, S),
    g2x2_check(Expected),
    S = Expected.
...
% Block size 1: majority is just the cell itself.
    g2x2_check(G),
    grid_scale_block_majority(G, 1, S),
    S = G.
...
% Padding by 0 returns the grid unchanged.
    g2x2_tb(G),
    grid_scale_pad(G, 0, x, P),
    P = G.

test(pad_dimensions) :-
% Padding 3x3 by 1 gives 5x5.
    g3x3_r(G),
    grid_scale_pad(G, 1, b, P),
    length(P, 5),
    P = [PR|_], length(PR, 5).
...
% 4x4 cropped to factor 2: already divisible, returns full grid.
    g4x4_up2(G),
    grid_scale_crop_to_factor(G, 2, C),
    C = G.
...
% g4x6_up2 (4 rows, 6 cols) cropped to factor 3: CH=(4//3)*3=3, CW=6.
    g4x6_up2(G),
    grid_scale_crop_to_factor(G, 3, C),
    length(C, 3),
    C = [R0|_], length(R0, 6).
...
% Factor 1 never crops.
    g3x3_r(G),
    grid_scale_crop_to_factor(G, 1, C),
    C = G.
...
% infer_tile then scale_factor recovers factor 2.
    ...
    grid_scale_scale_factor(Tile, G, N),
    N =:= 2.
