...
:- use_module('../prolog/grid_object_match.pl').

:- begin_tests(grid_object_match).
...
% --- grid_object_match_move_vector ---

test('AC-GOM-017: no movement gives mv(0,0)') :-
    or00(O1), or00(O2),
    grid_object_match_move_vector(match(O1, O2), V),
    V = mv(0, 0).
...
% --- grid_object_match_move_vectors ---
...
test('AC-GOM-021: empty pairs list gives empty vectors list') :-
    grid_object_match_move_vectors([], Vectors),
    Vectors = [].

% --- grid_object_match_constant_move ---

test('AC-GOM-022: all pairs same vector - constant move succeeds') :-
    or00(R1), or01(R2), ob11(B1), ob12(B2),
    Pairs = [match(R1, R2), match(B1, B2)],
    grid_object_match_constant_move(Pairs, V),
    V = mv(0, 1).
...
test('AC-GOM-023: different vectors - constant move fails') :-
    ...
    \+ grid_object_match_constant_move(Pairs, _).

test('AC-GOM-024: single pair always constant') :-
    or00(O1), og22(O2),
    grid_object_match_constant_move([match(O1, O2)], V),
    V = mv(2, 2).

% --- grid_object_match_infer_color_map ---

test('AC-GOM-025: no color changes - empty map') :-
    or00(O1), or01(O2),
    grid_object_match_infer_color_map([match(O1, O2)], Map),
    Map = [].
...
% --- grid_object_match_appeared ---

test('AC-GOM-028: no new colors - appeared is empty') :-
    or00(O1), ob11(O2),
    or01(O3), ob12(O4),
    grid_object_match_appeared([O1, O2], [O3, O4], Appeared),
    Appeared = [].

test('AC-GOM-029: one new color in Objs2') :-
    or00(O1), oy00(O2),
    grid_object_match_appeared([O1], [O1, O2], Appeared),
    Appeared = [O2].

% --- grid_object_match_disappeared ---

test('AC-GOM-030: no lost colors - disappeared is empty') :-
    or00(O1), ob11(O2),
    or01(O3), ob12(O4),
    grid_object_match_disappeared([O1, O2], [O3, O4], Disappeared),
    Disappeared = [].

test('AC-GOM-031: one color lost from Objs1') :-
    or00(O1), oy00(O2),
    grid_object_match_disappeared([O1, O2], [O1], Disappeared),
    Disappeared = [O2].

% --- grid_object_match_same_structure ---

test('AC-GOM-032: same color multiset - succeeds') :-
    or00(R1), ob11(B1),
    or01(R2), ob12(B2),
    grid_object_match_same_structure([R1, B1], [R2, B2]).

test('AC-GOM-033: different color multiset - fails') :-
    or00(R), ob11(B), og22(G),
    \+ grid_object_match_same_structure([R, B], [R, G]).

test('AC-GOM-034: same colors in different list order - succeeds') :-
    or00(R), ob11(B),
    or01(R2), ob12(B2),
    grid_object_match_same_structure([R, B], [B2, R2]).

% --- grid_object_match_count_change ---

test('AC-GOM-035: same count gives N=0') :-
    or00(R), ob11(B),
    grid_object_match_count_change([R], [B], N),
    N =:= 0.

test('AC-GOM-036: more objects in Objs2 gives positive N') :-
    or00(R), ob11(B), og22(G),
    grid_object_match_count_change([R], [B, G], N),
    N =:= 1.
...
test('AC-GOM-038: match nearest, color_diff finds all pairs changed') :-
    ...
    grid_object_match_match_nearest([R1, B1], [N1, N2], Pairs),
    grid_object_match_color_diff(Pairs, Same, Diff),
    Same = [],
    length(Diff, 2).
...
test('AC-GOM-039: match nearest then constant move') :-
    ...
    grid_object_match_match_nearest([R1, B1, G1], [R2, B2, G2], Pairs),
    grid_object_match_constant_move(Pairs, V),
    V = mv(0, 1).
...
test('AC-GOM-041: appeared and disappeared correct after color swap') :-
    ...
    grid_object_match_appeared([R, B], [B, G, Y], App),
    length(App, 2),
    memberchk(G, App), memberchk(Y, App),
    grid_object_match_disappeared([R, B], [B, G, Y], Dis),
    Dis = [R].

test('AC-GOM-042: unmatched_a and unmatched_b after partial color match') :-
    or00(R), ob11(B), og22(G),
    or01(R2),
    grid_object_match_match_color([R, B, G], [R2], Pairs),
    grid_object_match_unmatched_a(Pairs, [R, B, G], UA),
    length(UA, 2),
    memberchk(B, UA), memberchk(G, UA),
    grid_object_match_unmatched_b(Pairs, [R2], UB),
    UB = [].

test('AC-GOM-043: same structure check via match and structure') :-
    or00(R1), ob11(B1), og22(G1),
    or01(R2), ob12(B2), og23(G2),
    grid_object_match_same_structure([R1, B1, G1], [R2, B2, G2]),
    grid_object_match_count_change([R1, B1, G1], [R2, B2, G2], N),
    N =:= 0.
