/*  Round-trip tests for the cognitive-pack snapshot/restore exports.
    ...
    Proves that world_model, hypothesis, and goal_inference can each serialise their learned
...
:- use_module('../../hypothesis/prolog/hypothesis').
:- use_module('../../goal_inference/prolog/goal_inference').
...
% ===========================================================================
% hypothesis — a model's hypotheses and commitment survive the round-trip.
% ===========================================================================
test_hypo :-
    hypothesis_reset,
    % Build strong support for one hypothesis so it commits.
    forall(between(1, 8, _), hypothesis_support(g1, productive(act1))),
    hypothesis_contradict(g1, productive(act2)),
    hypothesis_update_commitment(g1),
    ( hypothesis_committed(g1, productive(act1))
    -> ok('HY-001 committed the well-supported hypothesis')
    ;  bad('HY-001') ),
    % Snapshot, wipe, restore.
    hypothesis_snapshot(g1, Snap),
    hypothesis_reset,
    hypothesis_restore(g1, Snap),
    ( hypothesis_committed(g1, productive(act1))
    -> ok('HY-002 restored commitment survives')
    ;  bad('HY-002') ),
    ( hypothesis_score(g1, productive(act1), Sc), Sc > 0.8
    -> ok('HY-003 restored evidence yields the same high score')
    ;  bad('HY-003') ).

% ===========================================================================
% goal_inference — accumulated goal evidence survives the round-trip.
% ===========================================================================
test_goalinfer :-
    goal_inference_reset,
    % Two wins that both light colour 4, and one loss on colour 7.
    goal_inference_observe([changed(1, 1, 0, 4)], win),
    goal_inference_observe([changed(2, 2, 0, 4)], win),
    goal_inference_observe([changed(3, 3, 0, 7)], game_over),
    ( goal_inference_hypothesise_goal(reach_colour(4))
    -> ok('GI-001 inferred the winning colour')
    ;  bad('GI-001') ),
    goal_inference_snapshot(Snap),
    goal_inference_reset,
    goal_inference_restore(Snap),
    ( goal_inference_hypothesise_goal(reach_colour(4)), goal_inference_win_count(2)
