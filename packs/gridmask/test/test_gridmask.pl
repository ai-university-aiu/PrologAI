...
test(compare_same) :-
    g3x3_r(G),
    gm_compare(G, G, x, Diff),
    Diff = [].

% tl vs br: diff at every non-bg cell of either.
test(compare_tl_br) :-
    g3x3_tl(GA), g3x3_br(GB),
    gm_compare(GA, GB, x, Diff),
    msort(Diff, S),
...
test(paste_at_origin) :-
    g3x3_x(G), g2x2_r(P),
    gm_paste(G, P, 0, 0, x, R),
    R = [[r,r,x],[r,r,x],[x,x,x]].

% Paste at (1,1).
test(paste_at_center) :-
    g3x3_x(G), g2x2_r(P),
    gm_paste(G, P, 1, 1, x, R),
    R = [[x,x,x],[x,r,r],[x,r,r]].

% Paste with all-bg patch: no change.
test(paste_bg_no_change) :-
    g3x3_r(G),
    gm_paste(G, [[x,x],[x,x]], 0, 0, x, R),
    R = G.

% --- gm_border_mask tests ---
