:- use_module('../prolog/grid_mark.pl').
...
test(mark_rect_4x4) :-
    bg(G), grid_mark_mark_rect(G, 1, 1, 2, 2, m, M),
...
test(mark_rect_full) :-
    bg(G), grid_mark_mark_rect(G, 0, 0, 3, 3, m, M),
    M = [[m,m,m,m],[m,m,m,m],[m,m,m,m],[m,m,m,m]].

% --- grid_mark_mark_border ---

% AC-GMK-016: border of 4x4 bg marks outer ring
test(mark_border_4x4) :-
    bg(G), grid_mark_mark_border(G, m, M),
...
test(mark_border_1x1) :-
    g11(G), grid_mark_mark_border(G, m, M),
    M = [[m]].
...
test(mark_diagonal_1x1) :-
    g11(G), grid_mark_mark_diagonal(G, m, M),
    M = [[m]].

% --- grid_mark_mark_anti_diagonal ---

% AC-GMK-022: anti-diagonal of 4x4 bg marks (0,3),(1,2),(2,1),(3,0)
test(mark_anti_diagonal_4x4) :-
    bg(G), grid_mark_mark_anti_diagonal(G, m, M),
...
% --- grid_mark_mark_cross ---

% AC-GMK-027: cross of 4x4 marks row 2 and col 2
test(mark_cross_4x4) :-
    bg(G), grid_mark_mark_cross(G, m, M),
...
test(combined_mark_row_count) :-
    bg(G), grid_mark_mark_row(G, 0, m, M),
    grid_mark_marked_cells(M, m, Cs),
    length(Cs, 4).

% AC-GMK-040: mark_col then is_marked at all column positions
test(combined_mark_col_check) :-
    bg(G), grid_mark_mark_col(G, 1, m, M),
    grid_mark_is_marked(M, 0, 1, m),
    grid_mark_is_marked(M, 3, 1, m).
...
test(combined_rect_count) :-
    bg(G), grid_mark_mark_rect(G, 0, 0, 1, 1, m, M),
    grid_mark_marked_cells(M, m, Cs),
    length(Cs, 4).
