...
test(color_delta_change) :-
    dot_r(A), dot_b(B),
    objdelta_color_delta(A, B, Delta),
    Delta == r-b.
...
test(apply_color_match) :-
    dot_r(Obj),
    objdelta_apply_color(r-b, Obj, Obj2),
    Obj2 == obj(b, [r(0,0)]).

test(apply_color_no_match, [fail]) :-
    dot_r(Obj),
    objdelta_apply_color(g-b, Obj, _).
...
test(apply_map_not_found, [fail]) :-
    Map = [g-y],
    dot_r(Obj),
    objdelta_apply_color_map(Map, Obj, _).
...
test(apply_map_all_basic) :-
    ...
    objdelta_apply_map_all(Map, [R, G, B], Objs2),
    Objs2 == [obj(b,[r(0,0)]), obj(y,[r(0,0)]), obj(b,[r(0,0)])].

test(apply_map_all_empty) :-
    objdelta_apply_map_all([r-b], [], Objs2),
    Objs2 == [].
...
test(common_cells_overlap) :-
    ...
    objdelta_common_cells(A, B, Cells),
    Cells == [r(0,0), r(1,0)].
...
test(cell_diff_basic) :-
    A = obj(r, [r(0,0), r(0,1)]),
    B = obj(b, [r(0,1), r(0,2)]),
    objdelta_cell_diff(A, B, Added, Removed),
    Added == [r(0,2)],
    Removed == [r(0,0)].
...
test(apply_map_all_unmapped) :-
    Map = [g-y],
    dot_r(R),
    objdelta_apply_map_all(Map, [R], Objs2),
    Objs2 == [obj(r,[r(0,0)])].
