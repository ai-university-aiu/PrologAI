...
:- module(row_signature, [
    row_signature_col_at/3,
    row_signature_all_cols/2,
    row_signature_row_freq/2,
    row_signature_col_freq/2,
    row_signature_modal_row/2,
    row_signature_modal_col/2,
    row_signature_uniq_rows/2,
    row_signature_uniq_cols/2,
    row_signature_dup_row_pairs/2,
    row_signature_dup_col_pairs/2,
    row_signature_row_palindrome/2,
    row_signature_col_palindrome/2,
    row_signature_rows_anagram/3,
    row_signature_cols_anagram/3
]).
...
% row_signature_col_at(+Grid, +C, -Col): Col is the ordered list of values in column C,
% enumerated top-to-bottom (row 0 first). Fails if C is out of range.
row_signature_col_at(Grid, C, Col) :-
% Collect the value at column C from every row in Grid order.
    findall(V, (member(Row, Grid), nth0(C, Row, V)), Col).

% row_signature_all_cols(+Grid, -Cols): Cols is the list of all columns in column order
% (column 0 first). Each element of Cols is a top-to-bottom list of values.
row_signature_all_cols(Grid, Cols) :-
% Determine column count from the first row; default 0 for empty grid.
    (Grid = [Fr|_] -> length(Fr, W) ; W = 0),
% Highest valid column index.
    W1 is W - 1,
% Collect each column as a list using row_signature_col_at.
    findall(Col, (between(0, W1, C), row_signature_col_at(Grid, C, Col)), Cols).

% row_signature_row_freq(+Grid, -Freqs): Freqs is the list of Row-N pairs where N is the
...
row_signature_row_freq(Grid, Freqs) :-
...
% row_signature_col_freq(+Grid, -Freqs): Freqs is the list of Col-N pairs where N is the
% number of times column list Col appears. Sorted by N descending.
row_signature_col_freq(Grid, Freqs) :-
...
% row_signature_modal_row(+Grid, -Row): Row is the row that appears most often in Grid.
% Ties broken by standard order descending (largest row wins, via msort + last).
% Fails if Grid is empty.
row_signature_modal_row(Grid, Row) :-
% Collect unique rows.
    sort(Grid, UniqRows),
...
% row_signature_modal_col(+Grid, -Col): Col is the column list that appears most often.
% Ties broken by largest column list (standard order, via msort + last).
% Fails if Grid is empty or has no columns.
row_signature_modal_col(Grid, Col) :-
% Extract all columns.
    row_signature_all_cols(Grid, Cols),
...
% row_signature_uniq_rows(+Grid, -Pairs): Pairs is the list of R-Row terms for each row index R
% whose row value appears exactly once in Grid. Ordered by row index ascending.
row_signature_uniq_rows(Grid, Pairs) :-
...
% row_signature_uniq_cols(+Grid, -Pairs): Pairs is the list of C-Col terms for each column
% index C whose column list appears exactly once. Ordered by column index ascending.
row_signature_uniq_cols(Grid, Pairs) :-
% Extract all columns.
    row_signature_all_cols(Grid, Cols),
...
% row_signature_dup_row_pairs(+Grid, -Pairs): Pairs is the list of R1-R2 index pairs where
% row R1 equals row R2 and R1 < R2. Ordered lexicographically by R1 then R2.
row_signature_dup_row_pairs(Grid, Pairs) :-
% Determine row index range.
    length(Grid, H), H1 is H - 1,
...
% row_signature_dup_col_pairs(+Grid, -Pairs): Pairs is the list of C1-C2 index pairs where
% column C1 equals column C2 and C1 < C2. Ordered by C1 then C2.
row_signature_dup_col_pairs(Grid, Pairs) :-
...
% row_signature_row_palindrome(+Grid, +R): succeeds if row R reads the same left-to-right
% and right-to-left (i.e., the row list equals its own reverse).
row_signature_row_palindrome(Grid, R) :-
% Extract row R.
    nth0(R, Grid, Row),
% Palindrome iff reverse equals original.
    reverse(Row, Row).

% row_signature_col_palindrome(+Grid, +C): succeeds if column C reads the same top-to-bottom
% and bottom-to-top (i.e., the column list equals its own reverse).
row_signature_col_palindrome(Grid, C) :-
% Extract column C as a list.
    row_signature_col_at(Grid, C, Col),
% Palindrome iff reverse equals original.
    reverse(Col, Col).

% row_signature_rows_anagram(+Grid, +R1, +R2): succeeds if rows R1 and R2 in Grid have the
% same multiset of values (i.e., msort of both rows produces the same list).
row_signature_rows_anagram(Grid, R1, R2) :-
% Extract both rows.
    nth0(R1, Grid, Row1),
    nth0(R2, Grid, Row2),
...
% row_signature_cols_anagram(+Grid, +C1, +C2): succeeds if columns C1 and C2 in Grid have
% the same multiset of values (i.e., msort of both columns produces the same list).
row_signature_cols_anagram(Grid, C1, C2) :-
% Extract both columns.
    row_signature_col_at(Grid, C1, Col1),
    row_signature_col_at(Grid, C2, Col2),
% Anagram iff sorted multisets are equal.
    msort(Col1, Sorted),
    msort(Col2, Sorted).
