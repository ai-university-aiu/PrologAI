...
test(add_remove_border) :-
    g4(G), gridcrop_add_border(G, 1, m, B),
    gridcrop_remove_border(B, 1, m, R), R = G.

:- end_tests(gridcrop_add_border).

:- begin_tests(gridcrop_remove_border).

% AC-GCR-028: remove 1-cell border from g5 yields [[b]].
test(remove_border_g5) :-
    g5(G), gridcrop_remove_border(G, 1, x, R), R = [[b]].

% AC-GCR-029: remove 1-cell border from g1 yields inner 2x2.
test(remove_border_g1) :-
    g1(G), gridcrop_remove_border(G, 1, b, R),
    R = [[x,x],[x,x]].

% AC-GCR-030: remove_border is the inverse of add_border.
test(remove_border_inverse) :-
    G = [[a,b],[c,d]],
    gridcrop_add_border(G, 1, z, B),
    gridcrop_remove_border(B, 1, z, R), R = G.

:- end_tests(gridcrop_remove_border).

:- begin_tests(gridcrop_expand_down).

% AC-GCR-031: expand_down g4 by 2 rows of x.
test(expand_down_g4) :-
    g4(G), gridcrop_expand_down(G, 2, x, E),
    E = [[b,b,b],[b,b,b],[b,b,b],[x,x,x],[x,x,x]].

% AC-GCR-032: expand_down by 0 rows yields unchanged grid.
test(expand_down_zero) :-
    g4(G), gridcrop_expand_down(G, 0, x, E), E = G.

% AC-GCR-033: expand_down 1x1 [[a]] by 1 row.
test(expand_down_1x1) :-
    G = [[a]], gridcrop_expand_down(G, 1, b, E),
    E = [[a],[b]].

:- end_tests(gridcrop_expand_down).

:- begin_tests(gridcrop_expand_right).

% AC-GCR-034: expand_right g4 by 2 columns of x.
test(expand_right_g4) :-
    g4(G), gridcrop_expand_right(G, 2, x, E),
    E = [[b,b,b,x,x],[b,b,b,x,x],[b,b,b,x,x]].

% AC-GCR-035: expand_right by 0 columns yields unchanged grid.
test(expand_right_zero) :-
    g4(G), gridcrop_expand_right(G, 0, x, E), E = G.

% AC-GCR-036: expand_right 1x1 [[a]] by 1 column.
test(expand_right_1x1) :-
    G = [[a]], gridcrop_expand_right(G, 1, b, E),
    E = [[a,b]].

:- end_tests(gridcrop_expand_right).

:- begin_tests(gridcrop_content_h).

% AC-GCR-037: content_h of g1 is 2 (rows 1-2).
test(content_h_g1) :-
    g1(G), gridcrop_content_h(G, b, H), H =:= 2.

% AC-GCR-038: content_h of g3 is 1 (single cell).
test(content_h_g3) :-
    g3(G), gridcrop_content_h(G, b, H), H =:= 1.

% AC-GCR-039: content_h of g2 is 3 (all rows used).
test(content_h_g2) :-
    g2(G), gridcrop_content_h(G, b, H), H =:= 3.

:- end_tests(gridcrop_content_h).

:- begin_tests(gridcrop_content_w).

% AC-GCR-040: content_w of g1 is 2 (cols 1-2).
test(content_w_g1) :-
    g1(G), gridcrop_content_w(G, b, W), W =:= 2.

% AC-GCR-041: content_w of g3 is 1 (single cell).
test(content_w_g3) :-
    g3(G), gridcrop_content_w(G, b, W), W =:= 1.

% AC-GCR-042: content_w of g2 is 5 (full width).
test(content_w_g2) :-
    g2(G), gridcrop_content_w(G, b, W), W =:= 5.

:- end_tests(gridcrop_content_w).
...
test(pad_then_trim) :-
    G = [[x,x],[x,x]],
    gridcrop_pad_to(G, b, 5, 5, Padded),
    gridcrop_trim(Padded, b, Trimmed),
    Trimmed = G.

% AC-GCR-044: add_border then crop_bbox recovers the inner content.
test(add_border_then_crop_bbox) :-
    G = [[r,g],[b,y]],
    gridcrop_add_border(G, 2, b, Bordered),
    gridcrop_crop_bbox(Bordered, b, Cropped),
    Cropped = G.
