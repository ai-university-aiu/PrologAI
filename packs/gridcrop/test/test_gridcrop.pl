...
:- end_tests(gcr_trim_rows).

:- begin_tests(gcr_trim_cols).

% AC-GCR-013: trim_cols g1 removes first and last column.
test(trim_cols_g1) :-
    g1(G), gcr_trim_cols(G, b, T),
    T = [[b,b],[x,x],[x,x],[b,b]].

% AC-GCR-014: trim_cols on g5 removes no columns.
test(trim_cols_g5) :-
    g5(G), gcr_trim_cols(G, b, T), T = G.
...
test(pad_then_trim) :-
    G = [[x,x],[x,x]],
    gcr_pad_to(G, b, 5, 5, Padded),
    gcr_trim(Padded, b, Trimmed),
    Trimmed = G.

% AC-GCR-044: add_border then crop_bbox recovers the inner content.
test(add_border_then_crop_bbox) :-
    G = [[r,g],[b,y]],
    gcr_add_border(G, 2, b, Bordered),
    gcr_crop_bbox(Bordered, b, Cropped),
    Cropped = G.
