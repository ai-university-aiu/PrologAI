...
:- module(distance, [
    dt_manhattan/5, dt_chebyshev/5,
    dt_euclidean_sq/5, dt_nearest/4,
    dt_farthest/4, dt_all_at/5,
    dt_within_manhattan/5, dt_within_chebyshev/5,
    dt_map_manhattan/3, dt_map_chebyshev/3,
    dt_ring_manhattan/5, dt_centroid/3,
    dt_diameter_manhattan/3, dt_diameter_chebyshev/3
]).
...
% dt_nearest(+Grid, +Val, +R0-C0, -R-C): find the nearest cell (by
...
% dt_farthest(+Grid, +Val, +R0-C0, -R-C): find the farthest cell (by
...
% dt_all_at(+Grid, +Val, +R0-C0, +D, -Cells): find all cells of value Val
...
% dt_within_manhattan(+Grid, +R0, +C0, +D, -Cells): all grid positions
...
% Collect all in-bounds positions with Manhattan distance <= D.
    ...
    findall(R2-C2, (
        between(0, H1, R2), between(0, W1, C2),
        dt_manhattan(R0, C0, R2, C2, Dist),
        Dist =< D
    ), Unsorted),
    sort(Unsorted, Cells).

% dt_within_chebyshev(+Grid, +R0, +C0, +D, -Cells): all grid positions
...
% Collect all in-bounds positions with Chebyshev distance <= D.
    ...
    findall(R2-C2, (
        between(0, H1, R2), between(0, W1, C2),
        dt_chebyshev(R0, C0, R2, C2, Dist),
        Dist =< D
    ), Unsorted),
    sort(Unsorted, Cells).

% dt_map_manhattan(+Grid, +Val, -MapGrid): produce a same-size grid where
    ...
    findall(RowD, (
        ...
        findall(DMin, (
            ...
            findall(D, (member(ValR-ValC, ValCells),
                        dt_manhattan(R, C, ValR, ValC, D)), Ds),
            (Ds = [] -> DMin = -1 ; min_list(Ds, DMin))
        ), RowD)
    ), MapGrid).

% dt_map_chebyshev(+Grid, +Val, -MapGrid): same-size grid where each cell
...
% For each cell compute the minimum Chebyshev distance to any Val-cell.
    ...
    findall(RowD, (
        ...
        findall(DMin, (
            between(0, W1, C),
            findall(D, (member(ValR-ValC, ValCells),
                        dt_chebyshev(R, C, ValR, ValC, D)), Ds),
            (Ds = [] -> DMin = -1 ; min_list(Ds, DMin))
        ), RowD)
    ), MapGrid).

% dt_ring_manhattan(+Grid, +R0, +C0, +D, -Cells): all in-bounds positions
% at exactly Manhattan distance D from (R0,C0). Returns R-C pairs sorted.
...
% dt_centroid(+Grid, +Val, -R-C): integer centroid (center of mass) of all
% cells with value Val in Grid. Fails if no such cells exist.
dt_centroid(Grid, Val, RC) :-
...
% dt_diameter_manhattan(+Cells, -D1-D2, -Diam): given a list of R-C pairs,
...
% Enumerate all ordered pairs; collect max distance.
    findall(Dist-A-B, (
        ...
        A = RA-CA, B = RB-CB,
        dt_manhattan(RA, CA, RB, CB, Dist)
    ), Triples),
    Triples \= [],
    max_list_key_(Triples, Diam-D1-D2).

% dt_diameter_chebyshev(+Cells, -D1-D2, -Diam): diameter under Chebyshev
