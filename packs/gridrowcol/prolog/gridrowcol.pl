...
% Compute the valid column index range.
    grc_dims_(Grid, _, W), W1 is W - 1,
% Collect every C whose column content equals Pattern.
    findall(C, (between(0, W1, C), grc_col_(Grid, C, Pattern)), Positions).
...
% A column C is unique if no other column C2 has the same content.
    findall(C,
        (between(0, W1, C),
         grc_col_(Grid, C, ColC),
         \+ (between(0, W1, C2), C2 \= C, grc_col_(Grid, C2, ColC))),
        Cols).

% grc_row_sort(+Grid, -Sorted)
...
% Extract all columns as a list of lists.
    findall(Col, (between(0, W1, C), grc_col_(Grid, C, Col)), Cols),
