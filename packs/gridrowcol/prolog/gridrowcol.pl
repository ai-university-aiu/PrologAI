:- module(gridrowcol, [
    gridrowcol_row/3,
    gridrowcol_col/3,
    gridrowcol_row_uniform/3,
    gridrowcol_col_uniform/3,
    gridrowcol_equal_rows/3,
    gridrowcol_equal_cols/3,
    gridrowcol_row_positions/3,
    gridrowcol_col_positions/3,
    gridrowcol_matching_rows/2,
    gridrowcol_matching_cols/2,
    gridrowcol_unique_rows/2,
    gridrowcol_unique_cols/2,
    gridrowcol_row_sort/2,
    gridrowcol_col_sort/2
]).
...
% gridrowcol_col_uniform(+Grid, +C, -Color)
...
% gridrowcol_equal_rows(+Grid, +R1, +R2)
...
% gridrowcol_equal_cols(+Grid, +C1, +C2)
% Succeeds if columns C1 and C2 of Grid have identical content.
gridrowcol_equal_cols(Grid, C1, C2) :-
% Extract column C1 as a list.
    gridrowcol_col_(Grid, C1, Col),
% Column C2 must produce the same list.
    gridrowcol_col_(Grid, C2, Col).

% gridrowcol_row_positions(+Grid, +Pattern, -Positions)
...
% gridrowcol_col_positions(+Grid, +Pattern, -Positions)
% Positions is the list of column indices C where column C equals Pattern.
gridrowcol_col_positions(Grid, Pattern, Positions) :-
% Compute the valid column index range.
    gridrowcol_dims_(Grid, _, W), W1 is W - 1,
% Collect every C whose column content equals Pattern.
    findall(C, (between(0, W1, C), gridrowcol_col_(Grid, C, Pattern)), Positions).

% gridrowcol_matching_rows(+Grid, -Pairs)
...
% gridrowcol_matching_cols(+Grid, -Pairs)
...
% gridrowcol_unique_rows(+Grid, -Rows)
...
% gridrowcol_unique_cols(+Grid, -Cols)
...
% A column C is unique if no other column C2 has the same content.
    findall(C,
        (between(0, W1, C),
         gridrowcol_col_(Grid, C, ColC),
         \+ (between(0, W1, C2), C2 \= C, gridrowcol_col_(Grid, C2, ColC))),
        Cols).

% gridrowcol_row_sort(+Grid, -Sorted)
...
% gridrowcol_col_sort(+Grid, -Sorted)
...
% Extract all columns as a list of lists.
    findall(Col, (between(0, W1, C), gridrowcol_col_(Grid, C, Col)), Cols),
