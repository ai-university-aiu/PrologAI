:- use_module('../prolog/grid_symmetry').
...
% --- grid_symmetry_sym_h ---
test(symmetry_transform_h_fully_symmetric, []) :-
    g3x3_hsym(G),
    grid_symmetry_sym_h(G).

test(symmetry_transform_h_d4_symmetric, []) :-
    g3x3_d4sym(G),
    grid_symmetry_sym_h(G).

test(symmetry_transform_h_fails_for_asym, [fail]) :-
    g3x3_none(G),
    grid_symmetry_sym_h(G).

% --- grid_symmetry_sym_v ---
test(symmetry_transform_v_fully_symmetric, []) :-
    g3x3_vsym(G),
    grid_symmetry_sym_v(G).

test(symmetry_transform_v_d4_symmetric, []) :-
    g3x3_d4sym(G),
    grid_symmetry_sym_v(G).

test(symmetry_transform_v_fails_for_asym, [fail]) :-
    g3x3_none(G),
    grid_symmetry_sym_v(G).

% --- grid_symmetry_sym_d1 ---
test(symmetry_transform_d1_passes, []) :-
    g3x3_d1sym(G),
    grid_symmetry_sym_d1(G).

test(symmetry_transform_d1_fails_non_square, [fail]) :-
    g2x4_hsym(G),
    grid_symmetry_sym_d1(G).

test(symmetry_transform_d1_fails_asym, [fail]) :-
    g3x3_none(G),
    grid_symmetry_sym_d1(G).

% --- grid_symmetry_sym_d2 ---
test(symmetry_transform_d2_passes, []) :-
    g3x3_d2sym(G),
    grid_symmetry_sym_d2(G).

test(symmetry_transform_d2_fails_asym, [fail]) :-
    g3x3_none(G),
    grid_symmetry_sym_d2(G).

% --- grid_symmetry_sym_rot90 ---
test(symmetry_transform_rot90_uniform, []) :-
    g3x3_rot90(G),
    grid_symmetry_sym_rot90(G).

test(symmetry_transform_rot90_fails_asym, [fail]) :-
    g3x3_none(G),
    grid_symmetry_sym_rot90(G).

test(symmetry_transform_rot90_fails_non_square, [fail]) :-
    g2x4_hsym(G),
    grid_symmetry_sym_rot90(G).

% --- grid_symmetry_sym_rot180 ---
test(symmetry_transform_rot180_passes, []) :-
    g3x3_rot180(G),
    grid_symmetry_sym_rot180(G).

test(symmetry_transform_rot180_2x2_sym, []) :-
    g2x2_sym(G),
    grid_symmetry_sym_rot180(G).

test(symmetry_transform_rot180_fails_asym, [fail]) :-
    g3x3_none(G),
    grid_symmetry_sym_rot180(G).

% --- grid_symmetry_symmetries ---
test(symmetries_d4, []) :-
    g3x3_d4sym(G),
    grid_symmetry_symmetries(G, Syms),
    memberchk(h, Syms), memberchk(v, Syms), memberchk(rot180, Syms).

test(symmetries_none_is_empty, []) :-
    g3x3_none(G),
    grid_symmetry_symmetries(G, []).
...
% --- grid_symmetry_complete_h ---
test(complete_h_top_symmetric_result, []) :-
    G = [[r,b],[x,x]],
    grid_symmetry_complete_h(G, top, R),
    R = [[r,b],[r,b]].

test(complete_h_bottom_symmetric_result, []) :-
    G = [[x,x],[r,b]],
    grid_symmetry_complete_h(G, bottom, R),
    R = [[r,b],[r,b]].

test(complete_h_already_symmetric, []) :-
    g3x3_hsym(G),
    grid_symmetry_complete_h(G, top, G).

% --- grid_symmetry_complete_v ---
test(complete_v_left_symmetric_result, []) :-
    G = [[r,x],[b,x]],
    grid_symmetry_complete_v(G, left, R),
    R = [[r,r],[b,b]].

test(complete_v_right_symmetric_result, []) :-
    G = [[x,r],[x,b]],
    grid_symmetry_complete_v(G, right, R),
    R = [[r,r],[b,b]].

test(complete_v_already_symmetric, []) :-
    g3x3_vsym(G),
    grid_symmetry_complete_v(G, left, G).

% --- grid_symmetry_complete_rot180 ---
test(complete_rot180_fills_bg, []) :-
    G = [[r,x],[x,x]],
    grid_symmetry_complete_rot180(G, x, R),
    R = [[r,x],[x,r]].
...
test(complete_rot180_already_symmetric, []) :-
    g3x3_rot180(G),
    grid_symmetry_complete_rot180(G, x, G).

% --- grid_symmetry_violations_h ---
test(violations_h_symmetric_is_empty, []) :-
    g3x3_hsym(G),
    grid_symmetry_violations_h(G, []).

test(violations_h_detects_violation, []) :-
    g3x3_h_partial(G),
    grid_symmetry_violations_h(G, Cells),
    Cells \= [].
...
% --- grid_symmetry_violations_v ---
test(violations_v_symmetric_is_empty, []) :-
    g3x3_vsym(G),
    grid_symmetry_violations_v(G, []).

test(violations_v_detects_violation, []) :-
    g3x3_none(G),
    grid_symmetry_violations_v(G, Cells),
    Cells \= [].

% --- grid_symmetry_violations_rot180 ---
test(violations_rot180_symmetric_is_empty, []) :-
    g3x3_rot180(G),
    grid_symmetry_violations_rot180(G, []).

test(violations_rot180_detects_violation, []) :-
    g3x3_none(G),
    grid_symmetry_violations_rot180(G, Cells),
    Cells \= [].

% --- grid_symmetry_score ---
test(score_h_fully_symmetric_is_1, []) :-
    g3x3_hsym(G),
    grid_symmetry_score(G, h, S),
    S =:= 1.0.

test(score_v_partially_symmetric, []) :-
    G = [[r,x,r],[r,x,b],[r,x,r]],
    grid_symmetry_score(G, v, S),
    S > 0.0,
    S < 1.0.

test(score_rot180_fully_symmetric, []) :-
    g3x3_rot180(G),
    grid_symmetry_score(G, rot180, S),
    S =:= 1.0.
...
test(complete_h_then_sym_h, []) :-
    G = [[r,b],[x,x]],
    grid_symmetry_complete_h(G, top, R),
    grid_symmetry_sym_h(R).

test(complete_v_then_sym_v, []) :-
    G = [[r,x],[b,x]],
    grid_symmetry_complete_v(G, left, R),
    grid_symmetry_sym_v(R).

test(symmetry_transform_rot180_implies_score_1, []) :-
    g3x3_rot180(G),
    grid_symmetry_score(G, rot180, S),
    S =:= 1.0.
