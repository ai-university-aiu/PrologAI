...
test(cols_2x1) :-
    interleave_cols([[1],[2]], [[3],[4]], R),
    R = [[1,3],[2,4]].

% interleave_derows/3 tests
...
test(weave_rows_empty) :-
    interleave_weave_rows([], 9, W),
    W = [].

% interleave_weave_cols/3 tests
...
% interleave_unweave_rows/2 tests
...
test(unweave_rows_one) :-
    interleave_unweave_rows([[1,2,3]], Core),
    Core = [[1,2,3]].

% Four-row grid: keep rows at indices 0 and 2.
test(unweave_rows_four) :-
    interleave_unweave_rows([[a],[b],[c],[d]], Core),
    Core = [[a],[c]].

% interleave_unweave_cols/2 tests
...
test(unweave_cols_one) :-
    interleave_unweave_cols([[5]], Core),
    Core = [[5]].

% Four-element row: keep indices 0 and 2.
test(unweave_cols_four) :-
    interleave_unweave_cols([[a,b,c,d]], Core),
    Core = [[a,c]].

% interleave_stride_rows/4 tests
...
test(stride_rows_step1) :-
    interleave_stride_rows([[a],[b],[c]], 1, 0, S),
    S = [[a],[b],[c]].

% interleave_stride_cols/4 tests
...
test(stride_cols_step2_start1) :-
    interleave_stride_cols([[1,2,3,4]], 2, 1, S),
    S = [[2,4]].
...
% interleave_select_rows/3 tests
...
test(select_rows_middle) :-
    interleave_select_rows([[a],[b],[c]], [1], S),
    S = [[b]].
...
% interleave_select_cols/3 tests
...
test(select_cols_all) :-
    interleave_select_cols([[p,q,r]], [0,1,2], S),
    S = [[p,q,r]].

% interleave_rows_n/2 tests
