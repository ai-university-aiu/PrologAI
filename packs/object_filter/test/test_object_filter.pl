:- use_module('../prolog/object_filter').
...
% object_filter_by_color/3 tests
test(by_color_single) :-
    scene(Objs),
    object_filter_by_color(Objs, r, Filtered),
    length(Filtered, 1),
    Filtered = [obj(r, _)].

test(by_color_none) :-
    scene(Objs),
    object_filter_by_color(Objs, z, Filtered),
    Filtered = [].

test(by_color_two) :-
    Objs = [obj(a, [r(0,0)]), obj(b, [r(0,1)]), obj(a, [r(0,2)])],
    object_filter_by_color(Objs, a, Filtered),
    length(Filtered, 2).

% object_filter_not_color/3 tests
test(not_color_excludes_one) :-
    scene(Objs),
    object_filter_not_color(Objs, r, Filtered),
    length(Filtered, 6),
    \+ member(obj(r, _), Filtered).

test(not_color_none_match) :-
    scene(Objs),
    object_filter_not_color(Objs, z, Filtered),
    length(Filtered, 7).

test(not_color_all_excluded) :-
    Objs = [obj(a, [r(0,0)]), obj(a, [r(1,0)])],
    object_filter_not_color(Objs, a, Filtered),
    Filtered = [].

% object_filter_exact_size/3 tests
test(exact_size_1) :-
    scene(Objs),
    object_filter_exact_size(Objs, 1, Filtered),
    length(Filtered, 1),
    Filtered = [obj(r, _)].

test(exact_size_3) :-
    scene(Objs),
    object_filter_exact_size(Objs, 3, Filtered),
    % hbar, vbar, L all have 3 cells
    length(Filtered, 3).

test(exact_size_8) :-
    scene(Objs),
    object_filter_exact_size(Objs, 8, Filtered),
    % frame and big both have 8 cells
    length(Filtered, 2).

test(exact_size_zero) :-
    scene(Objs),
    object_filter_exact_size(Objs, 9, Filtered),
    Filtered = [].

% object_filter_min_size/3 tests
test(min_size_4) :-
    scene(Objs),
    object_filter_min_size(Objs, 4, Filtered),
    % rect (4), frame (8), big (8)
    length(Filtered, 3).

test(min_size_8) :-
    scene(Objs),
    object_filter_min_size(Objs, 8, Filtered),
    % frame and big
    length(Filtered, 2).

test(min_size_1) :-
    scene(Objs),
    object_filter_min_size(Objs, 1, Filtered),
    % all 7 objects
    length(Filtered, 7).

% object_filter_max_size/3 tests
test(max_size_3) :-
    scene(Objs),
    object_filter_max_size(Objs, 3, Filtered),
    % dot (1), hbar (3), vbar (3), L (3)
    length(Filtered, 4).

test(max_size_1) :-
    scene(Objs),
    object_filter_max_size(Objs, 1, Filtered),
    % only dot
    length(Filtered, 1).

test(max_size_8) :-
    scene(Objs),
    object_filter_max_size(Objs, 8, Filtered),
    % all 7 objects
    length(Filtered, 7).

% object_filter_is_rect/2 tests
...
% object_filter_is_hline/2 tests
...
test(is_hline_explicit) :-
    ...
    object_filter_is_hline([H], Filtered),
    Filtered = [H].

% object_filter_is_vline/2 tests
...
test(is_vline_explicit) :-
    ...
    object_filter_is_vline([V], Filtered),
    Filtered = [V].

% object_filter_is_single/2 tests
...
% object_filter_is_hollow/2 tests
...
% object_filter_smallest/2 tests
test(smallest_is_dot) :-
    scene(Objs),
    object_filter_smallest(Objs, Smallest),
    Smallest = obj(r, _).

test(smallest_single_element) :-
    Obj = obj(a, [r(0,0),r(0,1),r(0,2)]),
    object_filter_smallest([Obj], Smallest),
    Smallest = Obj.
...
% object_filter_filter/3 tests (meta-predicate with custom Goal)
is_blue(obj(b, _)).

test(filter_custom_blue) :-
    scene(Objs),
    object_filter_filter(Objs, is_blue, Filtered),
    Filtered = [obj(b, _)].
...
% object_filter_partition/4 tests
test(partition_by_color) :-
    Objs = [obj(r, [r(0,0)]), obj(b, [r(0,1)]), obj(r, [r(0,2)])],
    object_filter_partition(Objs, is_blue, In, Out),
    In  = [obj(b, _)],
    length(Out, 2).
