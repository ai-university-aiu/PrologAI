...
griddist_min_manhattan_(R, C, Seeds, MinD) :-
...
griddist_min_chebyshev_(R, C, Seeds, MinD) :-
...
griddist_bfs_loop_(_, _, _, _, [], _, Acc, Acc) :- !.
griddist_bfs_loop_(Grid, H, W, Wall, [R-C-D|Queue], Visited, Acc, Result) :-
% Next-level distance.
    D1 is D + 1,
...
% Extend visited, queue, and accumulated results.
    ...
    griddist_bfs_loop_(Grid, H, W, Wall, NextQueue, NewVisited, NewAcc, Result).

% Find the nearest foreground color to (R,C) from a list of FR-FC-V triples.
griddist_nearest_fg_color_(R, C, FgCells, Color) :-
...
% Collect all positions that hold Color as BFS seeds.
    ...
    W1 is W - 1,
    findall(SR-SC,
        (between(0, H1, SR), between(0, W1, SC), griddist_cell_(Grid, SR, SC, Color)),
        Seeds),
% Build the distance grid cell by cell.
    findall(Row,
        (between(0, H1, R),
         findall(D,
             (between(0, W1, C), griddist_min_manhattan_(R, C, Seeds, D)),
             Row)),
        DistGrid).
...
% Build the Chebyshev distance grid.
    findall(Row,
        (between(0, H1, R),
         findall(D,
             (between(0, W1, C), griddist_min_chebyshev_(R, C, Seeds, D)),
             Row)),
        DistGrid).
...
griddist_nearest_coord(Grid, R, C, Color, [R2, C2]) :-
% Collect all Color cells with their distance from (R,C).
    ...
    findall(D-NR-NC,
        (between(0, H1, NR), between(0, W1, NC),
         griddist_cell_(Grid, NR, NC, Color),
...
% Compute distance map from Color cells.
    griddist_dist_map(Grid, Color, DMap),
    griddist_dims_(Grid, H, W),
    H1 is H - 1,
    W1 is W - 1,
    findall(Row,
...
% Collect all foreground (non-background) cells as FR-FC-V triples.
    findall(R-C-V,
        (between(0, H1, R), between(0, W1, C),
         griddist_cell_(Grid, R, C, V), V \= Bg),
        FgCells),
        ...
        (between(0, H1, R),
         findall(NV,
             (between(0, W1, C),
              griddist_cell_(Grid, R, C, V),
              (V = Bg, FgCells \= [] ->
                 griddist_nearest_fg_color_(R, C, FgCells, NV)
              ;  NV = V)),
             Row)),
        VGrid).
...
% Start cell must not be a wall.
    griddist_dims_(Grid, H, W),
    griddist_cell_(Grid, R0, C0, StartV),
    StartV \= Wall,
    H1 is H - 1,
    W1 is W - 1,
% BFS: initial queue and visited both contain the start cell at distance 0.
    griddist_bfs_loop_(Grid, H, W, Wall, [R0-C0-0], [R0-C0], [R0-C0-0], CellDists),
...
% Find background and compute distance from background for every cell.
    griddist_background_(Grid, Bg),
    griddist_dist_map(Grid, Bg, DMap),
    length(Palette, PLen),
    griddist_dims_(Grid, H, W),
    H1 is H - 1,
    W1 is W - 1,
...
% Build one distance map per color.
    griddist_dist_map(Grid, ColorA, DMapA),
    griddist_dist_map(Grid, ColorB, DMapB),
    griddist_dims_(Grid, H, W),
    H1 is H - 1,
    W1 is W - 1,
    findall(Row,
        (between(0, H1, R),
         findall(NV,
             (between(0, W1, C),
              griddist_cell_(Grid, R, C, V),
              griddist_cell_(DMapA, R, C, DA),
              griddist_cell_(DMapB, R, C, DB),
...
% Collect all finite distances (exclude 9999 = unreachable / no Color present).
    findall(D,
        (between(0, H1, R2), between(0, W1, C2),
         griddist_cell_(DMap, R2, C2, D), D < 9999),
        Dists),
    Dists \= [],
    max_list(Dists, MaxD),
% Return the first cell (row-major) that achieves MaxD.
    once((between(0, H1, R), between(0, W1, C), griddist_cell_(DMap, R, C, MaxD))).
