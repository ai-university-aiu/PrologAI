...
gd_min_manhattan_(R, C, Seeds, MinD) :-
...
gd_min_chebyshev_(R, C, Seeds, MinD) :-
...
gd_bfs_loop_(_, _, _, _, [], _, Acc, Acc) :- !.
gd_bfs_loop_(Grid, H, W, Wall, [R-C-D|Queue], Visited, Acc, Result) :-
% Next-level distance.
    D1 is D + 1,
...
% Extend visited, queue, and accumulated results.
    ...
    gd_bfs_loop_(Grid, H, W, Wall, NextQueue, NewVisited, NewAcc, Result).

% Find the nearest foreground color to (R,C) from a list of FR-FC-V triples.
gd_nearest_fg_color_(R, C, FgCells, Color) :-
...
% Collect all positions that hold Color as BFS seeds.
    ...
    W1 is W - 1,
    findall(SR-SC,
        (between(0, H1, SR), between(0, W1, SC), gd_cell_(Grid, SR, SC, Color)),
        Seeds),
% Build the distance grid cell by cell.
    findall(Row,
        (between(0, H1, R),
         findall(D,
             (between(0, W1, C), gd_min_manhattan_(R, C, Seeds, D)),
             Row)),
        DistGrid).
...
% Build the Chebyshev distance grid.
    findall(Row,
        (between(0, H1, R),
         findall(D,
             (between(0, W1, C), gd_min_chebyshev_(R, C, Seeds, D)),
             Row)),
        DistGrid).

% gd_nearest_coord(+Grid, +R, +C, +Color, -Coord)
        ...
        (between(0, H1, R),
         findall(NV,
             (between(0, W1, C),
              gd_cell_(Grid, R, C, V),
              (V = Bg, FgCells \= [] ->
                 gd_nearest_fg_color_(R, C, FgCells, NV)
              ;  NV = V)),
             Row)),
        VGrid).
...
% Start cell must not be a wall.
    gd_dims_(Grid, H, W),
    gd_cell_(Grid, R0, C0, StartV),
    StartV \= Wall,
    H1 is H - 1,
    W1 is W - 1,
% BFS: initial queue and visited both contain the start cell at distance 0.
    gd_bfs_loop_(Grid, H, W, Wall, [R0-C0-0], [R0-C0], [R0-C0-0], CellDists),
...
% Build one distance map per color.
    gd_dist_map(Grid, ColorA, DMapA),
    gd_dist_map(Grid, ColorB, DMapB),
    gd_dims_(Grid, H, W),
    H1 is H - 1,
    W1 is W - 1,
    findall(Row,
        (between(0, H1, R),
         findall(NV,
             (between(0, W1, C),
              gd_cell_(Grid, R, C, V),
              gd_cell_(DMapA, R, C, DA),
              gd_cell_(DMapB, R, C, DB),
...
% Collect all finite distances (exclude 9999 = unreachable / no Color present).
    findall(D,
        (between(0, H1, R2), between(0, W1, C2),
         gd_cell_(DMap, R2, C2, D), D < 9999),
        Dists),
    Dists \= [],
    max_list(Dists, MaxD),
% Return the first cell (row-major) that achieves MaxD.
    once((between(0, H1, R), between(0, W1, C), gd_cell_(DMap, R, C, MaxD))).
