% State the fact: this pack is named co_liken.
name(co_liken).
% State the fact: this is version 1.0.0.
version('1.0.0').
% State the fact: the title carries the work-package number.
title('PrologAI Causalontology Analogy — map the objects of one situation onto another so the relations line up, score the match, and transfer a known rule across the mapping (WP-419)').
