:- module(grid_histogram, [
    grid_histogram_row_hist/3,
    grid_histogram_col_hist/3,
    grid_histogram_all_row_hists/2,
    grid_histogram_all_col_hists/2,
    grid_histogram_modal_row/3,
    grid_histogram_modal_col/3,
    grid_histogram_row_count/4,
    grid_histogram_col_count/4,
    grid_histogram_row_entropy/3,
    grid_histogram_col_entropy/3,
    grid_histogram_max_row/3,
    grid_histogram_min_row/3,
    grid_histogram_rows_with/4,
    grid_histogram_cols_with/4
]).
...
grid_histogram_list_hist_(List, Hist) :-
% Sort preserving duplicates to group identical colors together.
    msort(List, Sorted),
% Count consecutive runs of each color.
    grid_histogram_count_runs_(Sorted, Hist).

% Base case: empty list produces empty histogram.
grid_histogram_count_runs_([], []).
% Inductive case: count the run of the head color.
grid_histogram_count_runs_([Color|Rest], [Color-N|Hist]) :-
% Count how many consecutive elements equal Color.
    grid_histogram_count_leading_(Rest, Color, 1, N, Remaining),
% Recurse on the remainder after the run.
    grid_histogram_count_runs_(Remaining, Hist).

% Count leading elements equal to Target, starting with accumulator Acc.
grid_histogram_count_leading_([], _, N, N, []).
grid_histogram_count_leading_([V|Rest], Target, Acc, N, Remaining) :-
    (V = Target ->
% V continues the run; increment accumulator.
        Acc1 is Acc + 1,
        grid_histogram_count_leading_(Rest, Target, Acc1, N, Remaining)
    ;
...
grid_histogram_modal_(Hist, BestColor) :-
...
% Build histogram from the list.
    grid_histogram_list_hist_(Row, Hist).

% grid_histogram_col_hist(+Grid, +C, -Hist)
...
% Extract column C as a top-to-bottom list.
    grid_histogram_col_(Grid, C, Col),
% Build histogram from the list.
    grid_histogram_list_hist_(Col, Hist).

% grid_histogram_all_row_hists(+Grid, -AllHists)
...
% Collect one histogram per row.
    findall(Hist, (between(0, H1, R), grid_histogram_row_hist(Grid, R, Hist)), AllHists).

% grid_histogram_all_col_hists(+Grid, -AllHists)
...
% Collect one histogram per column.
    findall(Hist, (between(0, W1, C), grid_histogram_col_hist(Grid, C, Hist)), AllHists).

% grid_histogram_modal_row(+Grid, +R, -Color)
% Color is the most frequent color in row R. Ties broken by last-in-sort order.
grid_histogram_modal_row(Grid, R, Color) :-
% Get the row histogram.
    grid_histogram_row_hist(Grid, R, Hist),
% Find the color with the highest count.
    grid_histogram_modal_(Hist, Color).

% grid_histogram_modal_col(+Grid, +C, -Color)
...
% grid_histogram_row_count(+Grid, +R, +Color, -Count)
...
% grid_histogram_col_count(+Grid, +C, +Color, -Count)
...
% grid_histogram_row_entropy(+Grid, +R, -N)
...
% grid_histogram_col_entropy(+Grid, +C, -N)
...
% grid_histogram_max_row(+Grid, +Color, -Row)
...
% Count Color in every row.
    length(Grid, H), H1 is H - 1,
    findall(N-R, (between(0, H1, R), grid_histogram_row_count(Grid, R, Color, N), N > 0), Pairs),
...
% grid_histogram_min_row(+Grid, +Color, -Row)
...
% grid_histogram_rows_with(+Grid, +Color, +N, -Rows)
% Rows is the list of row indices where Color appears at least N times.
grid_histogram_rows_with(Grid, Color, N, Rows) :-
% Compute the valid row index range.
    length(Grid, H), H1 is H - 1,
% Collect every R where the count of Color is >= N.
    findall(R, (between(0, H1, R), grid_histogram_row_count(Grid, R, Color, C), C >= N), Rows).

% grid_histogram_cols_with(+Grid, +Color, +N, -Cols)
...
% Collect every C where the count of Color is >= N.
    findall(C, (between(0, W1, C), grid_histogram_col_count(Grid, C, Color, Cnt), Cnt >= N), Cols).
