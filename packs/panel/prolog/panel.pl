...
% pn_is_divider_row(+Grid, +R, +Bg): row R of Grid is entirely Bg.
% Uses forall/2 to assert that every element of the row equals Bg.
pn_is_divider_row(Grid, R, Bg) :-
% Extract the row at 0-based index R.
    nth0(R, Grid, Row),
% Verify that every element in the row equals Bg.
    forall(member(V, Row), V == Bg).

% pn_is_divider_col(+Grid, +C, +Bg): column C of Grid is entirely Bg.
% Uses forall to check every row's element at position C.
pn_is_divider_col(Grid, C, Bg) :-
% Verify that for every row in Grid, the element at column C equals Bg.
    forall(member(Row, Grid), (nth0(C, Row, V), V == Bg)).

% pn_divider_rows(+Grid, +Bg, -Rows): sorted list of all-Bg row indices.
pn_divider_rows(Grid, Bg, Rows) :-
% Compute the highest valid row index.
    length(Grid, H), H1 is H - 1,
% Collect every index where pn_is_divider_row succeeds.
    findall(R, (between(0, H1, R), pn_is_divider_row(Grid, R, Bg)), Rows).

% pn_divider_cols(+Grid, +Bg, -Cols): sorted list of all-Bg column indices.
pn_divider_cols(Grid, Bg, Cols) :-
% Get column count from the first row.
    (Grid = [First|_] -> length(First, W) ; W = 0),
% Compute the highest valid column index.
    W1 is W - 1,
% Collect every column index where pn_is_divider_col succeeds.
    findall(C, (between(0, W1, C), pn_is_divider_col(Grid, C, Bg)), Cols).

% pn_split_rows(+Grid, +RowIdxs, -Parts): split at given row indices.
...
% pn_split_cols(+Grid, +ColIdxs, -Parts): split at given column indices.
% Each element of ColIdxs is excluded; parts are the column bands between dividers.
pn_split_cols(Grid, ColIdxs, Parts) :-
% Get column count from the first row.
    (Grid = [First|_] -> length(First, W) ; W = 0),
% Delegate to the recursive helper.
    pn_split_cols_(Grid, ColIdxs, 0, W, Parts).

% pn_split_cols_(+Grid, +Divs, +Start, +End, -Parts): accumulate column bands.
% Base case: extract the final column band.
pn_split_cols_(Grid, [], Start, End, [Part]) :- !,
% Extract columns from Start to End for every row.
    maplist(pn_slice_row_(Start, End), Grid, Part).
% Recursive case: extract band before divider, then recurse.
pn_split_cols_(Grid, [Div|Rest], Start, End, [Part|Parts]) :-
% Extract columns [Start, Div) for every row.
    maplist(pn_slice_row_(Start, Div), Grid, Part),
% Next band starts after the divider column.
    Next is Div + 1,
% Recurse.
    pn_split_cols_(Grid, Rest, Next, End, Parts).

% pn_slice_row_(+C0, +C1, +Row, -Sub): extract elements at columns [C0, C1).
...
% pn_auto_split_rows(+Grid, +Bg, -Parts): detect all-Bg rows and split at them.
pn_auto_split_rows(Grid, Bg, Parts) :-
% Find all divider row indices.
    pn_divider_rows(Grid, Bg, Divs),
% Split the grid at those positions.
    pn_split_rows(Grid, Divs, Parts).

% pn_auto_split_cols(+Grid, +Bg, -Parts): detect all-Bg columns and split at them.
pn_auto_split_cols(Grid, Bg, Parts) :-
% Find all divider column indices.
    pn_divider_cols(Grid, Bg, Divs),
% Split the grid at those positions.
    pn_split_cols(Grid, Divs, Parts).

% pn_strip_rows(+Grid, +Bg, -Stripped): remove all all-Bg rows.
% Stripped contains all rows whose index is not in the divider list.
pn_strip_rows(Grid, Bg, Stripped) :-
% Find all divider row indices.
    pn_divider_rows(Grid, Bg, Divs),
...
% pn_strip_cols(+Grid, +Bg, -Stripped): remove all all-Bg columns.
% Each row in Stripped contains only non-divider column values.
pn_strip_cols(Grid, Bg, Stripped) :-
% Find all divider column indices.
    pn_divider_cols(Grid, Bg, Divs),
% Get column count from the first row.
    (Grid = [First|_] -> length(First, W) ; W = 0),
...
% For each row, extract only the kept columns.
    maplist(pn_select_cols_(Kept), Grid, Stripped).

% pn_select_cols_(+Idxs, +Row, -Sub): extract elements at given column indices.
pn_select_cols_(Idxs, Row, Sub) :-
% Map nth0 over the index list to retrieve each element.
    maplist(pn_nth0_(Row), Idxs, Sub).

% pn_nth0_(+Row, +I, -V): retrieve element at 0-based index I from Row.
pn_nth0_(Row, I, V) :-
% Delegate to built-in nth0/3.
    nth0(I, Row, V).

% pn_quadrants(+Grid, -TL, -TR, -BL, -BR): split at midpoints into 4 quadrants.
% MidR = H // 2; MidC = W // 2.
% TL: rows [0, MidR), cols [0, MidC).
% TR: rows [0, MidR), cols [MidC, W).
% BL: rows [MidR, H), cols [0, MidC).
% BR: rows [MidR, H), cols [MidC, W).
pn_quadrants(Grid, TL, TR, BL, BR) :-
% Compute grid dimensions.
    length(Grid, H), MidR is H // 2,
    Grid = [FirstRow|_], length(FirstRow, W), MidC is W // 2,
% Extract top-left quadrant.
    pn_rect_(Grid, 0, 0, MidR, MidC, TL),
% Extract top-right quadrant.
    pn_rect_(Grid, 0, MidC, MidR, W, TR),
% Extract bottom-left quadrant.
    pn_rect_(Grid, MidR, 0, H, MidC, BL),
% Extract bottom-right quadrant.
    pn_rect_(Grid, MidR, MidC, H, W, BR).

% pn_rect_(+Grid, +R0, +C0, +R1, +C1, -Sub): extract rows [R0,R1) and cols [C0,C1).
pn_rect_(Grid, R0, C0, R1, C1, Sub) :-
...
% Collect each row in the range, sliced to [C0, C1).
    findall(SubRow, (
        between(R0, R1a, R),
        nth0(R, Grid, Row),
        pn_slice_row_(C0, C1, Row, SubRow)
    ), Sub).

% pn_n_col_parts(+Grid, +N, -Parts): split into N equal-width column strips.
% The grid width must be divisible by N. Each strip has width W // N.
pn_n_col_parts(Grid, N, Parts) :-
...
% Extract the strip from every row.
        maplist(pn_slice_row_(C0, C1), Grid, Part)
    ), Parts).

% pn_col_slices(+Grid, +Widths, -Parts): split columns by a list of widths.
% The elements of Widths must sum to the total grid width.
pn_col_slices(Grid, Widths, Parts) :-
% Compute the cumulative column end positions from Widths.
    pn_cumsum_(Widths, 0, Ends),
% Build each slice using width and end position.
    pn_col_slices_(Grid, Widths, Ends, Parts).

% pn_cumsum_(+List, +Acc, -Ends): cumulative sum of List starting from Acc.
...
pn_cumsum_([W|Rest], Acc, [End|Ends]) :-
    End is Acc + W,
% Recurse with the updated accumulator.
    pn_cumsum_(Rest, End, Ends).

% pn_col_slices_(+Grid, +Widths, +Ends, -Parts): build slices from precomputed ends.
% Base case: empty widths list.
pn_col_slices_(_, [], [], []) :- !.
% Recursive case: extract one slice and recurse.
pn_col_slices_(Grid, [W|WRest], [End|ERest], [Part|Parts]) :-
% Compute start position as end minus width.
    Start is End - W,
% Extract the column band [Start, End) from every row.
    maplist(pn_slice_row_(Start, End), Grid, Part),
% Recurse over remaining slices.
    pn_col_slices_(Grid, WRest, ERest, Parts).

% pn_has_dividers(+Grid, +Bg): succeed if Grid has at least one all-Bg row or column.
% Tests row dividers first; only tests column dividers if no row dividers found.
pn_has_dividers(Grid, Bg) :-
