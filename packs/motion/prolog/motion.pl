...
% mv_gravity_down(+Grid, +Bg, -Grid2): non-Bg cells sink to the bottom of each column.
...
% mv_gravity_up(+Grid, +Bg, -Grid2): non-Bg cells rise to the top of each column.
mv_gravity_up(Grid, Bg, Grid2) :-
    gd_size(Grid, _, Cols),
    C1 is Cols - 1,
    numlist(0, C1, ColIndices),
...
% mv_gravity_left(+Grid, +Bg, -Grid2): non-Bg cells slide to the left of each row.
mv_gravity_left(Grid, Bg, Grid2) :-
...
% mv_gravity_right(+Grid, +Bg, -Grid2): non-Bg cells slide to the right of each row.
mv_gravity_right(Grid, Bg, Grid2) :-
...
% GRID SHIFT
% mv_shift_grid(+Grid, +DR, +DC, +Bg, -Grid2): shift all content by (DR, DC).
...
% mv_slide_col(+Grid, +C, +Dir, -Grid2): slide column C up or down.
mv_slide_col(Grid, C, down, Grid2) :-
...
% mv_slide_row(+Grid, +R, +Dir, -Grid2): slide row R left or right.
mv_slide_row(Grid, R, left, Grid2) :-
    gd_row(Grid, R, Row),
...
mv_slide_row(Grid, R, right, Grid2) :-
    gd_row(Grid, R, Row),
    include(is_fg, Row, Fg),
    length(Row, Len),
    length(Fg, FgLen),
...
% OBJECT TRANSLATION
% mv_obj_translate(+Obj, +DR, +DC, -Obj2): shift all cells of an object by (DR,DC).
mv_obj_translate(obj(Color, Cells), DR, DC, obj(Color, NewCells)) :-
...
% mv_scene_translate(+Scene, +DR, +DC, -Scene2): translate all objects in a scene.
mv_scene_translate(scene(Rows, Cols, Bg, Objects), DR, DC, scene(Rows, Cols, Bg, Objects2)) :-
...
translate_obj(DR, DC, Obj, Obj2) :-
    mv_obj_translate(Obj, DR, DC, Obj2).


% SCENE TO GRID RENDERING
% mv_scene_to_grid(+Scene, -Grid): render a scene back to a raw grid.
...
% mv_scene_gravity(+Scene, -Scene2): render scene, apply column gravity, re-parse.
...
% DISTANCE METRICS
% mv_distance(+r(R1,C1), +r(R2,C2), -D): Manhattan distance between two cells.
mv_distance(r(R1, C1), r(R2, C2), D) :-
    DR is abs(R2 - R1),
    DC is abs(C2 - C1),
    D is DR + DC.

% mv_closest_cell(+Cells, +Ref, -Closest): cell in Cells closest to Ref.
mv_closest_cell([H|T], Ref, Closest) :-
% Compute distances to all cells.
    mv_distance(H, Ref, D0),
