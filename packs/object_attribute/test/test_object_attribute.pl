:- use_module('../prolog/object_attribute').
:- use_module(library(plunit)).

:- begin_tests(object_attribute).
...
test(total_cells_basic) :-
    ...
    % 1 + 3 = 4
    object_attribute_total_cells([R, B], N),
    N == 4.

test(total_cells_empty) :-
    object_attribute_total_cells([], N),
    N == 0.

test(total_cells_all_same) :-
    red_dot(D1), blue_dot(D2), green_dot(D3),
    object_attribute_total_cells([D1, D2, D3], N),
    N == 3.
...
test(color_counts_basic) :-
    ...
    object_attribute_color_counts([R, B, RB], Pairs),
    member(b-1, Pairs),
    member(r-2, Pairs).

test(color_counts_single_color) :-
    red_dot(D), red_bar(B), red_sq(S),
    object_attribute_color_counts([D, B, S], Pairs),
    Pairs == [r-3].

test(color_counts_empty) :-
    object_attribute_color_counts([], Pairs),
    Pairs == [].
...
test(cell_counts_by_color_basic) :-
    ...
    object_attribute_cell_counts_by_color([R, RB, B], Pairs),
    member(b-1, Pairs),
    member(r-4, Pairs).
...
test(cell_counts_by_color_same) :-
    ...
    % both size 1
    object_attribute_cell_counts_by_color([D, B], Pairs),
    member(b-1, Pairs),
    member(r-1, Pairs).
...
test(n_colors_three) :-
    ...
    object_attribute_n_colors([R, B, G], N),
    N == 3.

test(n_colors_one) :-
    red_dot(R), red_bar(RB),
    object_attribute_n_colors([R, RB], N),
    N == 1.

test(n_colors_empty) :-
    object_attribute_n_colors([], N),
    N == 0.
...
test(n_objs_of_color_basic) :-
    red_dot(R), red_bar(RB), blue_dot(B),
    object_attribute_n_objs_of_color([R, RB, B], r, N),
    N == 2.

test(n_objs_of_color_zero) :-
    red_dot(R),
    object_attribute_n_objs_of_color([R], b, N),
    N == 0.
...
test(dominant_color_basic) :-
    ...
    object_attribute_dominant_color([S, B, G], C),
    C == r.

test(dominant_color_single) :-
    red_dot(R),
    object_attribute_dominant_color([R], C),
    C == r.
...
test(rarest_color_basic) :-
    ...
    object_attribute_rarest_color([S, B, G], C),
    member(C, [b, g]).

test(rarest_color_single) :-
    blue_dot(B),
    object_attribute_rarest_color([B], C),
    C == b.
...
test(unique_color_basic) :-
    ...
    object_attribute_unique_color([R, B, RB], C),
    C == b.

test(unique_color_all_unique) :-
    red_dot(R), blue_dot(B), green_dot(G),
    % all appear once; object_attribute_unique_color/2 returns first alphabetically
    object_attribute_unique_color([R, B, G], _).

test(unique_color_none) :-
    red_dot(R), red_bar(RB),
    \+ object_attribute_unique_color([R, RB], _).
...
test(size_rank_basic) :-
    ...
    object_attribute_size_rank([D, B, S], [S, B, D]).

test(size_rank_single) :-
    red_dot(D),
    object_attribute_size_rank([D], [D]).
...
test(pos_rank_basic) :-
    ...
    object_attribute_pos_rank([Y, S], [S, Y]).
...
test(pos_rank_row_order) :-
    ...
    object_attribute_pos_rank([G, B, R], [R, B, G]).
...
test(majority_size_basic) :-
    ...
    object_attribute_majority_size([D1, D2, B, S], N),
    N == 1.

test(majority_size_single) :-
    red_bar(B),
    object_attribute_majority_size([B], N),
    N == 3.
...
test(all_same_color_true) :-
    red_dot(D), red_bar(B), red_sq(S),
    object_attribute_all_same_color([D, B, S]).

test(all_same_color_false) :-
    red_dot(D), blue_dot(B),
    \+ object_attribute_all_same_color([D, B]).
...
test(all_same_form_true) :-
    ...
    object_attribute_all_same_form([L, LF]).

test(all_same_form_false) :-
    lshape(L), red_bar(B),
    \+ object_attribute_all_same_form([L, B]).
...
test(all_same_form_dots) :-
    ...
    red_dot(R), blue_dot(B), green_dot(G),
    object_attribute_all_same_form([R, B, G]).
