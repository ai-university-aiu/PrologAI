...
test(roll_roundtrip) :-
    g5(G),
    grid_shift_roll_right(G, 2, R1),
    grid_shift_roll_left(R1, 2, R2),
    R2 = [[a,b,c],[d,e,f],[g,h,i]].

% AC-GSH-044: shift_right by 1 equals offset by (DR=0, DC=1) with same bg.
test(shift_vs_offset) :-
    g5(G),
    grid_shift_shift_right(G, 1, z, SR),
    grid_shift_offset(G, 0, 1, z, OFF),
    SR = OFF.
