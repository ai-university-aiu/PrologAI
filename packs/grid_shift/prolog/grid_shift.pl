:- module(grid_shift, [
    grid_shift_shift_right/4,
    grid_shift_shift_left/4,
    grid_shift_shift_down/4,
    grid_shift_shift_up/4,
    grid_shift_roll_right/3,
    grid_shift_roll_left/3,
    grid_shift_roll_down/3,
    grid_shift_roll_up/3,
    grid_shift_roll_row/4,
    grid_shift_roll_col/4,
    grid_shift_shift_color/6,
    grid_shift_offset/5,
    grid_shift_shift_row/5,
    grid_shift_shift_col/5
]).
...
% grid_shift_dims_(+Grid, -H, -W): grid dimensions.
grid_shift_dims_(Grid, H, W) :-
% Count rows for height.
    length(Grid, H),
% Use first row for width.
    Grid = [Row|_], length(Row, W).

% grid_shift_bg_row_(+W, +BgColor, -Row): create a row of W BgColor values.
grid_shift_bg_row_(W, BgColor, Row) :-
% Generate W copies of BgColor using between/3.
    findall(BgColor, between(1, W, _), Row).

% grid_shift_roll_list_(+List, +N, -Rolled): cyclically roll List right by N positions.
% The last N elements move to the front.
grid_shift_roll_list_(List, N, Rolled) :-
...
% grid_shift_transpose_(+Grid, -T): transpose a rectangular grid.
grid_shift_transpose_([], []) :- !.
grid_shift_transpose_([[]|_], []) :- !.
grid_shift_transpose_(Grid, [Heads|Tails]) :-
% Split each row into its first element (head) and the rest (tail).
    grid_shift_heads_tails_(Grid, Heads, RestRows),
    grid_shift_transpose_(RestRows, Tails).

% grid_shift_heads_tails_(+Rows, -Heads, -Tails): split rows into head elements and tail lists.
grid_shift_heads_tails_([], [], []).
grid_shift_heads_tails_([[H|T]|Rest], [H|Hs], [T|Ts]) :-
% Recurse over remaining rows.
    grid_shift_heads_tails_(Rest, Hs, Ts).

% grid_shift_shift_row_r_(+Row, +N, +BgColor, -Shifted): shift one row right by N, fill left with BgColor.
grid_shift_shift_row_r_(Row, N, BgColor, Shifted) :-
...
% grid_shift_shift_row_l_(+Row, +N, +BgColor, -Shifted): shift one row left by N, fill right with BgColor.
grid_shift_shift_row_l_(Row, N, BgColor, Shifted) :-
% Clamp N to row width.
    length(Row, W), N2 is min(N, W),
    KeepLen is W - N2,
...
% grid_shift_shift_right(+Grid, +N, +BgColor, -Result)
% Shift each row of Grid right by N cells; vacated left cells become BgColor.
grid_shift_shift_right(Grid, N, BgColor, Result) :-
% Apply row shift to every row.
    findall(NewRow,
        (member(GRow, Grid),
         grid_shift_shift_row_r_(GRow, N, BgColor, NewRow)),
        Result).

% grid_shift_shift_left(+Grid, +N, +BgColor, -Result)
% Shift each row left by N cells; vacated right cells become BgColor.
grid_shift_shift_left(Grid, N, BgColor, Result) :-
% Apply left row shift to every row.
    findall(NewRow,
        (member(GRow, Grid),
         grid_shift_shift_row_l_(GRow, N, BgColor, NewRow)),
        Result).

% grid_shift_shift_down(+Grid, +N, +BgColor, -Result)
...
% Roll left by N = roll right by (W - N mod W).
    findall(NewRow,
        (member(GRow, Grid),
         ...
         grid_shift_roll_list_(GRow, RN, NewRow)),
        Result).

% grid_shift_roll_down(+Grid, +N, -Result)
% Cyclically roll the entire grid down by N rows (last N rows wrap to top).
grid_shift_roll_down(Grid, N, Result) :-
% Roll the list of rows cyclically.
    grid_shift_roll_list_(Grid, N, Result).

% grid_shift_roll_up(+Grid, +N, -Result)
...
% Roll up by N = roll down by (H - N mod H).
    ...
    grid_shift_roll_list_(Grid, RN, Result).

% grid_shift_roll_row(+Grid, +R, +N, -Result)
...
% grid_shift_roll_col(+Grid, +C, +N, -Result)
...
% Transpose to treat columns as rows, roll the target row, transpose back.
    grid_shift_transpose_(Grid, TGrid),
    grid_shift_roll_row(TGrid, C, N, TRolled),
    grid_shift_transpose_(TRolled, Result).

% grid_shift_shift_color(+Grid, +Color, +DR, +DC, +BgColor, -Result)
...
% grid_shift_offset(+Grid, +DR, +DC, +BgColor, -Result)
...
% grid_shift_shift_row(+Grid, +R, +N, +BgColor, -Result)
...
% Apply shift only to the target row.
         (RI =:= R ->
             (N >= 0 ->
                 grid_shift_shift_row_r_(GRow, N, BgColor, NewRow)
             ;   N2 is -N,
                 grid_shift_shift_row_l_(GRow, N2, BgColor, NewRow)
             )
         ;   NewRow = GRow
         )),
        Result).

% grid_shift_shift_col(+Grid, +C, +N, +BgColor, -Result)
...
% Transpose to treat column C as a row, shift it, then transpose back.
    grid_shift_transpose_(Grid, TGrid),
    grid_shift_shift_row(TGrid, C, N, BgColor, TShifted),
    grid_shift_transpose_(TShifted, Result).
