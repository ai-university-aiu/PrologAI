...
% grid_chain_sublist_/4: extract Len elements starting at index Start from List.
grid_chain_sublist_(List, Start, Len, SubList) :-
...
% grid_chain_count_cell_diffs_/3: count positions where two same-length rows differ.
grid_chain_count_cell_diffs_([], [], 0).
grid_chain_count_cell_diffs_([V|T1], [V|T2], N) :- !,
% Matching values contribute zero to the count.
    grid_chain_count_cell_diffs_(T1, T2, N).
grid_chain_count_cell_diffs_([_|T1], [_|T2], N) :-
% Non-matching values increment count by one.
    grid_chain_count_cell_diffs_(T1, T2, N1),
    N is N1 + 1.

% grid_chain_count_row_diffs_/3: total count of differing cells across two same-shaped grids.
grid_chain_count_row_diffs_([], [], 0).
grid_chain_count_row_diffs_([R1|T1], [R2|T2], N) :-
    grid_chain_count_cell_diffs_(R1, R2, N1),
    grid_chain_count_row_diffs_(T1, T2, N2),
    N is N1 + N2.

% grid_chain_diff_mask_cells_/5: build a row marking differing positions with MarkColor.
grid_chain_diff_mask_cells_([], [], _, _, []).
grid_chain_diff_mask_cells_([V1|T1], [V2|T2], Bg, MC, [V|Vs]) :-
% Mark differing positions; unchanged positions become Bg.
    (V1 \= V2 -> V = MC ; V = Bg),
    grid_chain_diff_mask_cells_(T1, T2, Bg, MC, Vs).

% grid_chain_diff_mask_rows_/5: build a grid marking all differing positions.
grid_chain_diff_mask_rows_([], [], _, _, []).
grid_chain_diff_mask_rows_([R1|T1], [R2|T2], Bg, MC, [NR|Rest]) :-
    grid_chain_diff_mask_cells_(R1, R2, Bg, MC, NR),
    grid_chain_diff_mask_rows_(T1, T2, Bg, MC, Rest).

% --- PUBLIC PREDICATES ---

% grid_chain_pairs(+Grids, -Pairs)
...
% grid_chain_window(+Grids, +W, -Windows)
...
grid_chain_window(Grids, W, Windows) :-
    ...
    (W =< N ->
        Last is N - W,
        findall(Window,
            (between(0, Last, I), grid_chain_sublist_(Grids, I, W, Window)),
            Windows)
    ;
        Windows = []
    ).

% grid_chain_zip(+Grids1, +Grids2, -ZippedPairs)
% Zip two sequences element-wise: ZippedPairs = [[G1a,G1b],[G2a,G2b],...].
% Terminates at the shorter sequence.
grid_chain_zip([], _, []) :- !.
grid_chain_zip(_, [], []) :- !.
grid_chain_zip([G1|T1], [G2|T2], [[G1,G2]|Rest]) :-
    grid_chain_zip(T1, T2, Rest).

% grid_chain_take(+Grids, +N, -Taken)
...
% grid_chain_nth(+Grids, +N, -Grid)
% Return the 0-indexed N-th grid from the sequence.
grid_chain_nth(Grids, N, Grid) :-
    nth0(N, Grids, Grid).

% grid_chain_all_same(+Grids)
...
grid_chain_all_same([]) :- !.
grid_chain_all_same([_]) :- !.
grid_chain_all_same([G, G | Rest]) :-
    grid_chain_all_same([G | Rest]).

% grid_chain_dedup(+Grids, -Unique)
% Remove duplicate grids, preserving first occurrence.
% Two grids are duplicates if they are identical by unification.
grid_chain_dedup(Grids, Unique) :-
    grid_chain_dedup_(Grids, [], Unique).

% grid_chain_dedup_/3: accumulate seen grids and filter duplicates.
grid_chain_dedup_([], _, []).
grid_chain_dedup_([H|T], Seen, Result) :-
    (member(H, Seen) ->
        grid_chain_dedup_(T, Seen, Result)
    ;
        Result = [H|Rest],
        grid_chain_dedup_(T, [H|Seen], Rest)
    ).

% grid_chain_cycle(+Grid, +N, -Sequence)
% Build a sequence of N copies of Grid.
grid_chain_cycle(Grid, N, Sequence) :-
    findall(Grid, between(1, N, _), Sequence).

% grid_chain_interleave(+Grids1, +Grids2, -Interleaved)
% Interleave two sequences: [G1a, G1b, G2a, G2b, ...].
% Terminates when the shorter sequence is exhausted.
grid_chain_interleave([], _, []) :- !.
grid_chain_interleave(_, [], []) :- !.
grid_chain_interleave([G1|T1], [G2|T2], [G1, G2 | Rest]) :-
    grid_chain_interleave(T1, T2, Rest).

% grid_chain_split_at(+Grids, +N, -Before, -After)
% Split the sequence at index N: Before has the first N grids; After has the rest.
grid_chain_split_at(Grids, N, Before, After) :-
    length(Before, N),
    append(Before, After, Grids).

% grid_chain_reverse(+Grids, -Reversed)
% Reverse the sequence of grids.
grid_chain_reverse(Grids, Reversed) :-
    reverse(Grids, Reversed).

% grid_chain_diff_counts(+Grids, +BgColor, -Counts)
...
grid_chain_diff_counts(Grids, _Bg, Counts) :-
    grid_chain_pairs(Grids, Pairs),
    findall(N, (member([G1,G2], Pairs), grid_chain_count_row_diffs_(G1, G2, N)), Counts).

% grid_chain_changes_mask(+Grids, +BgColor, +MarkColor, -Mask)
