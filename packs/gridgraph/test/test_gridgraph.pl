...
test(component_cells_one, []) :-
    g5x5_block(G),
    ggr_component_cells(G, r, Components),
    length(Components, 1),
    [Cells] = Components,
    length(Cells, 9).
...
test(degree_equals_adj_of_length, []) :-
    g3x3_abc(G),
    ggr_adj_of(G, b, Nbrs),
    ggr_color_degree(G, b, N),
    length(Nbrs, N).

test(border_length_asymmetric, []) :-
    g3x3_rb(G),
    % r->x and x->r border lengths can differ (asymmetric)
    ggr_border_length(G, r, x, N1),
    ggr_border_length(G, x, r, N2),
    N1 \= N2.

test(merge_then_uniform, []) :-
    g3x3_rb(G),
    ggr_merge_colors(G, x, r, All_r),
    ggr_isolated_colors(All_r, [r]).

test(component_cells_count_eq_color_components, []) :-
    g5x5_two(G),
    ggr_component_cells(G, r, Comps),
    ggr_color_components(G, r, N),
    length(Comps, N).
