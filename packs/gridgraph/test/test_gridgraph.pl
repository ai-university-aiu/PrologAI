...
:- begin_tests(gridgraph).

% --- gridgraph_adj_colors ---
test(adj_colors_rb, []) :-
    g3x3_rb(G),
    % Only pair: r-x
    gridgraph_adj_colors(G, Pairs),
    Pairs = [r-x].
...
test(adj_colors_uniform, []) :-
    ...
    gridgraph_adj_colors(G, []).

% --- gridgraph_adj_of ---
test(adj_of_r_in_rb, []) :-
    g3x3_rb(G),
    gridgraph_adj_of(G, r, [x]).

test(adj_of_x_in_rb, []) :-
    g3x3_rb(G),
    gridgraph_adj_of(G, x, [r]).

test(adj_of_b_in_abc, []) :-
    g3x3_abc(G),
    % b is between a (above) and c (below)
    gridgraph_adj_of(G, b, Nbrs),
    msort(Nbrs, Sorted),
    Sorted = [a, c].

test(adj_of_uniform, []) :-
    g3x3_all_r(G),
    gridgraph_adj_of(G, r, []).

% --- gridgraph_adj_graph ---
test(adj_graph_rb, []) :-
    g3x3_rb(G),
    gridgraph_adj_graph(G, Graph),
    member(r-[x], Graph),
    member(x-[r], Graph).

test(adj_graph_uniform, []) :-
    g3x3_all_r(G),
    gridgraph_adj_graph(G, [r-[]]).

% --- gridgraph_are_adj ---
test(are_adj_r_x, []) :-
    g3x3_rb(G),
    gridgraph_are_adj(G, r, x).

test(are_not_adj_same, []) :-
    g3x3_rb(G),
    \+ gridgraph_are_adj(G, r, r).

test(are_adj_a_b, []) :-
    g3x3_abc(G),
    gridgraph_are_adj(G, a, b).

test(are_not_adj_a_c, []) :-
    g3x3_abc(G),
    % a (top row) and c (bottom row) do not share a border
    \+ gridgraph_are_adj(G, a, c).

% --- gridgraph_color_degree ---
test(degree_r_in_rb, []) :-
    g3x3_rb(G),
    gridgraph_color_degree(G, r, 1).

test(degree_b_in_abc, []) :-
    g3x3_abc(G),
    % b is adjacent to a and c -> degree 2
    gridgraph_color_degree(G, b, 2).

test(degree_uniform, []) :-
    g3x3_all_r(G),
    gridgraph_color_degree(G, r, 0).

% --- gridgraph_shared_border ---
test(shared_border_r_x, []) :-
    g3x3_rb(G),
    % r cells adjacent to x: (0,1),(1,0),(1,1)
    gridgraph_shared_border(G, r, x, Cells),
    length(Cells, 3).

test(shared_border_x_r, []) :-
    g3x3_rb(G),
    % x cells adjacent to r: (0,2),(1,2),(2,0),(2,1)
    gridgraph_shared_border(G, x, r, Cells),
    length(Cells, 4).

% --- gridgraph_border_length ---
test(border_length_r_x, []) :-
    g3x3_rb(G),
    gridgraph_border_length(G, r, x, 3).

test(border_length_x_r, []) :-
    g3x3_rb(G),
    gridgraph_border_length(G, x, r, 4).

% --- gridgraph_isolated_colors ---
test(isolated_uniform_r, []) :-
    g3x3_all_r(G),
    % r has no different-colored neighbors -> isolated
    gridgraph_isolated_colors(G, [r]).

test(isolated_none_rb, []) :-
    g3x3_rb(G),
    % Both r and x are adjacent to each other -> neither isolated
    gridgraph_isolated_colors(G, []).

% --- gridgraph_enclosed_colors ---
test(enclosed_x_in_ring, []) :-
    ...
    gridgraph_enclosed_colors(G, Inner, r),
    Inner = [x].
...
test(not_enclosed_r_in_block, []) :-
    ...
    gridgraph_enclosed_colors(G, Inner, x),
    Inner = [r].
...
test(spanning_v_abc, []) :-
    ...
    % No color spans v
    gridgraph_spanning_v(G, []).

test(spanning_v_full_col, []) :-
    g5x5_vspan(G),
    % r is in col 0 of row 0 and col 0 of row 4 -> r spans v
    gridgraph_spanning_v(G, Colors),
    member(r, Colors).

% --- gridgraph_merge_colors ---
test(merge_r_to_x, []) :-
    g3x3_rb(G),
    gridgraph_merge_colors(G, r, x, Result),
    Result = [[x,x,x],[x,x,x],[x,x,x]].

test(merge_noop, []) :-
    g3x3_rb(G),
    % Merge a color not in Grid -> no change
    gridgraph_merge_colors(G, z, y, G).

% --- gridgraph_color_components ---
test(components_one, []) :-
    g5x5_block(G),
    % 3x3 r block is all connected -> 1 component
    gridgraph_color_components(G, r, 1).

test(components_four_corners, []) :-
    g5x5_two(G),
    % 4 isolated r cells at corners -> 4 components
    gridgraph_color_components(G, r, 4).

test(components_uniform, []) :-
    g3x3_all_r(G),
    gridgraph_color_components(G, r, 1).

% --- gridgraph_component_cells ---
test(component_cells_one, []) :-
    g5x5_block(G),
    gridgraph_component_cells(G, r, Components),
    length(Components, 1),
    [Cells] = Components,
    length(Cells, 9).
...
test(degree_equals_adj_of_length, []) :-
    g3x3_abc(G),
    gridgraph_adj_of(G, b, Nbrs),
    gridgraph_color_degree(G, b, N),
    length(Nbrs, N).

test(border_length_asymmetric, []) :-
    g3x3_rb(G),
    % r->x and x->r border lengths can differ (asymmetric)
    gridgraph_border_length(G, r, x, N1),
    gridgraph_border_length(G, x, r, N2),
    N1 \= N2.

test(merge_then_uniform, []) :-
    g3x3_rb(G),
    gridgraph_merge_colors(G, x, r, All_r),
    gridgraph_isolated_colors(All_r, [r]).

test(component_cells_count_eq_color_components, []) :-
    g5x5_two(G),
    gridgraph_component_cells(G, r, Comps),
    gridgraph_color_components(G, r, N),
    length(Comps, N).
...
test(component_cells_all_in_correct_color, []) :-
    g5x5_block(G),
    gridgraph_component_cells(G, r, [Comp]),
