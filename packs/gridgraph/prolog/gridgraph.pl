:- module(gridgraph, [
    gridgraph_adj_colors/2,
    gridgraph_adj_of/3,
    gridgraph_adj_graph/2,
    gridgraph_are_adj/3,
    gridgraph_color_degree/3,
    gridgraph_shared_border/4,
    gridgraph_border_length/4,
    gridgraph_isolated_colors/2,
    gridgraph_enclosed_colors/3,
    gridgraph_spanning_h/2,
    gridgraph_spanning_v/2,
    gridgraph_merge_colors/4,
    gridgraph_color_components/3,
    gridgraph_component_cells/3
]).
...
gridgraph_bfs_same_(_, [], _, Visited, Visited).
gridgraph_bfs_same_(Grid, [RC|Queue], Color, Visited, Result) :-
    (memberchk(RC, Visited) ->
        gridgraph_bfs_same_(Grid, Queue, Color, Visited, Result)
    ;
        RC = R-C,
        NewVisited = [RC|Visited],
        findall(NR-NC,
            (gridgraph_nbr4_(R, C, NR, NC),
             gridgraph_cell_(Grid, NR, NC, Color),
             \+ memberchk(NR-NC, NewVisited)),
            Nbrs),
        append(Queue, Nbrs, NewQueue),
        gridgraph_bfs_same_(Grid, NewQueue, Color, NewVisited, Result)).

% --- PUBLIC PREDICATES ---

% gridgraph_adj_colors(+Grid, -Pairs)
...
% Collect all border pairs.
    ...
    findall(P,
        (between(0, H1, R), between(0, W1, C),
         gridgraph_cell_(Grid, R, C, V),
         once((gridgraph_nbr4_(R, C, NR, NC),
               gridgraph_cell_(Grid, NR, NC, NV),
               NV \= V,
               (V @< NV -> P = V-NV ; P = NV-V)))),
        RawPairs),
    list_to_set(RawPairs, Pairs).

% gridgraph_adj_of(+Grid, +Color, -Neighbors)
...
% Find all cells of Color with a differently-colored neighbor.
    ...
    findall(NV,
        (between(0, H1, R), between(0, W1, C),
         gridgraph_cell_(Grid, R, C, Color),
         gridgraph_nbr4_(R, C, NR, NC),
         gridgraph_cell_(Grid, NR, NC, NV),
         NV \= Color),
        RawNbrs),
...
% gridgraph_adj_graph(+Grid, -Graph)
...
% Build adjacency list for each color.
    findall(Color-Nbrs,
        (member(Color, Colors),
         gridgraph_adj_of(Grid, Color, Nbrs)),
        Graph).

% gridgraph_are_adj(+Grid, +C1, +C2)
...
% Find one C1 cell adjacent to a C2 cell.
    ...
    once((between(0, H1, R), between(0, W1, C),
          gridgraph_cell_(Grid, R, C, C1),
          gridgraph_nbr4_(R, C, NR, NC),
          gridgraph_cell_(Grid, NR, NC, C2))).

% gridgraph_color_degree(+Grid, +Color, -N)
...
% gridgraph_shared_border(+Grid, +C1, +C2, -Cells)
...
% gridgraph_border_length(+Grid, +C1, +C2, -N)
...
% gridgraph_isolated_colors(+Grid, -Colors)
...
gridgraph_isolated_colors(Grid, Colors) :-
% Get all colors.
    gridgraph_colors_(Grid, AllColors),
% Isolated colors have degree 0.
    findall(C,
        (member(C, AllColors),
         gridgraph_color_degree(Grid, C, 0)),
        Colors).

% gridgraph_enclosed_colors(+Grid, -InnerColors, +OuterColor)
...
% Find colors in first and last row.
    findall(C,
        (member(Row, Grid), member(C, Row),
         once((between(0, W1, C2), gridgraph_cell_(Grid, 0, C2, C))),
         once((between(0, W1, C3), gridgraph_cell_(Grid, H1, C3, C)))),
        RawColors),
    list_to_set(RawColors, Colors).

% gridgraph_merge_colors(+Grid, +FromColor, +ToColor, -Result)
...
% gridgraph_color_components(+Grid, +Color, -N)
...
% Count components by BFS.
    gridgraph_count_components_(Grid, AllCells, Color, 0, N).

% Count connected components by iterating BFS over unvisited cells.
gridgraph_count_components_(_, [], _, N, N).
gridgraph_count_components_(Grid, [RC|Rest], Color, Acc, N) :-
% BFS from RC to find its whole component.
    gridgraph_bfs_same_(Grid, [RC], Color, [], Component),
% Remove component cells from remaining.
    subtract(Rest, Component, Remaining),
    Acc1 is Acc + 1,
    gridgraph_count_components_(Grid, Remaining, Color, Acc1, N).

% gridgraph_component_cells(+Grid, +Color, -Components)
...
% Partition into components.
    gridgraph_partition_components_(Grid, AllCells, Color, [], Components).

% Partition Color cells into connected components.
gridgraph_partition_components_(_, [], _, Acc, Rev) :-
% Reverse accumulator for row-major order.
    reverse(Acc, Rev).
gridgraph_partition_components_(Grid, [RC|Rest], Color, Acc, Components) :-
% BFS from RC to find its component.
    gridgraph_bfs_same_(Grid, [RC], Color, [], Component),
% Remove component from remaining cells.
    subtract(Rest, Component, Remaining),
    gridgraph_partition_components_(Grid, Remaining, Color, [Component|Acc], Components).
