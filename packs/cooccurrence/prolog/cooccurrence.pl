...
:- module(cooccurrence, [
    cooccurrence_h_pairs/2,
    cooccurrence_v_pairs/2,
    cooccurrence_d_pairs_dr/2,
    cooccurrence_d_pairs_dl/2,
    cooccurrence_count_h/4,
    cooccurrence_count_v/4,
    cooccurrence_count_adj4/4,
    cooccurrence_always_adj4/3,
    cooccurrence_never_adj4/3,
    cooccurrence_isolated4/3,
    cooccurrence_border_vals/3,
    cooccurrence_shared_border/3,
    cooccurrence_row_transitions/2,
    cooccurrence_most_common_adj4/3
]).
...
% cooccurrence_h_pairs(+Grid, -Pairs): Pairs is the list of all ordered horizontal adjacent
...
cooccurrence_h_pairs(Grid, Pairs) :-
% For each row collect consecutive adjacent pairs via cooccurrence_row_adj_.
    findall(A-B, (member(Row, Grid), cooccurrence_row_adj_(Row, A, B)), Pairs).

% cooccurrence_v_pairs(+Grid, -Pairs): Pairs is the list of all ordered vertical adjacent
...
cooccurrence_v_pairs(Grid, Pairs) :-
% Determine column count from the first row; default 0 for empty grid.
    (Grid = [Fr | _] -> length(Fr, W) ; W = 0),
% Highest valid column index.
    W1 is W - 1,
% For each column, collect pairs from adjacent rows via cooccurrence_col_adj_.
    findall(A-B, (between(0, W1, C), cooccurrence_col_adj_(Grid, C, A, B)), Pairs).

% cooccurrence_d_pairs_dr(+Grid, -Pairs): Pairs is the ordered list of diagonal adjacent
% value pairs A-B going down-right: A at (R,C), B at (R+1,C+1).
cooccurrence_d_pairs_dr(Grid, Pairs) :-
...
% cooccurrence_d_pairs_dl(+Grid, -Pairs): Pairs is the ordered list of diagonal adjacent
% value pairs A-B going down-left: A at (R,C), B at (R+1,C-1).
cooccurrence_d_pairs_dl(Grid, Pairs) :-
% Determine grid bounds.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr | _] -> length(Fr, W) ; W = 0), W1 is W - 1,
...
% cooccurrence_count_h(+Grid, +V1, +V2, -N): N is the count of directed horizontal pairs
% where value V1 appears immediately to the left of value V2.
cooccurrence_count_h(Grid, V1, V2, N) :-
% Collect all horizontal pairs then filter to those matching V1-V2.
    cooccurrence_h_pairs(Grid, Pairs),
    include(=(V1-V2), Pairs, M),
    length(M, N).

% cooccurrence_count_v(+Grid, +V1, +V2, -N): N is the count of directed vertical pairs
% where value V1 appears immediately above value V2.
cooccurrence_count_v(Grid, V1, V2, N) :-
% Collect all vertical pairs then filter to those matching V1-V2.
    cooccurrence_v_pairs(Grid, Pairs),
    include(=(V1-V2), Pairs, M),
    length(M, N).

% cooccurrence_count_adj4(+Grid, +V1, +V2, -N): N is the total count of undirected
...
cooccurrence_count_adj4(Grid, V1, V2, N) :-
% Collect all horizontal and vertical ordered pairs.
    cooccurrence_h_pairs(Grid, H),
    cooccurrence_v_pairs(Grid, V),
    append(H, V, All),
...
% cooccurrence_always_adj4(+Grid, +V1, +V2): succeeds if every cell with value V1 has
% at least one 4-adjacent cell with value V2. Fails if Grid has no V1 cells.
cooccurrence_always_adj4(Grid, V1, V2) :-
% Fails if any V1 cell has no V2 among its 4-neighbors.
    \+ cooccurrence_has_nonadj4_(Grid, V1, V2).

% cooccurrence_never_adj4(+Grid, +V1, +V2): succeeds if no cell with value V1 is
% 4-adjacent to any cell with value V2.
cooccurrence_never_adj4(Grid, V1, V2) :-
% Fails if any V1-V2 shared edge exists.
    \+ cooccurrence_shared_border(Grid, V1, V2).

% cooccurrence_isolated4(+Grid, +V, -Cells): Cells is the sorted R-C list of positions
% with value V that have no 4-adjacent cell with the same value V.
cooccurrence_isolated4(Grid, V, Cells) :-
% Determine grid bounds.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr | _] -> length(Fr, W) ; W = 0), W1 is W - 1,
...
% cooccurrence_border_vals(+Grid, +V, -Vals): Vals is the sorted list of distinct values
% W (where W \= V) that appear in at least one 4-adjacent position to a V cell.
cooccurrence_border_vals(Grid, V, Vals) :-
% Determine grid bounds.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr | _] -> length(Fr, W) ; W = 0), W1 is W - 1,
...
% cooccurrence_shared_border(+Grid, +V1, +V2): succeeds if some cell with value V1 is
...
% cooccurrence_row_transitions(+Grid, -Triples): Triples is a list of A-B-N terms, each
% representing a directed horizontal adjacent pair A-B occurring N times.
% Sorted by count descending (most frequent first).
cooccurrence_row_transitions(Grid, Triples) :-
...
% cooccurrence_most_common_adj4(+Grid, +V, -W): W is the value most frequently 4-adjacent
...
cooccurrence_most_common_adj4(Grid, V, W) :-
% Get distinct values that border V.
    cooccurrence_border_vals(Grid, V, BVals),
% Fail if no other value borders V.
    BVals \= [],
% Count undirected adjacency of each border value.
    findall(N-X, (member(X, BVals), cooccurrence_count_adj4(Grid, V, X, N)), Counts),
% Sort ascending; last element has highest count (and highest value on tie).
    msort(Counts, Sorted),
    last(Sorted, _-W).

% cooccurrence_row_adj_(+Row, -A, -B): A and B are consecutive elements of Row.
% Backtracks over all adjacent pairs in left-to-right order.
cooccurrence_row_adj_([A, B | _], A, B).
cooccurrence_row_adj_([_ | T], A, B) :-
% Require at least two more elements to prevent out-of-bounds.
    T = [_ | _],
    cooccurrence_row_adj_(T, A, B).

% cooccurrence_col_adj_(+Grid, +C, -A, -B): A is the value in column C of some row R,
% B is the value in column C of row R+1. Backtracks over adjacent row pairs.
cooccurrence_col_adj_([R1, R2 | _], C, A, B) :-
    nth0(C, R1, A),
    nth0(C, R2, B).
cooccurrence_col_adj_([_ | T], C, A, B) :-
% Require at least two more rows to prevent out-of-bounds.
    T = [_ | _],
    cooccurrence_col_adj_(T, C, A, B).

% cooccurrence_has_nonadj4_(+Grid, +V1, +V2): internal helper. Succeeds if there exists
% a V1 cell that has NO 4-adjacent cell with value V2. Used by cooccurrence_always_adj4.
cooccurrence_has_nonadj4_(Grid, V1, V2) :-
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr | _] -> length(Fr, W) ; W = 0), W1 is W - 1,
    between(0, H1, R), between(0, W1, C),
