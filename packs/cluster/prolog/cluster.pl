...
% cl_chebyshev(+R1, +C1, +R2, +C2, -D): Chebyshev (L-inf) distance between two cells.
% D = max(|R2 - R1|, |C2 - C1|). This is the number of king moves between the cells.
cl_chebyshev(R1, C1, R2, C2, D) :-
...
% cl_manhattan(+R1, +C1, +R2, +C2, -D): Manhattan (L-1) distance between two cells.
% D = |R2 - R1| + |C2 - C1|. This is the minimum number of 4-connected steps.
cl_manhattan(R1, C1, R2, C2, D) :-
% Sum of absolute differences.
    D is abs(R2 - R1) + abs(C2 - C1).

% cl_euclidean_sq(+R1, +C1, +R2, +C2, -DSq): squared Euclidean distance.
% DSq = (R2-R1)^2 + (C2-C1)^2. Integer arithmetic; no square root needed for comparisons.
cl_euclidean_sq(R1, C1, R2, C2, DSq) :-
% Sum of squared differences.
    DSq is (R2 - R1) * (R2 - R1) + (C2 - C1) * (C2 - C1).

% cl_within(+R, +C, +D, +Cells, -Near): cells from Cells within Chebyshev D of (R,C).
% Near is the sublist of Cells whose Chebyshev distance to (R,C) is at most D.
cl_within(R, C, D, Cells, Near) :-
...
% cl_nearest_helper_: fold helper for finding the nearest cell. If-then-else for determinism.
cl_nearest_helper_(_, _, [], _, BestCell, BestCell) :- !.
cl_nearest_helper_(R, C, [Cell|Rest], BestD, BestCell, Result) :-
    Cell = CR-CC,
% Chebyshev distance from query point to this cell.
    D is max(abs(CR - R), abs(CC - C)),
    (D < BestD ->
% This cell is closer; update best.
        cl_nearest_helper_(R, C, Rest, D, Cell, Result)
    ;
% Keep current best.
        cl_nearest_helper_(R, C, Rest, BestD, BestCell, Result)
    ).

% cl_nearest(+R, +C, +Cells, -Nearest): cell in Cells closest to (R,C) by Chebyshev.
% Cells must be non-empty. Ties resolved by first occurrence in Cells.
cl_nearest(R, C, [First|Rest], Nearest) :-
% Initialize with the first cell.
    First = FR-FC,
    D0 is max(abs(FR - R), abs(FC - C)),
    cl_nearest_helper_(R, C, Rest, D0, First, Nearest).

% cl_farthest_helper_: fold helper for finding the farthest cell.
cl_farthest_helper_(_, _, [], _, BestCell, BestCell) :- !.
cl_farthest_helper_(R, C, [Cell|Rest], BestD, BestCell, Result) :-
    Cell = CR-CC,
    D is max(abs(CR - R), abs(CC - C)),
    (D > BestD ->
        cl_farthest_helper_(R, C, Rest, D, Cell, Result)
    ;
        cl_farthest_helper_(R, C, Rest, BestD, BestCell, Result)
    ).

% cl_farthest(+R, +C, +Cells, -Farthest): cell in Cells most distant from (R,C).
% Cells must be non-empty. Ties resolved by first occurrence.
cl_farthest(R, C, [First|Rest], Farthest) :-
    First = FR-FC,
    D0 is max(abs(FR - R), abs(FC - C)),
    cl_farthest_helper_(R, C, Rest, D0, First, Farthest).

% cl_center(+Cells, -CR, -CC): integer centroid of a non-empty cell set.
% CR = round(mean row), CC = round(mean column).
cl_center(Cells, CR, CC) :-
% Extract row indices.
...
% cl_all_pairs_: generate all unordered pairs from a list. Cut on base cases.
cl_all_pairs_([], []) :- !.
cl_all_pairs_([_], []) :- !.
cl_all_pairs_([H|T], Pairs) :-
% Pair H with each element of T.
    maplist([X, P]>>(P = H-X), T, HPs),
% Recurse for remaining pairs.
    cl_all_pairs_(T, RestPs),
    append(HPs, RestPs, Pairs).

% cl_diameter(+Cells, -D): maximum pairwise Chebyshev distance within a cell set.
% For a single cell, D = 0.
cl_diameter([_], 0) :- !.
cl_diameter(Cells, D) :-
% Generate all unordered pairs.
    cl_all_pairs_(Cells, Pairs),
...
% cl_spread(+Cells, -DR, -DC): bounding box dimensions of a non-empty cell set.
% DR = max_row - min_row; DC = max_col - min_col.
cl_spread(Cells, DR, DC) :-
...
% cl_group_by_color(+Grid, +Cells, -Colors, -Groups): group R-C cells by their value in Grid.
% Colors is a sorted list of distinct values; Groups[i] is the sublist of Cells with Colors[i].
cl_group_by_color(Grid, Cells, Colors, Groups) :-
...
% cl_sort_by_dist(+R, +C, +Cells, -Sorted): sort Cells by Chebyshev distance to (R,C).
% Ties preserve the original relative order (keysort is stable).
cl_sort_by_dist(R, C, Cells, Sorted) :-
...
% cl_min_pair_: fold helper for nearest pair (minimum Chebyshev).
cl_min_pair_([], _, Best, Best) :- !.
cl_min_pair_([H|T], BestD, Best0, Result) :-
    H = (PR1-PC1)-(PR2-PC2),
    D is max(abs(PR2-PR1), abs(PC2-PC1)),
    (D < BestD ->
        cl_min_pair_(T, D, H, Result)
    ;
        cl_min_pair_(T, BestD, Best0, Result)
    ).

% cl_nearest_pair(+Cells, -Cell1, -Cell2): pair of cells with minimum Chebyshev distance.
% Cells must have at least 2 elements. Ties resolved by first occurrence.
cl_nearest_pair(Cells, Cell1, Cell2) :-
% Generate all unordered pairs.
    cl_all_pairs_(Cells, [First|RestPairs]),
...
% Find the pair with minimum distance.
    cl_min_pair_(RestPairs, D0, First, BestPair),
    BestPair = Cell1-Cell2.

% cl_max_pair_: fold helper for farthest pair (maximum Chebyshev).
cl_max_pair_([], _, Best, Best) :- !.
cl_max_pair_([H|T], BestD, Best0, Result) :-
    H = (PR1-PC1)-(PR2-PC2),
    D is max(abs(PR2-PR1), abs(PC2-PC1)),
    (D > BestD ->
        cl_max_pair_(T, D, H, Result)
    ;
        cl_max_pair_(T, BestD, Best0, Result)
    ).

% cl_farthest_pair(+Cells, -Cell1, -Cell2): pair of cells with maximum Chebyshev distance.
% Cells must have at least 2 elements. Ties resolved by first occurrence.
cl_farthest_pair(Cells, Cell1, Cell2) :-
    cl_all_pairs_(Cells, [First|RestPairs]),
    First = (PR1-PC1)-(PR2-PC2),
    D0 is max(abs(PR2-PR1), abs(PC2-PC1)),
    cl_max_pair_(RestPairs, D0, First, BestPair),
    BestPair = Cell1-Cell2.

% cl_cells_in_band(+R1, +R2, +C1, +C2, +Cells, -Band): filter Cells to those
% whose row is in [R1,R2] and column is in [C1,C2]. Inclusive on all sides.
cl_cells_in_band(R1, R2, C1, C2, Cells, Band) :-
