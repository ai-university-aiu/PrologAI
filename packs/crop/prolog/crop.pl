...
% crop_grid_dims_(+Grid, -Rows, -Cols)
% Number of rows and columns in Grid.
crop_grid_dims_(Grid, Rows, Cols) :-
    length(Grid, Rows),
    (Rows =:= 0 -> Cols = 0 ; Grid = [First|_], length(First, Cols)).

% crop_row_(+Grid, +R, -Row)
% Row R (0-indexed) of Grid.
crop_row_(Grid, R, Row) :- nth0(R, Grid, Row).

% crop_col_val_(+Row, +C, -Val)
% Value at column C in a single row.
crop_col_val_(Row, C, Val) :- nth0(C, Row, Val).

% crop_bbox(+Grid, +BG, -Box)
...
crop_bbox(Grid, BG, bbox(MinR, MinC, MaxR, MaxC)) :-
    Grid = [_|_],
    crop_grid_dims_(Grid, Rows, Cols),
    Rows > 0, Cols > 0,
...
% crop_crop_bbox(+Grid, +R1, +C1, +R2, +C2, -Sub)
...
crop_crop_bbox(Grid, R1, C1, R2, C2, Sub) :-
    ...
    length(RowIds, Len),
    maplist(crop_crop_row_(Grid, C1, C2), RowIds, Sub).

% crop_crop_row_(+Grid, +C1, +C2, +R, -Row)
...
% crop_crop_content(+Grid, +BG, -Cropped)
% Cropped is Grid trimmed to the bounding box of cells that differ from BG.
crop_crop_content(Grid, BG, Cropped) :-
    crop_bbox(Grid, BG, bbox(R1, C1, R2, C2)),
    crop_crop_bbox(Grid, R1, C1, R2, C2, Cropped).

% crop_make_row_(+Color, +N, -Row)
% Row is a list of N copies of Color.
crop_make_row_(Color, N, Row) :-
    length(Row, N),
    maplist(=(Color), Row).

% crop_pad(+Grid, +Color, +N, -Padded)
% Padded adds N rows of Color above and below, and N columns of Color left
% and right of Grid.
crop_pad(Grid, Color, N, Padded) :-
    N >= 0,
    crop_grid_dims_(Grid, _Rows, Cols),
    PadCols is Cols + 2 * N,
    crop_make_row_(Color, N, PadSide),
    crop_make_row_(Color, PadCols, FullPadRow),
    length(TopRows, N), maplist(=(FullPadRow), TopRows),
    length(BotRows, N), maplist(=(FullPadRow), BotRows),
    maplist(crop_pad_row_(Color, PadSide), Grid, MidRows),
    append([TopRows, MidRows, BotRows], Padded).

% crop_pad_row_(+Color, +PadSide, +Row, -PaddedRow)
% Prepend and append PadSide to a single row.
crop_pad_row_(_Color, PadSide, Row, PaddedRow) :-
    append(PadSide, Row, Left),
    append(Left, PadSide, PaddedRow).

% crop_strip_border(+Grid, +N, -Inner)
...
% crop_stitch_h(+Left, +Right, -Joined)
...
% crop_stitch_v(+Top, +Bottom, -Joined)
...
% crop_center(+Grid, +Rows, +Cols, -Center)
% Center is the central Rows x Cols subgrid of Grid.
% Grid must have height >= Rows and width >= Cols.
crop_center(Grid, Rows, Cols, Center) :-
    crop_grid_dims_(Grid, GH, GW),
    GH >= Rows, GW >= Cols,
    R1 is (GH - Rows) // 2,
    C1 is (GW - Cols) // 2,
    R2 is R1 + Rows - 1,
    C2 is C1 + Cols - 1,
    crop_crop_bbox(Grid, R1, C1, R2, C2, Center).

% crop_quadrants(+Grid, -Q1, -Q2, -Q3, -Q4)
...
crop_quadrants(Grid, Q1, Q2, Q3, Q4) :-
    ...
    MidR is Rows // 2,
    MidC is Cols // 2,
    crop_split_h(Grid, MidR, Top, Bottom),
    crop_split_v(Top, MidC, Q1, Q2),
    crop_split_v(Bottom, MidC, Q3, Q4).
