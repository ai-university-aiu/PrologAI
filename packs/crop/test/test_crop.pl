...
test(bbox_basic) :-
    g4x4(G),
    crop_bbox(G, 0, bbox(1,1,2,2)).

test(bbox_3x3) :-
    g3x3(G),
    crop_bbox(G, 0, bbox(0,0,2,2)).

test(bbox_full) :-
    % All cells non-background.
    G = [[1,2],[3,4]],
    crop_bbox(G, 0, bbox(0,0,1,1)).
...
test(crop_bbox_basic) :-
    g4x4(G),
    crop_crop_bbox(G, 1, 1, 2, 2, Sub),
    Sub = [[1,2],[3,4]].

test(crop_bbox_full_row) :-
    G = [[1,2,3],[4,5,6],[7,8,9]],
    crop_crop_bbox(G, 0, 0, 2, 2, Sub),
    Sub = [[1,2,3],[4,5,6],[7,8,9]].

test(crop_bbox_single_cell) :-
    G = [[1,2,3],[4,5,6]],
    crop_crop_bbox(G, 1, 2, 1, 2, Sub),
    Sub = [[6]].

test(crop_bbox_single_row) :-
    g2x4(G),
    crop_crop_bbox(G, 0, 1, 0, 2, Sub),
    Sub = [[2,3]].
...
test(pad_basic) :-
    G = [[1,2],[3,4]],
    crop_pad(G, 0, 1, Padded),
    Padded = [[0,0,0,0],[0,1,2,0],[0,3,4,0],[0,0,0,0]].

test(pad_zero) :-
    G = [[1,2],[3,4]],
    crop_pad(G, 0, 0, Padded),
    Padded = [[1,2],[3,4]].

test(pad_two) :-
    G = [[5]],
    crop_pad(G, 0, 2, Padded),
...
test(strip_basic) :-
    g4x4(G),
    crop_strip_border(G, 1, Inner),
    Inner = [[1,2],[3,4]].

test(strip_zero) :-
    G = [[1,2],[3,4]],
    crop_strip_border(G, 0, Inner),
    Inner = [[1,2],[3,4]].

test(strip_fails_too_large, [fail]) :-
    G = [[1,2],[3,4]],
    crop_strip_border(G, 2, _).
...
test(split_h_basic) :-
    G = [[1,2],[3,4],[5,6],[7,8]],
    crop_split_h(G, 2, Top, Bottom),
    Top = [[1,2],[3,4]],
    Bottom = [[5,6],[7,8]].

test(split_h_one) :-
    G = [[1,2],[3,4],[5,6]],
    crop_split_h(G, 1, Top, Bottom),
    Top = [[1,2]],
    Bottom = [[3,4],[5,6]].
...
test(split_h_fails_zero, [fail]) :-
    G = [[1,2],[3,4]],
    crop_split_h(G, 0, _, _).

test(split_h_fails_full, [fail]) :-
    G = [[1,2],[3,4]],
    crop_split_h(G, 2, _, _).
...
test(split_v_one) :-
    G = [[1,2,3],[4,5,6]],
    crop_split_v(G, 1, Left, Right),
    Left = [[1],[4]],
    Right = [[2,3],[5,6]].

test(split_v_fails_zero, [fail]) :-
    G = [[1,2],[3,4]],
    crop_split_v(G, 0, _, _).

test(split_v_fails_full, [fail]) :-
    G = [[1,2],[3,4]],
    crop_split_v(G, 2, _, _).
...
test(embed_basic) :-
    ...
    Sub  = [[1,2],[3,4]],
    crop_embed(Base, Sub, 1, 1, Result),
    Result = [[0,0,0],[0,1,2],[0,3,4]].

test(embed_at_origin) :-
    Base = [[0,0,0],[0,0,0],[0,0,0]],
    Sub  = [[1,2],[3,4]],
    crop_embed(Base, Sub, 0, 0, Result),
...
test(center_basic) :-
    ...
    crop_center(G, 3, 3, Center),
    Center = [[7,8,9],[12,13,14],[17,18,19]].

test(center_1x1) :-
    G = [[1,2,3],[4,5,6],[7,8,9]],
    crop_center(G, 1, 1, Center),
    Center = [[5]].

test(center_exact_size) :-
    G = [[1,2],[3,4]],
    crop_center(G, 2, 2, Center),
    Center = [[1,2],[3,4]].
