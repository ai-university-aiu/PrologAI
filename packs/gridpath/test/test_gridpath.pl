...
test(all_reachable_obstacle_seed_fails, [fail]) :-
    g3x3_wall_mid(G),
    gridpath_all_reachable(G, 0, 1, w, _).

% --- gridpath_distance_map tests ---

% 1x1 grid: map has one entry (0,0)=0.
test(distance_map_1x1) :-
    g1x1(G),
    gridpath_distance_map(G, 0, 0, w, Map),
    Map = [0-0-0].
...
test(region_path_same_color) :-
    g4x4_two_colors(G),
    gridpath_region_path(G, r, r, w, Path),
    Path = [0-0].

% No Color1 present: fails.
test(region_path_no_color1_fails, [fail]) :-
    g3x3_open(G),
    gridpath_region_path(G, z, q, w, _).
