...
test(path_start_obstacle_fails, [fail]) :-
    g3x3_wall_mid(G),
    gpa_shortest_path(G, 0, 1, 0, 2, w, _).

% End cell is obstacle: fails.
test(path_end_obstacle_fails, [fail]) :-
    g3x3_wall_mid(G),
    gpa_shortest_path(G, 0, 0, 0, 1, w, _).

% 1x1 grid, same start and end.
test(path_1x1) :-
    g1x1(G),
    gpa_shortest_path(G, 0, 0, 0, 0, w, Path),
    Path = [0-0].

% --- gpa_path_length tests ---

% Adjacent: length 1.
test(path_length_adjacent) :-
    g3x3_open(G),
    gpa_path_length(G, 0, 0, 0, 1, w, N),
    N = 1.

% Same cell: length 0.
test(path_length_same) :-
    g3x3_open(G),
    gpa_path_length(G, 1, 1, 1, 1, w, N),
    N = 0.

% Corner to corner 3x3: Manhattan distance 4.
test(path_length_corner) :-
    g3x3_open(G),
    gpa_path_length(G, 0, 0, 2, 2, w, N),
    N = 4.

% Blocked: fails.
test(path_length_blocked_fails, [fail]) :-
    g3x3_wall_mid(G),
    gpa_path_length(G, 0, 0, 0, 2, w, _).
...
test(all_reachable_obstacle_seed_fails, [fail]) :-
    g3x3_wall_mid(G),
    gpa_all_reachable(G, 0, 1, w, _).

% --- gpa_distance_map tests ---

% 1x1 grid: map has one entry (0,0)=0.
test(distance_map_1x1) :-
    g1x1(G),
    gpa_distance_map(G, 0, 0, w, Map),
    Map = [0-0-0].
...
test(between_v_adjacent) :-
    gpa_between_v(0, 1, 2, Cells),
    Cells = [].

% One between.
test(between_v_one) :-
    gpa_between_v(0, 2, 1, Cells),
    Cells = [1-1].

% Three between.
test(between_v_three) :-
    gpa_between_v(0, 4, 2, Cells),
    Cells = [1-2, 2-2, 3-2].

% --- gpa_region_path tests ---

% r at (0,0), b at (3,3) in open 4x4: path exists.
test(region_path_exists) :-
    g4x4_two_colors(G),
    gpa_region_path(G, r, b, w, Path),
    Path = [0-0|_],
    last(Path, 3-3).
