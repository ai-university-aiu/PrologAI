...
test(reflect_h_single_row) :-
    sy_reflect_h([[1,2,3]], [[3,2,1]]).

test(reflect_h_1x1) :-
    sy_reflect_h([[7]], [[7]]).

:- end_tests(sy_reflect_h).

:- begin_tests(sy_reflect_v).
...
:- end_tests(sy_reflect_v).

:- begin_tests(sy_transpose).
...
test(transpose_1x3) :-
    sy_transpose([[1,2,3]], [[1],[2],[3]]).

test(transpose_empty) :-
    sy_transpose([], []).

:- end_tests(sy_transpose).

:- begin_tests(sy_rotate90).
...
:- end_tests(sy_rotate90).

:- begin_tests(sy_rotate180).
...
:- end_tests(sy_rotate180).

:- begin_tests(sy_rotate270).
...
:- end_tests(sy_rotate270).

:- begin_tests(sy_has_h_symm).

test(has_h_symm_yes) :-
    sy_has_h_symm([[1,2,1],[3,4,3]]).
...
:- end_tests(sy_has_h_symm).

:- begin_tests(sy_has_v_symm).

test(has_v_symm_yes) :-
    sy_has_v_symm([[1,2],[1,2]]).
...
:- end_tests(sy_has_v_symm).

:- begin_tests(sy_has_rot2_symm).

test(rot2_yes) :-
    % 1 2 / 2 1 rotated 180 = 1 2 / 2 1.
    sy_has_rot2_symm([[1,2],[2,1]]).
...
:- end_tests(sy_has_rot2_symm).

:- begin_tests(sy_has_rot4_symm).
...
:- end_tests(sy_has_rot4_symm).

:- begin_tests(sy_symmetries).

test(symms_all_same) :-
    % All-same grid has all 4 symmetries.
    all_same_grid(G),
    sy_symmetries(G, Symms),
    msort(Symms, Sorted),
