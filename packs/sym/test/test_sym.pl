...
test(reflect_h_single_row) :-
    sym_reflect_h([[1,2,3]], [[3,2,1]]).

test(reflect_h_1x1) :-
    sym_reflect_h([[7]], [[7]]).

:- end_tests(sym_reflect_h).

:- begin_tests(sym_reflect_v).
...
:- end_tests(sym_reflect_v).

:- begin_tests(sym_transpose).
...
test(transpose_1x3) :-
    sym_transpose([[1,2,3]], [[1],[2],[3]]).

test(transpose_empty) :-
    sym_transpose([], []).

:- end_tests(sym_transpose).

:- begin_tests(sym_rotate90).
...
:- end_tests(sym_rotate90).

:- begin_tests(sym_rotate180).
...
:- end_tests(sym_rotate180).

:- begin_tests(sym_rotate270).
...
:- end_tests(sym_rotate270).

:- begin_tests(sym_has_h_symm).

test(has_h_symm_yes) :-
    sym_has_h_symm([[1,2,1],[3,4,3]]).
...
:- end_tests(sym_has_h_symm).

:- begin_tests(sym_has_v_symm).

test(has_v_symm_yes) :-
    sym_has_v_symm([[1,2],[1,2]]).
...
:- end_tests(sym_has_v_symm).

:- begin_tests(sym_has_rot2_symm).

test(rot2_yes) :-
    % 1 2 / 2 1 rotated 180 = 1 2 / 2 1.
    sym_has_rot2_symm([[1,2],[2,1]]).
...
:- end_tests(sym_has_rot2_symm).

:- begin_tests(sym_has_rot4_symm).
...
:- end_tests(sym_has_rot4_symm).

:- begin_tests(sym_symmetries).

test(symms_all_same) :-
    % All-same grid has all 4 symmetries.
    all_same_grid(G),
    sym_symmetries(G, Symms),
    msort(Symms, Sorted),
