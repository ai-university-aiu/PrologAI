/*  PrologAI — Causalontology Clue Grounding Test Suite  (WP-417)
    ...
    Run with the full library path:
        swipl $LIB -g "run_tests, halt" -t "halt(1)" packs/co_ground/test/test_co_ground.pl
*/

% Declare this file as a test module.
:- module(test_co_ground, []).
% Load the PLUnit framework.
:- use_module(library(plunit)).
% Load the module under test.
:- use_module(library(co_ground)).

% Open the test block.
:- begin_tests(co_ground).

% "that looks like a key" grounds to a key-like continuant with dispositions.
test(ground_key_phrase) :-
    co_ground:gr_reset,
    co_ground:gr_ground('that looks like a key', obj7, A),
...
test(ground_word_list) :-
    co_ground:gr_reset,
    co_ground:gr_ground([that, is, a, lock], cell(3,4), A),
...
test(ground_door_goal) :-
    co_ground:gr_reset,
    co_ground:gr_ground('that looks like a door, let us walk through it', d1, A),
    assertion(memberchk(goal(traverse(d1)), A)).

% "pick that up" suggests a prioritized pickup action.
test(ground_pickup) :-
    co_ground:gr_reset,
    co_ground:gr_ground('pick that up', k9, A),
...
test(ground_preventive) :-
    co_ground:gr_reset,
    co_ground:gr_ground('do not touch that, it hurts', spike, A),
...
test(ground_reinforce) :-
    co_ground:gr_reset,
    co_ground:gr_ground('good, that worked', _, A),
    assertion(memberchk(reinforce(recent, positive), A)).

% An unrelated clue grounds to nothing, and is not a clue.
test(no_grounding) :-
    co_ground:gr_reset,
    co_ground:gr_ground('the weather is nice', x, A),
    assertion(A == []),
    assertion(\+ co_ground:gr_is_clue('the weather is nice')).

% A caller-added rule wins over the built-ins.
test(custom_rule) :-
    co_ground:gr_reset,
    co_ground:gr_rule_add(gem, R, [continuant(R, treasure), goal(collect(R))]),
    co_ground:gr_ground([grab, the, gem], g1, A),
