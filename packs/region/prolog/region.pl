...
% region_spans_from_seps_: compute non-separator spans from a sorted separator index list.
% Spans is a list of R0-R1 inclusive bounds for each section in [Lo..MaxIdx].
region_spans_from_seps_([], Lo, MaxIdx, Spans) :-
% Base: if Lo is within bounds, one final span covers Lo to MaxIdx.
    (Lo =< MaxIdx -> Spans = [Lo-MaxIdx] ; Spans = []).
region_spans_from_seps_([S|Rest], Lo, MaxIdx, Spans) :-
...
% Recursively find spans for the tail separators.
    region_spans_from_seps_(Rest, NextLo, MaxIdx, RestSpans),
...
% region_find_span_: find the span (S0-S1) in Spans that contains Idx.
% Uses first-match with cut; fails if Idx is in a separator position.
region_find_span_([S0-S1|_], Idx, S0-S1) :- Idx >= S0, Idx =< S1, !.
region_find_span_([_|Rest], Idx, Span) :-
% Continue searching the rest of the spans.
    region_find_span_(Rest, Idx, Span).

% region_extract_subgrid_: extract rows R0..R1 and cols C0..C1 inclusive from Grid.
region_extract_subgrid_(Grid, R0, R1, C0, C1, Region) :-
...
% region_is_sep_row(+Grid, +R, +Sep): succeed if every cell in row R equals Sep.
region_is_sep_row(Grid, R, Sep) :-
% Extract the specified row.
    nth0(R, Grid, Row),
% Every element in the row must equal Sep.
    maplist(=(Sep), Row).

% region_is_sep_col(+Grid, +C, +Sep): succeed if every cell in column C equals Sep.
region_is_sep_col(Grid, C, Sep) :-
% For every row, the Cth cell must equal Sep.
    maplist([Row]>>(nth0(C, Row, Sep)), Grid).

% region_sep_rows(+Grid, +Sep, -IdxList): sorted list of row indices where every cell equals Sep.
region_sep_rows(Grid, Sep, IdxList) :-
% Count grid rows to build the index range.
    length(Grid, NR),
    NR1 is NR - 1,
    numlist(0, NR1, AllRows),
% Keep only rows that are entirely Sep.
    include([R]>>(region_is_sep_row(Grid, R, Sep)), AllRows, IdxList).

% region_sep_cols(+Grid, +Sep, -IdxList): sorted list of column indices where every cell equals Sep.
region_sep_cols(Grid, Sep, IdxList) :-
...
% Keep only columns that are entirely Sep.
    include([C]>>(region_is_sep_col(Grid, C, Sep)), AllCols, IdxList).

% region_spans_h(+Grid, +Sep, -Spans): row spans of non-separator horizontal sections.
% Spans is a list of R0-R1 inclusive pairs, one per section, in order.
region_spans_h(Grid, Sep, Spans) :-
% Get the maximum row index.
    length(Grid, NR),
    NR1 is NR - 1,
% Find all separator rows.
    region_sep_rows(Grid, Sep, SepRows),
% Compute the non-separator spans.
    region_spans_from_seps_(SepRows, 0, NR1, Spans).

% region_spans_v(+Grid, +Sep, -Spans): column spans of non-separator vertical sections.
region_spans_v(Grid, Sep, Spans) :-
% Get the maximum column index from the first row.
    Grid = [FirstRow|_],
    length(FirstRow, NC),
    NC1 is NC - 1,
% Find all separator columns.
    region_sep_cols(Grid, Sep, SepCols),
% Compute the non-separator spans.
    region_spans_from_seps_(SepCols, 0, NC1, Spans).

% region_cut_h(+Grid, +Sep, -Parts): split Grid at separator rows.
% Parts is a list of sub-grids, one per horizontal section. Separator rows are excluded.
region_cut_h(Grid, Sep, Parts) :-
...
% region_cut_v(+Grid, +Sep, -Parts): split Grid at separator columns.
% Parts is a list of sub-grids, one per vertical section. Separator columns are excluded.
region_cut_v(Grid, Sep, Parts) :-
...
% region_sections(+Grid, +Sep, -Matrix): split Grid at both separator rows and separator columns.
% Matrix is a list of rows of sub-grids: Matrix[I][J] is the sub-grid at row-section I, col-section J.
region_sections(Grid, Sep, Matrix) :-
% First cut horizontally to get horizontal strips.
    region_cut_h(Grid, Sep, HStrips),
% Then cut each strip vertically to get the column sections.
    maplist([Strip, VSections]>>(region_cut_v(Strip, Sep, VSections)), HStrips, Matrix).

% region_section_h(+Grid, +Sep, +N, -Section): N-th horizontal section (1-indexed).
% Fails if N is out of range.
region_section_h(Grid, Sep, N, Section) :-
...
% region_section_v(+Grid, +Sep, +N, -Section): N-th vertical section (1-indexed).
region_section_v(Grid, Sep, N, Section) :-
...
% region_count_h(+Grid, +Sep, -N): number of horizontal sections in Grid.
region_count_h(Grid, Sep, N) :-
% Compute spans; count equals number of spans.
    region_spans_h(Grid, Sep, Spans),
    length(Spans, N).

% region_count_v(+Grid, +Sep, -N): number of vertical sections in Grid.
region_count_v(Grid, Sep, N) :-
% Compute spans; count equals number of spans.
    region_spans_v(Grid, Sep, Spans),
    length(Spans, N).

% region_region(+Grid, +Sep, +R, +C, -Region): sub-grid of the section containing cell (R,C).
...
region_region(Grid, Sep, R, C, Region) :-
% Find all horizontal section spans.
    region_spans_h(Grid, Sep, HSpans),
% Find all vertical section spans.
    region_spans_v(Grid, Sep, VSpans),
% Find which horizontal span contains row R.
    region_find_span_(HSpans, R, R0-R1),
% Find which vertical span contains column C.
    region_find_span_(VSpans, C, C0-C1),
