...
test(mark_cells_list) :-
    bg(G), gmk_mark_cells(G, [0-0, 1-1, 2-2, 3-3], m, M),
...
test(mark_rect_4x4) :-
    bg(G), gmk_mark_rect(G, 1, 1, 2, 2, m, M),
...
test(mark_rect_full) :-
    bg(G), gmk_mark_rect(G, 0, 0, 3, 3, m, M),
...
test(mark_diagonal_1x1) :-
    g11(G), gmk_mark_diagonal(G, m, M),
    M = [[m]].

% --- gmk_mark_anti_diagonal ---

% AC-GMK-022: anti-diagonal of 4x4 bg marks (0,3),(1,2),(2,1),(3,0)
test(mark_anti_diagonal_4x4) :-
    bg(G), gmk_mark_anti_diagonal(G, m, M),
...
test(combined_mark_row_count) :-
    bg(G), gmk_mark_row(G, 0, m, M),
    gmk_marked_cells(M, m, Cs),
    length(Cs, 4).

% AC-GMK-040: mark_col then is_marked at all column positions
test(combined_mark_col_check) :-
    bg(G), gmk_mark_col(G, 1, m, M),
    gmk_is_marked(M, 0, 1, m),
    gmk_is_marked(M, 3, 1, m).
...
test(combined_rect_count) :-
    bg(G), gmk_mark_rect(G, 0, 0, 1, 1, m, M),
    gmk_marked_cells(M, m, Cs),
    length(Cs, 4).
