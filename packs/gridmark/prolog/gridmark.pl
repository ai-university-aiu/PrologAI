:- module(gridmark, [
    gridmark_mark_cell/5,
    gridmark_mark_cells/4,
    gridmark_mark_row/4,
    gridmark_mark_col/4,
    gridmark_mark_rect/7,
    gridmark_mark_border/3,
    gridmark_mark_diagonal/3,
    gridmark_mark_anti_diagonal/3,
    gridmark_mark_corners/3,
    gridmark_mark_cross/3,
    gridmark_marked_cells/3,
    gridmark_is_marked/4,
    gridmark_mark_checkerboard/3,
    gridmark_erase/4
]).
...
gridmark_mark_row_(GRow, ColSet, MarkColor, NewRow) :-
...
gridmark_mark_positions_(Grid, PairSet, MarkColor, Marked) :-
    gridmark_dims_(Grid, H, W),
    H1 is H - 1, W1 is W - 1,
    findall(NewRow,
        (between(0, H1, R),
...
% gridmark_mark_cell(+Grid, +R, +C, +MarkColor, -Marked)
% Marked is Grid with the cell at (R, C) replaced by MarkColor.
gridmark_mark_cell(Grid, R, C, MarkColor, Marked) :-
    gridmark_mark_positions_(Grid, [R-C], MarkColor, Marked).

% gridmark_mark_cells(+Grid, +Cells, +MarkColor, -Marked)
% Marked is Grid with every cell in Cells (a list of R-C pairs) replaced by MarkColor.
gridmark_mark_cells(Grid, Cells, MarkColor, Marked) :-
    gridmark_mark_positions_(Grid, Cells, MarkColor, Marked).

% gridmark_mark_row(+Grid, +R, +MarkColor, -Marked)
% Marked is Grid with every cell in row R replaced by MarkColor.
gridmark_mark_row(Grid, R, MarkColor, Marked) :-
    gridmark_dims_(Grid, H, _),
    H1 is H - 1,
    findall(NewRow,
        (between(0, H1, Row),
         nth0(Row, Grid, GRow),
...
% gridmark_mark_col(+Grid, +C, +MarkColor, -Marked)
...
% gridmark_mark_rect(+Grid, +R0, +C0, +R1, +C1, +MarkColor, -Marked)
...
% gridmark_mark_border(+Grid, +MarkColor, -Marked)
...
% Mark top row, then bottom row, then left and right columns.
    gridmark_mark_rect(Grid, 0, 0, 0, W1, MarkColor, M1),
    gridmark_mark_rect(M1, H1, 0, H1, W1, MarkColor, M2),
    gridmark_mark_col(M2, 0, MarkColor, M3),
    gridmark_mark_col(M3, W1, MarkColor, Marked).

% gridmark_mark_diagonal(+Grid, +MarkColor, -Marked)
...
gridmark_mark_diagonal(Grid, MarkColor, Marked) :-
    ...
    gridmark_mark_positions_(Grid, Pairs, MarkColor, Marked).

% gridmark_mark_anti_diagonal(+Grid, +MarkColor, -Marked)
...
% gridmark_mark_corners(+Grid, +MarkColor, -Marked)
...
% gridmark_mark_cross(+Grid, +MarkColor, -Marked)
...
gridmark_mark_cross(Grid, MarkColor, Marked) :-
    ...
    gridmark_mark_row(Grid, CenterR, MarkColor, M1),
    gridmark_mark_col(M1, CenterC, MarkColor, Marked).

% gridmark_marked_cells(+Grid, +MarkColor, -Cells)
...
% gridmark_is_marked(+Grid, +R, +C, +MarkColor)
% Succeeds if cell (R, C) in Grid has color MarkColor.
gridmark_is_marked(Grid, R, C, MarkColor) :-
    nth0(R, Grid, Row),
    nth0(C, Row, MarkColor).

% gridmark_mark_checkerboard(+Grid, +MarkColor, -Marked)
