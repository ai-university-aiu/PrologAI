/*  PrologAI — RuVector HTTP Backend Unit Tests (PR 2, update)
    ...
    Shadow store tests (AC-RV-008 through AC-RV-012) access vbr_shadow/5
    with the module qualifier backend_ruvector:vbr_shadow/5 because that
    ...
    Acceptance criteria:
        ...
        AC-RV-008: vbr_insert/4 records a shadow entry in vbr_shadow/5
        AC-RV-009: vbr_delete/2 removes the shadow entry for that Id
        AC-RV-010: vbr_close/1 removes all shadow entries for the collection
        AC-RV-011: vbr_rebuild/1 succeeds gracefully when no shadows exist
        AC-RV-012: vbr_shadow_count/2 returns the correct count after inserts
*/
...
test(insert_no_crash,
     [setup(retractall(backend_ruvector:vbr_shadow(shadow_coll_2,_,_,_,_))),
      cleanup(retractall(backend_ruvector:vbr_shadow(shadow_coll_2,_,_,_,_)))]) :-
...
test(close_no_crash,
     [setup(retractall(backend_ruvector:vbr_shadow(close_coll_6,_,_,_,_)))]) :-
    % Create a local Ref term pointing at a non-existent server.
...
% AC-RV-008: vbr_insert/4 must record a vbr_shadow/5 entry after a call.
test(insert_records_shadow,
     [setup(retractall(backend_ruvector:vbr_shadow(shadow_coll_8,_,_,_,_))),
      cleanup(retractall(backend_ruvector:vbr_shadow(shadow_coll_8,_,_,_,_)))]) :-
    % Create a Ref pointing at a port that will refuse connections.
    ...
    vbr_insert(Ref, sv_1, [1.0, 2.0, 3.0, 4.0], meta(shadow)),
    % Verify that a shadow entry now exists for this collection and Id.
    backend_ruvector:vbr_shadow(shadow_coll_8, 'http://localhost:19999', sv_1, _, _).

% AC-RV-009: vbr_delete/2 must remove the shadow entry for the deleted Id.
test(delete_removes_shadow,
     [setup((retractall(backend_ruvector:vbr_shadow(shadow_coll_9,_,_,_,_)),
             assertz(backend_ruvector:vbr_shadow(shadow_coll_9,'http://localhost:19999',sv_del,[1.0],m)))),
      cleanup(retractall(backend_ruvector:vbr_shadow(shadow_coll_9,_,_,_,_)))]) :-
    % Create a Ref for the test collection.
    Ref = rv_ref(shadow_coll_9, 'http://localhost:19999'),
    % Verify the shadow exists before delete.
    backend_ruvector:vbr_shadow(shadow_coll_9, 'http://localhost:19999', sv_del, _, _),
    % Call delete to remove the shadow entry.
    vbr_delete(Ref, sv_del),
    % Verify no shadow remains for this Id.
    \+ backend_ruvector:vbr_shadow(shadow_coll_9, 'http://localhost:19999', sv_del, _, _).

% AC-RV-010: vbr_close/1 must remove ALL shadow entries for the collection.
test(close_clears_shadows,
     [setup((retractall(backend_ruvector:vbr_shadow(shadow_coll_10,_,_,_,_)),
             assertz(backend_ruvector:vbr_shadow(shadow_coll_10,'http://localhost:19999',a,[1.0],m)),
             assertz(backend_ruvector:vbr_shadow(shadow_coll_10,'http://localhost:19999',b,[2.0],m)))),
      cleanup(retractall(backend_ruvector:vbr_shadow(shadow_coll_10,_,_,_,_)))]) :-
    % Create a Ref for the test collection.
    Ref = rv_ref(shadow_coll_10, 'http://localhost:19999'),
    % Verify two shadows exist before close.
    vbr_shadow_count(Ref, CountBefore),
    CountBefore =:= 2,
    % Call close to drop the collection and clear all shadows.
    vbr_close(Ref),
    % Verify no shadows remain after close.
    vbr_shadow_count(Ref, CountAfter),
    CountAfter =:= 0.

% AC-RV-011: vbr_rebuild/1 must succeed gracefully when no shadows exist.
test(rebuild_no_shadows,
     [setup(retractall(backend_ruvector:vbr_shadow(empty_coll_11,_,_,_,_)))]) :-
    % Create a Ref pointing at a port that will refuse connections.
    Ref = rv_ref(empty_coll_11, 'http://localhost:19999'),
    % Call rebuild on a collection with no shadows; expect graceful success.
    vbr_rebuild(Ref).

% AC-RV-012: vbr_shadow_count/2 must return the correct count after inserts.
test(shadow_count_correct,
     [setup(retractall(backend_ruvector:vbr_shadow(count_coll_12,_,_,_,_))),
      cleanup(retractall(backend_ruvector:vbr_shadow(count_coll_12,_,_,_,_)))]) :-
    % Create a Ref for the count test collection.
    Ref = rv_ref(count_coll_12, 'http://localhost:19999'),
    % Verify count starts at zero.
    vbr_shadow_count(Ref, C0), C0 =:= 0,
    % Insert three vectors (HTTP will fail silently; shadows must be recorded).
    vbr_insert(Ref, c1, [1.0, 0.0], m1),
    vbr_insert(Ref, c2, [0.0, 1.0], m2),
    vbr_insert(Ref, c3, [0.5, 0.5], m3),
    % Verify the count is now three.
    vbr_shadow_count(Ref, C3), C3 =:= 3.
