...
:- module(cooccur, [
    cooccur_h_pairs/2,
    cooccur_v_pairs/2,
    cooccur_d_pairs_dr/2,
    cooccur_d_pairs_dl/2,
    cooccur_count_h/4,
    cooccur_count_v/4,
    cooccur_count_adj4/4,
    cooccur_always_adj4/3,
    cooccur_never_adj4/3,
    cooccur_isolated4/3,
    cooccur_border_vals/3,
    cooccur_shared_border/3,
    cooccur_row_transitions/2,
    cooccur_most_common_adj4/3
]).
...
% cooccur_h_pairs(+Grid, -Pairs): Pairs is the list of all ordered horizontal adjacent
...
cooccur_h_pairs(Grid, Pairs) :-
% For each row collect consecutive adjacent pairs via cooccur_row_adj_.
    findall(A-B, (member(Row, Grid), cooccur_row_adj_(Row, A, B)), Pairs).

% cooccur_v_pairs(+Grid, -Pairs): Pairs is the list of all ordered vertical adjacent
...
cooccur_v_pairs(Grid, Pairs) :-
% Determine column count from the first row; default 0 for empty grid.
    (Grid = [Fr | _] -> length(Fr, W) ; W = 0),
% Highest valid column index.
    W1 is W - 1,
% For each column, collect pairs from adjacent rows via cooccur_col_adj_.
    findall(A-B, (between(0, W1, C), cooccur_col_adj_(Grid, C, A, B)), Pairs).

% cooccur_d_pairs_dr(+Grid, -Pairs): Pairs is the ordered list of diagonal adjacent
% value pairs A-B going down-right: A at (R,C), B at (R+1,C+1).
cooccur_d_pairs_dr(Grid, Pairs) :-
...
% cooccur_d_pairs_dl(+Grid, -Pairs): Pairs is the ordered list of diagonal adjacent
% value pairs A-B going down-left: A at (R,C), B at (R+1,C-1).
cooccur_d_pairs_dl(Grid, Pairs) :-
% Determine grid bounds.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr | _] -> length(Fr, W) ; W = 0), W1 is W - 1,
...
% cooccur_count_h(+Grid, +V1, +V2, -N): N is the count of directed horizontal pairs
% where value V1 appears immediately to the left of value V2.
cooccur_count_h(Grid, V1, V2, N) :-
% Collect all horizontal pairs then filter to those matching V1-V2.
    cooccur_h_pairs(Grid, Pairs),
    include(=(V1-V2), Pairs, M),
    length(M, N).

% cooccur_count_v(+Grid, +V1, +V2, -N): N is the count of directed vertical pairs
% where value V1 appears immediately above value V2.
cooccur_count_v(Grid, V1, V2, N) :-
% Collect all vertical pairs then filter to those matching V1-V2.
    cooccur_v_pairs(Grid, Pairs),
    include(=(V1-V2), Pairs, M),
    length(M, N).

% cooccur_count_adj4(+Grid, +V1, +V2, -N): N is the total count of undirected
...
cooccur_count_adj4(Grid, V1, V2, N) :-
% Collect all horizontal and vertical ordered pairs.
    cooccur_h_pairs(Grid, H),
    cooccur_v_pairs(Grid, V),
    append(H, V, All),
...
% cooccur_always_adj4(+Grid, +V1, +V2): succeeds if every cell with value V1 has
% at least one 4-adjacent cell with value V2. Fails if Grid has no V1 cells.
cooccur_always_adj4(Grid, V1, V2) :-
% Fails if any V1 cell has no V2 among its 4-neighbors.
    \+ cooccur_has_nonadj4_(Grid, V1, V2).

% cooccur_never_adj4(+Grid, +V1, +V2): succeeds if no cell with value V1 is
% 4-adjacent to any cell with value V2.
cooccur_never_adj4(Grid, V1, V2) :-
% Fails if any V1-V2 shared edge exists.
    \+ cooccur_shared_border(Grid, V1, V2).

% cooccur_isolated4(+Grid, +V, -Cells): Cells is the sorted R-C list of positions
% with value V that have no 4-adjacent cell with the same value V.
cooccur_isolated4(Grid, V, Cells) :-
% Determine grid bounds.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr | _] -> length(Fr, W) ; W = 0), W1 is W - 1,
...
% cooccur_border_vals(+Grid, +V, -Vals): Vals is the sorted list of distinct values
% W (where W \= V) that appear in at least one 4-adjacent position to a V cell.
cooccur_border_vals(Grid, V, Vals) :-
% Determine grid bounds.
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr | _] -> length(Fr, W) ; W = 0), W1 is W - 1,
...
% cooccur_shared_border(+Grid, +V1, +V2): succeeds if some cell with value V1 is
...
% cooccur_most_common_adj4(+Grid, +V, -W): W is the value most frequently 4-adjacent
...
cooccur_most_common_adj4(Grid, V, W) :-
% Get distinct values that border V.
    cooccur_border_vals(Grid, V, BVals),
% Fail if no other value borders V.
    BVals \= [],
% Count undirected adjacency of each border value.
    findall(N-X, (member(X, BVals), cooccur_count_adj4(Grid, V, X, N)), Counts),
% Sort ascending; last element has highest count (and highest value on tie).
    msort(Counts, Sorted),
    last(Sorted, _-W).

% cooccur_row_adj_(+Row, -A, -B): A and B are consecutive elements of Row.
% Backtracks over all adjacent pairs in left-to-right order.
cooccur_row_adj_([A, B | _], A, B).
cooccur_row_adj_([_ | T], A, B) :-
% Require at least two more elements to prevent out-of-bounds.
    T = [_ | _],
    cooccur_row_adj_(T, A, B).

% cooccur_col_adj_(+Grid, +C, -A, -B): A is the value in column C of some row R,
% B is the value in column C of row R+1. Backtracks over adjacent row pairs.
cooccur_col_adj_([R1, R2 | _], C, A, B) :-
    nth0(C, R1, A),
    nth0(C, R2, B).
cooccur_col_adj_([_ | T], C, A, B) :-
% Require at least two more rows to prevent out-of-bounds.
    T = [_ | _],
    cooccur_col_adj_(T, C, A, B).

% cooccur_has_nonadj4_(+Grid, +V1, +V2): internal helper. Succeeds if there exists
% a V1 cell that has NO 4-adjacent cell with value V2. Used by cooccur_always_adj4.
cooccur_has_nonadj4_(Grid, V1, V2) :-
    length(Grid, H), H1 is H - 1,
    (Grid = [Fr | _] -> length(Fr, W) ; W = 0), W1 is W - 1,
    between(0, H1, R), between(0, W1, C),
