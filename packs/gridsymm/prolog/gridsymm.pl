:- module(gridsymm, [
    ...
    gridsymm_sym_d1/1,
    gridsymm_sym_d2/1,
    gridsymm_sym_rot90/1,
    gridsymm_sym_rot180/1,
    gridsymm_symmetries/2,
    gridsymm_complete_h/3,
    gridsymm_complete_v/3,
    gridsymm_complete_rot180/3,
    gridsymm_violations_h/2,
    gridsymm_violations_v/2,
    gridsymm_violations_rot180/2,
    gridsymm_score/3
]).
...
% Fail if any top-half row does not match its mirror.
    \+ (between(0, MaxR, R),
        between(0, W1, C),
        R2 is H1 - R,
        gridsymm_cell_(Grid, R, C, V1),
        gridsymm_cell_(Grid, R2, C, V2),
        V1 \= V2).
...
% Fail if any left-half column does not match its mirror.
    \+ (between(0, H1, R),
        ...
        C2 is W1 - C,
        gridsymm_cell_(Grid, R, C, V1),
        gridsymm_cell_(Grid, R, C2, V2),
        V1 \= V2).

% gridsymm_sym_d1(+Grid)
...
% Check only upper-triangle cells (R < C).
    \+ (between(0, H1, R),
        ...
        R < C,
        gridsymm_cell_(Grid, R, C, V1),
        gridsymm_cell_(Grid, C, R, V2),
        V1 \= V2).

% gridsymm_sym_d2(+Grid)
...
% Check only cells in the upper anti-triangle.
    \+ (between(0, H1, R),
        ...
        R2 is H1 - C,
        C2 is H1 - R,
        gridsymm_cell_(Grid, R2, C2, V2),
        V1 \= V2).

% gridsymm_sym_rot90(+Grid)
...
% Check that each cell matches its 90-CW-rotated counterpart.
    \+ (between(0, H1, R),
        ...
        gridsymm_cell_(Grid, R, C, V1),
        R2 is H1 - C,
        gridsymm_cell_(Grid, R2, R, V2),
        V1 \= V2).

% gridsymm_sym_rot180(+Grid)
...
% Check horizontal (top-bottom) symmetry.
    (gridsymm_sym_h(Grid) -> SH = [h] ; SH = []),
% Check vertical (left-right) symmetry.
    (gridsymm_sym_v(Grid) -> SV = [v] ; SV = []),
% Check diagonal symmetries only for square grids.
    (H =:= W ->
        (gridsymm_sym_d1(Grid) -> SD1 = [d1] ; SD1 = []),
        (gridsymm_sym_d2(Grid) -> SD2 = [d2] ; SD2 = []),
        (gridsymm_sym_rot90(Grid) -> SR90 = [rot90] ; SR90 = [])
    ;
        SD1 = [], SD2 = [], SR90 = []
    ),
% Check 180-rotation symmetry.
    (gridsymm_sym_rot180(Grid) -> SR180 = [rot180] ; SR180 = []),
% Concatenate all present symmetries.
    append([SH, SV, SD1, SD2, SR90, SR180], Syms).

% --- SYMMETRY COMPLETION ---

% gridsymm_complete_h(+Grid, +Dir, -Result)
...
% Build result using min/max row index depending on direction.
    gridsymm_build_(H, W,
        [R, C, V]>>(R2 is H1 - R,
                    ...
                    gridsymm_cell_(Grid, OR, C, V)),
        Result).

% gridsymm_complete_v(+Grid, +Dir, -Result)
...
% Build result using min/max column index depending on direction.
    gridsymm_build_(H, W,
        [R, C, V]>>(C2 is W1 - C,
                    ...
                    gridsymm_cell_(Grid, R, OC, V)),
        Result).

% gridsymm_complete_rot180(+Grid, +BgColor, -Result)
...
% For each cell: keep non-BgColor; for BgColor cells, use rotational mirror.
    gridsymm_build_(H, W,
        [R, C, V]>>(gridsymm_cell_(Grid, R, C, GV),
                    (GV \= BgColor ->
                        V = GV
                    ;
                        R2 is H1 - R,
                        C2 is W1 - C,
                        gridsymm_cell_(Grid, R2, C2, MV),
                        (MV \= BgColor -> V = MV ; V = BgColor))),
        Result).

% --- VIOLATION DETECTION ---

% gridsymm_violations_h(+Grid, -Cells)
...
% gridsymm_violations_v(+Grid, -Cells)
...
% gridsymm_violations_rot180(+Grid, -Cells)
...
% gridsymm_score(+Grid, +Type, -Score)
