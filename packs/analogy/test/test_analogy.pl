/*  PrologAI — Analogy Pack Test Suite  (PR 57)

    Acceptance tests for all ay_* predicates.

    Run with:
        swipl -g "run_tests, halt" test_analogy.pl
*/

% Load the PLUnit test framework.
:- use_module(library(plunit)).

% Load the grid pack (needed for gd_* calls in test bodies).
:- use_module(library(grid)).

% Load the module under test.
:- use_module('../prolog/analogy').
...
test(isometry_identity, nondet) :-
    ...
    ay_shape_isometry(Cells, identity, Norm),
...
test(isometry_rot90_4x, nondet) :-
    ...
    ay_normalize_shape(Cells, N0),
    ay_shape_isometry(N0, rot90, N1),
    ay_shape_isometry(N1, rot90, N2),
    ay_shape_isometry(N2, rot90, N3),
    ay_shape_isometry(N3, rot90, N4),
...
test(isometry_ref_h_twice, nondet) :-
    ...
    Cells = [r(0,0), r(0,1), r(1,0)],
    ay_normalize_shape(Cells, N0),
    ay_shape_isometry(N0, ref_h, N1),
    ay_shape_isometry(N1, ref_h, N2),
    N2 = N0.
...
test(isometry_ref_d1, nondet) :-
    ...
    Cells = [r(0,0), r(0,1), r(0,2)],
    ay_shape_isometry(Cells, ref_d1, Transposed),
...
test(isometry_candidates_rot90, nondet) :-
    ...
    ay_shape_isometry(S1, rot90, S2),
    % The candidates should include rot90.
    ay_isometry_candidates(S1, S2, Isos),
    member(rot90, Isos).
...
test(match_objects_shape, nondet) :-
    ...
    Out2 = [r(3,3), r(4,3)],
    % Find matches.
    ay_match_objects([In1, In2], [Out1, Out2], Pairs),
...
test(object_offset) :-
    ...
    Cells1 = [r(0,0), r(0,1)],
    Cells2 = [r(1,2), r(1,3)],
    ay_object_offset(Cells1, Cells2, offset(1,2)).
...
test(apply_color_map) :-
    ...
    G = [[1,2],[2,1]],
    % Apply map: 1->3, 2->4.
    ay_apply_color_map(G, [1-3, 2-4], G2),
    G2 = [[3,4],[4,3]].
...
test(solve_rot90, nondet) :-
    ...
    % Solve.
    ay_solve_from_examples(Train, GTest, Predicted),
