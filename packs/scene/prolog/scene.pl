...
% BACKGROUND COLOR
% sc_bg_color(+Grid, -BgColor): the most frequently occurring color in Grid.
...
% OBJECT EXTRACTION
% sc_objects(+Grid, -Objects): all foreground objects as obj(Color, Cells) terms.
...
% SCENE CONSTRUCTION
% sc_grid_to_scene(+Grid, -Scene): build scene(Rows, Cols, BgColor, Objects).
sc_grid_to_scene(Grid, scene(Rows, Cols, BgColor, Objects)) :-
% Get grid dimensions.
...
% sc_obj_color(+Obj, -Color): extract the color from an obj term.
sc_obj_color(obj(Color, _), Color).

% sc_obj_cells(+Obj, -Cells): extract the cell list from an obj term.
sc_obj_cells(obj(_, Cells), Cells).

% sc_obj_size(+Obj, -Size): number of cells in the object.
sc_obj_size(obj(_, Cells), Size) :-
% Count the cells.
    length(Cells, Size).

% sc_obj_bbox(+Obj, -TL, -BR): axis-aligned bounding box of the object.
sc_obj_bbox(obj(_, Cells), TL, BR) :-
% Delegate to grid pack bounding box predicate.
    gd_bounding_box(Cells, TL, BR).

% sc_obj_shape(+Obj, -Shape): normalized cell set (translated to (0,0), sorted).
sc_obj_shape(obj(_, Cells), Shape) :-
% Normalize the cell set to canonical position.
    sc_normalize_cells(Cells, Shape).

% sc_obj_centroid(+Obj, -R, -C): center of mass of the object, rounded.
...
% CELL NORMALIZATION
% sc_normalize_cells(+Cells, -Normalized): translate to (0,0) top-left, sort.
...
% FILTERING
% sc_objects_of_color(+Objects, +Color, -Filtered): keep only objects of Color.
...
% sc_sort_by_size(+Objects, +Order, -Sorted): Order is asc or desc.
...
% sc_sort_by_size with desc order: sort ascending then reverse.
sc_sort_by_size(Objects, desc, Sorted) :-
% Sort ascending first.
    sc_sort_by_size(Objects, asc, Asc),
...
% sc_largest(+Objects, -Largest): the object with the most cells.
...
% sc_smallest(+Objects, -Smallest): the object with the fewest cells.
sc_smallest(Objects, Smallest) :-
% Sort ascending, smallest is first.
    sc_sort_by_size(Objects, asc, [Smallest|_]).


% COUNTING
% sc_count(+Objects, -Count): total number of objects.
sc_count(Objects, Count) :-
% Count all elements in the list.
    length(Objects, Count).

% sc_count_of_color(+Objects, +Color, -Count): count objects with given color.
...
% Get top-left of Obj1.
    sc_obj_bbox(Obj1, r(R1, _), _),
% Get top-left of Obj2.
    sc_obj_bbox(Obj2, r(R2, _), _),
% Obj1 is above if its min row is strictly less.
    R1 < R2.

% sc_below(+Obj1, +Obj2): Obj1 is below Obj2 (greater row index).
sc_below(Obj1, Obj2) :-
% Obj1 is below if Obj2 is above Obj1.
    sc_above(Obj2, Obj1).

% sc_left_of(+Obj1, +Obj2): Obj1 is to the left of Obj2 (lower col index).
sc_left_of(Obj1, Obj2) :-
% Get top-left of Obj1.
    sc_obj_bbox(Obj1, r(_, C1), _),
% Get top-left of Obj2.
    sc_obj_bbox(Obj2, r(_, C2), _),
% Obj1 is left-of if its min col is strictly less.
    C1 < C2.

% sc_right_of(+Obj1, +Obj2): Obj1 is to the right of Obj2 (greater col index).
sc_right_of(Obj1, Obj2) :-
% Obj1 is right-of if Obj2 is left-of Obj1.
    sc_left_of(Obj2, Obj1).

% sc_cells_touching(+Obj1, +Obj2): any cell of Obj1 is 4-connected to any cell of Obj2.
sc_cells_touching(obj(_, Cells1), obj(_, Cells2)) :-
...
% sc_contained_in(+Obj1, +Obj2): bbox of Obj1 is inside bbox of Obj2.
sc_contained_in(Obj1, Obj2) :-
% Get bounding box of Obj1.
    sc_obj_bbox(Obj1, r(R1min, C1min), r(R1max, C1max)),
% Get bounding box of Obj2.
    sc_obj_bbox(Obj2, r(R2min, C2min), r(R2max, C2max)),
...
% sc_same_shape(+Obj1, +Obj2): true if both objects have equal normalized shapes.
sc_same_shape(Obj1, Obj2) :-
% Get normalized shape of Obj1.
    sc_obj_shape(Obj1, Shape1),
% Get normalized shape of Obj2.
    sc_obj_shape(Obj2, Shape2),
% Compare shapes for identity.
    Shape1 == Shape2.

% sc_same_color(+Obj1, +Obj2): true if both objects have the same color.
sc_same_color(obj(Color, _), obj(Color, _)).
