% State the fact: this pack is named regulation.
name(regulation).
% State the fact: this is version 1.0.0.
version('1.0.0').
% State the fact: the title carries the work-package number.
title('PrologAI Causalontology Regulation — refine a behaviour from feedback, classify each result into the four flavours (confirming, disappointing, serendipitous, shocking), and split a rule that both succeeds and fails by finding the discriminating feature (WP-423)').
