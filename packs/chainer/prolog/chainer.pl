/*  PrologAI — Generic Chainer and Meta-Reasoning  (Specification PR 34)
    ...
    Rule base:
        ...
        node_facts.  `chainer_rule_base/2` declares one from a list of
    ...
    Control policies:
        ...
        chainer_chain/5 accepts policy(Name) in Options and records outcomes.
        chainer_control_policy/3 queries stats and finds the best policy.

    Predicates:
        chainer_chain/5          — +Direction, +RuleBase, +Start, -Result, +Options
        chainer_rule_base/2      — +Name, +Rules
        chainer_control_policy/3 — +RuleBase, ?PolicyName, ?Stats
*/
...
chainer_chain(backward, RuleBase, Goal, Result, Options) :-
    ...
    chainer_b(RuleBase, Goal, MaxD, MaxS, 0, Steps, Chain),
...
% Define a clause for 'pai chain': succeed when the following conditions hold.
chainer_chain(forward, RuleBase, Start, Result, Options) :-
    % State a fact for 'option' with the arguments listed below.
    option(max_steps(MaxS), Options, 100),
    % State a fact for 'option' with the arguments listed below.
    option(policy(Policy), Options, default),
...
find_chain(RuleBase, Goal, D, MaxS, S0, Steps, Chain) :-
    ...
    % State a fact for 'dfs b' with the arguments listed below.
    chainer_b(RuleBase, Ante, D, MaxS, S0, S1, SubChain),
...
%
%   chainer_control_policy(+RuleBase, ?PolicyName, ?Stats)
