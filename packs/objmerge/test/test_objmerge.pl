...
test(union_overlap) :-
    ...
    objmerge_union_cells(B, B2, x, U),
    U = obj(x, Cells),
    length(Cells, 5).

test(union_self) :-
    dot(D),
    objmerge_union_cells(D, D, x, U),
    U = obj(x, [r(0,0)]).
...
test(intersect_overlap) :-
    ...
    objmerge_intersect_cells(B, B2, x, I),
    I = obj(x, [r(0,2)]).

test(intersect_disjoint) :-
    dot(D), dot_right(DR),
    objmerge_intersect_cells(D, DR, x, I),
    I = obj(x, []).

test(intersect_sq) :-
    sq(S), sq2(S2),
    % overlap at r(1,1) only
    objmerge_intersect_cells(S, S2, x, I),
    I = obj(x, [r(1,1)]).
...
test(diff_disjoint) :-
    dot(D1), dot_right(D2),
    objmerge_diff_cells(D1, D2, x, Diff),
    Diff = obj(x, [r(0,0)]).

test(diff_same) :-
    dot(D),
    objmerge_diff_cells(D, D, x, Diff),
    Diff = obj(x, []).
...
test(sym_diff_partial) :-
    ...
    objmerge_sym_diff_cells(B, B2, x, S),
    S = obj(x, Cells),
    length(Cells, 4).

test(sym_diff_disjoint) :-
    dot(D1), dot_right(D2),
    objmerge_sym_diff_cells(D1, D2, x, S),
    S = obj(x, Cells),
    length(Cells, 2).

test(sym_diff_same) :-
    dot(D),
    objmerge_sym_diff_cells(D, D, x, S),
    S = obj(x, []).
...
test(concat_h_gap0) :-
    ...
    objmerge_concat_h(D, DB, 0, obj(r, Cells)),
    msort(Cells, SC),
    SC == [r(0,0),r(1,1)].
...
test(concat_h_gap1) :-
    ...
    objmerge_concat_h(D, DR, 1, obj(r, Cells)),
    msort(Cells, SC),
    SC == [r(0,0),r(0,2)].
...
test(concat_h_bars) :-
    ...
    objmerge_concat_h(B1, B2, 0, obj(r, Cells)),
    length(Cells, 6),
    member(r(0,3), Cells),
    member(r(0,5), Cells).
...
test(concat_v_gap0) :-
    ...
    objmerge_concat_v(D, DR, 0, obj(r, Cells)),
    msort(Cells, SC),
    SC == [r(0,0),r(1,1)].
...
test(concat_v_bars) :-
    ...
    objmerge_concat_v(V1, V2, 0, obj(g, Cells)),
    length(Cells, 6),
    member(r(3,0), Cells),
    member(r(5,0), Cells).
...
test(merge_list_basic) :-
    ...
    objmerge_merge_list([D, DR, DB], x, M),
    M = obj(x, Cells),
    length(Cells, 3).
...
test(merge_list_with_overlap) :-
    ...
    objmerge_merge_list([D, B], x, M),
    M = obj(x, Cells),
    length(Cells, 3).

test(merge_list_single) :-
    dot(D),
    objmerge_merge_list([D], x, M),
    M = obj(x, [r(0,0)]).
...
test(subtract_list_basic) :-
    ...
    objmerge_subtract_list(B, [D], x, R),
    R = obj(x, Cells),
    length(Cells, 2),
    \+ member(r(0,0), Cells).
...
test(expand_bbox_sq) :-
    ...
    objmerge_expand_bbox(S, Expanded),
...
test(expand_bbox_lshape) :-
    ...
    objmerge_expand_bbox(L, Expanded),
...
test(n_components4_one) :-
    bar_h(B),
    objmerge_n_components4(B, N),
    N == 1.

test(n_components4_two) :-
    disconnected(D),
    objmerge_n_components4(D, N),
    N == 2.

test(n_components4_sq) :-
    sq(S),
    objmerge_n_components4(S, N),
    N == 1.
...
test(union_three_separate) :-
    dot(D), dot_right(DR), dot_below(DB),
    objmerge_union_cells(D, DR, x, U1),
    objmerge_union_cells(U1, DB, x, U2),
    U2 = obj(x, Cells),
    length(Cells, 3).
