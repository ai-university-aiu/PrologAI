...
test(left_right_symmetry) :-
    g2(G), gsn_dist_right(G, 0, 3, b, DR), gsn_dist_left(G, 0, 3, b, DL),
    DR =:= 2, DL =:= 1.

% AC-GSN-040: scan_row and scan_col count check.
test(scan_count) :-
    g2(G), gsn_scan_row(G, 0, b, R), length(R, NR), NR =:= 2,
    gsn_scan_col(G, 6, b, C), length(C, NC), NC =:= 1.

% AC-GSN-041: dist_down then dist_up from between two x cells.
test(dist_down_up) :-
    g1(G), gsn_dist_down(G, 1, 2, b, DD), gsn_dist_up(G, 4, 2, b, DU),
    DD =:= 2, DU =:= 1.

% AC-GSN-042: blocked checks match first_* existence.
test(blocked_vs_first) :-
    g1(G),
    gsn_blocked_right(G, 2, 0, b),
    gsn_first_right(G, 2, 0, b, 3-y).

% AC-GSN-043: all-bg grid: no hits in any direction.
test(all_bg_no_hits) :-
    g3(G),
    \+ gsn_blocked_right(G, 1, 0, b),
    \+ gsn_blocked_left(G, 1, 3, b),
    \+ gsn_blocked_down(G, 0, 1, b),
    \+ gsn_blocked_up(G, 3, 1, b).

% AC-GSN-044: scan_col g2 col 6 has one hit.
test(scan_col_g2_c6) :-
    g2(G), gsn_scan_col(G, 6, b, Pairs), Pairs = [2-x].
