...
% and columns.
:- module(permute, [
    permute_permute_rows/3,
    permute_permute_cols/3,
    permute_swap_rows/4,
    permute_swap_cols/4,
    permute_cycle_rows/3,
    permute_cycle_cols/3,
    permute_find_row_perm/3,
    permute_find_col_perm/3,
    permute_sort_rows/2,
    permute_sort_cols/2,
    permute_insert_row/4,
    permute_delete_row/3,
    permute_insert_col/4,
    permute_delete_col/3
]).
...
% permute_col_: extract column C from Grid as a list of values (top to bottom).
permute_col_(Grid, C, Col) :-
% For each row in Grid, take the Cth element.
    maplist([Row, V]>>(nth0(C, Row, V)), Grid, Col).

% permute_first_row_idx_: find the 0-based index of the first row in Grid
% that equals TargetRow. Fails if no matching row exists.
permute_first_row_idx_(Grid, TargetRow, I) :-
% nth0 will try indices in order; cut commits to the first match.
    nth0(I, Grid, TargetRow), !.

% permute_first_col_idx_: find the 0-based column index whose column list
% in Grid equals TargetCol. Fails if no matching column exists.
permute_first_col_idx_(Grid, TargetCol, I) :-
% Enumerate columns until the extracted column matches TargetCol.
    Grid = [FirstRow|_], length(FirstRow, NC), NC1 is NC - 1,
    between(0, NC1, I),
    permute_col_(Grid, I, TargetCol), !.

% permute_permute_rows(+Grid, +Perm, -Result): reorder rows of Grid according
% to index permutation Perm. Result[i] = Grid[Perm[i]].
permute_permute_rows(Grid, Perm, Result) :-
% For each index I in Perm, retrieve row I from Grid.
    maplist([I, Row]>>(nth0(I, Grid, Row)), Perm, Result).

% permute_permute_cols(+Grid, +Perm, -Result): reorder columns of Grid according
% to index permutation Perm. Result column j = Grid column Perm[j].
permute_permute_cols(Grid, Perm, Result) :-
% For each row, reorder its cells by Perm.
    maplist([GRow, ResRow]>>(
        maplist([C, Val]>>(nth0(C, GRow, Val)), Perm, ResRow)
    ), Grid, Result).

% permute_swap_rows(+Grid, +R1, +R2, -Result): exchange rows R1 and R2 in Grid.
permute_swap_rows(Grid, R1, R2, Result) :-
% Extract both rows.
    nth0(R1, Grid, RowA),
    nth0(R2, Grid, RowB),
...
% permute_swap_cols(+Grid, +C1, +C2, -Result): exchange columns C1 and C2.
permute_swap_cols(Grid, C1, C2, Result) :-
...
% permute_insert_row(+Grid, +R, +NewRow, -Result): insert NewRow before position R.
% Existing rows at R and beyond are shifted down by one.
permute_insert_row(Grid, R, NewRow, Result) :-
...
% permute_delete_col(+Grid, +C, -Result): remove column C from every row.
% Fails if C is out of range for any row.
permute_delete_col(Grid, C, Result) :-
