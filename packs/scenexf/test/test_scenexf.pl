...
test(recolor_basic) :-
    ...
    scenexf_recolor([R, B], r, g, [obj(g,[r(0,0)]), B]).

test(recolor_no_match) :-
    red_dot(R),
    scenexf_recolor([R], b, g, [R]).

test(recolor_all_match) :-
    red_dot(D), red_bar(B),
    scenexf_recolor([D, B], r, y, [obj(y,[r(0,0)]), obj(y,[r(0,0),r(0,1),r(0,2)])]).

% --- scenexf_recolor_all/3 ---

test(recolor_all_basic) :-
    red_dot(R), blue_bar(B),
    scenexf_recolor_all([R, B], g, [obj(g,[r(0,0)]), obj(g,[r(1,0),r(1,1),r(1,2)])]).

test(recolor_all_empty) :-
    scenexf_recolor_all([], g, []).

% --- scenexf_apply_color_map/3 ---

test(apply_color_map_basic) :-
    red_dot(R), blue_dot(B),
    scenexf_apply_color_map([R, B], [r-g, b-y], [obj(g,[r(0,0)]), obj(y,[r(0,1)])]).

test(apply_color_map_partial) :-
    red_dot(R), blue_dot(B),
    % only r->g in map; b unchanged
    scenexf_apply_color_map([R, B], [r-g], [obj(g,[r(0,0)]), B]).

test(apply_color_map_empty_map) :-
    red_dot(R), blue_dot(B),
    scenexf_apply_color_map([R, B], [], [R, B]).

% --- scenexf_shift/4 ---

test(shift_basic) :-
    red_dot(R),
    scenexf_shift([R], 2, 3, [obj(r,[r(2,3)])]).

test(shift_negative) :-
    lshape(L),
    % L is at r(0,0),r(1,0),r(1,1) — shift down by -0 is no-op; shift right by 0
    scenexf_shift([L], 0, 0, [L]).

test(shift_multiple) :-
    red_dot(R), blue_dot(B),
    scenexf_shift([R, B], 1, 1, [obj(r,[r(1,1)]), obj(b,[r(1,2)])]).

% --- scenexf_to_origin/2 ---

test(to_origin_basic) :-
    Shifted = obj(r, [r(3,4)]),
    scenexf_to_origin([Shifted], [obj(r,[r(0,0)])]).

test(to_origin_scene) :-
    % Scene with min row=1, min col=1
    A = obj(r, [r(1,1)]), B = obj(b, [r(2,3)]),
    scenexf_to_origin([A, B], [obj(r,[r(0,0)]), obj(b,[r(1,2)])]).

test(to_origin_already_at_origin) :-
    red_dot(R), blue_dot(B),
    scenexf_to_origin([R, B], [R, B]).
...
test(reflect_h_basic) :-
    ...
    red_dot(R),
    scenexf_reflect_h([R], 5, [obj(r,[r(0,4)])]).

test(reflect_h_bar) :-
    % Width=3; bar at cols 0,1,2 -> cols 2,1,0 (same cells sorted differently)
    red_bar(B),
    scenexf_reflect_h([B], 3, [obj(r,Cells)]),
    sort(Cells, Sorted),
...
test(top_n_basic) :-
    ...
    scenexf_top_n([D, B, BB], 2, TopN),
    length(TopN, 2),
    \+ member(D, TopN).

test(top_n_one) :-
    red_dot(D), red_bar(B),
    scenexf_top_n([D, B], 1, [B]).

test(top_n_all) :-
    red_dot(D), red_bar(B),
    scenexf_top_n([D, B], 5, TopN),
...
test(reflect_v_multiple) :-
    ...
    scenexf_reflect_v([A, B], 3, [obj(r,[r(2,0)]), obj(b,[r(0,1)])]).

test(apply_color_map_swap) :-
    % swap r<->b
    red_dot(R), blue_dot(B),
    scenexf_apply_color_map([R, B], [r-b, b-r], [obj(b,[r(0,0)]), obj(r,[r(0,1)])]).

test(top_n_exact) :-
    red_dot(D), red_bar(B),
    % exactly 2 objects, N=2 => return both sorted desc
    scenexf_top_n([D, B], 2, TopN),
    length(TopN, 2).

test(sort_pos_single) :-
    red_dot(R),
    scenexf_sort_pos([R], [R]).

test(recolor_all_single) :-
    red_dot(R),
    scenexf_recolor_all([R], b, [obj(b,[r(0,0)])]).
