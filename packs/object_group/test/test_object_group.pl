:- use_module('../prolog/object_group').
:- use_module(library(plunit)).
:- use_module(library(lists), [memberchk/2]).

:- begin_tests(object_group).
...
test(by_color_basic) :-
    ...
    object_group_by_color([R, B, RB], Groups),
    memberchk(b-[B], Groups),
    memberchk(r-[R,RB], Groups).

test(by_color_single) :-
    red_dot(R),
    object_group_by_color([R], Groups),
    Groups == [r-[R]].

test(by_color_empty) :-
    object_group_by_color([], Groups),
    Groups == [].

test(by_color_all_same) :-
    red_dot(D), red_bar(B), red_sq(S),
    object_group_by_color([D, B, S], Groups),
    Groups == [r-[D,B,S]].

test(by_color_three) :-
    red_dot(R), blue_dot(B), green_dot(G),
    object_group_by_color([R, B, G], Groups),
    length(Groups, 3),
...
test(by_size_basic) :-
    red_dot(D), red_bar(B), red_sq(S),
    object_group_by_size([D, B, S], Groups),
...
test(by_size_shared) :-
    ...
    object_group_by_size([D, BD, B, BB], Groups),
...
test(by_form_basic) :-
    ...
    object_group_by_form([L, LF, D, BD], Groups),
    % two distinct forms: L-shape and single-dot
    length(Groups, 2),
    % both groups have 2 members
    object_group_n_members(Groups, 2, TwoGroups),
    length(TwoGroups, 2).

test(by_form_single_dot) :-
    red_dot(D),
    object_group_by_form([D], [_-[D]]).
...
test(by_row_basic) :-
    ...
    object_group_by_row([D, BD, G], Groups),
...
test(by_row_all_same) :-
    ...
    object_group_by_row([D, BD], Groups),
    Groups == [0-[D,BD]].
...
test(by_row_three_rows) :-
    ...
    red_sq(S),    % row 0
    yellow_sq(YS), % row 2
    object_group_by_row([D, B, G, S, YS], Groups),
    length(Groups, 3),
    memberchk(0-Row0, Groups),
    length(Row0, 3).
...
test(by_col_basic) :-
    ...
    object_group_by_col([D, BD, G], Groups),
...
test(by_col_all_same) :-
    ...
    % both at col 0
    object_group_by_col([D, G], Groups),
    Groups == [0-[D,G]].
...
test(n_members_none) :-
    ...
    object_group_n_members(Groups, 5, Selected),
    Selected == [].

test(n_members_two) :-
    red_dot(R), blue_dot(B), red_bar(RB),
    object_group_by_color([R, B, RB], Groups),
    object_group_n_members(Groups, 2, Selected),
    length(Selected, 1),
    Selected = [r-_].
...
test(singletons_none) :-
    ...
    object_group_by_color([R, B, RB, BB], Groups),
...
test(largest_basic) :-
    red_dot(R), blue_dot(B), red_bar(RB), red_sq(S),
    object_group_by_color([R, B, RB, S], Groups),
    object_group_largest(Groups, Largest),
    Largest = r-_.
...
test(smallest_basic) :-
    red_dot(R), blue_dot(B), red_bar(RB),
    object_group_by_color([R, B, RB], Groups),
    object_group_smallest(Groups, Smallest),
    Smallest = b-_.
...
test(all_same_size_true) :-
    red_dot(R), blue_dot(B), green_dot(G),
    object_group_by_color([R, B, G], Groups),
    object_group_all_same_size(Groups).
...
test(filter_size_basic) :-
    red_dot(R), blue_dot(B), red_bar(RB), red_sq(S),
    object_group_by_color([R, B, RB, S], Groups),
    object_group_filter_size(Groups, 2, 4, Filtered),
    length(Filtered, 1),
    Filtered = [r-_].
...
test(filter_size_all) :-
    red_dot(R), blue_dot(B),
    object_group_by_color([R, B], Groups),
    object_group_filter_size(Groups, 1, 1, Filtered),
    length(Filtered, 2).

test(filter_size_none) :-
    red_dot(R), blue_dot(B),
    object_group_by_color([R, B], Groups),
    object_group_filter_size(Groups, 5, 10, Filtered),
    Filtered == [].
