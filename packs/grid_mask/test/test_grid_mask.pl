...
test(intersect_overlap) :-
    g3x3_tl(GA), g3x3_br(GB),
    grid_mask_intersect(GA, GB, x, g, R),
    R = [[x,x,x],[x,g,x],[x,x,x]].
...
% --- grid_mask_invert tests ---

% Invert tl: r cells become x, x cells become r.
test(invert_tl) :-
    g3x3_tl(G),
    grid_mask_invert(G, x, r, R),
    R = [[x,x,r],[x,x,r],[r,r,r]].
...
test(mask_apply_tl_mask) :-
    ...
    grid_mask_mask_apply(G, Mask, x, x, R),
    R = [[r,r,x],[r,r,x],[x,x,x]].
...
test(mask_apply_empty) :-
    ...
    grid_mask_mask_apply(G, Mask, x, b, R),
    R = [[b,b,b],[b,b,b],[b,b,b]].

% --- grid_mask_extract tests ---

% Extract cells [0-0, 0-1, 1-0] from g3x3_r.
test(extract_cells) :-
    g3x3_r(G),
    grid_mask_extract(G, [0-0,0-1,1-0], x, R),
    R = [[r,r,x],[r,x,x],[x,x,x]].

% Extract no cells: all bg.
test(extract_empty) :-
    g3x3_r(G),
    grid_mask_extract(G, [], x, R),
    R = [[x,x,x],[x,x,x],[x,x,x]].
...
test(extract_all) :-
    ...
    grid_mask_extract(G, Cells, x, R),
    R = G.

% --- grid_mask_stamp tests ---

% Stamp 2x2 [[s,s],[s,s]] at (0,0) on 3x3 all-r.
test(stamp_topleft) :-
    g3x3_r(G), stamp2x2(S),
    grid_mask_stamp(G, S, 0, 0, R),
    R = [[s,s,r],[s,s,r],[r,r,r]].

% Stamp at (1,1) on 3x3 all-r.
test(stamp_center) :-
    g3x3_r(G), stamp2x2(S),
    grid_mask_stamp(G, S, 1, 1, R),
    R = [[r,r,r],[r,s,s],[r,s,s]].

% Stamp partially outside (at (2,2) on 3x3): only top-left cell visible.
test(stamp_partial) :-
    g3x3_r(G), stamp2x2(S),
    grid_mask_stamp(G, S, 2, 2, R),
    R = [[r,r,r],[r,r,r],[r,r,s]].

% --- grid_mask_compare tests ---

% Same grids: empty diff.
test(compare_same) :-
    g3x3_r(G),
    grid_mask_compare(G, G, x, Diff),
    Diff = [].

% tl vs br: diff at every non-bg cell of either.
test(compare_tl_br) :-
    g3x3_tl(GA), g3x3_br(GB),
    grid_mask_compare(GA, GB, x, Diff),
    msort(Diff, S),
...
test(border_mask_n1) :-
    g3x3_r(G),
    grid_mask_border_mask(G, 1, Mask),
...
test(border_mask_4x4_n2) :-
    g4x4_open(G),
    grid_mask_border_mask(G, 2, Mask),
...
test(invert_then_union) :-
    g3x3_tl(G),
    grid_mask_invert(G, x, r, Inv),
    grid_mask_union(G, Inv, x, R),
    grid_mask_equal(R, [[r,r,r],[r,r,r],[r,r,r]]).

% Sub then stamp round-trip: extract sub-grid and stamp back gives original.
test(sub_then_stamp) :-
    g3x3_r(G),
    grid_mask_sub(G, 0, 0, 2, 2, Sub),
    grid_mask_stamp(G, Sub, 0, 0, R),
    grid_mask_equal(R, G).
