...
% object terms as produced by grid_object_all_objects/3.
:- use_module(library(lists), [member/2]).

% --- PRIVATE HELPERS ---

% grid_relation_cells_/2: extract the Cells list from an ob/3 term.
grid_relation_cells_(ob(_,Cells,_), Cells).

% grid_relation_bbox_/2: extract the BBox r0(R0,C0,R1,C1) from an ob/3 term.
grid_relation_bbox_(ob(_,_,BBox), BBox).

% grid_relation_manhattan_/4: Manhattan distance between two cells r(RA,CA) and r(RB,CB).
grid_relation_manhattan_(r(RA,CA), r(RB,CB), D) :-
    DR is abs(RA - RB), DC is abs(CA - CB), D is DR + DC.

% --- PUBLIC PREDICATES ---

% grid_relation_touching(+ObjA, +ObjB)
% Succeed if ObjA and ObjB share a 4-adjacent cell edge.
% Two cells are 4-adjacent if they differ by exactly 1 in exactly one coordinate.
grid_relation_touching(ObjA, ObjB) :-
    grid_relation_cells_(ObjA, CellsA),
    grid_relation_cells_(ObjB, CellsB),
...
grid_relation_min_distance(ObjA, ObjB, D) :-
    ...
    findall(Dist,
        ( member(CA, CellsA), member(CB, CellsB),
          grid_relation_manhattan_(CA, CB, Dist) ),
        Dists),
    Dists \= [],
    min_list(Dists, D).

% grid_relation_above(+ObjA, +ObjB)
% Succeed if ObjA is entirely above ObjB: ObjA's last row < ObjB's first row.
grid_relation_above(ObjA, ObjB) :-
    grid_relation_bbox_(ObjA, r0(_,_,R1A,_)),
    grid_relation_bbox_(ObjB, r0(R0B,_,_,_)),
    R1A < R0B.

% grid_relation_below(+ObjA, +ObjB)
% Succeed if ObjA is entirely below ObjB: ObjA's first row > ObjB's last row.
grid_relation_below(ObjA, ObjB) :-
    grid_relation_bbox_(ObjA, r0(R0A,_,_,_)),
    grid_relation_bbox_(ObjB, r0(_,_,R1B,_)),
    R0A > R1B.

% grid_relation_left_of(+ObjA, +ObjB)
% Succeed if ObjA is entirely left of ObjB: ObjA's last col < ObjB's first col.
grid_relation_left_of(ObjA, ObjB) :-
    grid_relation_bbox_(ObjA, r0(_,_,_,C1A)),
    grid_relation_bbox_(ObjB, r0(_,C0B,_,_)),
    C1A < C0B.

% grid_relation_right_of(+ObjA, +ObjB)
% Succeed if ObjA is entirely right of ObjB: ObjA's first col > ObjB's last col.
grid_relation_right_of(ObjA, ObjB) :-
    grid_relation_bbox_(ObjA, r0(_,C0A,_,_)),
    grid_relation_bbox_(ObjB, r0(_,_,_,C1B)),
    C0A > C1B.

% grid_relation_bbox_contains(+ObjA, +ObjB)
% Succeed if ObjA's bbox fully contains ObjB's bbox on all four sides.
grid_relation_bbox_contains(ObjA, ObjB) :-
    grid_relation_bbox_(ObjA, r0(R0A,C0A,R1A,C1A)),
    grid_relation_bbox_(ObjB, r0(R0B,C0B,R1B,C1B)),
    R0A =< R0B, C0A =< C0B, R1A >= R1B, C1A >= C1B.

% grid_relation_bbox_overlap(+ObjA, +ObjB)
...
% grid_relation_cells_overlap(+ObjA, +ObjB)
% Succeed if ObjA and ObjB share at least one identical cell position.
grid_relation_cells_overlap(ObjA, ObjB) :-
    grid_relation_cells_(ObjA, CellsA),
    grid_relation_cells_(ObjB, CellsB),
...
% grid_relation_same_rows(+ObjA, +ObjB)
% Succeed if ObjA's and ObjB's row ranges overlap.
grid_relation_same_rows(ObjA, ObjB) :-
    grid_relation_bbox_(ObjA, r0(R0A,_,R1A,_)),
    grid_relation_bbox_(ObjB, r0(R0B,_,R1B,_)),
    R0A =< R1B, R0B =< R1A.

% grid_relation_same_cols(+ObjA, +ObjB)
% Succeed if ObjA's and ObjB's column ranges overlap.
grid_relation_same_cols(ObjA, ObjB) :-
    grid_relation_bbox_(ObjA, r0(_,C0A,_,C1A)),
    grid_relation_bbox_(ObjB, r0(_,C0B,_,C1B)),
    C0A =< C1B, C0B =< C1A.

% grid_relation_direction(+ObjA, +ObjB, -Dir)
...
% grid_relation_all_relations(+ObjA, +ObjB, -Relations)
% Relations is the list of all true spatial relation atoms between ObjA and ObjB.
...
grid_relation_all_relations(ObjA, ObjB, Relations) :-
    findall(Rel,
        ( member(Rel-Goal,
            [ touching    - grid_relation_touching(ObjA, ObjB)
            , above       - grid_relation_above(ObjA, ObjB)
            , below       - grid_relation_below(ObjA, ObjB)
            , left_of     - grid_relation_left_of(ObjA, ObjB)
            , right_of    - grid_relation_right_of(ObjA, ObjB)
            , bbox_contains - grid_relation_bbox_contains(ObjA, ObjB)
            , bbox_overlap  - grid_relation_bbox_overlap(ObjA, ObjB)
            , cells_overlap - grid_relation_cells_overlap(ObjA, ObjB)
            , same_rows   - grid_relation_same_rows(ObjA, ObjB)
            , same_cols   - grid_relation_same_cols(ObjA, ObjB)
            ]),
          call(Goal) ),
        Relations).
