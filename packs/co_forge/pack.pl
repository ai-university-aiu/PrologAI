% State the fact: this pack is named co_forge.
name(co_forge).
% State the fact: this is version 1.0.0.
version('1.0.0').
% State the fact: the title carries the work-package number.
title('PrologAI Causalontology Concept Formation — induce categories from observed feature bundles by clustering items that share enough features, coin a concept from the shared core, and classify new items into it (WP-420)').
