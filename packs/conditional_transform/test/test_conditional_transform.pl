:- use_module('../prolog/conditional_transform').
:- use_module(library(plunit)).

:- begin_tests(conditional_transform).
...
test(split_color_basic) :-
    ...
    conditional_transform_split_color([R, B, RB], r, Match, NoMatch),
    Match == [R, RB],
    NoMatch == [B].

test(split_color_none_match) :-
    red_dot(R), blue_dot(B),
    conditional_transform_split_color([R, B], g, Match, NoMatch),
    Match == [],
    NoMatch == [R, B].

test(split_color_all_match) :-
    red_dot(R), red_bar(RB),
    conditional_transform_split_color([R, RB], r, Match, NoMatch),
    Match == [R, RB],
    NoMatch == [].
...
test(merge_basic) :-
    red_dot(R), blue_dot(B),
    conditional_transform_merge([R], [B], [R, B]).

test(merge_empty_left) :-
    blue_dot(B),
    conditional_transform_merge([], [B], [B]).

test(merge_empty_both) :-
    conditional_transform_merge([], [], []).

% --- conditional_transform_recolor_matching/4 ---

test(recolor_matching_basic) :-
    red_dot(R), blue_dot(B), red_bar(RB),
    conditional_transform_recolor_matching([R, B, RB], r, g, Out),
    Out == [obj(g,[r(0,0)]), B, obj(g,[r(0,0),r(0,1),r(0,2)])].

test(recolor_matching_no_match) :-
    red_dot(R), blue_dot(B),
    conditional_transform_recolor_matching([R, B], g, y, [R, B]).

% --- conditional_transform_shift_color/5 ---

test(shift_color_basic) :-
    red_dot(R), blue_dot(B),
    conditional_transform_shift_color([R, B], r, 2, 3, Out),
    Out == [obj(r,[r(2,3)]), B].

test(shift_color_no_match) :-
    red_dot(R),
    conditional_transform_shift_color([R], b, 1, 1, [R]).

test(shift_color_multiple) :-
    red_dot(R), red_bar(RB), blue_dot(B),
    conditional_transform_shift_color([R, RB, B], r, 1, 0, Out),
...
test(swap_colors_basic) :-
    red_dot(R), blue_dot(B),
    conditional_transform_swap_colors([R, B], r, b, [obj(b,[r(0,0)]), obj(r,[r(0,1)])]).

test(swap_colors_no_effect_other) :-
    green_dot(G),
    conditional_transform_swap_colors([G], r, b, [G]).

test(swap_colors_single) :-
    red_dot(R),
    conditional_transform_swap_colors([R], r, g, [obj(g,[r(0,0)])]).
...
test(recolor_by_size_gt) :-
    ...
    conditional_transform_recolor_by_size([R, RB], 1, gt, y, Out),
    Out == [R, obj(y,[r(0,0),r(0,1),r(0,2)])].

test(recolor_by_size_eq) :-
    red_dot(R), blue_dot(B), red_bar(RB),
    % size == 1: dots qualify
    conditional_transform_recolor_by_size([R, B, RB], 1, eq, g, Out),
    Out == [obj(g,[r(0,0)]), obj(g,[r(0,1)]), RB].

test(recolor_by_size_lt) :-
    red_dot(R), red_bar(RB),
    % size < 3: dot qualifies
    conditional_transform_recolor_by_size([R, RB], 3, lt, g, Out),
    Out == [obj(g,[r(0,0)]), RB].
...
test(move_color_basic) :-
    red_dot(R), blue_dot(B),
    conditional_transform_move_color([R, B], r, 1, 1, Out),
    Out == [obj(r,[r(1,1)]), B].
...
test(unique_size_basic) :-
    ...
    conditional_transform_unique_size([R, B, L], L).

test(unique_size_no_unique) :-
    red_dot(R), blue_dot(B),
    % both size 1, no unique size
    \+ conditional_transform_unique_size([R, B], _).

test(unique_size_single) :-
    red_dot(R),
    % only one object — its size is unique
    conditional_transform_unique_size([R], R).
...
test(recolor_unique_size_basic) :-
    ...
    conditional_transform_recolor_unique_size([R, B, L], g, Out),
...
test(split_size_gt) :-
    red_dot(R), red_bar(RB),
    conditional_transform_split_size([R, RB], 1, gt, Match, NoMatch),
    Match == [RB],
    NoMatch == [R].

test(split_size_eq) :-
    red_dot(R), blue_dot(B), red_bar(RB),
    conditional_transform_split_size([R, B, RB], 1, eq, Match, NoMatch),
    Match == [R, B],
    NoMatch == [RB].

test(split_size_le) :-
    red_dot(R), lshape(L), red_bar(RB),
    conditional_transform_split_size([R, L, RB], 3, le, Match, NoMatch),
    Match == [R, L, RB],
    NoMatch == [].
...
test(split_color_single_match) :-
    red_dot(R),
    conditional_transform_split_color([R], r, [R], []).
...
test(recolor_matching_all) :-
    red_dot(R), red_bar(RB),
    conditional_transform_recolor_matching([R, RB], r, g, Out),
    Out == [obj(g,[r(0,0)]), obj(g,[r(0,0),r(0,1),r(0,2)])].

test(shift_color_all_match) :-
    red_dot(R), red_bar(RB),
    conditional_transform_shift_color([R, RB], r, 1, 0, Out),
...
test(swap_colors_third_unchanged) :-
    ...
    conditional_transform_swap_colors([G, R, B], r, b, Out),
    Out == [G, obj(b,[r(0,0)]), obj(r,[r(0,1)])].

test(recolor_by_size_ge) :-
    red_dot(R), red_bar(RB),
    % size >= 1: all qualify
    conditional_transform_recolor_by_size([R, RB], 1, ge, y, Out),
    Out == [obj(y,[r(0,0)]), obj(y,[r(0,0),r(0,1),r(0,2)])].

test(recolor_by_size_no_match) :-
    red_dot(R), blue_dot(B),
    % size > 5: no object qualifies
    conditional_transform_recolor_by_size([R, B], 5, gt, g, [R, B]).

test(split_size_ge) :-
    red_dot(R), lshape(L), red_bar(RB),
    % size >= 3: lshape and bar qualify
    conditional_transform_split_size([R, L, RB], 3, ge, Match, NoMatch),
    Match == [L, RB],
    NoMatch == [R].
...
test(recolor_unique_size_no_unique) :-
    ...
    red_dot(R), blue_dot(B),
    \+ conditional_transform_recolor_unique_size([R, B], g, _).

:- end_tests(conditional_transform).

:- begin_tests(conditional_transform_infer_gate).
...
test(infer_gate_finds_a_gate) :-
    ...
    conditional_transform_infer_gate([PA, PB, PC, PD], [], gate_color(3)).
...
test(infer_gate_returns_gate_color_term) :-
    ...
    conditional_transform_infer_gate([PA, PB, PC, PD], [], Gate),
    Gate = gate_color(_).
...
test(infer_gate_fails_no_distinction) :-
    ...
    \+ conditional_transform_infer_gate([PA, PB], [], _).

% Fails when only one pair exists (cannot split into two non-empty groups).
test(infer_gate_fails_single_pair) :-
    PA = pair([obj(1, [r(0,0)])], []),
    \+ conditional_transform_infer_gate([PA], [], _).

:- end_tests(conditional_transform_infer_gate).
