...
% logic_and(+Grid1, +Grid2, +Bg, -Grid3)
% Grid3[r,c] = Grid1[r,c] if both Grid1 and Grid2 are non-Bg at that cell; else Bg.
logic_and(Grid1, Grid2, Bg, Grid3) :-
    % Apply AND row-wise across paired rows.
    maplist(logic_and_row_(Bg), Grid1, Grid2, Grid3).

% logic_and_row_(+Bg, +Row1, +Row2, -Row3): AND two rows cell by cell.
logic_and_row_(Bg, Row1, Row2, Row3) :-
    % Apply AND to each paired cell.
    maplist(logic_and_cell_(Bg), Row1, Row2, Row3).

% logic_and_cell_(+Bg, +C1, +C2, -C3): C3 = C1 if both non-Bg; else Bg.
logic_and_cell_(Bg, C1, C2, C3) :-
    % Both cells must be non-Bg to pass the C1 value through.
    (C1 \== Bg, C2 \== Bg -> C3 = C1 ; C3 = Bg).

% logic_or(+Grid1, +Grid2, +Bg, -Grid3)
% Grid3[r,c] = Grid1[r,c] if Grid1 non-Bg; else Grid2[r,c] if Grid2 non-Bg; else Bg.
logic_or(Grid1, Grid2, Bg, Grid3) :-
    % Apply OR row-wise across paired rows.
    maplist(logic_or_row_(Bg), Grid1, Grid2, Grid3).

% logic_or_row_(+Bg, +Row1, +Row2, -Row3): OR two rows cell by cell.
logic_or_row_(Bg, Row1, Row2, Row3) :-
    % Apply OR to each paired cell.
    maplist(logic_or_cell_(Bg), Row1, Row2, Row3).

% logic_or_cell_(+Bg, +C1, +C2, -C3): prefer C1; fall back to C2; else Bg.
logic_or_cell_(Bg, C1, C2, C3) :-
...
% logic_xor(+Grid1, +Grid2, +Bg, -Grid3)
% Grid3[r,c] = non-Bg value if exactly one of Grid1[r,c], Grid2[r,c] is non-Bg; else Bg.
logic_xor(Grid1, Grid2, Bg, Grid3) :-
    % Apply XOR row-wise across paired rows.
    maplist(logic_xor_row_(Bg), Grid1, Grid2, Grid3).

% logic_xor_row_(+Bg, +Row1, +Row2, -Row3): XOR two rows cell by cell.
logic_xor_row_(Bg, Row1, Row2, Row3) :-
    % Apply XOR to each paired cell.
    maplist(logic_xor_cell_(Bg), Row1, Row2, Row3).

% logic_xor_cell_(+Bg, +C1, +C2, -C3): C1 if only C1 non-Bg; C2 if only C2 non-Bg; else Bg.
logic_xor_cell_(Bg, C1, C2, C3) :-
...
% logic_not(+Grid, +Bg, +Fg, -Grid2)
% Grid2[r,c] = Fg if Grid[r,c] == Bg; else Bg. Inverts foreground and background.
logic_not(Grid, Bg, Fg, Grid2) :-
    % Apply NOT row-wise.
    maplist(logic_not_row_(Bg, Fg), Grid, Grid2).

% logic_not_row_(+Bg, +Fg, +Row, -Row2): NOT one row cell by cell.
logic_not_row_(Bg, Fg, Row, Row2) :-
    % Apply NOT to each cell.
    maplist(logic_not_cell_(Bg, Fg), Row, Row2).

% logic_not_cell_(+Bg, +Fg, +Cell, -Cell2): Bg -> Fg; non-Bg -> Bg.
logic_not_cell_(Bg, Fg, Cell, Cell2) :-
    % Background becomes foreground; foreground becomes background.
    (Cell == Bg -> Cell2 = Fg ; Cell2 = Bg).

% logic_diff(+Grid1, +Grid2, +Bg, -Grid3)
% Grid3[r,c] = Grid1[r,c] if Grid1 is non-Bg there and Grid2 is Bg there; else Bg.
logic_diff(Grid1, Grid2, Bg, Grid3) :-
    % Apply diff row-wise across paired rows.
    maplist(logic_diff_row_(Bg), Grid1, Grid2, Grid3).

% logic_diff_row_(+Bg, +Row1, +Row2, -Row3): subtract Row2 presence from Row1.
logic_diff_row_(Bg, Row1, Row2, Row3) :-
    % Apply diff to each paired cell.
    maplist(logic_diff_cell_(Bg), Row1, Row2, Row3).

% logic_diff_cell_(+Bg, +C1, +C2, -C3): C1 only if C1 non-Bg and C2 == Bg; else Bg.
logic_diff_cell_(Bg, C1, C2, C3) :-
    % Retain C1 only where Grid2 has no content.
    (C1 \== Bg, C2 == Bg -> C3 = C1 ; C3 = Bg).

% logic_overlay(+Base, +Top, +Bg, -Result)
...
logic_overlay(Base, Top, Bg, Result) :-
    ...
    maplist(logic_overlay_row_(Bg), Base, Top, Result).

% logic_overlay_row_(+Bg, +BaseRow, +TopRow, -ResultRow): overlay one row pair.
logic_overlay_row_(Bg, BaseRow, TopRow, ResultRow) :-
    % Apply overlay to each paired cell.
    maplist(logic_overlay_cell_(Bg), BaseRow, TopRow, ResultRow).

% logic_overlay_cell_(+Bg, +Base, +Top, -Result): Top wins if non-Bg; else keep Base.
logic_overlay_cell_(Bg, Base, Top, Result) :-
    % Top's non-Bg value overwrites Base.
    (Top \== Bg -> Result = Top ; Result = Base).

% logic_mask_apply(+Mask, +Grid, +Bg, -Grid2)
% Grid2[r,c] = Grid[r,c] where Mask[r,c] is non-Bg; else Bg.
logic_mask_apply(Mask, Grid, Bg, Grid2) :-
    % Apply mask row-wise across paired rows.
    maplist(logic_mask_apply_row_(Bg), Mask, Grid, Grid2).

% logic_mask_apply_row_(+Bg, +MaskRow, +GridRow, -Row2): apply mask to one row.
logic_mask_apply_row_(Bg, MaskRow, GridRow, Row2) :-
    % Apply mask cell-wise.
    maplist(logic_mask_apply_cell_(Bg), MaskRow, GridRow, Row2).

% logic_mask_apply_cell_(+Bg, +M, +G, -C2): keep G if M non-Bg; else Bg.
logic_mask_apply_cell_(Bg, M, G, C2) :-
    % Non-Bg mask cell passes the grid value through; Bg mask cell blocks it.
    (M \== Bg -> C2 = G ; C2 = Bg).

% logic_mask_from(+Grid, +Bg, +Fg, -Mask)
% Mask[r,c] = Fg if Grid[r,c] is non-Bg; else Bg. Creates a binary presence mask.
logic_mask_from(Grid, Bg, Fg, Mask) :-
    % Apply mask creation row-wise.
    maplist(logic_mask_from_row_(Bg, Fg), Grid, Mask).

% logic_mask_from_row_(+Bg, +Fg, +Row, -MaskRow): create mask for one row.
logic_mask_from_row_(Bg, Fg, Row, MaskRow) :-
    % Create mask cell-wise.
    maplist(logic_mask_from_cell_(Bg, Fg), Row, MaskRow).

% logic_mask_from_cell_(+Bg, +Fg, +Cell, -M): non-Bg -> Fg; Bg -> Bg.
logic_mask_from_cell_(Bg, Fg, Cell, M) :-
    % Mark presence with Fg; absence stays Bg.
    (Cell == Bg -> M = Bg ; M = Fg).

% logic_any_row(+Grid, +Bg, -Flags)
% Flags is a list with one element per row: 1 if any cell in that row is non-Bg; else 0.
logic_any_row(Grid, Bg, Flags) :-
    % Check each row independently.
    maplist(logic_any_row_flag_(Bg), Grid, Flags).

% logic_any_row_flag_(+Bg, +Row, -Flag): Flag = 1 if any cell non-Bg; else 0.
logic_any_row_flag_(Bg, Row, Flag) :-
    % member/2 inside -> backtracks to find a non-Bg cell.
    (member(Cell, Row), Cell \== Bg -> Flag = 1 ; Flag = 0).

% logic_any_col(+Grid, +Bg, -Flags)
...
logic_any_col(Grid, Bg, Flags) :-
    ...
    ; Grid = [FirstRow|_],
      ...
      maplist(logic_any_col_flag_(Grid, Bg), CIdxs, Flags)).

% logic_any_col_flag_(+Grid, +Bg, +C, -Flag): Flag = 1 if any cell in col C non-Bg.
logic_any_col_flag_(Grid, Bg, C, Flag) :-
    % Scan all rows for col C; stop at first non-Bg cell.
    (member(Row, Grid), nth0(C, Row, Cell), Cell \== Bg -> Flag = 1 ; Flag = 0).

% logic_all_row(+Grid, +Bg, -Flags)
% Flags is a list with one element per row: 1 if all cells in that row are non-Bg; else 0.
logic_all_row(Grid, Bg, Flags) :-
    % Check each row independently.
    maplist(logic_all_row_flag_(Bg), Grid, Flags).

% logic_all_row_flag_(+Bg, +Row, -Flag): Flag = 1 if all cells non-Bg; else 0.
logic_all_row_flag_(Bg, Row, Flag) :-
    % forall succeeds vacuously on empty rows.
    (forall(member(Cell, Row), Cell \== Bg) -> Flag = 1 ; Flag = 0).

% logic_all_col(+Grid, +Bg, -Flags)
...
logic_all_col(Grid, Bg, Flags) :-
    ...
    ; Grid = [FirstRow|_],
      ...
      NColsM1 is NCols - 1,
      numlist(0, NColsM1, CIdxs),
      maplist(logic_all_col_flag_(Grid, Bg), CIdxs, Flags)).

% logic_all_col_flag_(+Grid, +Bg, +C, -Flag): Flag = 1 if all cells in col C non-Bg.
logic_all_col_flag_(Grid, Bg, C, Flag) :-
    % forall checks every row; nth0 extracts the cell at column C.
    (forall(member(Row, Grid), (nth0(C, Row, Cell), Cell \== Bg))
    -> Flag = 1 ; Flag = 0).

% logic_eq(+Grid1, +Grid2, -BoolGrid)
% BoolGrid[r,c] = 1 if Grid1[r,c] == Grid2[r,c]; else 0.
logic_eq(Grid1, Grid2, BoolGrid) :-
    % Apply equality row-wise across paired rows.
    maplist(logic_eq_row_, Grid1, Grid2, BoolGrid).

% logic_eq_row_(+Row1, +Row2, -BoolRow): equality for one row pair.
logic_eq_row_(Row1, Row2, BoolRow) :-
    % Apply equality cell-wise.
    maplist(logic_eq_cell_, Row1, Row2, BoolRow).

% logic_eq_cell_(+C1, +C2, -B): 1 if C1 == C2; else 0.
logic_eq_cell_(C1, C2, B) :-
    % Term equality (==) covers atoms, integers, and compound terms.
    (C1 == C2 -> B = 1 ; B = 0).

% logic_neq(+Grid1, +Grid2, -BoolGrid)
% BoolGrid[r,c] = 1 if Grid1[r,c] \== Grid2[r,c]; else 0.
logic_neq(Grid1, Grid2, BoolGrid) :-
    % Apply inequality row-wise across paired rows.
    maplist(logic_neq_row_, Grid1, Grid2, BoolGrid).

% logic_neq_row_(+Row1, +Row2, -BoolRow): inequality for one row pair.
logic_neq_row_(Row1, Row2, BoolRow) :-
    % Apply inequality cell-wise.
    maplist(logic_neq_cell_, Row1, Row2, BoolRow).

% logic_neq_cell_(+C1, +C2, -B): 1 if C1 \== C2; else 0.
logic_neq_cell_(C1, C2, B) :-
