...
% --- symtab_color_feature ---

test('AC-ST-001: symtab_color_feature extracts color(r) from red object') :-
    red1(R), symtab_color_feature(R, F), F = color(r).

test('AC-ST-002: symtab_color_feature extracts color(b) from blue object') :-
    blue1(B), symtab_color_feature(B, F), F = color(b).

test('AC-ST-003: symtab_color_feature extracts color(g) from green object') :-
    green1(G), symtab_color_feature(G, F), F = color(g).

% --- symtab_size_feature ---

test('AC-ST-004: symtab_size_feature extracts size(1) from single-cell object') :-
    red1(R), symtab_size_feature(R, F), F = size(1).

test('AC-ST-005: symtab_size_feature extracts size(2) from two-cell object') :-
    red2(R), symtab_size_feature(R, F), F = size(2).

test('AC-ST-006: symtab_size_feature extracts size(3) from three-cell object') :-
    blue3(B), symtab_size_feature(B, F), F = size(3).

% --- symtab_hole_count ---

test('AC-ST-007: symtab_hole_count returns 0 for solid single cell') :-
    red1(R), symtab_hole_count(R, N), N =:= 0.

test('AC-ST-008: symtab_hole_count returns 0 for solid two-cell object') :-
    red2(R), symtab_hole_count(R, N), N =:= 0.

test('AC-ST-009: symtab_hole_count returns positive for L-shape with missing corner') :-
    hollow(H), symtab_hole_count(H, N), N > 0.

test('AC-ST-010: symtab_hole_count for 1x1 solid returns 0') :-
    O = ob(r, [r(5,5)], r0(5,5,5,5)),
    symtab_hole_count(O, N), N =:= 0.

% --- symtab_lookup ---

test('AC-ST-011: symtab_lookup finds first matching entry') :-
    Table = [sym(color(r), b), sym(color(b), g)],
    symtab_lookup(Table, color(r), V),
    V = b.

test('AC-ST-012: symtab_lookup finds second entry') :-
    Table = [sym(color(r), b), sym(color(b), g)],
    symtab_lookup(Table, color(b), V),
    V = g.

test('AC-ST-013: symtab_lookup fails for unknown feature') :-
    Table = [sym(color(r), b)],
    \+ symtab_lookup(Table, color(g), _).

% --- symtab_entry_consistent ---

test('AC-ST-014: entry consistent with empty table') :-
    symtab_entry_consistent(sym(color(r), b), []).

test('AC-ST-015: entry consistent when table maps same F to same V') :-
    symtab_entry_consistent(sym(color(r), b), [sym(color(r), b)]).

test('AC-ST-016: entry inconsistent when table maps F to different V') :-
    \+ symtab_entry_consistent(sym(color(r), b), [sym(color(r), g)]).

test('AC-ST-017: entry consistent when table maps different F') :-
    symtab_entry_consistent(sym(color(r), b), [sym(color(g), y)]).

% --- symtab_position_feature ---

test('AC-ST-018: position feature top-left quadrant') :-
    O = ob(r, [r(0,0)], r0(0,0,0,0)),
    symtab_position_feature(O, dims(10,10), pos(tl)).

test('AC-ST-019: position feature top-right quadrant') :-
    O = ob(r, [r(0,8)], r0(0,8,0,8)),
    symtab_position_feature(O, dims(10,10), pos(tr)).

test('AC-ST-020: position feature bottom-left quadrant') :-
    O = ob(r, [r(8,0)], r0(8,0,8,0)),
    symtab_position_feature(O, dims(10,10), pos(bl)).

test('AC-ST-021: position feature bottom-right quadrant') :-
    O = ob(r, [r(8,8)], r0(8,8,8,8)),
    symtab_position_feature(O, dims(10,10), pos(br)).
...
test('AC-ST-025: candidate symbols picks smallest quartile') :-
    ...
    symtab_candidate_symbols([O1, O2, O3, O4], Cands),
...
test('AC-ST-027: single object always a candidate') :-
    red1(R), symtab_candidate_symbols([R], Cands), Cands = [R].
...
test('AC-ST-028: apply color table - red becomes blue') :-
    ...
    symtab_apply_table(Table, InObjs, Map),
...
test('AC-ST-030: apply table to empty objs gives empty map') :-
    Table = [sym(color(r), b)],
    symtab_apply_table(Table, [], Map),
    Map = [].

test('AC-ST-031: empty table gives empty map') :-
    InObjs = [ob(r,[r(0,0)],r0(0,0,0,0))],
    symtab_apply_table([], InObjs, Map),
    Map = [].
...
test('AC-ST-044: best table selects highest scoring candidate') :-
    ...
    Good = [sym(color(r), b)],
    Bad  = [sym(color(g), y)],
    symtab_best_table([Good, Bad], [A], Best),
    Best = Good.
