...
% AC-GPT-008: overlay with no transparent cells = same as grid_patch_place.
test(overlay_no_transparency) :-
    base4(G), p2x2(P),
    grid_patch_overlay(G, P, 1, 1, z, R1),
    grid_patch_place(G, P, 1, 1, R2),
    R1 = R2.
...
:- end_tests(grid_patch_overlay).

:- begin_tests(grid_patch_match_at).

% AC-GPT-010: [[x,b],[b,x]] matches checker at (0,0).
test(match_checker_00) :-
    checker(G), grid_patch_match_at(G, [[x,b],[b,x]], 0, 0).

% AC-GPT-011: [[b,x],[x,b]] matches checker at (0,1).
test(match_checker_01) :-
    checker(G), grid_patch_match_at(G, [[b,x],[x,b]], 0, 1).

% AC-GPT-012: all-x patch does NOT match checker at (0,0).
test(match_fail) :-
    checker(G), \+ grid_patch_match_at(G, [[x,x],[x,x]], 0, 0).

:- end_tests(grid_patch_match_at).

:- begin_tests(grid_patch_find_all).

% AC-GPT-013: find all [[x,b],[b,x]] in checker: 5 positions.
test(find_all_checker) :-
    checker(G),
    grid_patch_find_all(G, [[x,b],[b,x]], Pos),
    length(Pos, 5).

% AC-GPT-014: find [[b,b],[b,b]] in base4: 9 positions (3x3 possible offsets).
test(find_all_bg_in_base4) :-
    base4(G), p2x2b(P),
    grid_patch_find_all(G, P, Pos),
    length(Pos, 9).

% AC-GPT-015: no match returns empty list.
test(find_all_none) :-
    base4(G),
    grid_patch_find_all(G, [[m,m],[m,m]], Pos),
    Pos = [].

:- end_tests(grid_patch_find_all).

:- begin_tests(grid_patch_count).

% AC-GPT-016: 5 occurrences of [[x,b],[b,x]] in checker.
test(count_checker) :-
    checker(G),
    grid_patch_count(G, [[x,b],[b,x]], N), N =:= 5.

% AC-GPT-017: zero occurrences of all-m patch in base4.
test(count_zero) :-
    base4(G), grid_patch_count(G, [[m,m],[m,m]], N), N =:= 0.

:- end_tests(grid_patch_count).

:- begin_tests(grid_patch_scatter).

% AC-GPT-018: scatter 1x1 [[m]] at [(0,0),(2,2)] in base4.
test(scatter_two) :-
    base4(G), p1x1(P),
    grid_patch_scatter(G, P, [0-0, 2-2], R),
    nth0(0, R, R0), nth0(0, R0, m),
    nth0(2, R, R2), nth0(2, R2, m).

% AC-GPT-019: scatter with empty list leaves grid unchanged.
test(scatter_empty) :-
    checker(G), grid_patch_scatter(G, [[m]], [], R), R = G.

% AC-GPT-020: scatter same position twice uses last placement.
test(scatter_same_twice) :-
    base4(G), p1x1(P),
    grid_patch_scatter(G, P, [1-1, 1-1], R),
    nth0(1, R, Row), nth0(1, Row, m).

:- end_tests(grid_patch_scatter).

:- begin_tests(grid_patch_tile_fill).

% AC-GPT-021: tile [[x,b],[b,x]] to 4x4 = checker.
test(tile_fill_4x4) :-
    grid_patch_tile_fill([[x,b],[b,x]], 4, 4, G),
    checker(G).

% AC-GPT-022: tile [[x]] to 3x3 = all-x.
test(tile_fill_1x1) :-
    grid_patch_tile_fill([[x]], 3, 3, G),
    G = [[x,x,x],[x,x,x],[x,x,x]].

% AC-GPT-023: tile 1x2 [[a,b]] to 2x4.
test(tile_fill_1x2) :-
    grid_patch_tile_fill([[a,b]], 2, 4, G),
    G = [[a,b,a,b],[a,b,a,b]].

:- end_tests(grid_patch_tile_fill).

:- begin_tests(grid_patch_h).

% AC-GPT-024: height of 2x2 patch.
test(h_2x2) :- p2x2(P), grid_patch_h(P, H), H =:= 2.

% AC-GPT-025: height of 1x1 patch.
test(h_1x1) :- p1x1(P), grid_patch_h(P, H), H =:= 1.

:- end_tests(grid_patch_h).

:- begin_tests(grid_patch_w).

% AC-GPT-026: width of 2x2 patch.
test(w_2x2) :- p2x2(P), grid_patch_w(P, W), W =:= 2.

% AC-GPT-027: width of 4x4 checker.
test(w_4x4) :- checker(G), grid_patch_w(G, W), W =:= 4.

:- end_tests(grid_patch_w).

:- begin_tests(grid_patch_size).

% AC-GPT-028: size of 2x2 patch.
test(size_2x2) :- p2x2(P), grid_patch_size(P, H, W), H =:= 2, W =:= 2.

% AC-GPT-029: size of 4x4 checker.
test(size_4x4) :- checker(G), grid_patch_size(G, H, W), H =:= 4, W =:= 4.

:- end_tests(grid_patch_size).

:- begin_tests(grid_patch_eq).

% AC-GPT-030: two equal patches unify.
test(eq_same) :- p2x2(P), grid_patch_eq(P, P).

% AC-GPT-031: different patches fail.
test(eq_diff) :- p2x2(P), p2x2b(Q), \+ grid_patch_eq(P, Q).

:- end_tests(grid_patch_eq).

:- begin_tests(grid_patch_inpaint).

% AC-GPT-032: inpaint 2x2 block at (1,1) with m in base4.
test(inpaint_block) :-
    base4(G),
    grid_patch_inpaint(G, 1, 1, 2, 2, m, R),
    R = [[b,b,b,b],[b,m,m,b],[b,m,m,b],[b,b,b,b]].

% AC-GPT-033: inpaint entire grid with x.
test(inpaint_full) :-
    base3(G),
    grid_patch_inpaint(G, 0, 0, 2, 2, x, R),
    R = [[x,x,x],[x,x,x],[x,x,x]].

% AC-GPT-034: inpaint single cell.
test(inpaint_single) :-
    checker(G),
    grid_patch_inpaint(G, 0, 0, 0, 0, z, R),
    nth0(0, R, Row0), nth0(0, Row0, z),
    nth0(0, R, Row0b), nth0(1, Row0b, b).

:- end_tests(grid_patch_inpaint).

:- begin_tests(grid_patch_replace_first).

% AC-GPT-035: replace first [[x,b],[b,x]] in checker with [[m,m],[m,m]].
test(replace_first_checker) :-
    checker(G),
    grid_patch_replace_first(G, [[x,b],[b,x]], [[m,m],[m,m]], R),
    nth0(0, R, Row0), Row0 = [m,m,x,b].

% AC-GPT-036: replace first occurrence in striped grid.
test(replace_first_stripe) :-
    stripe(G),
    grid_patch_replace_first(G, [[x,b],[x,b]], [[a,a],[a,a]], R),
    nth0(0, R, Row0), Row0 = [a,a,x,b].

% AC-GPT-037: replace_first fails when no match.
test(replace_first_nomatch) :-
    base4(G),
    \+ grid_patch_replace_first(G, [[m,m]], _, _).

:- end_tests(grid_patch_replace_first).
...
test(extract_then_place) :-
    checker(G),
    grid_patch_extract(G, 0, 0, 1, 1, Patch),
    base4(Blank),
    grid_patch_place(Blank, Patch, 0, 0, R),
...
test(overlay_then_extract) :-
    base4(G), p2x2(P),
    grid_patch_overlay(G, P, 1, 1, z, R),
    grid_patch_extract(R, 1, 1, 2, 2, Q),
    Q = [[x,x],[x,x]].
...
test(inpaint_then_place) :-
    checker(G),
    grid_patch_extract(G, 0, 0, 1, 1, Patch),
    grid_patch_inpaint(G, 0, 0, 1, 1, b, Blank),
    grid_patch_place(Blank, Patch, 0, 0, Restored),
    nth0(0, Restored, R0), nth0(0, R0, x).

% AC-GPT-042: scatter then count.
test(scatter_then_count) :-
    base4(G), p1x1(P),
    grid_patch_scatter(G, P, [0-0, 1-1, 2-2, 3-3], R),
    grid_patch_count(R, [[m]], N), N =:= 4.

% AC-GPT-043: replace_first reduces the match count.
test(replace_first_reduces_count) :-
    checker(G),
    grid_patch_count(G, [[x,b],[b,x]], N0),
    grid_patch_replace_first(G, [[x,b],[b,x]], [[b,b],[b,b]], R),
    grid_patch_count(R, [[x,b],[b,x]], N1),
    N1 < N0.

% AC-GPT-044: grid_patch_size and tile_fill agree on dimensions.
test(size_tile_agree) :-
    Patch = [[a,b],[c,d]],
    grid_patch_tile_fill(Patch, 6, 6, G),
    grid_patch_size(G, H, W), H =:= 6, W =:= 6.
