...
% ob_bbox_(+Obj, -MinR, -MinC, -MaxR, -MaxC): private bounding box helper.
% Collects all row and column indices then finds min and max via library predicates.
ob_bbox_(obj(_, Cells), MinR, MinC, MaxR, MaxC) :-
...
% ob_bbox_h(+Obj, -H): H is the bounding box height (number of rows spanned by Obj).
% H = MaxR - MinR + 1; minimum is 1 for a single-row object.
ob_bbox_h(Obj, H) :-
% Compute the bounding box to get the row span.
    ob_bbox_(Obj, MinR, _, MaxR, _),
% Height = max row - min row + 1.
    H is MaxR - MinR + 1.

% ob_bbox_w(+Obj, -W): W is the bounding box width (number of columns spanned by Obj).
% W = MaxC - MinC + 1; minimum is 1 for a single-column object.
ob_bbox_w(Obj, W) :-
% Compute the bounding box to get the column span.
    ob_bbox_(Obj, _, MinC, _, MaxC),
% Width = max col - min col + 1.
    W is MaxC - MinC + 1.

% ob_bbox_area(+Obj, -Area): Area is the area of Obj's bounding box (H * W).
...
% ob_is_rect(+Obj): true when Obj is a solid rectangle with no holes.
...
% ob_is_hline(+Obj): true when all cells of Obj lie in a single row.
% The bounding box height is 1 for a horizontal line.
ob_is_hline(Obj) :-
% A horizontal line spans exactly one row.
    ob_bbox_h(Obj, 1).

% ob_is_vline(+Obj): true when all cells of Obj lie in a single column.
% The bounding box width is 1 for a vertical line.
ob_is_vline(Obj) :-
% A vertical line spans exactly one column.
    ob_bbox_w(Obj, 1).

% ob_is_single(+Obj): true when Obj has exactly one cell (H=1, W=1).
...
% ob_is_square_bbox(+Obj): true when the bounding box of Obj is a square (H = W).
% This does NOT require the object itself to fill the square; use ob_is_rect too.
...
% ob_holes(+Obj, -HoleCells): HoleCells is the list of cells in the bounding box
...
% Get the bounding box to enumerate all grid positions within it.
    Obj = obj(_, Cells),
    ob_bbox_(Obj, MinR, MinC, MaxR, MaxC),
...
% ob_n_holes(+Obj, -N): N is the number of hole cells in Obj's bounding box.
...
% ob_is_hollow(+Obj): true when Obj has at least one hole in its bounding box.
...
% ob_is_frame(+Obj): true when Obj is a hollow rectangular frame.
...
% ob_cell_exposed_(+r(R,C), +Cells, -N): N exposed edges for cell r(R,C).
% An edge is exposed when the 4-adjacent neighbor is not in Cells.
ob_cell_exposed_(r(R,C), Cells, N) :-
...
% ob_perimeter(+Obj, -P): P is the total number of exposed cell edges in Obj.
...
% Sum exposed edges across all cells.
    findall(N, (member(Cell, Cells), ob_cell_exposed_(Cell, Cells, N)), Ns),
    sum_list(Ns, P).

% ob_dense_hull(+Obj, -HullObj): HullObj is the solid rectangle covering Obj's bounding box.
...
ob_dense_hull(Obj, obj(Color, Hull)) :-
