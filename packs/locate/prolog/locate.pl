...
:- module(locate, [
    lc_subgrid_at/4, lc_find_sub/4, lc_all_sub/3, lc_subgrid_count/3,
    lc_row_pattern/3, lc_all_row_pattern/3, lc_col_pattern/3,
    lc_all_col_pattern/3, lc_row_prefix/4, lc_row_suffix/3,
    lc_anchor/4, lc_row_count/3, lc_row_contains/3, lc_col_contains/3
]).
...
% lc_subgrid_at(+Grid, +Sub, +R0, +C0): succeed if Sub appears at top-left (R0,C0) in Grid.
...
% lc_find_sub(+Grid, +Sub, -R0, -C0): first row-major position where Sub matches Grid.
...
% lc_all_sub(+Grid, +Sub, -Positions): list of all R0-C0 where Sub matches Grid.
...
% lc_subgrid_count(+Grid, +Sub, -N): count how many times Sub appears in Grid.
lc_subgrid_count(Grid, Sub, N) :-
    % collect all positions then measure the result list
    lc_all_sub(Grid, Sub, Positions),
    length(Positions, N).

% lc_row_pattern(+Grid, +Pattern, -R): first row index where row equals Pattern.
...
% lc_all_row_pattern(+Grid, +Pattern, -Rows): all row indices where row equals Pattern.
...
% lc_col_pattern(+Grid, +Pattern, -C): first column index whose value list equals Pattern.
...
% lc_all_col_pattern(+Grid, +Pattern, -Cols): all column indices whose value list equals Pattern.
...
% lc_row_prefix(+Grid, +R, +Prefix, -Rest): decompose grid row R as Prefix ++ Rest.
...
% lc_row_suffix(+Grid, +R, +Suffix): succeed if grid row R ends with Suffix.
...
% lc_anchor(+Grid, +V, -R, -C): succeed if exactly one cell in Grid equals V.
...
% lc_col_contains(+Grid, +V, -Cols): sorted list of column indices that contain V.
