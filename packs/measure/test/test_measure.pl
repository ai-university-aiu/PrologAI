...
test(area_single) :-
    single_cell(R), measure_area(R, N), N =:= 1.

test(area_hline) :-
    hline(R), measure_area(R, N), N =:= 3.

test(area_square) :-
    square2_region(R), measure_area(R, N), N =:= 4.

test(area_lshape) :-
    lshape(R), measure_area(R, N), N =:= 3.
...
test(bbox_single) :-
    single_cell(R), measure_bbox(R, bbox(2,3,2,3)).

test(bbox_hline) :-
    hline(R), measure_bbox(R, bbox(1,0,1,2)).

test(bbox_vline) :-
    vline(R), measure_bbox(R, bbox(0,1,2,1)).

test(bbox_square) :-
    square2_region(R), measure_bbox(R, bbox(0,0,1,1)).

test(bbox_lshape) :-
    lshape(R), measure_bbox(R, bbox(0,0,1,1)).
...
test(bbox_size_single) :-
    single_cell(R), measure_bbox_size(R, W, H), W =:= 1, H =:= 1.

test(bbox_size_hline) :-
    hline(R), measure_bbox_size(R, W, H), W =:= 3, H =:= 1.

test(bbox_size_vline) :-
    vline(R), measure_bbox_size(R, W, H), W =:= 1, H =:= 3.

test(bbox_size_square) :-
    square2_region(R), measure_bbox_size(R, W, H), W =:= 2, H =:= 2.
...
test(diameter_single) :-
    single_cell(R), measure_diameter(R, D), D =:= 0.

test(diameter_hline) :-
    % hline spans (1,0) to (1,2): max distance = 2.
    hline(R), measure_diameter(R, D), D =:= 2.

test(diameter_square2) :-
    % 2x2: max distance between corners = (0,0) to (1,1) = 2.
    square2_region(R), measure_diameter(R, D), D =:= 2.

test(diameter_vline) :-
    % vline (0,1) to (2,1): max = 2.
    vline(R), measure_diameter(R, D), D =:= 2.
...
test(extent_single) :-
    single_cell(R), measure_extent(R, N, D), N =:= 1, D =:= 1.

test(extent_hline) :-
    % area=3, bbox=3x1=3; extent = 3/3 = 1.
    hline(R), measure_extent(R, N, D), N =:= 3, D =:= 3.

test(extent_lshape) :-
    % area=3, bbox=2x2=4; extent = 3/4.
    lshape(R), measure_extent(R, N, D), N =:= 3, D =:= 4.

test(extent_square) :-
    % area=4, bbox=2x2=4; extent = 4/4 = 1.
    square2_region(R), measure_extent(R, N, D), N =:= 4, D =:= 4.
...
test(aspect_square) :-
    square2_region(R), measure_aspect(R, N, D), N =:= 2, D =:= 2.

test(aspect_hline) :-
    % W=3, H=1: max=3, min=1.
    hline(R), measure_aspect(R, N, D), N =:= 3, D =:= 1.

test(aspect_vline) :-
    % W=1, H=3: max=3, min=1.
    vline(R), measure_aspect(R, N, D), N =:= 3, D =:= 1.

test(aspect_single) :-
    single_cell(R), measure_aspect(R, N, D), N =:= 1, D =:= 1.
...
test(row_span_single) :-
    single_cell(R), measure_row_span(R, S), S =:= 1.

test(row_span_hline) :-
    hline(R), measure_row_span(R, S), S =:= 1.

test(row_span_vline) :-
    vline(R), measure_row_span(R, S), S =:= 3.

test(row_span_lshape) :-
    lshape(R), measure_row_span(R, S), S =:= 2.
...
test(col_span_single) :-
    single_cell(R), measure_col_span(R, S), S =:= 1.

test(col_span_hline) :-
    hline(R), measure_col_span(R, S), S =:= 3.

test(col_span_vline) :-
    vline(R), measure_col_span(R, S), S =:= 1.

test(col_span_lshape) :-
    lshape(R), measure_col_span(R, S), S =:= 2.
...
test(centroid_single) :-
    single_cell(R), measure_centroid(R, AvgR, AvgC), AvgR =:= 2, AvgC =:= 3.

test(centroid_hline) :-
    % hline: rows all 1, cols 0,1,2 -> avg col = 1.
    hline(R), measure_centroid(R, AR, AC), AR =:= 1, AC =:= 1.

test(centroid_square2) :-
    % 2x2: rows 0,0,1,1 = avg 0; cols 0,1,0,1 = avg 0.
    square2_region(R), measure_centroid(R, AR, AC), AR =:= 0, AC =:= 0.

test(centroid_vline) :-
    % vline: rows 0,1,2 -> avg 1; col always 1.
    vline(R), measure_centroid(R, AR, AC), AR =:= 1, AC =:= 1.
...
test(radius_single) :-
    single_cell(R), measure_radius(R, Rad), Rad =:= 0.

test(radius_hline) :-
    % centroid = (1,1); Chebyshev dist to (1,0) or (1,2) = 1.
    hline(R), measure_radius(R, Rad), Rad =:= 1.

test(radius_vline) :-
    % centroid = (1,1); dist to (0,1) or (2,1) = 1.
    vline(R), measure_radius(R, Rad), Rad =:= 1.

test(radius_square2) :-
    % centroid = (0,0); dist to (1,1) = max(1,1) = 1.
    square2_region(R), measure_radius(R, Rad), Rad =:= 1.
...
test(border_single) :-
    single_cell(R), measure_border_count(R, N), N =:= 1.

test(border_hline) :-
    hline(R), measure_border_count(R, N), N =:= 3.

test(border_sq3) :-
    % 3x3: 8 border cells, 1 interior.
    sq3(R), measure_border_count(R, N), N =:= 8.

test(border_interior_sum) :-
    % border + interior = area.
    sq3(R), measure_area(R, A), measure_border_count(R, B), measure_interior_count(R, I),
    A =:= B + I.
...
test(color_count_one) :-
    measure_color_count([[0,0],[0,0]], N), N =:= 1.

test(color_count_two) :-
    measure_color_count([[0,1],[0,1]], N), N =:= 2.

test(color_count_all_different) :-
    measure_color_count([[1,2],[3,4]], N), N =:= 4.

test(color_count_arc) :-
    measure_color_count([[0,1,2],[3,4,5],[6,7,8]], N), N =:= 9.
