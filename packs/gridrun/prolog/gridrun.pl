:- module(gridrun, [
    gridrun_row_runs/3,
    gridrun_col_runs/3,
    gridrun_all_row_runs/2,
    gridrun_all_col_runs/2,
    gridrun_decode/2,
    gridrun_run_count/3,
    gridrun_uniform_row/3,
    gridrun_uniform_col/3,
    gridrun_is_striped_h/1,
    gridrun_is_striped_v/1,
    gridrun_stripe_colors_h/2,
    gridrun_stripe_colors_v/2,
    gridrun_max_run/4,
    gridrun_alternating/2
]).
...
gridrun_list_runs_([], []) :- !.
gridrun_list_runs_([V|Rest], Runs) :-
    gridrun_run_acc_(V, 1, Rest, Runs).

% Accumulate a run of V; when the color changes, emit V-N and start new run.
gridrun_run_acc_(V, N, [], [V-N]) :- !.
gridrun_run_acc_(V, N, [V|Rest], Runs) :- !,
% Same color: extend the current run.
    N1 is N + 1,
    gridrun_run_acc_(V, N1, Rest, Runs).
gridrun_run_acc_(V, N, [W|Rest], [V-N|Runs]) :-
% Different color: emit run, start fresh.
    gridrun_run_acc_(W, 1, Rest, Runs).

% Extract column C of Grid as a flat list.
gridrun_col_list_(Grid, C, Col) :-
    gridrun_dims_(Grid, H, _),
    H1 is H - 1,
...
gridrun_all_row_runs(Grid, AllRuns) :-
    gridrun_dims_(Grid, H, _),
    H1 is H - 1,
    findall(Runs, (between(0, H1, R), gridrun_row_runs(Grid, R, Runs)), AllRuns).

% gridrun_all_col_runs(+Grid, -AllRuns)
% AllRuns is a list with one element per column: the run-length encoding of that column.
gridrun_all_col_runs(Grid, AllRuns) :-
    gridrun_dims_(Grid, _, W),
    W1 is W - 1,
    findall(Runs, (between(0, W1, C), gridrun_col_runs(Grid, C, Runs)), AllRuns).
...
gridrun_is_striped_h(Grid) :-
    gridrun_dims_(Grid, H, _),
    H1 is H - 1,
    \+ (between(0, H1, R), \+ gridrun_uniform_row(Grid, R, _)).
...
gridrun_is_striped_v(Grid) :-
    gridrun_dims_(Grid, _, W),
    W1 is W - 1,
    \+ (between(0, W1, C), \+ gridrun_uniform_col(Grid, C, _)).
...
gridrun_stripe_colors_h(Grid, Colors) :-
    gridrun_dims_(Grid, H, _),
    H1 is H - 1,
    findall(Color, (between(0, H1, R), gridrun_uniform_row(Grid, R, Color)), Colors),
    length(Colors, H).
...
gridrun_stripe_colors_v(Grid, Colors) :-
    gridrun_dims_(Grid, _, W),
    W1 is W - 1,
    findall(Color, (between(0, W1, C), gridrun_uniform_col(Grid, C, Color)), Colors),
    length(Colors, W).
