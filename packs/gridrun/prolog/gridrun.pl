...
grl_list_runs_([], []) :- !.
grl_list_runs_([V|Rest], Runs) :-
    grl_run_acc_(V, 1, Rest, Runs).

% Accumulate a run of V; when the color changes, emit V-N and start new run.
grl_run_acc_(V, N, [], [V-N]) :- !.
grl_run_acc_(V, N, [V|Rest], Runs) :- !,
% Same color: extend the current run.
    N1 is N + 1,
    grl_run_acc_(V, N1, Rest, Runs).
grl_run_acc_(V, N, [W|Rest], [V-N|Runs]) :-
% Different color: emit run, start fresh.
    grl_run_acc_(W, 1, Rest, Runs).

% Extract column C of Grid as a flat list.
grl_col_list_(Grid, C, Col) :-
    grl_dims_(Grid, H, _),
    H1 is H - 1,
...
grl_all_row_runs(Grid, AllRuns) :-
    grl_dims_(Grid, H, _),
    H1 is H - 1,
    findall(Runs, (between(0, H1, R), grl_row_runs(Grid, R, Runs)), AllRuns).
...
grl_all_col_runs(Grid, AllRuns) :-
    grl_dims_(Grid, _, W),
    W1 is W - 1,
    findall(Runs, (between(0, W1, C), grl_col_runs(Grid, C, Runs)), AllRuns).
...
grl_is_striped_h(Grid) :-
    grl_dims_(Grid, H, _),
    H1 is H - 1,
    \+ (between(0, H1, R), \+ grl_uniform_row(Grid, R, _)).
...
grl_is_striped_v(Grid) :-
    grl_dims_(Grid, _, W),
    W1 is W - 1,
    \+ (between(0, W1, C), \+ grl_uniform_col(Grid, C, _)).
...
grl_stripe_colors_h(Grid, Colors) :-
    grl_dims_(Grid, H, _),
    H1 is H - 1,
    findall(Color, (between(0, H1, R), grl_uniform_row(Grid, R, Color)), Colors),
    length(Colors, H).
...
grl_stripe_colors_v(Grid, Colors) :-
    grl_dims_(Grid, _, W),
    W1 is W - 1,
    findall(Color, (between(0, W1, C), grl_uniform_col(Grid, C, Color)), Colors),
    length(Colors, W).
