...
% gridalign_nonbg_cells_/3: collect all p(R,C) positions of non-bg cells in Grid.
gridalign_nonbg_cells_(Grid, Bg, Cells) :-
    length(Grid, H),
    (H > 0 ->
...
% gridalign_floor_avg_/2: integer floor-average of a non-empty list.
gridalign_floor_avg_(List, Avg) :-
    length(List, N), N > 0,
    sumlist(List, S),
    Avg is S // N.

% gridalign_grid_size_/3: (H, W) dimensions. Returns 0,0 for empty grid.
gridalign_grid_size_([], 0, 0) :- !.
gridalign_grid_size_(Grid, H, W) :-
    length(Grid, H),
    Grid = [Row0|_], length(Row0, W).

% --- PUBLIC PREDICATES ---

% gridalign_center_of_mass(+Grid, +Bg, -r(R,C))
...
gridalign_center_of_mass(Grid, Bg, r(R, C)) :-
    ...
    gridalign_floor_avg_(Rs, R),
    gridalign_floor_avg_(Cs, C).

% gridalign_nonbg_bbox(+Grid, +Bg, -BBox)
...
% gridalign_bbox_center(+Grid, +Bg, -Center)
...
% gridalign_translate(+Grid, +DR, +DC, +Bg, -Result)
...
% gridalign_overlap_count(+Grid1, +Grid2, +Bg, -Count)
...
gridalign_overlap_count(Grid1, Grid2, Bg, Count) :-
    gridalign_grid_size_(Grid1, H, W),
    H1 is H - 1, W1 is W - 1,
    findall(1,
...
% gridalign_overlap_score(+Grid1, +Grid2, +Bg, -Score)
% Score = OverlapCount / NonbgCount(Grid1), as a float.
% Score = 0.0 if Grid1 has no non-bg cells.
gridalign_overlap_score(Grid1, Grid2, Bg, Score) :-
    gridalign_nonbg_cells_(Grid1, Bg, Cells1),
    length(Cells1, N1),
    (N1 =:= 0 ->
        Score = 0.0
    ;
        gridalign_overlap_count(Grid1, Grid2, Bg, Ov),
        Score is Ov / N1
    ).

% gridalign_iou(+Grid1, +Grid2, +Bg, -IoU)
...
% gridalign_find_offset(+Grid1, +Grid2, +Bg, +SearchR, -o(BestDR, BestDC))
...
% Collect Grid1 non-bg positions as a set for fast lookup.
    gridalign_nonbg_cells_(Grid1, Bg, S1),
% Collect Grid2 non-bg positions with values.
    gridalign_nonbg_cells_(Grid2, Bg, S2),
    Neg is -SearchR,
    findall(cv(Ov,DR,DC),
...
% Find the best candidate.
    gridalign_pick_best_offset_(Candidates, 0, 0, 0, BestDR, BestDC).

% gridalign_pick_best_offset_/6: fold over candidates, tracking best Ov and (DR,DC).
gridalign_pick_best_offset_([], _, BDR, BDC, BDR, BDC).
gridalign_pick_best_offset_([cv(Ov,DR,DC)|Rest], BestOv, BestDR, BestDC, OutDR, OutDC) :-
    Dist is abs(DR) + abs(DC),
    BestDist is abs(BestDR) + abs(BestDC),
    (   Ov > BestOv
    ;   Ov =:= BestOv, Dist < BestDist
    ) ->
        gridalign_pick_best_offset_(Rest, Ov, DR, DC, OutDR, OutDC)
    ;
        gridalign_pick_best_offset_(Rest, BestOv, BestDR, BestDC, OutDR, OutDC).

% gridalign_align_centers(+Grid1, +Grid2, +Bg, -Aligned)
% Translate Grid2 so its centroid matches Grid1's. Returns Grid2 unchanged
% if either grid has no non-bg cells.
gridalign_align_centers(Grid1, Grid2, Bg, Aligned) :-
    (   gridalign_center_of_mass(Grid1, Bg, r(R1, C1)),
        gridalign_center_of_mass(Grid2, Bg, r(R2, C2))
    ->  DR is R1 - R2, DC is C1 - C2,
        gridalign_translate(Grid2, DR, DC, Bg, Aligned)
    ;   Aligned = Grid2
    ).

% gridalign_align_bbox_centers(+Grid1, +Grid2, +Bg, -Aligned)
% Translate Grid2 so its bbox center matches Grid1's. Returns Grid2 unchanged
% if either grid has no non-bg cells.
gridalign_align_bbox_centers(Grid1, Grid2, Bg, Aligned) :-
    (   gridalign_bbox_center(Grid1, Bg, r(R1, C1)),
        gridalign_bbox_center(Grid2, Bg, r(R2, C2))
    ->  DR is R1 - R2, DC is C1 - C2,
        gridalign_translate(Grid2, DR, DC, Bg, Aligned)
    ;   Aligned = Grid2
    ).

% gridalign_nonbg_offset(+Grid1, +Grid2, +Bg, -o(DR,DC))
...
% gridalign_place_at(+Canvas, +Grid2, +R0, +C0, +Bg, -Result)
...
% gridalign_anchor_to(+Canvas, +Grid2, +PR, +PC, +QR, +QC, +Bg, -Result)
...
% gridalign_match_any_offset(+Grid1, +Grid2, +Bg, -Offsets)
