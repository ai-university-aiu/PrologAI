...
test(above_basic) :-
    ...
    lq_above([R, B, BB], B, Found),
    Found == [R].

test(above_none) :-
    red_dot(R), red_bar(B),
    % Ref is red_dot at row 0; nothing above
    lq_above([R, B], R, Found),
    Found == [].
...
test(below_basic) :-
    ...
    lq_below([R, B, BB], B, Found),
    Found == [BB].

test(below_none) :-
    blue_bar(BB), red_dot(R),
    lq_below([R, BB], BB, Found),
    Found == [].
...
test(left_of_basic) :-
    ...
    lq_left_of([R, BD], BD, Found),
    Found == [R].

test(left_of_none) :-
    red_dot(R), blue_dot_r(BD),
    lq_left_of([R, BD], R, Found),
    Found == [].
...
test(touching8_includes_diagonal) :-
    ...
    lq_touching8([DN], R, Found),
    Found == [DN].

test(touching8_not_distant) :-
    red_dot(R), yellow_dot_br(Y),
    lq_touching8([Y], R, Found),
    Found == [].
...
test(overlapping_basic) :-
    ...
    lq_overlapping([Shared], R, Found),
    Found == [Shared].

test(overlapping_none) :-
    red_dot(R), blue_dot_r(BD),
    lq_overlapping([BD], R, Found),
    Found == [].
...
test(nearest_basic) :-
    ...
    lq_nearest([BD, Y], R, Nearest),
    Nearest == BD.

test(nearest_single) :-
    red_dot(R), blue_dot_r(BD),
    lq_nearest([BD], R, Nearest),
    Nearest == BD.
...
test(farthest_basic) :-
    ...
    lq_farthest([BD, Y], R, Farthest),
    Farthest == Y.

test(farthest_single) :-
    red_dot(R), green_dot_b(G),
    lq_farthest([G], R, Farthest),
    Farthest == G.
...
test(n_touching4_basic) :-
    ...
    lq_n_touching4([TB, TR, DN], R, N),
    N == 2.

test(n_touching4_none) :-
    red_dot(R), yellow_dot_br(Y),
    lq_n_touching4([Y], R, N),
    N == 0.
...
test(n_touching4_all) :-
    ...
    LeftN = obj(w, [r(1,0)]),
    RightN = obj(w, [r(1,3)]),
    lq_n_touching4([TopN, BotN, LeftN, RightN], A, N),
    N == 4.
