:- use_module('../prolog/objlocate').
:- use_module(library(plunit)).
:- use_module(library(lists), [memberchk/2]).

:- begin_tests(objlocate).
...
test(above_basic) :-
    ...
    objlocate_above([R, B, BB], B, Found),
    Found == [R].

test(above_none) :-
    red_dot(R), red_bar(B),
    % Ref is red_dot at row 0; nothing above
    objlocate_above([R, B], R, Found),
    Found == [].
...
test(above_multiple) :-
    ...
    objlocate_above([R, B, BB], BB, Found),
    length(Found, 2).
...
test(below_basic) :-
    ...
    objlocate_below([R, B, BB], B, Found),
    Found == [BB].

test(below_none) :-
    blue_bar(BB), red_dot(R),
    objlocate_below([R, BB], BB, Found),
    Found == [].
...
test(left_of_basic) :-
    ...
    objlocate_left_of([R, BD], BD, Found),
    Found == [R].

test(left_of_none) :-
    red_dot(R), blue_dot_r(BD),
    objlocate_left_of([R, BD], R, Found),
    Found == [].
...
test(right_of_basic) :-
    red_dot(R),      % col 0
    blue_dot_r(BD),  % col 2
    objlocate_right_of([R, BD], R, Found),
    Found == [BD].
...
test(touching4_basic) :-
    ...
    objlocate_touching4([TB, TR, DN], R, Found),
    length(Found, 2),
    memberchk(TB, Found),
    memberchk(TR, Found).
...
test(touching8_includes_diagonal) :-
    ...
    objlocate_touching8([DN], R, Found),
    Found == [DN].

test(touching8_not_distant) :-
    red_dot(R), yellow_dot_br(Y),
    objlocate_touching8([Y], R, Found),
    Found == [].
...
test(overlapping_basic) :-
    ...
    objlocate_overlapping([Shared], R, Found),
    Found == [Shared].

test(overlapping_none) :-
    red_dot(R), blue_dot_r(BD),
    objlocate_overlapping([BD], R, Found),
    Found == [].
...
test(same_color_basic) :-
    ...
    objlocate_same_color([R, RB, BB], R, Found),
    length(Found, 2),
    memberchk(R, Found),
    memberchk(RB, Found).
...
test(same_color_none) :-
    ...
    objlocate_same_color([BB, BD], R, Found),
    Found == [].
...
test(same_form_basic) :-
    ...
    objlocate_same_form([L, LF, R], L, Found),
    length(Found, 2),
    memberchk(L, Found),
    memberchk(LF, Found).
...
test(same_form_none) :-
    ...
    objlocate_same_form([RB], R, Found),
    Found == [].
...
test(aligned_h_basic) :-
    ...
    objlocate_aligned_h([R, BD, G], R, Found),
    length(Found, 2),
    memberchk(R, Found),
    memberchk(BD, Found).
...
test(aligned_v_basic) :-
    ...
    objlocate_aligned_v([R, G, BD], R, Found),
    length(Found, 2),
    memberchk(R, Found),
    memberchk(G, Found).
...
test(nearest_basic) :-
    ...
    objlocate_nearest([BD, Y], R, Nearest),
    Nearest == BD.

test(nearest_single) :-
    red_dot(R), blue_dot_r(BD),
    objlocate_nearest([BD], R, Nearest),
    Nearest == BD.
...
test(farthest_basic) :-
    ...
    objlocate_farthest([BD, Y], R, Farthest),
    Farthest == Y.

test(farthest_single) :-
    red_dot(R), green_dot_b(G),
    objlocate_farthest([G], R, Farthest),
    Farthest == G.
...
test(n_touching4_basic) :-
    ...
    objlocate_n_touching4([TB, TR, DN], R, N),
    N == 2.

test(n_touching4_none) :-
    red_dot(R), yellow_dot_br(Y),
    objlocate_n_touching4([Y], R, N),
    N == 0.
...
test(n_touching4_all) :-
    ...
    LeftN = obj(w, [r(1,0)]),
    RightN = obj(w, [r(1,3)]),
    objlocate_n_touching4([TopN, BotN, LeftN, RightN], A, N),
    N == 4.
...
test(below_multiple) :-
    ...
    red_bar(B),   % row 1
    blue_bar(BB), % row 3
    objlocate_below([R, B, BB], R, Found),
    length(Found, 2).
...
test(touching8_basic_multi) :-
    ...
    objlocate_touching8([TB, TR, DN], R, Found),
    length(Found, 3).

test(overlapping_self) :-
    red_dot(R),
    objlocate_overlapping([R], R, Found),
    Found == [R].

test(same_color_empty) :-
    red_dot(R),
    objlocate_same_color([], R, Found),
    Found == [].

test(same_form_dots_all) :-
    red_dot(R), blue_dot_r(BD), green_dot_b(G),
    % all are single-dot form
    objlocate_same_form([R, BD, G], R, Found),
    length(Found, 3).

test(aligned_v_empty) :-
    red_dot(R),
    objlocate_aligned_v([], R, Found),
    Found == [].
...
test(nearest_three) :-
    ...
    objlocate_nearest([BD, G, Y], R, Nearest),
