...
test(invert_basic) :-
    mask_3x3(M),
    noise_mask_invert(M, 3, Inv),
    Inv = [[0,1,0],[1,0,1],[0,1,0]].

test(invert_all_zeros) :-
    M = [[0,0],[0,0]],
    noise_mask_invert(M, 2, Inv),
    Inv = [[1,1],[1,1]].
...
test(and_basic) :-
    ...
    noise_mask_and(M1, M2, R),
    R = [[0,0,0],[0,0,0],[0,0,0]].

test(and_with_self) :-
    % AND with self = self.
    mask_3x3(M),
    noise_mask_and(M, M, R),
    R = M.
...
test(mask_from_color_absent) :-
    ...
    G = [[1,1],[1,1]],
    noise_mask_from_color(G, 9, M),
    M = [[0,0],[0,0]].

test(mask_from_color_all) :-
    % Uniform grid: all ones.
    G = [[3,3],[3,3]],
    noise_mask_from_color(G, 3, M),
    M = [[1,1],[1,1]].
...
test(to_region_empty) :-
    ...
    noise_mask_to_region([[0,0],[0,0]], R),
    R = [].
...
test(region_to_mask_basic) :-
    ...
    noise_region_to_mask(Region, 3, 3, M),
    M = [[1,0,1],[0,0,0],[1,0,1]].

test(region_to_mask_empty) :-
    noise_region_to_mask([], 2, 2, M),
    M = [[0,0],[0,0]].
...
test(color_count_basic) :-
    color_grid(G),
    noise_color_count(G, 2, N),
    N =:= 4.

test(color_count_absent) :-
    G = [[1,1],[1,1]],
    noise_color_count(G, 9, N),
    N =:= 0.

test(color_count_all) :-
    G = [[3,3],[3,3]],
    noise_color_count(G, 3, N),
    N =:= 4.
