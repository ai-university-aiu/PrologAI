...
test(by_color_single) :-
    scene(Objs),
    of_by_color(Objs, r, Filtered),
    length(Filtered, 1),
    Filtered = [obj(r, _)].

test(by_color_none) :-
    scene(Objs),
    of_by_color(Objs, z, Filtered),
    Filtered = [].

test(by_color_two) :-
    Objs = [obj(a, [r(0,0)]), obj(b, [r(0,1)]), obj(a, [r(0,2)])],
    of_by_color(Objs, a, Filtered),
    length(Filtered, 2).

% of_not_color/3 tests
test(not_color_excludes_one) :-
    scene(Objs),
    of_not_color(Objs, r, Filtered),
    length(Filtered, 6),
    \+ member(obj(r, _), Filtered).

test(not_color_none_match) :-
    scene(Objs),
    of_not_color(Objs, z, Filtered),
    length(Filtered, 7).

test(not_color_all_excluded) :-
    Objs = [obj(a, [r(0,0)]), obj(a, [r(1,0)])],
    of_not_color(Objs, a, Filtered),
    Filtered = [].

% of_exact_size/3 tests
test(exact_size_1) :-
    scene(Objs),
    of_exact_size(Objs, 1, Filtered),
    length(Filtered, 1),
    Filtered = [obj(r, _)].

test(exact_size_3) :-
    scene(Objs),
    of_exact_size(Objs, 3, Filtered),
    % hbar, vbar, L all have 3 cells
    length(Filtered, 3).

test(exact_size_8) :-
    scene(Objs),
    of_exact_size(Objs, 8, Filtered),
    % frame and big both have 8 cells
    length(Filtered, 2).

test(exact_size_zero) :-
    scene(Objs),
    of_exact_size(Objs, 9, Filtered),
    Filtered = [].

% of_min_size/3 tests
test(min_size_4) :-
    scene(Objs),
    of_min_size(Objs, 4, Filtered),
    % rect (4), frame (8), big (8)
    length(Filtered, 3).

test(min_size_8) :-
    scene(Objs),
    of_min_size(Objs, 8, Filtered),
    % frame and big
    length(Filtered, 2).

test(min_size_1) :-
    scene(Objs),
    of_min_size(Objs, 1, Filtered),
    % all 7 objects
    length(Filtered, 7).
...
test(max_size_3) :-
    scene(Objs),
    of_max_size(Objs, 3, Filtered),
    % dot (1), hbar (3), vbar (3), L (3)
    length(Filtered, 4).

test(max_size_1) :-
    scene(Objs),
    of_max_size(Objs, 1, Filtered),
    % only dot
    length(Filtered, 1).

test(max_size_8) :-
    scene(Objs),
    of_max_size(Objs, 8, Filtered),
    % all 7 objects
    length(Filtered, 7).
...
test(is_vline_explicit) :-
    ...
    of_is_vline([V], Filtered),
    Filtered = [V].

% of_is_single/2 tests
...
% of_filter/3 tests (meta-predicate with custom Goal)
is_blue(obj(b, _)).

test(filter_custom_blue) :-
    scene(Objs),
    of_filter(Objs, is_blue, Filtered),
    Filtered = [obj(b, _)].
