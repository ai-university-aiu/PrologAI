...
% Scale up grid by integer factor F (each cell becomes F x F block).
    gridresize_scale_up/3,
% Scale down by integer factor F, sampling top-left of each block.
    gridresize_scale_down/4,
% Scale down by integer factor F using mode (most common value) of each block.
    gridresize_scale_down_mode/4,
% Scale up by exactly 2 (shorthand for gridresize_scale_up with F=2).
    gridresize_double/2,
...
% gridresize_dims_/3: (H, W) of a grid; W=0 for empty grid.
gridresize_dims_(Grid, H, W) :-
    length(Grid, H),
    ( H > 0 -> Grid = [Row0|_], length(Row0, W) ; W = 0 ).

% gridresize_cell_/4: value at (R, C) in Grid.
gridresize_cell_(Grid, R, C, V) :-
    nth0(R, Grid, Row), nth0(C, Row, V).

% gridresize_count_/3: count occurrences of Val in list.
gridresize_count_([], _, 0).
gridresize_count_([H|T], Val, N) :-
    gridresize_count_(T, Val, N0),
    ( H = Val -> N is N0 + 1 ; N = N0 ).

% gridresize_mode_/2: most common element; on ties the msort-first value wins.
gridresize_mode_(List, Mode) :-
    sort(List, Vals),
    findall(neg(Neg,V), (member(V, Vals), gridresize_count_(List, V, N), Neg is -N), Keyed),
...
gridresize_scale_down(Grid, F, _Bg, Result) :-
    gridresize_dims_(Grid, H, W),
    NewH is H // F, NewW is W // F,
    ( NewH > 0, NewW > 0 ->
        NewH1 is NewH - 1, NewW1 is NewW - 1,
...
gridresize_scale_down_mode(Grid, F, _Bg, Result) :-
    gridresize_dims_(Grid, H, W),
    NewH is H // F, NewW is W // F,
    ( NewH > 0, NewW > 0 ->
                 ...
                 (between(0, NewW1, C),
                  ...
                  findall(BV,
                      (between(BR, BRE, SR), between(BC, BCE, SC),
                       SR < H, SC < W,
                       gridresize_cell_(Grid, SR, SC, BV)),
                      Block),
                  ( Block = [] ->
                      gridresize_cell_(Grid, BR, BC, V)
                  ;
                      gridresize_mode_(Block, V)
                  )),
                 Row)),
            Result)
    ;
...
% Outer parens ensure gridresize_dims_ runs before the if-then-else.
gridresize_resize(Grid, NewH, NewW, Bg, Result) :-
    gridresize_dims_(Grid, H, W),
    (   ( NewH =:= 0 ; NewW =:= 0 )
    ->  Result = []
...
gridresize_crop(Grid, R0, C0, R1, C1, Result) :-
    ...
    ;   findall(Row,
            (between(ClR0, ClR1, R),
             findall(V,
                 (between(ClC0, ClC1, C), gridresize_cell_(Grid, R, C, V)),
                 Row)),
            Result)
    ).
...
gridresize_fit_in(Grid, H, W, Bg, Result) :-
    ...
    ;   ScaleH is H / H0, ScaleW is W / W0,
        ...
        gridresize_resize(Grid, NewH, NewW, Bg, Scaled),
        gridresize_embed_in(Scaled, H, W, Bg, Result)
    ).
