:- module(grid_position, [
    grid_position_top_half/2,
    grid_position_bottom_half/2,
    grid_position_left_half/2,
    grid_position_right_half/2,
    grid_position_quadrant/3,
    grid_position_even_rows/2,
    grid_position_odd_rows/2,
    grid_position_even_cols/2,
    grid_position_odd_cols/2,
    grid_position_checkerboard/3,
    grid_position_center_cell/3,
    grid_position_corners/2,
    grid_position_cross_h/3,
    grid_position_cross_v/3
]).
...
grid_position_subgrid_(Grid, R0, R1, C0, C1, Sub) :-
...
grid_position_row_parity_(Row, Parity, SubRow) :-
...
% grid_position_top_half(+Grid, -TopGrid)
...
% grid_position_bottom_half(+Grid, -BotGrid)
...
% grid_position_left_half(+Grid, -LeftGrid)
% LeftGrid is the sub-grid consisting of columns [0, W//2).
grid_position_left_half(Grid, LeftGrid) :-
% Compute column midpoint.
    grid_position_dims_(Grid, _, W), WMid is W // 2,
...
% grid_position_quadrant(+Grid, +Quad, -SubGrid)
...
% Handle degenerate ranges (empty row or col span).
    (R0 > R1 -> SubGrid = [] ;
     ...
     ;
        grid_position_subgrid_(Grid, R0, R1, C0, C1, SubGrid)).

% grid_position_even_rows(+Grid, -EvenGrid)
...
% grid_position_odd_rows(+Grid, -OddGrid)
...
% grid_position_corners(+Grid, -Corners)
...
% grid_position_cross_h(+Grid, -R, -Row)
...
% grid_position_cross_v(+Grid, -C, -Col)
