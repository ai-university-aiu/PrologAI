...
test(nonbg_rows_one) :-
    project_nonbg_rows([[0,0],[1,0],[0,0]], 0, Rows),
    Rows = [1].

% All rows are non-BG.
test(nonbg_rows_all) :-
    project_nonbg_rows([[1,2],[3,4]], 0, Rows),
    Rows = [0,1].

% All BG: no non-BG rows.
test(nonbg_rows_none) :-
    project_nonbg_rows([[0,0],[0,0]], 0, Rows),
    Rows = [].
...
% One non-BG column.
test(nonbg_cols_one) :-
    project_nonbg_cols([[0,1,0],[0,0,0]], 0, Cols),
    Cols = [1].

% All columns are non-BG.
test(nonbg_cols_all) :-
    project_nonbg_cols([[1,0,1],[0,2,0]], 0, Cols),
    Cols = [0,1,2].

% All BG: empty list.
test(nonbg_cols_none) :-
    project_nonbg_cols([[0,0],[0,0]], 0, Cols),
    Cols = [].
...
test(row_counts_mixed) :-
    project_row_counts([[1,0,1],[0,0,0],[0,2,0]], 0, Counts),
    Counts = [2,0,1].

% All BG rows: all zero.
test(row_counts_all_bg) :-
    project_row_counts([[0,0],[0,0]], 0, Counts),
    Counts = [0,0].

% Full row: count equals row width.
test(row_counts_full_row) :-
    project_row_counts([[1,2,3]], 0, Counts),
    Counts = [3].
...
test(col_first_row1) :-
    project_col_first([[0,0],[1,0],[0,0]], 0, 0, R),
    R =:= 1.

% Non-BG appears at last row.
test(col_first_last_row) :-
    project_col_first([[0,0],[0,0],[0,1]], 1, 0, R),
    R =:= 2.

% Non-BG at row 0 (topmost position).
test(col_first_row0) :-
    project_col_first([[1,0],[0,0]], 0, 0, R),
    R =:= 0.
...
test(col_last_row1) :-
    project_col_last([[1,0],[1,0],[0,0]], 0, 0, R),
    R =:= 1.

% Non-BG cells at rows 0 and 2; last is row 2.
test(col_last_row2) :-
    project_col_last([[0,1],[0,0],[0,1]], 1, 0, R),
    R =:= 2.

% Single non-BG cell at bottom row.
test(col_last_bottom) :-
    project_col_last([[0,0],[0,0],[1,0]], 0, 0, R),
    R =:= 2.
...
test(row_first_col2) :-
    project_row_first([[0,0,1,0]], 0, 0, C),
    C =:= 2.

% In row 1 the first non-BG is at column 3.
test(row_first_row1) :-
    project_row_first([[0,1,0,1],[0,0,0,2]], 1, 0, C),
    C =:= 3.

% Non-BG at column 0 (leftmost position).
test(row_first_col0) :-
    project_row_first([[1,0,0]], 0, 0, C),
    C =:= 0.
