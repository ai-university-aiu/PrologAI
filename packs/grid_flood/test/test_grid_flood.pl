...
% --- grid_flood_region_cells tests ---

% Cells of 2x2 r block in g3x3_rb.
test(region_cells_block) :-
    g3x3_rb(G),
    grid_flood_region_cells(G, 0, 0, Cells),
    msort(Cells, S),
...
test(region_cells_all) :-
    g3x3_r(G),
    grid_flood_region_cells(G, 1, 1, Cells),
    length(Cells, 9).

% --- grid_flood_region_size tests ---

% 2x2 red block.
test(region_size_4) :-
    g3x3_rb(G),
    grid_flood_region_size(G, 0, 0, N),
    N = 4.
...
% --- grid_flood_region_bbox tests ---

% 2x2 red block in g3x3_rb: bbox rows 0-1, cols 0-1.
test(region_bbox_block) :-
    g3x3_rb(G),
    grid_flood_region_bbox(G, 0, 0, MinR, MinC, MaxR, MaxC),
...
test(region_bbox_l_shape) :-
    g3x3_rb(G),
    grid_flood_region_bbox(G, 0, 2, MinR, MinC, MaxR, MaxC),
    MinR = 0, MinC = 0, MaxR = 2, MaxC = 2.

% --- grid_flood_enclosed_cells tests ---

% b cells inside the ring in g4x4_ring are enclosed.
test(enclosed_cells_ring) :-
    g4x4_ring(G),
    grid_flood_enclosed_cells(G, b, Enclosed),
    msort(Enclosed, S),
...
test(enclosed_cells_center) :-
    g5x5_hole(G),
    grid_flood_enclosed_cells(G, r, Enclosed),
    Enclosed = [2-2].

% --- grid_flood_fill_enclosed tests ---

% Fill enclosed b cells in g4x4_ring with r.
test(fill_enclosed_fills_holes) :-
    g4x4_ring(G),
    grid_flood_fill_enclosed(G, b, r, R),
...
test(components_empty) :-
    g3x3_r(G),
    grid_flood_components(G, b, Comps),
    Comps = [].
...
% --- grid_flood_is_connected tests ---

% 3x3 all-red is 4-connected.
test(is_connected_4_true) :-
    g3x3_r(G),
    grid_flood_is_connected(G, r, 4).
...
test(region_cells_then_fill) :-
    ...
    grid_flood_region_cells(G, 0, 0, Cells),
    length(Cells, 4),
    grid_flood_fill(G, 0, 0, x, R),
    grid_flood_n_components(R, r, 0),
    grid_flood_n_components(R, x, 1).

:- end_tests(grid_flood).
