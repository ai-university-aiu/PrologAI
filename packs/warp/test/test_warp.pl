...
test(shear_h_step0) :-
    wr_shear_h([[a,b,c],[d,e,f]], 0, 0, Out),
    Out == [[a,b,c],[d,e,f]].

% Test 13: shear horizontal step=2; row 2 shifts by 4 and loses all content.
test(shear_h_step2) :-
    wr_shear_h([[a,b,c,d],[e,f,g,h],[i,j,k,l]], 2, 0, Out),
...
test(shear_v_step0) :-
    wr_shear_v([[a,b,c],[d,e,f],[g,h,i]], 0, 0, Out),
...
test(unshear_h_step2_round_trip) :-
    ...
    wr_shear_h(Grid, 2, 0, Sh),
    wr_unshear_h(Sh, 2, 0, Back),
    Back == Grid.
...
test(unshear_v_round_trip) :-
    ...
    wr_shear_v(Grid, 1, 0, Sh),
    wr_unshear_v(Sh, 1, 0, Back),
    Back == Grid.
...
test(cyclic_v_shift_h) :-
    wr_cyclic_v([[1,2],[3,4],[5,6]], 3, Out),
    Out == [[1,2],[3,4],[5,6]].

% Test 26: cyclic shift by 0 = identity.
test(cyclic_v_shift0) :-
    wr_cyclic_v([[1,2],[3,4],[5,6]], 0, Out),
    Out == [[1,2],[3,4],[5,6]].

% wr_cyclic_shear_h tests.
...
test(cyclic_shear_h_step0) :-
    wr_cyclic_shear_h([[1,2,3],[4,5,6]], 0, Out),
    Out == [[1,2,3],[4,5,6]].

% wr_cyclic_shear_v tests.
...
test(cyclic_shear_v_step0) :-
    wr_cyclic_shear_v([[1,2,3],[4,5,6]], 0, Out),
    Out == [[1,2,3],[4,5,6]].

% wr_skew_offsets tests.

% Test 31: offsets [0,1,2] equivalent to shear_h step=1.
test(skew_offsets_shear_equiv) :-
    Grid = [[a,b,c],[d,e,f],[g,h,i]],
    wr_skew_offsets(Grid, [0,1,2], 0, Out),
    wr_shear_h(Grid, 1, 0, Expected),
    Out == Expected.

% Test 32: all-zero offsets = identity.
test(skew_offsets_all_zero) :-
    wr_skew_offsets([[a,b,c],[d,e,f]], [0,0], 0, Out),
    Out == [[a,b,c],[d,e,f]].
...
test(skew_offsets_step2_equiv) :-
    ...
    wr_skew_offsets(Grid, [0,2,4], 0, Out),
    wr_shear_h(Grid, 2, 0, Expected),
    Out == Expected.

% wr_transpose_anti tests.
...
test(transpose_anti_involution) :-
    ...
    wr_transpose_anti(Grid, Temp),
    wr_transpose_anti(Temp, Back),
    Back == Grid.

% wr_find_shear_h tests.

% Test 39: find_shear_h identifies step=1 from a known shear result.
test(find_shear_h_step1) :-
    Grid = [[1,2,3],[4,5,6],[7,8,9]],
    wr_shear_h(Grid, 1, 0, Sheared),
    wr_find_shear_h(Grid, Sheared, 0, Step),
    Step == 1.

% Test 40: find_shear_h finds step=0 when grids are identical.
test(find_shear_h_step0) :-
    Grid = [[1,2],[3,4]],
    wr_find_shear_h(Grid, Grid, 0, Step),
    Step == 0.
