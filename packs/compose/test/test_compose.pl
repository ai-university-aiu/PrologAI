...
% is_small(+Grid): Grid has at most 2 rows (used for cp_until termination).
...
% cp_apply dispatches to the named rule correctly.
test(apply_reflect_h) :-
    grid_2x2(G),
    cp_apply(gd_reflect_h, G, G2),
    gd_equal(G2, [[3,4],[1,2]]).

% cp_apply works with a partial application (cp_const).
test(apply_const) :-
    grid_2x2(G),
    cp_apply(cp_const([[9,9],[9,9]]), G, G2),
    gd_equal(G2, [[9,9],[9,9]]).

% cp_identity returns the same grid.
test(identity) :-
    grid_2x2(G),
    cp_identity(G, G2),
    gd_equal(G, G2).

% cp_const ignores the input and returns the constant.
test(const) :-
    grid_2x2(G),
    cp_const([[0,0],[0,0]], G, G2),
    gd_equal(G2, [[0,0],[0,0]]).
...
test(pipe_two) :-
    grid_3x3(G),
    cp_pipe([gd_rotate90, gd_rotate90], G, G2),
...
test(pipe_three_reflect) :-
    grid_2x2(G),
    cp_pipe([gd_reflect_h, gd_reflect_h, gd_reflect_h], G, G2),
    gd_reflect_h(G, G2Expected),
    gd_equal(G2, G2Expected).
...
% cp_pipe_n 0 times is identity.
test(pipe_n_zero) :-
    grid_2x2(G),
    cp_pipe_n(gd_reflect_h, 0, G, G2),
    gd_equal(G, G2).

% cp_pipe_n 1 time = single apply.
test(pipe_n_one) :-
    grid_2x2(G),
    cp_pipe_n(gd_reflect_h, 1, G, G2),
    gd_reflect_h(G, G2Expected),
    gd_equal(G2, G2Expected).

% cp_pipe_n 4 rotations of 90 degrees = identity.
test(pipe_n_four_rotations) :-
    grid_3x3(G),
    cp_pipe_n(gd_rotate90, 4, G, G2),
    gd_equal(G, G2).

% cp_pipe_n 2 reflect_h = identity.
test(pipe_n_two_reflect) :-
    grid_2x2(G),
    cp_pipe_n(gd_reflect_h, 2, G, G2),
    gd_equal(G, G2).
...
% cp_repeat 0 = identity.
test(repeat_zero) :-
    grid_2x2(G),
    cp_repeat(gd_reflect_h, 0, G, G2),
    gd_equal(G, G2).

% cp_repeat 2 with reflect_h = identity.
test(repeat_two_reflect) :-
    grid_2x2(G),
    cp_repeat(gd_reflect_h, 2, G, G2),
    gd_equal(G, G2).

% cp_repeat 4 with rotate90 = identity.
test(repeat_four_rotate) :-
    grid_3x3(G),
    cp_repeat(gd_rotate90, 4, G, G2),
    gd_equal(G, G2).
...
% cp_until stops immediately when condition already holds.
test(until_already_done) :-
    grid_2x2(G),
% is_small/1 holds for a 2-row grid.
    cp_until(gd_rotate90, is_small, G, G2),
    gd_equal(G, G2).

% cp_until converges after applying the rule once (rotate 3x3 -> 3x3; is_small never holds).
...
% Condition: grid equals [[1,2],[3,4]] which is already true at start.
    cp_until(gd_reflect_h, gd_equal(G), G, G2),
    gd_equal(G, G2).
...
% cp_fixed_point on identity rule converges in one step.
test(fixed_point_identity) :-
    grid_2x2(G),
    cp_fixed_point(cp_identity, G, G2),
    gd_equal(G, G2).

% cp_fixed_point with reflect_h applied twice = identity, so converges after two steps.
...
% cp_map_rows applies the rule to each row independently.
test(map_rows_add1) :-
    grid_2x2(G),
    cp_map_rows(add1_row, G, G2),
    gd_cell(G2, 0, 0, 2),
    gd_cell(G2, 0, 1, 3),
    gd_cell(G2, 1, 0, 4),
    gd_cell(G2, 1, 1, 5).

% cp_map_rows with reverse reverses each row independently.
test(map_rows_reverse) :-
    grid_2x3(G),
    cp_map_rows(rev_list, G, G2),
    gd_equal(G2, [[3,2,1],[6,5,4]]).

% cp_map_rows on a uniform grid with add1 increments all cells.
...
% cp_map_cols with reverse reverses each column independently.
test(map_cols_reverse) :-
    grid_2x2(G),
% Reversing each column of [[1,2],[3,4]] gives [[3,4],[1,2]] = same as gd_reflect_h.
    cp_map_cols(rev_list, G, G2),
    gd_reflect_h(G, GExpected),
    gd_equal(G2, GExpected).

% cp_map_cols with identity on each column gives back the original grid.
test(map_cols_identity) :-
    grid_3x3(G),
    cp_map_cols(cp_identity, G, G2),
    gd_equal(G, G2).
...
% cp_zip with max_cell: result cell = max of the two input cells.
test(zip_max) :-
    GridA = [[1,4],[3,2]],
    GridB = [[2,3],[1,5]],
    cp_zip(max_cell, GridA, GridB, GridC),
    gd_equal(GridC, [[2,4],[3,5]]).

% cp_zip with min_cell: result cell = min of the two input cells.
test(zip_min) :-
    GridA = [[1,4],[3,2]],
    GridB = [[2,3],[1,5]],
    cp_zip(min_cell, GridA, GridB, GridC),
    gd_equal(GridC, [[1,3],[1,2]]).

% cp_zip of a grid with itself using max = identity.
test(zip_self_max) :-
    grid_2x2(G),
    cp_zip(max_cell, G, G, G2),
    gd_equal(G, G2).

% cp_zip with add_cell: elementwise addition.
test(zip_add) :-
    GridA = [[1,2],[3,4]],
    GridB = [[10,20],[30,40]],
    cp_zip(add_cell, GridA, GridB, GridC),
...
% cp_fold over empty list returns the initial accumulator.
test(fold_empty, nondet) :-
    grid_2x2(G),
    cp_fold(overlay_grids, G, [], G2),
    gd_equal(G, G2).

% cp_fold over one element applies the rule once.
test(fold_one, nondet) :-
    Base = [[0,0],[0,0]],
    Patch = [[1,2],[3,4]],
    cp_fold(overlay_grids, Base, [Patch], Result),
    gd_equal(Result, [[1,2],[3,4]]).

% cp_fold over two elements applies the rule twice in sequence.
test(fold_two, nondet) :-
    Base = [[0,0],[0,0]],
    Patch1 = [[1,0],[0,0]],
    Patch2 = [[0,2],[0,0]],
    cp_fold(overlay_grids, Base, [Patch1, Patch2], Result),
