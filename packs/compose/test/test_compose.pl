...
is_uniform(Grid) :- grid_colors(Grid, [_]).
% is_small(+Grid): Grid has at most 2 rows (used for compose_until termination).
is_small(Grid) :- grid_size(Grid, Rows, _), Rows =< 2.
% overlay_grids(+G1, +G2, -G3): overlay G2 onto G1 at (0,0).
overlay_grids(G1, G2, G3) :- grid_size(G2, R, C), R1 is R-1, C1 is C-1,
    grid_crop(G2, 0, 0, R1, C1, Patch), grid_overlay(G1, Patch, 0, 0, G3).
...
double_rotate(G, G2) :- grid_rotate90(G, G1), grid_rotate90(G1, G2).
...
test(pipe_single) :-
    grid_2x2(G),
    compose_pipe([grid_reflect_h], G, G2),
    grid_reflect_h(G, G2Expected),
    grid_equal(G2, G2Expected).

% Two-rule pipeline: rotate90 then rotate90 = rotate180.
test(pipe_two) :-
    grid_3x3(G),
    compose_pipe([grid_rotate90, grid_rotate90], G, G2),
    grid_rotate180(G, G2Expected),
    grid_equal(G2, G2Expected).

% Three-rule pipeline: reflect_h three times = reflect_h (odd count).
test(pipe_three_reflect) :-
    grid_2x2(G),
    compose_pipe([grid_reflect_h, grid_reflect_h, grid_reflect_h], G, G2),
    grid_reflect_h(G, G2Expected),
    grid_equal(G2, G2Expected).
...
test(pipe_n_zero) :-
    grid_2x2(G),
    compose_pipe_n(grid_reflect_h, 0, G, G2),
    grid_equal(G, G2).

% compose_pipe_n 1 time = single apply.
test(pipe_n_one) :-
    grid_2x2(G),
    compose_pipe_n(grid_reflect_h, 1, G, G2),
    grid_reflect_h(G, G2Expected),
    grid_equal(G2, G2Expected).
...
test(pipe_n_four_rotations) :-
    grid_3x3(G),
    compose_pipe_n(grid_rotate90, 4, G, G2),
    grid_equal(G, G2).

% compose_pipe_n 2 reflect_h = identity.
test(pipe_n_two_reflect) :-
    grid_2x2(G),
    compose_pipe_n(grid_reflect_h, 2, G, G2),
    grid_equal(G, G2).
...
% is_uniform/1 succeeds on uniform grid; ThenRule = reflect_h, ElseRule = reflect_v.
    compose_branch(is_uniform, grid_reflect_h, grid_reflect_v, G, G2),
...
% is_small/1 holds for a 2-row grid.
    compose_until(grid_rotate90, is_small, G, G2),
    grid_equal(G, G2).
...
% Condition: grid equals [[1,2],[3,4]] which is already true at start.
    compose_until(grid_reflect_h, grid_equal(G), G, G2),
    grid_equal(G, G2).
...
test(map_cols_reverse) :-
    grid_2x2(G),
% Reversing each column of [[1,2],[3,4]] gives [[3,4],[1,2]] = same as grid_reflect_h.
    compose_map_cols(rev_list, G, G2),
    grid_reflect_h(G, GExpected),
    grid_equal(G2, GExpected).
...
% Second overlay writes 2 at (0,1) on top of first result.
    grid_cell(Result, 0, 0, 1),
    grid_cell(Result, 0, 1, 2).
