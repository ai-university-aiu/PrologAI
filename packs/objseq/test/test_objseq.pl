...
test(color_seq_basic) :-
    ...
    objseq_color_seq([A,B,C], Colors),
    Colors == [r,b,g].

test(color_seq_single) :-
    bar2(X),
    objseq_color_seq([X], Colors),
    Colors == [b].

test(color_seq_empty) :-
    objseq_color_seq([], Colors),
    Colors == [].
...
test(size_seq_basic) :-
    ...
    objseq_size_seq([A,B,C], Sizes),
    Sizes == [2,3,4].

test(size_seq_single) :-
    dot1(X),
    objseq_size_seq([X], Sizes),
    Sizes == [1].

test(size_seq_empty) :-
    objseq_size_seq([], Sizes),
    Sizes == [].
...
test(centroid_seq_basic) :-
    dot1(A), dot2(B), dot3(C),
    objseq_centroid_seq([A,B,C], Cs),
    Cs == [r(0,0), r(0,2), r(0,4)].

test(centroid_seq_bar) :-
    bar3(X),
    objseq_centroid_seq([X], Cs),
    Cs == [r(2,1)].

test(centroid_seq_empty) :-
    objseq_centroid_seq([], Cs),
    Cs == [].
...
test(step_seq_basic) :-
    ...
    objseq_step_seq([A,B,C], Steps),
    Steps == [dr(2,0), dr(2,0)].

test(step_seq_one_element) :-
    dot1(X),
    objseq_step_seq([X], Steps),
    Steps == [].

test(step_seq_empty) :-
    objseq_step_seq([], Steps),
    Steps == [].
...
test(is_growing_yes) :-
    bar2(A), bar3(B), bar4(C),
    objseq_is_growing([A,B,C]).

test(is_growing_no_equal, [fail]) :-
    bar2(A), bar2(B),
    objseq_is_growing([A,B]).
...
test(is_shrinking_yes) :-
    bar4(A), bar3(B), bar2(C),
    objseq_is_shrinking([A,B,C]).

test(is_shrinking_no_equal, [fail]) :-
    bar3(A), bar3(B),
    objseq_is_shrinking([A,B]).
...
test(size_period_1) :-
    ...
    C = obj(g,[r(2,0)]),
    objseq_size_period([A,B,C], P, Cycle),
    P == 1, Cycle == [1].
...
test(next_centroid_row) :-
    col0(A), col1(B), col2(C),
    objseq_next_centroid([A,B,C], r(NR,NC)),
    NR == 6, NC == 0.
...
test(zip_colors_basic) :-
    ...
    objseq_zip_colors([A,B,C], [X,Y,Z], Pairs),
    Pairs == [r-b, b-g, g-y].

test(zip_colors_empty) :-
    objseq_zip_colors([], [], Pairs),
    Pairs == [].
