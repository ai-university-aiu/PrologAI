:- use_module('../prolog/scene_compare').
:- use_module(library(plunit)).

:- begin_tests(scene_compare).
...
test(n_objs_basic) :-
    ...
    scene_compare_n_objs([R, B], N),
    N == 2.

test(n_objs_empty) :-
    scene_compare_n_objs([], N),
    N == 0.

test(n_objs_single) :-
    red_dot(R),
    scene_compare_n_objs([R], N),
    N == 1.
...
test(total_cells_basic) :-
    red_dot(R), red_bar(B),
    scene_compare_total_cells([R, B], N),
    N == 4.

test(total_cells_empty) :-
    scene_compare_total_cells([], N),
    N == 0.
...
test(colors_basic) :-
    ...
    scene_compare_colors([R, B, G], Colors),
    Colors == [b, g, r].

test(colors_duplicates_removed) :-
    red_dot(R), red_bar(RB),
    scene_compare_colors([R, RB], Colors),
    Colors == [r].

test(colors_empty) :-
    scene_compare_colors([], Colors),
    Colors == [].
...
test(forms_basic) :-
    red_dot(R), red_bar(RB),
    scene_compare_forms([R, RB], Forms),
...
test(same_forms_true) :-
    ...
    scene_compare_same_forms([R], [B]).

test(same_forms_false) :-
    red_dot(R), red_bar(RB),
    \+ scene_compare_same_forms([R], [RB]).

% --- scene_compare_added_colors/3 ---

test(added_colors_basic) :-
    red_dot(R), blue_dot(B), green_dot(G),
    scene_compare_added_colors([R, B], [R, B, G], Added),
    Added == [g].

test(added_colors_none) :-
    red_dot(R), blue_dot(B),
    scene_compare_added_colors([R, B], [R, B], Added),
    Added == [].

test(added_colors_all_new) :-
    red_dot(R), blue_dot(B),
    scene_compare_added_colors([], [R, B], Added),
    Added == [b, r].
...
test(removed_colors_basic) :-
    red_dot(R), blue_dot(B), green_dot(G),
    scene_compare_removed_colors([R, B, G], [R, B], Removed),
    Removed == [g].

test(removed_colors_none) :-
    red_dot(R), blue_dot(B),
    scene_compare_removed_colors([R, B], [R, B], Removed),
    Removed == [].
...
test(added_forms_basic) :-
    red_dot(R), red_bar(RB),
    scene_compare_added_forms([R], [R, RB], Added),
    length(Added, 1).

test(added_forms_none) :-
    red_dot(R), blue_dot(B),
    % same single-cell form
    scene_compare_added_forms([R], [B], Added),
    Added == [].
...
test(removed_forms_basic) :-
    red_dot(R), red_bar(RB),
    scene_compare_removed_forms([R, RB], [R], Removed),
    length(Removed, 1).
...
test(n_color_change_basic) :-
    ...
    scene_compare_n_color_change([R, B], [R, G], N),
    N == 2.

test(n_color_change_none) :-
    red_dot(R), blue_dot(B),
    scene_compare_n_color_change([R, B], [R, B], N),
    N == 0.

test(n_color_change_only_added) :-
    red_dot(R), blue_dot(B), green_dot(G),
    scene_compare_n_color_change([R, B], [R, B, G], N),
    N == 1.
...
test(any_change_color) :-
    red_dot(R), blue_dot(B), green_dot(G),
    scene_compare_any_change([R, B], [R, G]).

test(any_change_count) :-
    red_dot(R), blue_dot(B), green_dot(G),
    scene_compare_any_change([R, B], [R, B, G]).

test(any_change_form) :-
    red_dot(R), red_bar(RB),
    scene_compare_any_change([R], [RB]).

test(no_change_equal) :-
    red_dot(R), blue_dot(B),
    \+ scene_compare_any_change([R, B], [R, B]).
...
test(no_change_same_colors_forms_count) :-
    ...
    \+ scene_compare_any_change([R, BB], [BD, RB]).

test(total_cells_three) :-
    red_dot(R), red_bar(B), red_sq(S),
    scene_compare_total_cells([R, B, S], N),
    N == 8.
...
test(n_color_change_all_replaced) :-
    ...
    scene_compare_n_color_change([R, B], [G, G2], N),
    N == 3.

test(added_colors_empty_before) :-
    red_dot(R),
    scene_compare_added_colors([], [R], Added),
    Added == [r].

test(removed_forms_none) :-
    red_dot(R), blue_dot(B),
    scene_compare_removed_forms([R], [B], Removed),
    Removed == [].

test(forms_three_distinct) :-
    red_dot(R), red_bar(RB), lshape(L),
    scene_compare_forms([R, RB, L], Forms),
    length(Forms, 3).

:- end_tests(scene_compare).
