...
:- begin_tests(score_exact).

test(exact_same) :-
    % Two identical grids.
    score_exact([[1,2],[3,4]], [[1,2],[3,4]]).
...
test(exact_fail) :-
    ...
    \+ score_exact([[1,2],[3,4]], [[1,2],[3,5]]).

:- end_tests(score_exact).

:- begin_tests(score_cell_match).

test(match_all) :-
    % All cells match.
    score_cell_match([[1,2],[3,4]], [[1,2],[3,4]], N),
    N =:= 4.

test(match_none) :-
    % No cells match.
    score_cell_match([[1,2],[3,4]], [[5,6],[7,8]], N),
    N =:= 0.

test(match_half) :-
    % Half the cells match.
    score_cell_match([[1,2],[3,4]], [[1,9],[3,9]], N),
    N =:= 2.

test(match_single) :-
    % Single cell.
    score_cell_match([[7]], [[7]], N),
    N =:= 1.

:- end_tests(score_cell_match).

:- begin_tests(score_cell_total).

test(total_2x2) :-
    score_cell_total([[1,2],[3,4]], N),
    N =:= 4.

test(total_3x3) :-
    score_cell_total([[1,2,3],[4,5,6],[7,8,9]], N),
    N =:= 9.

test(total_1x1) :-
    score_cell_total([[5]], N),
    N =:= 1.
...
:- end_tests(score_pairs_score).

:- begin_tests(score_perfect).

test(perfect_yes) :-
    % Identity rule produces exact match.
    score_perfect([[1,2],[3,4]], [[1,2],[3,4]], identity_rule_).

test(perfect_no) :-
    % Rule produces wrong output.
    \+ score_perfect([[1,2]], [[0,0]], identity_rule_).

:- end_tests(score_perfect).

:- begin_tests(score_pairs_perfect).

test(pairs_perfect_all) :-
    % Identity rule: all pairs perfect.
    Pairs = [[[1,2]]-[[1,2]], [[3,4]]-[[3,4]]],
    score_pairs_perfect(Pairs, identity_rule_).
...
:- end_tests(score_pairs_perfect).

:- begin_tests(score_rank).
...
test(rank_all_same) :-
    ...
    Expected = [[1,2]],
    score_rank([[[1,2]], [[1,2]]], Expected, Ranked),
    length(Ranked, 2),
    Ranked = [1.0-_|_].

:- end_tests(score_rank).

:- begin_tests(score_best).
...
:- end_tests(score_best).

:- begin_tests(score_threshold).

test(threshold_all_pass) :-
    % All candidates pass 0.5 threshold.
    Expected = [[1,2]],
    score_threshold([[[1,2]], [[1,9]]], Expected, 0.5, Filtered),
    length(Filtered, 2).

test(threshold_none_pass) :-
    % No candidate meets 1.0 threshold when none is perfect.
    Expected = [[1,2]],
    score_threshold([[[5,6]], [[7,8]]], Expected, 1.0, Filtered),
    Filtered = [].

test(threshold_some_pass) :-
    % Only exact matches pass 1.0 threshold.
    Expected = [[1,2]],
    score_threshold([[[1,2]], [[0,0]]], Expected, 1.0, Filtered),
    length(Filtered, 1).

:- end_tests(score_threshold).
