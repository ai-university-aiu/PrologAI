...
% legend_grid_dims_/3: grid dimensions.
legend_grid_dims_(Grid, Rows, Cols) :-
    length(Grid, Rows),
    (Grid = [Row|_] -> length(Row, Cols) ; Cols = 0).

% legend_cell_color_/3: color of a cell in a grid (0-indexed).
legend_cell_color_(Grid, r(R,C), Color) :-
    nth0(R, Grid, Row), nth0(C, Row, Color).

% legend_flood_region_/4: grow a connected region from seed r(R,C) with same color.
legend_flood_region_(Grid, Seed, BgColor, Region) :-
    legend_cell_color_(Grid, Seed, Color),
    Color \= BgColor,
    legend_grid_dims_(Grid, Rows, Cols),
    ...
    findall(r(RR,CC), (member(RR, RowRange), member(CC, ColRange),
                       legend_cell_color_(Grid, r(RR,CC), Color)), AllColorCells),
    legend_bfs_(AllColorCells, [Seed], [], Region).

% legend_bfs_/4: breadth-first search to find 4-connected component.
legend_bfs_(AllCells, [r(R,C)|Queue], Visited, Region) :-
    ...
    legend_bfs_(AllCells, SortedQueue, [r(R,C)|Visited], Region).
legend_bfs_(_, [r(R,C)|Queue], Visited, Region) :-
    member(r(R,C), Visited),
    !,
    legend_bfs_(_, Queue, Visited, Region).
legend_bfs_(_, [], Visited, Region) :-
    sort(Visited, Region).

% legend_region_min_max_/5: min/max row and col of a region.
legend_region_min_max_(Region, MinR, MinC, MaxR, MaxC) :-
...
% legend_touching_/2: succeed if two regions share at least one adjacent cell pair.
legend_touching_(RegA, RegB) :-
    member(r(R,C), RegA),
...
% legend_normalize_shape_/2: translate region to origin (min row and col = 0).
legend_normalize_shape_(Region, Normalized) :-
    findall(R, member(r(R,_), Region), Rows),
    findall(C, member(r(_,C), Region), Cols),
...
% legend_detect_legend(+Grid, +BgColor, -LegendRegion)
...
legend_detect_legend(Grid, BgColor, LegendRegion) :-
    legend_all_regions(Grid, BgColor, Regions),
    include(legend_is_small_(Grid), Regions, SmallRegions),
    SmallRegions \= [],
    (SmallRegions = [LegendRegion] ->
        true
    ;
        include(legend_isolated_(Regions), SmallRegions, Isolated),
        (Isolated = [LegendRegion|_] -> true ;
         SmallRegions = [LegendRegion|_])
    ).

% legend_is_small_/2: helper wrapper for include.
legend_is_small_(Grid, Region) :-
    legend_grid_dims_(Grid, Rows, Cols),
    TotalArea is Rows * Cols,
    legend_region_area(Region, A),
    A * 4 =< TotalArea.

% legend_isolated_/2: helper wrapper for include.
legend_isolated_(AllRegions, Region) :-
    findall(Other, (member(Other, AllRegions), Other \= Region), Others),
    \+ (member(Other, Others), legend_touching_(Region, Other)).

% legend_is_legend(+Region, +ContentRegions)
...
legend_is_legend(Region, ContentRegions) :-
    ...
    (ContentRegions = [] ->
        A =< 9
    ;
        findall(CA, (member(CR, ContentRegions), legend_region_area(CR, CA)), CAs),
        max_list(CAs, MaxCA),
        A * 2 =< MaxCA
    ),
    \+ (member(CR, ContentRegions), legend_touching_(Region, CR)).

% legend_legend_entries(+LegendRegion, -Entries)
...
% legend_consistent_legend(+Pairs, -Legend)
...
legend_consistent_legend(Pairs, Legend) :-
    ...
    legend_all_regions(FirstIn, 0, FirstRegions),
    include(legend_is_small_(FirstIn), FirstRegions, FirstSmall),
    (FirstSmall = [CandRegion|_] ->
        legend_legend_entries(CandRegion, Legend),
        maplist(legend_has_similar_region_(CandRegion), RestPairs)
    ;
        Legend = []
    ).

% legend_has_similar_region_/2: succeed if a pair input has a region of same size.
legend_has_similar_region_(CandRegion, pair(In, _)) :-
    legend_region_area(CandRegion, A),
    legend_all_regions(In, 0, Regions),
    member(Region, Regions),
    legend_region_area(Region, A),
    !.

% legend_region_bbox(+Region, +BgColor, -BBox)
% BBox = r0(MinR, MinC, MaxR, MaxC) for the region's bounding box.
legend_region_bbox(Region, _BgColor, r0(MinR, MinC, MaxR, MaxC)) :-
    legend_region_min_max_(Region, MinR, MinC, MaxR, MaxC).

% legend_separated(+RegionA, +RegionB)
% Succeed if RegionA and RegionB are not touching (no adjacent cells).
legend_separated(RegionA, RegionB) :-
    \+ legend_touching_(RegionA, RegionB).

% legend_region_area(+Region, -Area)
    ...
    (Rows =:= 0 -> Regions = [] ;
        ...
        findall(r(R,C),
            (member(R, RowRange), member(C, ColRange),
             legend_cell_color_(Grid, r(R,C), V), V \= BgColor),
            AllNonBg),
        legend_connected_components_(Grid, BgColor, AllNonBg, [], Regions)
    ).

% legend_connected_components_/5: iteratively peel off components.
legend_connected_components_(_, _, [], Acc, Acc).
legend_connected_components_(Grid, BgColor, [Seed|Rest], Acc, Regions) :-
    legend_flood_region_(Grid, Seed, BgColor, Component),
    subtract(Rest, Component, Remaining),
    legend_connected_components_(Grid, BgColor, Remaining, [Component|Acc], Regions).

% legend_region_color(+Region, +Grid, -Color)
% Color is the most common color of cells in Region (non-bg assumed to be consistent).
legend_region_color(Region, Grid, Color) :-
    findall(V, (member(Cell, Region), legend_cell_color_(Grid, Cell, V)), Colors),
    msort(Colors, Sorted),
    last(Sorted, Color).

% legend_is_small(+Region, +Grid, -Bool)
% Bool is true if region area <= 1/4 of total grid area.
legend_is_small(Region, Grid, true) :-
    legend_grid_dims_(Grid, Rows, Cols),
    TotalArea is Rows * Cols,
    legend_region_area(Region, A),
    A * 4 =< TotalArea,
    !.
legend_is_small(_, _, false).

% legend_same_shape(+Region1, +Region2)
% Succeed if two regions have the same normalized shape (same relative cell pattern).
legend_same_shape(Region1, Region2) :-
    legend_normalize_shape_(Region1, N1),
    legend_normalize_shape_(Region2, N2),
    N1 = N2.

% legend_entry_boundaries(+LegendRegion, -Boundaries)
...
% legend_color_map(+LegendRegion, +Grid, -ColorMap)
...
legend_color_map(LegendRegion, Grid, ColorMap) :-
    legend_entry_boundaries(LegendRegion, Rows),
    findall(cm(Color, Idx),
        (nth0(Idx, Rows, Row),
         findall(C, member(r(Row,C), LegendRegion), Cols),
         Cols \= [],
         hd_(Cols, Col),
         legend_cell_color_(Grid, r(Row, Col), Color)),
        Raw),
    sort(Raw, ColorMap).
...
% legend_position(+Region, +Grid, -Position)
