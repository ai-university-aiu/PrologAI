...
test(by_form_matching) :-
    ...
    mx_by_form([L], [LF], Pairs),
    Pairs == [L-LF].

test(by_form_no_match) :-
    red_dot(R), red_bar(RB),
    mx_by_form([R], [RB], Pairs),
    Pairs == [].
...
test(by_nearest_basic) :-
    ...
    mx_by_nearest([R, RF], [B, R11], Pairs),
    Pairs == [R-R11, RF-B].

test(by_nearest_single) :-
    red_dot(R),
    blue_dot22(B),
    mx_by_nearest([R], [B], Pairs),
    Pairs == [R-B].
...
test(by_nearest_two) :-
    ...
    mx_by_nearest([R1, R2], [B1, B2], Pairs),
    Pairs == [R1-B1, R2-B2].

test(by_nearest_empty_list2) :-
    red_dot(R),
    mx_by_nearest([R], [], Pairs),
    Pairs == [].
...
test(unmatched_basic) :-
    red_dot(R), blue_dot(B), green_dot(G),
    Pairs = [R-B],
    mx_unmatched(Pairs, [R, G], [B, G], Um1, Um2),
    Um1 == [G],
    Um2 == [G].
...
test(unmatched_all_matched) :-
    red_dot(R), blue_dot(B),
    Pairs = [R-B],
    mx_unmatched(Pairs, [R], [B], Um1, Um2),
    Um1 == [],
    Um2 == [].

test(unmatched_empty_pairs) :-
    red_dot(R), blue_dot(B),
    mx_unmatched([], [R], [B], Um1, Um2),
    Um1 == [R],
    Um2 == [B].
...
test(filter_changed_basic) :-
    ...
    mx_filter_changed_color([R-B, R-G], Changed),
    length(Changed, 2).

test(filter_changed_none) :-
    red_dot(R1), red_dot_far(R2),
    mx_filter_changed_color([R1-R2], Changed),
    Changed == [].

test(filter_changed_mixed) :-
    red_dot(R), blue_dot(B), red_dot_far(R2),
    mx_filter_changed_color([R-B, R-R2], Changed),
    Changed == [R-B].
...
test(filter_same_basic) :-
    ...
    mx_filter_same_color([R-R2, R-B], Same),
    Same == [R-R2].

test(filter_same_empty) :-
    red_dot(R), blue_dot(B),
    mx_filter_same_color([R-B], Same),
    Same == [].
...
test(color_deltas_basic) :-
    red_dot(R), blue_dot(B), green_dot(G),
    mx_color_deltas([R-B, R-G], Deltas),
    Deltas == [r-b, r-g].

test(color_deltas_empty) :-
    mx_color_deltas([], Deltas),
    Deltas == [].
...
test(all_same_color_delta_true) :-
    ...
    % Both pairs: r -> b
    mx_all_same_color_delta([R1-B1, R2-B2]).

test(all_same_color_delta_false) :-
    red_dot(R), blue_dot(B), green_dot(G),
    % Pairs: r->b and r->g — not same delta
    \+ mx_all_same_color_delta([R-B, R-G]).
...
test(all_same_pos_delta_two_equal) :-
    ...
    \+ mx_all_same_pos_delta([R1-R2, B1-B2]).
