...
test(hot_spots_center, []) :-
    ...
    gcv_hot_spots(G, 1, r, 1, Cells),
    length(Cells, 9).

test(hot_spots_none, []) :-
    g3x3_all_x(G),
    gcv_hot_spots(G, 1, r, 1, []).
...
test(hot_spots_high_threshold, []) :-
    ...
    gcv_hot_spots(G, 1, r, 9, Cells),
    Cells = [1-1].

% --- gcv_dilate_sq ---
test(dilate_sq_n0_identity, []) :-
    g3x3_rb(G),
    gcv_dilate_sq(G, r, 0, G).

test(dilate_sq_n1_dot_fills, []) :-
    g5x5_dot(G),
    gcv_dilate_sq(G, r, 1, Result),
...
test(dilate_sq_uniform_unchanged, []) :-
    g3x3_all_r(G),
    gcv_dilate_sq(G, r, 1, G).

% --- gcv_erode_sq ---
test(erode_sq_n0_identity, []) :-
    g3x3_all_r(G),
    gcv_erode_sq(G, r, 0, G).
...
test(density_map_matches_count_in, []) :-
    ...
    gcv_count_in(G, 1, 1, 1, r, V).

test(dilate_sq_increases_fg, []) :-
    g5x5_dot(G),
    gcv_dilate_sq(G, r, 1, D),
