...
:- module(wavefront, [
    wavefront_passable/3,
    wavefront_bfs/4,
    wavefront_reachable/4,
    wavefront_unreachable/4,
    wavefront_at_dist/3,
    wavefront_within_dist/3,
    wavefront_dist_of/3,
    wavefront_max_dist/2,
    wavefront_all_dists/2,
    wavefront_path_exists/4,
    wavefront_paint_bg/5,
    wavefront_multi_wave/4,
    wavefront_collision/4,
    wavefront_enclosed/3
]).
...
% wavefront_4adj_(+R, +C, -NR, -NC): enumerate four orthogonal neighbors of r(R,C).
% Used internally by wavefront_bfs_ to expand the wavefront one cell at a time.
wavefront_4adj_(R, C, NR, C)  :- NR is R - 1.
% Step downward by one row.
wavefront_4adj_(R, C, NR, C)  :- NR is R + 1.
% Step left by one column.
wavefront_4adj_(R, C, R,  NC) :- NC is C - 1.
% Step right by one column.
wavefront_4adj_(R, C, R,  NC) :- NC is C + 1.

% wavefront_passable(+Grid, +PassColor, -Cells): sorted list of all r(R,C) positions in
% Grid whose value equals PassColor. These are the cells the wavefront can traverse.
wavefront_passable(Grid, PassColor, Cells) :-
...
% wavefront_bfs_(+Queue, +Grid, +PassColor, +Enqueued, +Acc, -DistPairs): BFS worker.
...
wavefront_bfs_([], _, _, _, Acc, Sorted) :-
...
wavefront_bfs_([D-r(R,C)|Queue], Grid, PassColor, Enqueued, Acc, DistPairs) :-
...
% Recurse with current cell added to the result accumulator.
    wavefront_bfs_(NewQueue, Grid, PassColor, NewEnqueued, [D-r(R,C)|Acc], DistPairs).

% wavefront_bfs(+Grid, +Seeds, +PassColor, -DistPairs): BFS from all Seeds through
...
wavefront_bfs(Grid, Seeds, PassColor, DistPairs) :-
...
% Run BFS; the accumulator starts empty.
    wavefront_bfs_(InitQueue, Grid, PassColor, Enqueued, [], DistPairs).

% wavefront_reachable(+Grid, +Seeds, +PassColor, -Cells): sorted list of all cells
% (seeds and PassColor cells) that appear in the BFS distance map.
wavefront_reachable(Grid, Seeds, PassColor, Cells) :-
% Run BFS to get the full distance map.
    wavefront_bfs(Grid, Seeds, PassColor, DP),
...
% wavefront_unreachable(+Grid, +Seeds, +PassColor, -Cells): sorted list of PassColor cells
% that are NOT reachable from Seeds (isolated passable pockets with no BFS path).
wavefront_unreachable(Grid, Seeds, PassColor, Cells) :-
% Collect all cells whose value equals PassColor.
    wavefront_passable(Grid, PassColor, All),
% Collect all cells that the BFS reached.
    wavefront_reachable(Grid, Seeds, PassColor, Reached),
% Cells in All but not in Reached are unreachable PassColor cells.
    subtract(All, Reached, Cells).

% wavefront_at_dist(+DistPairs, +D, -Cells): sorted list of cells at exactly distance D.
wavefront_at_dist(DistPairs, D, Cells) :-
% Filter DistPairs to entries matching the given distance D exactly.
    findall(Cell, member(D-Cell, DistPairs), Raw),
% Sort for canonical ordering.
    sort(Raw, Cells).

% wavefront_within_dist(+DistPairs, +MaxD, -Cells): sorted list of cells at distance <= MaxD.
wavefront_within_dist(DistPairs, MaxD, Cells) :-
% Filter DistPairs to entries whose distance does not exceed MaxD.
    findall(Cell, (member(D-Cell, DistPairs), D =< MaxD), Raw),
% Sort for canonical ordering and duplicate removal.
    sort(Raw, Cells).

% wavefront_dist_of(+DistPairs, +Cell, -D): distance D of Cell in DistPairs.
% Fails if Cell does not appear in DistPairs (not reachable).
wavefront_dist_of(DistPairs, Cell, D) :-
% Search for the unique D-Cell pair; memberchk stops at first match.
    memberchk(D-Cell, DistPairs).

% wavefront_max_dist(+DistPairs, -MaxD): maximum wave distance present in DistPairs.
% Fails if DistPairs is empty.
wavefront_max_dist(DistPairs, MaxD) :-
...
% wavefront_all_dists(+DistPairs, -Dists): sorted list of distinct distance values in DistPairs.
wavefront_all_dists(DistPairs, Dists) :-
...
% wavefront_path_exists(+Grid, +CellA, +CellB, +PassColor): true when CellB is reachable
% from CellA via BFS through PassColor cells. CellA is always reachable (distance 0).
wavefront_path_exists(Grid, CellA, CellB, PassColor) :-
% Run BFS from CellA as the sole seed.
    wavefront_bfs(Grid, [CellA], PassColor, DP),
% Succeed iff CellB appears anywhere in the distance map; cut via memberchk.
    memberchk(_-CellB, DP).

% wavefront_paint_bg(+Grid, +Seeds, +PassColor, +MaxD, -NewGrid): replace each PassColor cell
...
wavefront_paint_bg(Grid, Seeds, PassColor, MaxD, NewGrid) :-
...
% wavefront_multi_wave_(+ColorDPs, +R, +C, -WinColor): helper for wavefront_multi_wave.
...
wavefront_multi_wave_(ColorDPs, R, C, WinColor) :-
...
% wavefront_multi_wave(+Grid, +ColoredSeeds, +PassColor, -Painted): paint each PassColor
...
wavefront_multi_wave(Grid, ColoredSeeds, PassColor, Painted) :-
...
% wavefront_collision(+Grid, +ColoredSeeds, +PassColor, -Cells): sorted list of PassColor
...
wavefront_collision(Grid, ColoredSeeds, PassColor, Cells) :-
...
% Gather all PassColor positions in the grid.
    wavefront_passable(Grid, PassColor, PassCells),
...
% wavefront_enclosed(+Grid, +PassColor, -Cells): sorted list of PassColor cells that are
...
wavefront_enclosed(Grid, PassColor, Cells) :-
...
% Run BFS from all border PassColor seeds to find the exterior-reachable cells.
    wavefront_bfs(Grid, BorderSeeds, PassColor, DP),
