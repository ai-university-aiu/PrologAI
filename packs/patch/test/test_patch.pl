...
test(match_all_zeros) :-
    ...
    pa_match_all(Grid, [[0,0],[0,0]], Positions),
    length(Positions, 4).
...
test(match_bg_all_positions) :-
    Grid = [[1,2],[3,4]],
    pa_match_bg(Grid, [[0,0],[0,0]], 0, Positions),
    length(Positions, 1).
...
% A patch with no background wildcards behaves like pa_match_all.
test(match_bg_no_wildcard) :-
    Grid = [[1,2,3],[4,5,6],[7,8,9]],
    pa_match_bg(Grid, [[1,2],[4,5]], 0, Positions),
    Positions = [0-0].
...
test(unique_four_patches) :-
    Grid = [[1,2],[3,4]],
    pa_unique_patches(Grid, 1, 1, Unique),
    length(Unique, 4).
...
test(most_common_two_one) :-
    Grid = [[1],[1],[2]],
    pa_most_common_patch(Grid, 1, 1, Patch, Count),
    Patch = [[1]],
    Count = 2.
