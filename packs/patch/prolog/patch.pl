...
% pa_take_cols_: extract W consecutive values from Row starting at column C0.
...
% pa_extract(+Grid, +R0, +C0, +H, +W, -Patch): Patch is the H x W sub-grid
...
% Collect sub-rows for each row in the range.
    findall(SubRow, (
        between(R0, R1, R),
        nth0(R, Grid, FullRow),
        pa_take_cols_(FullRow, C0, W, SubRow)
    ), Patch).

% pa_dims(+Patch, -H, -W): H is the number of rows and W is the number of
...
% pa_match(+Grid, +Patch, -R0, -C0): non-deterministically enumerate all
...
% pa_match_all(+Grid, +Patch, -Positions): Positions is the list of all R0-C0
% pairs (in row-major enumeration order) where Patch occurs exactly in Grid.
pa_match_all(Grid, Patch, Positions) :-
    findall(R0-C0, pa_match(Grid, Patch, R0, C0), Positions).

% pa_match_count(+Grid, +Patch, -N): N is the number of positions in Grid
% where Patch occurs exactly.
pa_match_count(Grid, Patch, N) :-
    pa_match_all(Grid, Patch, Positions),
    length(Positions, N).

% pa_cell_bg_match_: test that a patch cell PV matches a grid cell SV with
...
% pa_row_bg_match_: test that all corresponding cells in a patch row and a
% grid row satisfy the background wildcard match.
pa_row_bg_match_([], [], _Bg).
pa_row_bg_match_([PV | PRest], [SV | SRest], Bg) :-
    pa_cell_bg_match_(PV, SV, Bg),
    pa_row_bg_match_(PRest, SRest, Bg).

% pa_patch_bg_match_: test that all rows of Patch match the corresponding rows
% of Sub under the background wildcard rule.
pa_patch_bg_match_([], [], _Bg).
pa_patch_bg_match_([PR | PRest], [SR | SRest], Bg) :-
    pa_row_bg_match_(PR, SR, Bg),
    pa_patch_bg_match_(PRest, SRest, Bg).

% pa_match_bg(+Grid, +Patch, +Bg, -Positions): Positions is the list of all
...
pa_match_bg(Grid, Patch, Bg, Positions) :-
    ...
    findall(R0-C0, (
        between(0, MaxR0, R0),
        between(0, MaxC0, C0),
        pa_extract(Grid, R0, C0, H, W, Sub),
        pa_patch_bg_match_(Patch, Sub, Bg)
    ), Positions).

% pa_slides(+Grid, +H, +W, -Patches): Patches is the list of all overlapping
...
% pa_run_count_: count consecutive occurrences of P at the head of a sorted
% list; Rest is the list tail after the run ends.
pa_run_count_([], _, 0, []).
pa_run_count_([Q | Rest], P, N, Remaining) :-
% Use structural equality for comparing list-of-list patch terms.
    (Q == P ->
        pa_run_count_(Rest, P, N1, Remaining),
        N is N1 + 1
    ;
        N = 0, Remaining = [Q | Rest]
    ).

% pa_count_groups_: convert a sorted list into a list of Count-Element pairs.
pa_count_groups_([], []).
pa_count_groups_([P | Rest], [N-P | Groups]) :-
% Count occurrences of P at the head of the sorted list.
    pa_run_count_(Rest, P, N1, Remaining),
    N is N1 + 1,
    pa_count_groups_(Remaining, Groups).

% pa_most_common_patch(+Grid, +H, +W, -Patch, -Count): Patch is the H x W
...
% pa_is_uniform(+Patch, -Color): succeed if every cell in Patch has the same
...
% pa_flip_h(+Patch, -Flipped): Flipped is Patch with each row reversed
...
% pa_flip_v(+Patch, -Flipped): Flipped is Patch with the row order reversed
% (vertical/top-bottom mirror).
pa_flip_v(Patch, Flipped) :-
% Reverse the list of rows.
    reverse(Patch, Flipped).

% pa_rot90(+Patch, -Rotated): Rotated is Patch rotated 90 degrees clockwise.
