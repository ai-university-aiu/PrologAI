...
test(fill_hull_triangle) :-
    Grid = [[0,0,0],[0,0,0],[0,0,0]],
    hu_fill_hull(Grid, [0-0, 0-2, 2-0], 3, Out),
    Out = [[3,3,3],[3,3,0],[3,0,0]].

% hu_concavities/4 tests
...
test(concavities_none) :-
    Grid = [[0,0],[0,0]],
    hu_concavities(Grid, [0-0,0-1,1-0,1-1], 0, Missing),
    Missing = [].
