...
% hu_sort_xy_: sort cells by (C, R) - column then row - for monotone chain.
hu_sort_xy_(Cells, Sorted) :-
...
% hu_cross_: signed cross product of vectors (A-to-B) and (A-to-C) in R-C coords.
% In screen coordinates (R increases downward), positive Z is a counterclockwise
% turn and negative Z is a clockwise turn.
hu_cross_(R1-C1, R2-C2, R3-C3, Z) :-
    Z is (C2 - C1) * (R3 - R1) - (R2 - R1) * (C3 - C1).

% hu_trim_: remove points from the accumulator head while the last three
...
hu_trim_([B, A | Rest], P, Trimmed) :-
    hu_cross_(A, B, P, Z),
    Z >= 0, !,
    hu_trim_([A | Rest], P, Trimmed).
hu_trim_(Acc, _, Acc).

% hu_build_half_: build one half-hull from points in monotone order.
% Returns the half-hull in reverse (most recently added point at head).
hu_build_half_([], RevHull, RevHull).
hu_build_half_([P | Ps], Acc, RevHull) :-
% Trim clockwise-turn violations then prepend the new point.
    hu_trim_(Acc, P, TrimmedAcc),
    hu_build_half_(Ps, [P | TrimmedAcc], RevHull).

% hu_convex_hull(+Cells, -Hull): convex hull of Cells using Andrew's monotone
...
% hu_hull_size(+Cells, -N): number of vertices in the convex hull of Cells.
hu_hull_size(Cells, N) :-
    hu_convex_hull(Cells, Hull),
    length(Hull, N).

% hu_shoelace_: compute the signed twice-area of a closed polygon via the
% shoelace formula: sum of (C_i * R_{i+1} - C_{i+1} * R_i) over all edges.
hu_shoelace_(Hull, A2) :-
    Hull = [First | _],
    append(Hull, [First], Closed),
    hu_shoelace_sum_(Closed, 0, A2).

% hu_shoelace_sum_: accumulate shoelace terms over consecutive vertex pairs.
hu_shoelace_sum_([_], Acc, Acc) :- !.
hu_shoelace_sum_([R1-C1, R2-C2 | Rest], Acc, A2) :-
    Term is C1 * R2 - C2 * R1,
    Acc1 is Acc + Term,
    hu_shoelace_sum_([R2-C2 | Rest], Acc1, A2).

% hu_hull_area2(+Cells, -A2): twice the area of the convex hull (integer >= 0).
...
hu_hull_area2(Cells, A2) :-
    ...
    (N < 3 ->
        A2 = 0
    ;
        hu_shoelace_(Hull, SA),
        A2 is abs(SA)
    ).

% hu_edge_test_: test if point P is on the inside (or boundary) of directed
% edge A-to-B in a clockwise hull. For CW hull, inside means Z =< 0.
hu_edge_test_(RA-CA, RB-CB, RP-CP) :-
    Z is (CB - CA) * (RP - RA) - (RB - RA) * (CP - CA),
    Z =< 0.

% hu_all_edges_: check all consecutive edges in a closed polygon list.
% Succeeds if point P passes the edge test for every edge.
hu_all_edges_([_], _) :- !.
hu_all_edges_([A, B | Rest], P) :-
    hu_edge_test_(A, B, P),
    hu_all_edges_([B | Rest], P).

% hu_in_hull(+R, +C, +Hull): succeed if (R,C) is inside or on the convex hull.
% Handles degenerate hulls with 0, 1, or 2 vertices separately.
hu_in_hull(_, _, []) :- !, fail.
hu_in_hull(R, C, [R1-C1]) :- !, R =:= R1, C =:= C1.
hu_in_hull(R, C, [R1-C1, R2-C2]) :- !,
...
% For a proper polygon (3+ vertices): all edge cross products must be =< 0.
    Hull = [First | _],
    append(Hull, [First], Closed),
    hu_all_edges_(Closed, R-C).

% hu_dims_: extract row count and column count from a grid.
hu_dims_(Grid, NR, NC) :-
    length(Grid, NR),
    (NR > 0 -> Grid = [FR | _], length(FR, NC) ; NC = 0).

% hu_cells_in_hull(+Grid, +Cells, -InHull): sorted list of all R-C cells in
...
% hu_set_cell_: replace the value at (R,C) in a grid.
hu_set_cell_(Grid, R, C, Val, NewGrid) :-
    nth0(R, Grid, OldRow, RestRows),
    nth0(C, OldRow, _, RestCols),
    nth0(C, NewRow, Val, RestCols),
    nth0(R, NewGrid, NewRow, RestRows).

% hu_set_cells_: apply a list of cell replacements to a grid.
hu_set_cells_(Grid, [], _, Grid) :- !.
hu_set_cells_(Grid0, [R-C | Rest], Val, Out) :-
    hu_set_cell_(Grid0, R, C, Val, Grid1),
    hu_set_cells_(Grid1, Rest, Val, Out).

% hu_fill_hull(+Grid, +Cells, +Color, -Out): paint all cells inside the convex
% hull of Cells with Color. Cells outside the hull are unchanged.
hu_fill_hull(Grid, Cells, Color, Out) :-
    hu_cells_in_hull(Grid, Cells, InHull),
    hu_set_cells_(Grid, InHull, Color, Out).

% hu_concavities(+Grid, +Cells, +Bg, -Missing): sorted list of Bg-valued grid
...
% hu_dist2_: squared Euclidean distance between two R-C cells.
hu_dist2_(R1-C1, R2-C2, D2) :-
    D2 is (R2 - R1) * (R2 - R1) + (C2 - C1) * (C2 - C1).

% hu_diameter(+Cells, -P1, -P2, -D2): the pair of cells with maximum squared
...
% hu_aspect(+Cells, -H, -W): height (row-span) and width (col-span) of the
...
hu_aspect(Cells, H, W) :-
    ...
    (Hull = [] ->
        H = 0, W = 0
    ;
        hu_bbox_(Hull, MinR, MinC, MaxR, MaxC),
        H is MaxR - MinR,
        W is MaxC - MinC
    ).

% hu_dot_: dot product of edge vectors (A-to-B) and (B-to-C).
% Zero dot product means a 90-degree angle at B.
hu_dot_(R1-C1, R2-C2, R3-C3, D) :-
    D is (R2 - R1) * (R3 - R2) + (C2 - C1) * (C3 - C2).

% hu_is_rect(+Cells): succeed if the convex hull of Cells has exactly 4
% vertices and all four interior angles are exactly 90 degrees.
hu_is_rect(Cells) :-
    hu_convex_hull(Cells, Hull),
    length(Hull, 4),
    Hull = [A, B, C, D],
    hu_dot_(A, B, C, D1), D1 =:= 0,
    hu_dot_(B, C, D, D2), D2 =:= 0,
    hu_dot_(C, D, A, D3), D3 =:= 0,
    hu_dot_(D, A, B, D4), D4 =:= 0.

% hu_edge_len2_: squared length of edge from A to B.
hu_edge_len2_(R1-C1, R2-C2, L2) :-
    L2 is (R2 - R1) * (R2 - R1) + (C2 - C1) * (C2 - C1).

% hu_perim2_sum_: sum squared edge lengths over a closed vertex list.
hu_perim2_sum_([_], Acc, Acc) :- !.
hu_perim2_sum_([A, B | Rest], Acc, P2) :-
    hu_edge_len2_(A, B, L2),
    Acc1 is Acc + L2,
    hu_perim2_sum_([B | Rest], Acc1, P2).

% hu_hull_perim2(+Cells, -P2): sum of squared edge lengths of the convex hull
...
hu_hull_perim2(Cells, P2) :-
    ...
    ;
        Hull = [First | _],
        append(Hull, [First], Closed),
        hu_perim2_sum_(Closed, 0, P2)
    ).

% hu_sum_list_: sum a list of integers recursively.
hu_sum_list_([], 0).
hu_sum_list_([H | T], S) :-
    hu_sum_list_(T, S1),
    S is S1 + H.

% hu_centroid(+Cells, -AvgR, -AvgC): centroid of the convex hull vertices.
...
hu_centroid(Cells, AvgR, AvgC) :-
    ...
    hu_sum_list_(Rs, SumR),
    hu_sum_list_(Cs, SumC),
    AvgR is SumR // N,
    AvgC is SumC // N.

% hu_hull_contains(+Cells, +SubCells): succeed if every cell in SubCells lies
% inside or on the convex hull of Cells.
hu_hull_contains(Cells, SubCells) :-
    hu_convex_hull(Cells, Hull),
    forall(member(R-C, SubCells), hu_in_hull(R, C, Hull)).
