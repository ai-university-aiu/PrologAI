...
% --- gsg_is_sep_row ---
test(is_sep_row_yes, []) :-
    g_row1sep(G), gsg_is_sep_row(G, 1, s).

test(is_sep_row_no, []) :-
    g3x3(G), \+ gsg_is_sep_row(G, 0, _).

test(is_sep_row_binds_color, []) :-
    g_row1sep(G), gsg_is_sep_row(G, 1, C), C = s.

% --- gsg_is_sep_col ---
test(is_sep_col_yes, []) :-
    g_cols02sep(G), gsg_is_sep_col(G, 0, s).

test(is_sep_col_no, []) :-
    g3x3(G), \+ gsg_is_sep_col(G, 0, _).

test(is_sep_col_binds_color, []) :-
    g_cols02sep(G), gsg_is_sep_col(G, 2, C), C = s.

% --- gsg_sep_rows ---
test(sep_rows_one, []) :-
    g_row1sep(G), gsg_sep_rows(G, s, [1]).

test(sep_rows_none, []) :-
    g3x3(G), gsg_sep_rows(G, s, []).

test(sep_rows_two, []) :-
    g_rows02sep(G), gsg_sep_rows(G, s, [0,2]).

% --- gsg_sep_cols ---
test(sep_cols_two, []) :-
    g_cols02sep(G), gsg_sep_cols(G, s, [0,2]).

test(sep_cols_none, []) :-
    g3x3(G), gsg_sep_cols(G, s, []).

test(sep_cols_one_1x1, []) :-
    g1x1_sep(G), gsg_sep_cols(G, s, [0]).

% --- gsg_split_h ---
test(split_h_two_segs, []) :-
    g_row1sep(G), gsg_split_h(G, s, [[[a,b,c]],[[d,e,f]]]).

test(split_h_no_sep, []) :-
    g3x3(G), gsg_split_h(G, s, [G]).

test(split_h_border_sep, []) :-
    % Sep at top and bottom: one segment in the middle
    g_border_sep(G), gsg_split_h(G, s, [[[a,b,c],[d,e,f]]]).

test(split_h_consecutive_sep, []) :-
    % Two consecutive sep rows at top: one segment at bottom
    g_consec_sep(G), gsg_split_h(G, s, [[[a,b,c]]]).

% --- gsg_split_v ---
test(split_v_two_segs, []) :-
    g_cols02sep(G), gsg_split_v(G, s, [[[a],[b],[c]]]).

test(split_v_no_sep, []) :-
    g3x3(G), gsg_split_v(G, s, [G]).

test(split_v_border_cols, []) :-
    % Sep at left and right: one segment in the middle
    g_border_cols(G), gsg_split_v(G, s, [[[a,b],[c,d],[e,f]]]).

% --- gsg_segment_count_h ---
test(segment_count_h_two, []) :-
    g_row1sep(G), gsg_segment_count_h(G, s, 2).

test(segment_count_h_one, []) :-
    g3x3(G), gsg_segment_count_h(G, s, 1).

% --- gsg_segment_count_v ---
test(segment_count_v_one, []) :-
    % g_cols02sep has sep at 0 and 2, content in middle col: 1 segment
    g_cols02sep(G), gsg_segment_count_v(G, s, 1).

test(segment_count_v_one_nosep, []) :-
    g3x3(G), gsg_segment_count_v(G, s, 1).
...
test(panels_four, []) :-
    ...
    g_both_sep(G), gsg_panels(G, s, Panels), length(Panels, 4).

test(panels_two, []) :-
    % g_row1sep: only horizontal sep → 2 row segs, each 1 col seg
    g_row1sep(G), gsg_panels(G, s, Panels), length(Panels, 2).

test(panels_one, []) :-
    g3x3(G), gsg_panels(G, s, [G]).

% --- gsg_panel_count ---
test(panel_count_four, []) :-
    g_both_sep(G), gsg_panel_count(G, s, 4).

test(panel_count_two, []) :-
    g_row1sep(G), gsg_panel_count(G, s, 2).

test(panel_count_one, []) :-
    g3x3(G), gsg_panel_count(G, s, 1).

% --- gsg_trim_h ---
test(trim_h_removes_borders, []) :-
    g_border_sep(G), gsg_trim_h(G, s, [[a,b,c],[d,e,f]]).

test(trim_h_noop, []) :-
    g3x3(G), gsg_trim_h(G, s, G).

test(trim_h_consecutive, []) :-
    % Two sep rows at top removed, content at bottom kept
    g_consec_sep(G), gsg_trim_h(G, s, [[a,b,c]]).

% --- gsg_trim_v ---
test(trim_v_removes_borders, []) :-
    g_border_cols(G), gsg_trim_v(G, s, [[a,b],[c,d],[e,f]]).

test(trim_v_noop, []) :-
    g3x3(G), gsg_trim_v(G, s, G).
