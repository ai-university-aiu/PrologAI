/*  PrologAI — Causalontology Priming  (WP-422, Layer 397)
    ...
    Predicates:
      priming_reset/0            -- forget every association
      priming_link_add/3         -- +From, +To, +Weight
      priming_link/3             -- ?From, ?To, ?Weight
      priming_neighbors/2        -- +Node, -Neighbours
      priming_node_count/1       -- -N
      priming_spread/3           -- +Sources, +Decay, -Activations   (Node-Level, sorted by node)
      priming_activation/4       -- +Sources, +Decay, +Node, -Level   (0.0 if unreached)
      priming_primed/4           -- +Sources, +Decay, +K, -TopK       (best K primed non-sources)
*/
...
:- module(priming, [
    % priming_reset/0: forget every association.
    priming_reset/0,
    % priming_link_add/3: add a weighted association.
    priming_link_add/3,
    % priming_link/3: query the associations.
    priming_link/3,
    % priming_neighbors/2: the nodes a node links to.
    priming_neighbors/2,
    % priming_node_count/1: how many distinct nodes appear in the graph.
    priming_node_count/1,
    % priming_spread/3: activation of every reached node from the sources.
    priming_spread/3,
    % priming_activation/4: the activation level of one node.
    priming_activation/4,
    % priming_primed/4: the best K primed nodes (excluding the sources).
    priming_primed/4
]).
...
% priming_link_add/3: add a weighted association, without duplication.
priming_link_add(From, To, Weight) :-
    % Store it unless the exact same link already exists.
    ( link(From, To, Weight) -> true ; assertz(link(From, To, Weight)) ).

% priming_link/3: expose the associations.
priming_link(From, To, Weight) :-
    % Read the stored link.
    link(From, To, Weight).

% priming_neighbors/2: the distinct nodes a node links to.
priming_neighbors(Node, Neighbours) :-
...
% priming_node_count/1: how many distinct nodes appear in the graph.
priming_node_count(N) :-
    % Gather every node from both ends of every link.
    priming_graph_nodes(Nodes),
    % Count them.
    length(Nodes, N).

% priming_spread/3: spread activation from the sources across the graph.
priming_spread(Sources, Decay, Activations) :-
    % Normalise the sources into Node-Level pairs (default level 1.0).
    priming_norm_sources(Sources, Pairs),
    % Seed the activation map with the source levels.
    priming_seed(Pairs, Seed),
    % Convergence needs at most as many passes as there are nodes.
    priming_all_nodes(Pairs, AllNodes),
    length(AllNodes, Passes),
    % Relax the graph that many times.
    priming_relax_n(Passes, Decay, Seed, Final),
    % Return the activations as a node-sorted Node-Level list.
    assoc_to_list(Final, Activations).

% priming_activation/4: the activation level of one node (0.0 if it was never reached).
priming_activation(Sources, Decay, Node, Level) :-
    % Spread once, then look the node up.
    priming_spread(Sources, Decay, Activations),
    ( memberchk(Node-L, Activations) -> Level = L ; Level = 0.0 ).

% priming_primed/4: the best K primed nodes, excluding the sources themselves.
priming_primed(Sources, Decay, K, TopK) :-
    ...
    priming_take(Ranked, K, Best),
    findall(Node-Level, member(Level-Node, Best), TopK).

% ---- internal --------------------------------------------------------------

% priming_norm_sources/2: turn a source list into Node-Level pairs (default 1.0).
priming_norm_sources([], []).
% Each element is either a Node-Level pair or a bare node (level 1.0).
priming_norm_sources([X | Xs], [Node-Level | Rest]) :-
    ...
    priming_norm_sources(Xs, Rest).

% priming_seed/2: build the initial activation map from the source pairs.
priming_seed(Pairs, Seed) :-
    % Start from an empty association map.
    empty_assoc(Empty),
    % Fold each source's level into the map.
    foldl(priming_put_source, Pairs, Empty, Seed).

% priming_put_source/3: put one source level into the activation map.
priming_put_source(Node-Level, In, Out) :-
    % Record the node at its seed level.
    put_assoc(Node, In, Level, Out).

% priming_all_nodes/2: every node in the graph plus every source node.
priming_all_nodes(SourcePairs, AllNodes) :-
    % The graph's own nodes.
    priming_graph_nodes(GraphNodes),
    % The source nodes.
...
% priming_graph_nodes/1: every distinct node appearing in a link.
priming_graph_nodes(Nodes) :-
...
% priming_relax_n/4: run the relaxation pass a fixed number of times.
priming_relax_n(0, _, A, A) :-
    % Zero passes leave the map unchanged (committed).
    !.
priming_relax_n(K, Decay, A, Out) :-
    % With passes remaining, run one pass and recurse.
    K > 0,
    priming_pass(Decay, A, A1),
    K1 is K - 1,
    priming_relax_n(K1, Decay, A1, Out).

% priming_pass/3: one relaxation pass — every link may raise its target's activation.
priming_pass(Decay, Ain, Aout) :-
    ...
    foldl(priming_relax_edge(Decay, Ain), Edges, Ain, Aout).

% priming_relax_edge/5: raise the target's activation if this edge offers more.
priming_relax_edge(Decay, Ain, edge(From, To, Weight), AccIn, AccOut) :-
...
% priming_take/3: take at most K elements from the front of a list.
priming_take(_, K, []) :-
...
priming_take([X | Xs], K, [X | Ys]) :-
    % One fewer to take from the rest.
    K1 is K - 1,
    priming_take(Xs, K1, Ys).
