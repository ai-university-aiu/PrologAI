/*  PrologAI — Compositional Vector Binding (Vector Symbolic Architectures)  (PR 38)
    ...
    Predicates:
        vector_symbolic_architecture_bind/3            — +RoleVec, +FillerVec, -BoundVec
        vector_symbolic_architecture_unbind/3          — +BoundVec, +RoleVec, -FillerVec
        vector_symbolic_architecture_bundle/2          — +VecList, -BundledVec
        vector_symbolic_architecture_cleanup/3         — +QueryVec, +Lexicon, -NearestId
        vector_symbolic_architecture_embed/2           — +Term, -Vec
        vector_symbolic_architecture_embed_compound/2  — +CompoundTerm, -Vec
        vector_symbolic_architecture_role_vector/2     — +PositionIndex, -Vec
        vector_symbolic_architecture_set_algebra/1     — +algebra(map|hrr)
        vector_symbolic_architecture_current_algebra/1 — -Algebra
        vector_symbolic_architecture_set_dimension/1   — +Dim
*/

% Declare this file as the 'vsa' module and list its exported predicates.
:- module(vector_symbolic_architecture, [
...
% Declare 'vector_symbolic_architecture_active_algebra/1' as dynamic — its facts may be added or removed at runtime.
:- dynamic vector_symbolic_architecture_active_algebra/1.
% Declare 'vector_symbolic_architecture_dim/1' as dynamic — its facts may be added or removed at runtime.
:- dynamic vector_symbolic_architecture_dim/1.
% State the fact: vsa active algebra(map).
vector_symbolic_architecture_active_algebra(map).
% State the fact: vsa dim(64).
vector_symbolic_architecture_dim(64).

% Define a clause for 'vsa set algebra': succeed when the following conditions hold.
vector_symbolic_architecture_set_algebra(Alg) :-
    % State a fact for 'memberchk' with the arguments listed below.
    memberchk(Alg, [map, hrr]),
    % Remove all matching facts from the runtime knowledge base.
    retractall(vector_symbolic_architecture_active_algebra(_)),
    % Add a new fact or rule to the runtime knowledge base.
    assertz(vector_symbolic_architecture_active_algebra(Alg)).

% Define a clause for 'vsa current algebra': succeed when the following conditions hold.
vector_symbolic_architecture_current_algebra(Alg) :- vector_symbolic_architecture_active_algebra(Alg).

% Define a clause for 'vsa set dimension': succeed when the following conditions hold.
vector_symbolic_architecture_set_dimension(Dim) :-
    % Check that 'integer(Dim), Dim' is greater than '0'.
    integer(Dim), Dim > 0,
    % Remove all matching facts from the runtime knowledge base.
    retractall(vector_symbolic_architecture_dim(_)),
    % Add a new fact or rule to the runtime knowledge base.
    assertz(vector_symbolic_architecture_dim(Dim)).
...
vector_symbolic_architecture_cosine(A, B, Score) :-
...
vector_symbolic_architecture_normalize(Vec, Norm) :-
    % State a fact for 'foldl' with the arguments listed below.
...
vector_symbolic_architecture_embed(Term, Vec) :-
    % State a fact for 'vsa dim' with the arguments listed below.
    vector_symbolic_architecture_dim(Dim),
...
vector_symbolic_architecture_role_vector(Pos, Vec) :-
    ...
    vector_symbolic_architecture_embed(RoleName, Vec).

% ---------------------------------------------------------------------------
% vector_symbolic_architecture_bind/3 — binding operator
%
...
vector_symbolic_architecture_bind(A, B, C) :-
    % State a fact for 'vsa active algebra' with the arguments listed below.
    vector_symbolic_architecture_active_algebra(Alg),
    % State the fact: bind alg(Alg, A, B, C).
    vector_symbolic_architecture_bind_alg(Alg, A, B, C).

% Define a clause for 'bind alg': succeed when the following conditions hold.
vector_symbolic_architecture_bind_alg(map, A, B, C) :-
    % State the fact: maplist([X, Y, Z]>>(Z is X * Y), A, B, C).
    maplist([X, Y, Z]>>(Z is X * Y), A, B, C).
% Define a clause for 'bind alg': succeed when the following conditions hold.
vector_symbolic_architecture_bind_alg(hrr, A, B, C) :-
    % State the fact: circ convolve(A, B, C).
    vector_symbolic_architecture_circ_convolve(A, B, C).

% ---------------------------------------------------------------------------
% vector_symbolic_architecture_unbind/3 — unbinding operator (approximate inverse of bind)
%
...
vector_symbolic_architecture_unbind(Bound, Role, Filler) :-
    % State a fact for 'vsa active algebra' with the arguments listed below.
    vector_symbolic_architecture_active_algebra(Alg),
    % State the fact: unbind alg(Alg, Bound, Role, Filler).
    vector_symbolic_architecture_unbind_alg(Alg, Bound, Role, Filler).

% Define a clause for 'unbind alg': succeed when the following conditions hold.
vector_symbolic_architecture_unbind_alg(map, Bound, Role, Filler) :-
    % State the fact: maplist([R, B, F]>>(F is R * B), Role, Bound, Filler).
    maplist([R, B, F]>>(F is R * B), Role, Bound, Filler).
% Define a clause for 'unbind alg': succeed when the following conditions hold.
vector_symbolic_architecture_unbind_alg(hrr, Bound, Role, Filler) :-
    % State a fact for 'hrr inv' with the arguments listed below.
    vector_symbolic_architecture_hrr_inv(Role, RoleInv),
    % State the fact: circ convolve(Bound, RoleInv, Filler).
    vector_symbolic_architecture_circ_convolve(Bound, RoleInv, Filler).

% ---------------------------------------------------------------------------
% vector_symbolic_architecture_bundle/2 — superposition (element-wise sum, then normalize)
% ---------------------------------------------------------------------------

% Define a clause for 'pai bundle': succeed when the following conditions hold.
vector_symbolic_architecture_bundle([V|Vs], Bundled) :-
    % State a fact for 'foldl' with the arguments listed below.
    foldl([Vec, Acc, NAcc]>>(
        % Continue the multi-line expression started above.
    ...
    vector_symbolic_architecture_normalize(Sum, Bundled).

% ---------------------------------------------------------------------------
% vector_symbolic_architecture_cleanup/3 — nearest clean vector in a lexicon
%
...
vector_symbolic_architecture_cleanup(QueryVec, Lexicon, NearestId) :-
    % Collect all matching Template values into a list (findall — never fails, returns empty list if none).
    findall(Sim-Id, (
        % Continue the multi-line expression started above.
        member(Id-Vec, Lexicon),
        % Continue the multi-line expression started above.
        vector_symbolic_architecture_cosine(QueryVec, Vec, Sim)
    % Continue the multi-line expression started above.
    ), Pairs),
...
% ---------------------------------------------------------------------------
% vector_symbolic_architecture_embed_compound/2 — compositional embedding of a compound term
%
...
vector_symbolic_architecture_embed_compound(Term, Vec) :-
    % Execute: Term =.. [Functor|Args],.
    Term =.. [Functor|Args],
    % State a fact for 'vsa embed' with the arguments listed below.
    vector_symbolic_architecture_embed(Functor, RelVec),
    % State a fact for 'embed args' with the arguments listed below.
    vector_symbolic_architecture_embed_args(Args, 1, BoundVecs),
    % State the fact: pai bundle([RelVec|BoundVecs], Vec).
    vector_symbolic_architecture_bundle([RelVec|BoundVecs], Vec).

% State the fact: embed args([], _, []).
vector_symbolic_architecture_embed_args([], _, []).
% Define a clause for 'embed args': succeed when the following conditions hold.
vector_symbolic_architecture_embed_args([Arg|Rest], Pos, [Bound|Bounds]) :-
    % State a fact for 'vsa role vector' with the arguments listed below.
    vector_symbolic_architecture_role_vector(Pos, RoleVec),
    % State a fact for 'vsa embed' with the arguments listed below.
    vector_symbolic_architecture_embed(Arg, FillerVec),
    ...
    vector_symbolic_architecture_embed_args(Rest, Pos1, Bounds).

% ---------------------------------------------------------------------------
% HRR helpers — circular convolution and conjugate inverse
% ---------------------------------------------------------------------------

% vector_symbolic_architecture_circ_convolve(+A, +B, -C): C[k] = Σ_j A[j] * B[(k-j) mod N]
...
% vector_symbolic_architecture_hrr_inv(+Vec, -Inv): conjugate reversal — Inv[0]=Vec[0], Inv[k]=Vec[N-k]
% Define a clause for 'hrr inv': succeed when the following conditions hold.
vector_symbolic_architecture_hrr_inv([A0|ARest], Inv) :-
