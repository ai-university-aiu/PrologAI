...
% gridstamp_min_list_/2: minimum value in a non-empty list.
gridstamp_min_list_([H|T], Min) :- gridstamp_min_acc_(T, H, Min).
% Base: accumulator holds the minimum.
gridstamp_min_acc_([], M, M).
% Recursive: update accumulator if new element is smaller.
gridstamp_min_acc_([H|T], Cur, Min) :-
    (H < Cur -> gridstamp_min_acc_(T, H, Min) ; gridstamp_min_acc_(T, Cur, Min)).

% gridstamp_max_list_/2: maximum value in a non-empty list.
gridstamp_max_list_([H|T], Max) :- gridstamp_max_acc_(T, H, Max).
% Base: accumulator holds the maximum.
gridstamp_max_acc_([], M, M).
% Recursive: update accumulator if new element is larger.
gridstamp_max_acc_([H|T], Cur, Max) :-
    (H > Cur -> gridstamp_max_acc_(T, H, Max) ; gridstamp_max_acc_(T, Cur, Max)).

% gridstamp_matches_at_/5: succeeds if Pattern exactly matches Grid subgrid at (R0,C0).
% Fails immediately if any cell in Pattern differs from the corresponding Grid cell.
gridstamp_matches_at_(Grid, Pattern, R0, C0, _Bg) :-
...
% gridstamp_canvas(+H, +W, +Bg, -Canvas)
...
% gridstamp_stamp(+Grid, +Stamp, +R0, +C0, +Bg, -Result)
...
% gridstamp_stamp_all(+Grid, +Stamp, +Positions, +Bg, -Result)
...
gridstamp_stamp_all(Grid, _, [], _, Grid) :- !.
% Recursive case: stamp at the first position, then recurse.
gridstamp_stamp_all(Grid, Stamp, [r(R,C)|Rest], Bg, Result) :-
    gridstamp_stamp(Grid, Stamp, R, C, Bg, Grid1),
    gridstamp_stamp_all(Grid1, Stamp, Rest, Bg, Result).

% gridstamp_scatter(+Grid, +Color, +Positions, +Bg, -Result)
...
gridstamp_scatter(Grid, Color, Positions, _Bg, Result) :-
% Determine Grid dimensions.
    length(Grid, GH), GH1 is GH - 1,
    Grid = [GRow0|_], length(GRow0, GW), GW1 is GW - 1,
...
% gridstamp_find_matches(+Grid, +Pattern, +Bg, -Matches)
...
% Collect all top-left positions where the pattern matches.
     findall(r(R,C),
         (between(0, MaxR, R),
          between(0, MaxC, C),
          gridstamp_matches_at_(Grid, Pattern, R, C, Bg)),
         Matches)).

% gridstamp_stamp_count(+Grid, +Pattern, +Bg, -Count)
...
% gridstamp_pad(+Grid, +H, +W, +Bg, -Result)
...
% gridstamp_unpad(+Grid, +Bg, -Result)
...
% Collect rows and cols of all non-bg cells.
     ...
     (Rs = [] -> Result = [] ;
      findall(C,
          ...
          Cs),
      gridstamp_min_list_(Rs, MinR), gridstamp_max_list_(Rs, MaxR),
      gridstamp_min_list_(Cs, MinC), gridstamp_max_list_(Cs, MaxC),
...
% gridstamp_replicate_h(+Grid, +N, +Bg, -Result)
...
gridstamp_replicate_h(Grid, N, _Bg, Result) :-
    length(Grid, H), H1 is H - 1,
...
% gridstamp_center(+Grid, +H, +W, +Bg, -Result)
...
% gridstamp_extract(+Grid, +R0, +C0, +H, +W, -Subgrid)
...
% gridstamp_replace(+Grid, +R0, +C0, +Patch, -Result)
