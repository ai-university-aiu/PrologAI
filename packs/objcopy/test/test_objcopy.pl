...
test(place_at_origin) :-
    dot(D),
    tc_place_at(D, 0, 0, D2),
    D2 == obj(r, [r(0,0)]).

test(place_at_move) :-
    dot(D),
    tc_place_at(D, 3, 5, D2),
    D2 == obj(r, [r(3,5)]).

test(place_at_bar) :-
    bar_h(B),
    tc_place_at(B, 2, 4, B2),
...
test(tile_row_basic) :-
    dot(D),
    tc_tile_row(D, 3, 2, Objs),
    Objs == [obj(r,[r(0,0)]), obj(r,[r(0,2)]), obj(r,[r(0,4)])].

test(tile_row_one) :-
    bar_h(B),
    tc_tile_row(B, 1, 10, Objs),
    length(Objs, 1).
...
test(tile_col_basic) :-
    dot(D),
    tc_tile_col(D, 3, 2, Objs),
    Objs == [obj(r,[r(0,0)]), obj(r,[r(2,0)]), obj(r,[r(4,0)])].

test(tile_col_one) :-
    dot(D),
    tc_tile_col(D, 1, 5, Objs),
    Objs == [obj(r,[r(0,0)])].
...
test(at_positions_basic) :-
    dot(D),
    tc_at_positions(D, [r(1,0), r(3,5)], Objs),
    Objs == [obj(r,[r(1,0)]), obj(r,[r(3,5)])].

test(at_positions_empty) :-
    dot(D),
    tc_at_positions(D, [], Objs),
    Objs == [].
