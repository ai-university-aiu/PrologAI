:- module(gridedge, [
    gridedge_edge_cells/3,
    gridedge_edge_cells8/3,
    gridedge_is_edge/3,
    gridedge_boundary/4,
    gridedge_inner_border/3,
    gridedge_outer_border/3,
    gridedge_edge_grid/4,
    gridedge_edge_count/3,
    gridedge_neighbors_diff/4,
    gridedge_corners/3,
    gridedge_endpoints/3,
    gridedge_smooth_cells/3,
    gridedge_transition_map/2,
    gridedge_edge_color_count/4
]).
...
% gridedge_edge_cells(+Grid, +FgColor, -Cells)
...
% Collect cells that are FgColor and have at least one differently-colored neighbor.
    findall(R-C,
        (between(0, H1, R), between(0, W1, C),
         gridedge_cell_(Grid, R, C, FgColor),
         once((gridedge_nbr4_(R, C, NR, NC),
               gridedge_cell_(Grid, NR, NC, NV),
               NV \= FgColor))),
        Cells).

% gridedge_edge_cells8(+Grid, +FgColor, -Cells)
% Like gridedge_edge_cells but uses 8-connected neighbor check.
gridedge_edge_cells8(Grid, FgColor, Cells) :-
% Get grid dimensions.
    gridedge_dims_(Grid, H, W),
% Compute last indices.
...
% Collect cells that are FgColor and have at least one 8-neighbor of different color.
    findall(R-C,
        (between(0, H1, R), between(0, W1, C),
         gridedge_cell_(Grid, R, C, FgColor),
         once((gridedge_nbr8_(R, C, NR, NC),
               gridedge_cell_(Grid, NR, NC, NV),
               NV \= FgColor))),
        Cells).

% gridedge_is_edge(+Grid, +R-C, -Bool)
...
% gridedge_boundary(+Grid, +Color1, +Color2, -Cells)
...
% Collect Color1 cells adjacent to at least one Color2 cell.
    findall(R-C,
        (between(0, H1, R), between(0, W1, C),
         gridedge_cell_(Grid, R, C, Color1),
         once((gridedge_nbr4_(R, C, NR, NC),
               gridedge_cell_(Grid, NR, NC, Color2)))),
        Cells).

% gridedge_inner_border(+Grid, +BgColor, -Cells)
...
% BgColor cells adjacent to non-background is the boundary of the background region.
    gridedge_boundary_to_nonbg_(Grid, BgColor, Cells).
...
gridedge_boundary_to_nonbg_(Grid, BgColor, Cells) :-
    ...
    findall(R-C,
        (between(0, H1, R), between(0, W1, C),
         gridedge_cell_(Grid, R, C, BgColor),
         once((gridedge_nbr4_(R, C, NR, NC),
               gridedge_cell_(Grid, NR, NC, NV),
               NV \= BgColor))),
        Cells).

% gridedge_outer_border(+Grid, +FgColor, -Cells)
...
% Collect non-FgColor cells adjacent to at least one FgColor cell.
    findall(R-C,
        (between(0, H1, R), between(0, W1, C),
         gridedge_cell_(Grid, R, C, V), V \= FgColor,
         once((gridedge_nbr4_(R, C, NR, NC),
               gridedge_cell_(Grid, NR, NC, FgColor)))),
        Cells).

% --- EDGE GRIDS ---

% gridedge_edge_grid(+Grid, +FgColor, +EdgeColor, -Result)
% Result is a binary grid: FgColor edge cells become EdgeColor; all other cells
% retain their original color.
gridedge_edge_grid(Grid, FgColor, EdgeColor, Result) :-
% Get edge cells.
    gridedge_edge_cells(Grid, FgColor, Edges),
% Get dimensions.
    gridedge_dims_(Grid, H, W),
% Build result: edge cells get EdgeColor; others keep original value.
    gridedge_build_(H, W,
        [R, C, V]>>(gridedge_cell_(Grid, R, C, GV),
                    (memberchk(R-C, Edges) -> V = EdgeColor ; V = GV)),
        Result).

% gridedge_edge_count(+Grid, +FgColor, -N)
...
% gridedge_neighbors_diff(+Grid, +R, +C, -Neighbors)
...
% gridedge_corners(+Grid, +FgColor, -Cells)
...
% gridedge_endpoints(+Grid, +FgColor, -Cells)
...
% gridedge_smooth_cells(+Grid, +FgColor, -Cells)
...
% gridedge_transition_map(+Grid, -Result)
...
% gridedge_edge_color_count(+Grid, +FgColor, +BgColor, -N)
...
% Get the boundary of FgColor touching BgColor.
    gridedge_boundary(Grid, FgColor, BgColor, Cells),
% Count those cells.
    length(Cells, N).
