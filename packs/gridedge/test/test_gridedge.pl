...
% --- gridedge_edge_cells8 ---
test(edge_cells8_rb_all, []) :-
    g3x3_rb(G),
    gridedge_edge_cells8(G, r, Cells),
...
test(is_edge_boundary_cell, []) :-
    ...
    gridedge_is_edge(G, 0-1, yes).

test(is_edge_interior_uniform, []) :-
    g3x3_all_r(G),
    gridedge_is_edge(G, 1-1, no).

test(is_edge_center_of_ring, []) :-
    g5x5_ring(G),
    gridedge_is_edge(G, 2-2, no).

test(is_edge_border_of_ring, []) :-
    g5x5_ring(G),
    gridedge_is_edge(G, 1-1, yes).

% --- gridedge_boundary ---
test(boundary_r_touching_x, []) :-
    g3x3_rb(G),
    gridedge_boundary(G, r, x, Cells),
...
test(edge_grid_uniform_unchanged, []) :-
    g3x3_all_r(G),
    gridedge_edge_grid(G, r, e, G).

% --- gridedge_edge_count ---
test(edge_count_rb, []) :-
    g3x3_rb(G),
    gridedge_edge_count(G, r, 3).

test(edge_count_uniform_zero, []) :-
    g3x3_all_r(G),
    gridedge_edge_count(G, r, 0).

test(edge_count_cross, []) :-
    g3x3_cross(G),
    gridedge_edge_count(G, r, 4).

% --- gridedge_neighbors_diff ---
test(neighbors_diff_corner_cell, []) :-
    g3x3_rb(G),
    gridedge_neighbors_diff(G, 0, 0, Nbrs),
...
test(corners_uniform_none, []) :-
    g3x3_all_r(G),
    gridedge_corners(G, r, []).

% --- gridedge_endpoints ---
test(endpoints_cross_arms, []) :-
    g3x3_cross(G),
    gridedge_endpoints(G, r, Cells),
...
% --- gridedge_smooth_cells ---
test(smooth_cells_interior, []) :-
    g5x5_ring(G),
    gridedge_smooth_cells(G, r, Cells),
...
% --- gridedge_transition_map ---
test(transition_map_uniform_all_zero, []) :-
    g3x3_all_x(G),
    gridedge_transition_map(G, R),
...
% --- gridedge_edge_color_count ---
test(edge_color_count_r_to_x, []) :-
    g3x3_rb(G),
    gridedge_edge_color_count(G, r, x, N),
    N =:= 3.

test(edge_color_count_none, []) :-
    g3x3_all_r(G),
    gridedge_edge_color_count(G, r, x, 0).

% --- Combined tests ---
test(edge_cells_plus_smooth_partition, []) :-
    g5x5_ring(G),
    gridedge_edge_cells(G, r, ECells),
    gridedge_smooth_cells(G, r, SCells),
