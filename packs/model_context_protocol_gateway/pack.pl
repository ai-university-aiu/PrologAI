% State the fact: name(model_context_protocol_gateway).
name(model_context_protocol_gateway).
% State the fact: version('1.0.0').
version('1.0.0').
% State the fact: title('PrologAI MCP Gateway — Model Context Protocol 1.0 HTTP server exposing PrologAI to the AI agent ecosystem (PR 14)').
title('PrologAI MCP Gateway — Model Context Protocol 1.0 HTTP server exposing PrologAI to the AI agent ecosystem (PR 14)').
