/*  PrologAI — MCP Gateway  (Specification Section 3.12, PR 14)
    ...
    model_context_protocol_gateway_start/1  — start the HTTP server on Port (default 7474).
    ...
    Tool endpoints (POST /tools/<name>):
      ...
      body_command     — wraps mind_body_dispatch_command/2
      sona_learn       — wraps synaptic_ontological_neural_aggregator_absorb/1
      sona_recall      — wraps synaptic_ontological_neural_aggregator_retrieve/3
...
:- module(model_context_protocol_gateway, [
    % Continue the multi-line expression started above.
    model_context_protocol_gateway_start/1,   % +Port
    % Supply 'model_context_protocol_gateway_stop/0' as the next argument to the expression above.
    model_context_protocol_gateway_stop/0,
    % Continue the multi-line expression started above.
    model_context_protocol_gateway_set_api_key/1,     % +Key (atom)
    % Continue the multi-line expression started above.
    model_context_protocol_gateway_get_api_key/1      % -Key
...
% Declare 'model_context_protocol_gateway_api_key/1' as dynamic — its facts may be added or removed at runtime.
:- dynamic model_context_protocol_gateway_api_key/1.
% State the fact: mcp api key('prologai-dev-key').
model_context_protocol_gateway_api_key('prologai-dev-key').

% Define a clause for 'mcp set api key': succeed when the following conditions hold.
model_context_protocol_gateway_set_api_key(Key) :-
    % Remove all matching facts from the runtime knowledge base.
    retractall(model_context_protocol_gateway_api_key(_)),
    % Add a new fact or rule to the runtime knowledge base.
    assertz(model_context_protocol_gateway_api_key(Key)).
...
model_context_protocol_gateway_get_api_key(Key) :-
    ...
    model_context_protocol_gateway_api_key(Key).

% ---------------------------------------------------------------------------
% model_context_protocol_gateway_start/1
% ---------------------------------------------------------------------------
...
model_context_protocol_gateway_start(Port) :-
    % Execute: ( model_context_protocol_gateway_active_port(Port).
    ( model_context_protocol_gateway_active_port(Port)
    % If the condition above succeeded, perform the following action.
    ->  true   % already running on this port
    % Otherwise (else branch), perform the following action.
    ;   model_context_protocol_gateway_stop,   % stop any previously running server
        % Continue the multi-line expression started above.
        http_handler(root(tools), model_context_protocol_gateway_dispatch_tool, [prefix]),
        % Continue the multi-line expression started above.
        http_server(http_dispatch, [port(Port)]),
        % Continue the multi-line expression started above.
        retractall(model_context_protocol_gateway_active_port(_)),
        % Continue the multi-line expression started above.
        assertz(model_context_protocol_gateway_active_port(Port))
    % Close the expression opened above.
    ).
...
% Declare 'model_context_protocol_gateway_active_port/1' as dynamic — its facts may be added or removed at runtime.
:- dynamic model_context_protocol_gateway_active_port/1.

% ---------------------------------------------------------------------------
% model_context_protocol_gateway_stop/0
% ---------------------------------------------------------------------------

% Execute: model_context_protocol_gateway_stop :-.
...
% ---------------------------------------------------------------------------
% model_context_protocol_gateway_dispatch_tool/1 — HTTP handler for all /tools/<name> requests
% ---------------------------------------------------------------------------

% Define a clause for 'mcp dispatch tool': succeed when the following conditions hold.
model_context_protocol_gateway_dispatch_tool(Request) :-
    % Authenticate
    % Execute: ( model_context_protocol_gateway_check_auth(Request).
    ( model_context_protocol_gateway_check_auth(Request)
    % If the condition above succeeded, perform the following action.
    ->  true
    % Otherwise (else branch), perform the following action.
    ...
    % State a fact for 'catch' with the arguments listed below.
        ...
        % Continue the multi-line expression started above.
        model_context_protocol_gateway_tool(ToolName, Params, Result),
...
model_context_protocol_gateway_tool(lattice_query, Params, Results) :-
...
% Define a clause for 'dispatch tool': succeed when the following conditions hold.
model_context_protocol_gateway_tool(lattice_inscribe, Params, Id) :-
...
% Define a clause for 'dispatch tool': succeed when the following conditions hold.
model_context_protocol_gateway_tool(lattice_excise, Params, ok) :-
...
% Define a clause for 'dispatch tool': succeed when the following conditions hold.
model_context_protocol_gateway_tool(actor_list, _Params, Names) :-
    % State the fact: cyclic actor list(Names).
    cyclic_actor_list(Names).

% Define a clause for 'dispatch tool': succeed when the following conditions hold.
model_context_protocol_gateway_tool(actor_stop, Params, ok) :-
...
% Define a clause for 'dispatch tool': succeed when the following conditions hold.
model_context_protocol_gateway_tool(sentinels_list, Params, SentinelTerms) :-
...
% Define a clause for 'dispatch tool': succeed when the following conditions hold.
model_context_protocol_gateway_tool(body_enroll, Params, ok) :-
...
% Define a clause for 'dispatch tool': succeed when the following conditions hold.
model_context_protocol_gateway_tool(body_signal, Params, ok) :-
    % Check that 'Address' is unifiable with 'Params.get(address, 'herald://unknown')'.
    Address = Params.get(address, 'herald://unknown'),
...
% Define a clause for 'dispatch tool': succeed when the following conditions hold.
model_context_protocol_gateway_tool(body_command, Params, ok) :-
    % Check that 'Address' is unifiable with 'Params.get(address, 'herald://unknown')'.
    Address = Params.get(address, 'herald://unknown'),
...
% Define a clause for 'dispatch tool': succeed when the following conditions hold.
model_context_protocol_gateway_tool(sona_learn, Params, ok) :-
...
% Define a clause for 'dispatch tool': succeed when the following conditions hold.
model_context_protocol_gateway_tool(assess_all, Params, ReportAtom) :-
...
% Define a clause for 'dispatch tool': succeed when the following conditions hold.
model_context_protocol_gateway_tool(Unknown, _Params, _Result) :-
    % State the fact: throw(error(unknown_tool(Unknown), model_context_protocol_gateway_tool/3)).
    throw(error(unknown_tool(Unknown), model_context_protocol_gateway_tool/3)).
