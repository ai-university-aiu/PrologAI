...
test(spiral_2x2, []) :-
    ...
    gsp_spiral(G, [0-0, 0-1, 1-1, 1-0]).

% --- gsp_read_spiral ---
test(read_3x3, []) :-
    g3x3(G),
    % Clockwise: (0,0)=a,(0,1)=b,(0,2)=c,(1,2)=f,(2,2)=i,(2,1)=h,(2,0)=g,(1,0)=d,(1,1)=e
    gsp_read_spiral(G, [a, b, c, f, i, h, g, d, e]).
...
test(read_checker, []) :-
    ...
    gsp_read_spiral(G, [r, x, r, x, r, x, r, x, r]).
...
test(frame_spiral_0_3x3_order, []) :-
    ...
    gsp_frame_spiral(G, 0, [0-0, 0-1, 0-2, 1-2, 2-2, 2-1, 2-0, 1-0]).

% --- gsp_all_frame_spirals ---
...
test(spiral_uniform_3x3_no, []) :-
    g3x3(G),
    gsp_spiral_uniform(G, no).

% --- gsp_spiral_reversed ---
test(spiral_reversed_1x1, []) :-
    g1x1(G),
    gsp_spiral_reversed(G, [0-0]).

test(spiral_reversed_3x3_first, []) :-
    g3x3(G),
    gsp_spiral_reversed(G, [1-1|_]).
...
test(spiral_count_r_checker, []) :-
    ...
    gsp_spiral_count(G, r, 5).

test(spiral_count_x_checker, []) :-
    g3x3_checker(G),
    % Checkerboard has 4 x's
    gsp_spiral_count(G, x, 4).

test(spiral_count_1x1, []) :-
    g1x1(G),
    gsp_spiral_count(G, a, 1).

% --- gsp_fill_spiral ---
test(fill_spiral_alias, []) :-
    g3x3(G),
    % gsp_fill_spiral is alias for gsp_write_spiral
    gsp_fill_spiral(G, [r,r,r,r,r,r,r,r,r], Result),
...
test(spiral_slice_first_3, []) :-
    ...
    gsp_spiral_slice(G, 0, 2, [0-0, 0-1, 0-2]).

test(spiral_slice_last_1, []) :-
    g3x3(G),
    % Position 8 = center
    gsp_spiral_slice(G, 8, 8, [1-1]).

% Combined
test(read_write_roundtrip, []) :-
    g4x4(G),
    gsp_read_spiral(G, Vals),
    gsp_write_spiral(G, Vals, Result),
