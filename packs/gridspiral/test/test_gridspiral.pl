...
:- begin_tests(gridspiral).

% --- gridspiral_spiral ---
test(spiral_3x3_length, []) :-
    g3x3(G),
    gridspiral_spiral(G, Cells),
    % 3x3 = 9 cells
    length(Cells, 9).
...
test(spiral_1x1, []) :-
    g1x1(G),
    gridspiral_spiral(G, [0-0]).

test(spiral_2x2, []) :-
    g2x2(G),
    % Clockwise: top-left, top-right, bottom-right, bottom-left
    gridspiral_spiral(G, [0-0, 0-1, 1-1, 1-0]).

% --- gridspiral_read_spiral ---
test(read_3x3, []) :-
    g3x3(G),
    % Clockwise: (0,0)=a,(0,1)=b,(0,2)=c,(1,2)=f,(2,2)=i,(2,1)=h,(2,0)=g,(1,0)=d,(1,1)=e
    gridspiral_read_spiral(G, [a, b, c, f, i, h, g, d, e]).

test(read_1x1, []) :-
    g1x1(G),
    gridspiral_read_spiral(G, [a]).

test(read_checker, []) :-
    g3x3_checker(G),
    % Spiral: (0,0)=r,(0,1)=x,(0,2)=r,(1,2)=x,(2,2)=r,(2,1)=x,(2,0)=r,(1,0)=x,(1,1)=r
    gridspiral_read_spiral(G, [r, x, r, x, r, x, r, x, r]).

% --- gridspiral_write_spiral ---
test(write_spiral_partial, []) :-
    g3x3(G),
    % Write only 2 values: (0,0) and (0,1) change, rest unchanged
    gridspiral_write_spiral(G, [r, x], Result),
...
test(write_spiral_full_3x3, []) :-
    ...
    gridspiral_write_spiral(G, [p,p,p,p,p,p,p,p,p], Result),
    Result = [[p,p,p],[p,p,p],[p,p,p]].

% --- gridspiral_spiral_length ---
test(spiral_length_3x3, []) :-
    g3x3(G),
    gridspiral_spiral_length(G, 9).

test(spiral_length_2x4, []) :-
    g2x4(G),
    gridspiral_spiral_length(G, 8).

test(spiral_length_1x1, []) :-
    g1x1(G),
    gridspiral_spiral_length(G, 1).

% --- gridspiral_spiral_index ---
test(spiral_index_corner, []) :-
    g3x3(G),
    % Top-left corner is index 0
    gridspiral_spiral_index(G, 0, 0, 0).

test(spiral_index_center, []) :-
    g3x3(G),
    % Center (1,1) is last = index 8
    gridspiral_spiral_index(G, 1, 1, 8).

test(spiral_index_2x2_last, []) :-
    g2x2(G),
    % (1,0) is last in 2x2 spiral = index 3
    gridspiral_spiral_index(G, 1, 0, 3).

% --- gridspiral_nth_spiral ---
test(nth_spiral_0, []) :-
    g3x3(G),
    % Position 0 = (0,0)
    gridspiral_nth_spiral(G, 0, 0, 0).

test(nth_spiral_last, []) :-
    g3x3(G),
    % Position 8 = center (1,1)
    gridspiral_nth_spiral(G, 8, 1, 1).

% --- gridspiral_frame_spiral ---
test(frame_spiral_0_3x3_count, []) :-
    g3x3(G),
    gridspiral_frame_spiral(G, 0, Cells),
    % Outer frame of 3x3 = 8 cells
    length(Cells, 8).

test(frame_spiral_1_3x3_count, []) :-
    g3x3(G),
    gridspiral_frame_spiral(G, 1, Cells),
...
test(frame_spiral_0_3x3_order, []) :-
    ...
    gridspiral_frame_spiral(G, 0, [0-0, 0-1, 0-2, 1-2, 2-2, 2-1, 2-0, 1-0]).

% --- gridspiral_all_frame_spirals ---
test(all_frame_spirals_3x3, []) :-
    g3x3(G),
    gridspiral_all_frame_spirals(G, Spirals),
    length(Spirals, 2).
...
% --- gridspiral_spiral_uniform ---
test(spiral_uniform_1x1, []) :-
    g1x1(G),
    gridspiral_spiral_uniform(G, yes).
...
test(spiral_uniform_3x3_no, []) :-
    g3x3(G),
    gridspiral_spiral_uniform(G, no).

% --- gridspiral_spiral_reversed ---
test(spiral_reversed_1x1, []) :-
    g1x1(G),
    gridspiral_spiral_reversed(G, [0-0]).

test(spiral_reversed_3x3_first, []) :-
    g3x3(G),
    gridspiral_spiral_reversed(G, [1-1|_]).

test(spiral_reversed_3x3_last, []) :-
    g3x3(G),
    gridspiral_spiral_reversed(G, Rev),
    last(Rev, 0-0).

% --- gridspiral_spiral_count ---
test(spiral_count_r_checker, []) :-
    g3x3_checker(G),
    % Checkerboard has 5 r's
    gridspiral_spiral_count(G, r, 5).

test(spiral_count_x_checker, []) :-
    g3x3_checker(G),
    % Checkerboard has 4 x's
    gridspiral_spiral_count(G, x, 4).

test(spiral_count_1x1, []) :-
    g1x1(G),
    gridspiral_spiral_count(G, a, 1).

% --- gridspiral_fill_spiral ---
test(fill_spiral_alias, []) :-
    g3x3(G),
    % gridspiral_fill_spiral is alias for gridspiral_write_spiral
    gridspiral_fill_spiral(G, [r,r,r,r,r,r,r,r,r], Result),
    Result = [[r,r,r],[r,r,r],[r,r,r]].

% --- gridspiral_rotate_spiral ---
test(rotate_by_0, []) :-
    g3x3(G),
    % Rotate by 0 = identity
    gridspiral_rotate_spiral(G, 0, Result),
    Result = [[a,b,c],[d,e,f],[g,h,i]].

test(rotate_by_length, []) :-
    g3x3(G),
    % Rotate by 9 (= full cycle mod 9 = 0) = identity
    gridspiral_rotate_spiral(G, 9, Result),
    Result = [[a,b,c],[d,e,f],[g,h,i]].
...
test(rotate_checker_inverts, []) :-
    ...
    g3x3_checker(G),
    gridspiral_rotate_spiral(G, 1, Result),
    nth0(0, Result, Row0), nth0(0, Row0, x),
    nth0(0, Result, Row0b), nth0(1, Row0b, r).

% --- gridspiral_spiral_slice ---
test(spiral_slice_first_3, []) :-
    g3x3(G),
    % Positions 0..2 = top row
    gridspiral_spiral_slice(G, 0, 2, [0-0, 0-1, 0-2]).

test(spiral_slice_last_1, []) :-
    g3x3(G),
    % Position 8 = center
    gridspiral_spiral_slice(G, 8, 8, [1-1]).

% Combined
test(read_write_roundtrip, []) :-
    g4x4(G),
    gridspiral_read_spiral(G, Vals),
    gridspiral_write_spiral(G, Vals, Result),
...
test(spiral_1x3, []) :-
    ...
    gridspiral_spiral(G, [0-0, 0-1, 0-2]).
