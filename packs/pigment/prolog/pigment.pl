...
:- module(pigment, [
    % pg_recolor_all/3: set every obj in the scene to a fixed color.
    pg_recolor_all/3,
    % pg_recolor_one/4: change all objs with color From to color To; keep others unchanged.
    pg_recolor_one/4,
    % pg_swap/4: swap colors C1 and C2 throughout the scene.
    pg_swap/4,
    % pg_apply_table/3: apply a From-To color table; objs not in table keep their color.
    pg_apply_table/3,
    % pg_apply_table_strict/3: apply table; keep only objs whose color has a table entry.
    pg_apply_table_strict/3,
    % pg_infer_table/3: infer color table from two scenes with identical cell structures.
    pg_infer_table/3,
    % pg_zip_recolor/3: recolor each obj with the corresponding color from a Colors list.
    pg_zip_recolor/3,
    % pg_majority_to/3: recolor all objs sharing the most frequent color to a new color.
    pg_majority_to/3,
    % pg_minority_to/3: recolor all objs sharing the least frequent color to a new color.
    pg_minority_to/3,
    % pg_unique_to/3: recolor objs whose color appears exactly once to a new color.
    pg_unique_to/3,
    % pg_shared_to/3: recolor objs whose color appears more than once to a new color.
    pg_shared_to/3,
    % pg_invert_table/2: produce the inverse mapping by swapping From and To in each pair.
    pg_invert_table/2,
    % pg_table_from/2: sorted list of From colors appearing in the table.
    pg_table_from/2,
    % pg_consistent/1: succeed if no From color maps to two different To colors.
    pg_consistent/1
]).
...
% pg_count_by_color_(+Objs, -Counts): private helper producing sorted Color-N pairs.
...
% pg_recolor_all(+Objs, +Color, -Out): set every obj's color to Color.
...
% pg_recolor_one(+Objs, +From, +To, -Out): change color From to To everywhere.
...
% pg_swap(+Objs, +C1, +C2, -Out): exchange colors C1 and C2 throughout the scene.
...
% pg_apply_table(+Objs, +Table, -Out): apply color table; keep color if not in table.
...
% pg_apply_table_strict(+Objs, +Table, -Out): apply table; exclude objs not in table.
...
% pg_infer_table(+Objs1, +Objs2, -Table): infer color mapping from same-cell obj pairs.
...
% pg_zip_recolor(+Objs, +Colors, -Out): recolor each obj with the matching Colors list entry.
...
% pg_majority_to(+Objs, +Color, -Out): recolor all majority-color objs to Color.
...
% pg_minority_to(+Objs, +Color, -Out): recolor all minority-color objs to Color.
...
% pg_unique_to(+Objs, +Color, -Out): recolor objs with a unique color to Color.
...
% pg_shared_to(+Objs, +Color, -Out): recolor objs with a shared color to Color.
...
% pg_invert_table(+Table, -Inv): produce the inverse by swapping each From-To pair.
pg_invert_table(Table, Inv) :-
% Swap every From-To pair to To-From.
    findall(To-From, member(From-To, Table), Inv).

% pg_table_from(+Table, -Froms): sorted list of From colors in the table.
pg_table_from(Table, Froms) :-
% Collect From values and deduplicate with sort/2.
    findall(F, member(F-_, Table), Fs),
    sort(Fs, Froms).

% pg_consistent(+Table): succeed if no From color maps to two different To colors.
pg_consistent(Table) :-
