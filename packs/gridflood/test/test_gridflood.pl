...
test(region_cells_all) :-
    g3x3_r(G),
    gf_region_cells(G, 1, 1, Cells),
    length(Cells, 9).

% --- gf_region_size tests ---
...
test(region_bbox_l_shape) :-
    g3x3_rb(G),
    gf_region_bbox(G, 0, 2, MinR, MinC, MaxR, MaxC),
    MinR = 0, MinC = 0, MaxR = 2, MaxC = 2.

% --- gf_enclosed_cells tests ---
...
test(enclosed_cells_center) :-
    g5x5_hole(G),
    gf_enclosed_cells(G, r, Enclosed),
    Enclosed = [2-2].

% --- gf_fill_enclosed tests ---
...
test(region_cells_then_fill) :-
    ...
    length(Cells, 4),
    gf_fill(G, 0, 0, x, R),
    gf_n_components(R, r, 0),
    gf_n_components(R, x, 1).
