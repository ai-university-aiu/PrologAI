...
test(unreached_is_zero) :-
    setup_graph,
    co_prime:pr_activation([key], 1.0, nowhere, L),
    assertion(L =:= 0.0).

% Decay fades associations by distance.
test(decay_fades) :-
    setup_graph,
    co_prime:pr_activation([key], 0.5, lock, LLock),
    % key(1.0) * 0.9 * 0.5 = 0.45
    assertion(abs(LLock - 0.45) < 1e-9),
    co_prime:pr_activation([key], 0.5, door, LDoor),
    % 0.45 * 0.8 * 0.5 = 0.18
...
test(primed_ranking) :-
    setup_graph,
    co_prime:pr_primed([key], 1.0, 2, Top),
