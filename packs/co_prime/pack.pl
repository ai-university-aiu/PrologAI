% State the fact: this pack is named co_prime.
name(co_prime).
% State the fact: this is version 1.0.0.
version('1.0.0').
% State the fact: the title carries the work-package number.
title('PrologAI Causalontology Priming — spreading activation over an association graph, so that whatever is active makes its neighbours relevant, with decay by distance (WP-422)').
