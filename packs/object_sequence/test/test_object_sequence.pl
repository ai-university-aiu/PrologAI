:- use_module('../prolog/object_sequence').
:- use_module(library(plunit)).

:- begin_tests(object_sequence).
...
test(color_seq_basic) :-
    ...
    object_sequence_color_seq([A,B,C], Colors),
    Colors == [r,b,g].

test(color_seq_single) :-
    bar2(X),
    object_sequence_color_seq([X], Colors),
    Colors == [b].

test(color_seq_empty) :-
    object_sequence_color_seq([], Colors),
    Colors == [].
...
test(size_seq_basic) :-
    ...
    object_sequence_size_seq([A,B,C], Sizes),
    Sizes == [2,3,4].

test(size_seq_single) :-
    dot1(X),
    object_sequence_size_seq([X], Sizes),
    Sizes == [1].

test(size_seq_empty) :-
    object_sequence_size_seq([], Sizes),
    Sizes == [].
...
test(centroid_seq_basic) :-
    dot1(A), dot2(B), dot3(C),
    object_sequence_centroid_seq([A,B,C], Cs),
    Cs == [r(0,0), r(0,2), r(0,4)].

test(centroid_seq_bar) :-
    bar3(X),
    object_sequence_centroid_seq([X], Cs),
    Cs == [r(2,1)].

test(centroid_seq_empty) :-
    object_sequence_centroid_seq([], Cs),
    Cs == [].
...
test(step_seq_basic) :-
    ...
    object_sequence_step_seq([A,B,C], Steps),
    Steps == [dr(2,0), dr(2,0)].

test(step_seq_one_element) :-
    dot1(X),
    object_sequence_step_seq([X], Steps),
    Steps == [].

test(step_seq_empty) :-
    object_sequence_step_seq([], Steps),
    Steps == [].
...
test(is_growing_yes) :-
    bar2(A), bar3(B), bar4(C),
    object_sequence_is_growing([A,B,C]).

test(is_growing_no_equal, [fail]) :-
    bar2(A), bar2(B),
    object_sequence_is_growing([A,B]).
...
test(is_shrinking_yes) :-
    bar4(A), bar3(B), bar2(C),
    object_sequence_is_shrinking([A,B,C]).

test(is_shrinking_no_equal, [fail]) :-
    bar3(A), bar3(B),
    object_sequence_is_shrinking([A,B]).
...
test(const_step_yes) :-
    col0(A), col1(B), col2(C),
    object_sequence_const_step([A,B,C], DR, DC),
    DR == 2, DC == 0.

test(const_step_fail_inconsistent, [fail]) :-
    off1(A), off2(B), off3(C),
    object_sequence_const_step([A,B,C], _, _).

test(const_step_fail_single, [fail]) :-
    dot1(X),
    object_sequence_const_step([X], _, _).

% --- object_sequence_const_row_step/2 ---

test(const_row_step_yes) :-
    col0(A), col1(B), col2(C),
    object_sequence_const_row_step([A,B,C], DR),
    DR == 2.
...
test(size_period_1) :-
    ...
    C = obj(g,[r(2,0)]),
    object_sequence_size_period([A,B,C], P, Cycle),
    P == 1, Cycle == [1].
...
test(size_period_2) :-
    ...
    C = obj(g,[r(1,0)]),
    D = obj(y,[r(1,1),r(1,2)]),
    object_sequence_size_period([A,B,C,D], P, Cycle),
    P == 2, Cycle == [1,2].

test(size_period_full) :-
    A = obj(r,[r(0,0)]), B = obj(b,[r(0,1),r(0,2)]),
    C = obj(g,[r(0,3),r(0,4),r(0,5)]),
    object_sequence_size_period([A,B,C], P, _),
    P == 3.
...
test(collinear_horizontal) :-
    dot1(A), dot2(B), dot3(C),
    object_sequence_collinear([A,B,C]).
...
test(next_centroid_row) :-
    col0(A), col1(B), col2(C),
    object_sequence_next_centroid([A,B,C], r(NR,NC)),
    NR == 6, NC == 0.
...
test(next_centroid_fail_inconsistent, [fail]) :-
    off1(A), off2(B), off3(C),
    object_sequence_next_centroid([A,B,C], _).
...
test(zip_colors_basic) :-
    ...
    object_sequence_zip_colors([A,B,C], [X,Y,Z], Pairs),
    Pairs == [r-b, b-g, g-y].

test(zip_colors_empty) :-
    object_sequence_zip_colors([], [], Pairs),
    Pairs == [].

:- end_tests(object_sequence).
