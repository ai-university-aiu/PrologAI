:- module(grid_frame, [
    grid_frame_cell_depth/4,
    grid_frame_frame/3,
    grid_frame_max_depth/2,
    grid_frame_frame_count/2,
    grid_frame_peel/2,
    grid_frame_all_frames/2,
    grid_frame_frame_uniform/3,
    grid_frame_frame_colors/3,
    grid_frame_set_frame/4,
    grid_frame_count_in_frame/4,
    grid_frame_uniform_frames/2,
    grid_frame_fill_frames/3,
    grid_frame_innermost/2,
    grid_frame_onion_layers/2
]).
...
grid_frame_build_(H, W, Goal, Grid) :-
...
grid_frame_all_eq_([]).
grid_frame_all_eq_([_]).
grid_frame_all_eq_([A, A | Rest]) :- grid_frame_all_eq_([A | Rest]).

% --- PUBLIC PREDICATES ---

% grid_frame_cell_depth(+Grid, +R, +C, -D)
...
% Build inner grid by shifting indices by 1.
    grid_frame_build_(H1, W1,
        [R, C, V]>>(R1 is R + 1, C1 is C + 1, grid_frame_cell_(Grid, R1, C1, V)),
        Inner).

% grid_frame_all_frames(+Grid, -Frames)
...
grid_frame_all_frames(Grid, Frames) :-
% Compute max depth.
    grid_frame_max_depth(Grid, MaxD),
% Collect frame cell-lists in depth order.
    findall(Cells, (between(0, MaxD, D), grid_frame_frame(Grid, D, Cells)), Frames).

% grid_frame_frame_uniform(+Grid, +D, -Bool)
...
% Collect values of all cells in this frame.
    findall(V, (member(R-C, Cells), grid_frame_cell_(Grid, R, C, V)), Vals),
% Test uniformity.
    (grid_frame_all_eq_(Vals) -> Bool = yes ; Bool = no).

% grid_frame_frame_colors(+Grid, +D, -Colors)
...
% grid_frame_set_frame(+Grid, +D, +Color, -Result)
% Result is Grid with all cells at frame depth D replaced by Color.
grid_frame_set_frame(Grid, D, Color, Result) :-
% Get dimensions.
    grid_frame_dims_(Grid, H, W),
% Build result: replace depth-D cells, keep others.
    grid_frame_build_(H, W,
        [R, C, V]>>(grid_frame_depth_(H, W, R, C, Dep),
                    grid_frame_cell_(Grid, R, C, Orig),
                    (Dep =:= D -> V = Color ; V = Orig)),
        Result).

% grid_frame_count_in_frame(+Grid, +D, +Color, -Count)
% Count is the number of cells of Color at frame depth D.
grid_frame_count_in_frame(Grid, D, Color, Count) :-
% Get cells at depth D.
    grid_frame_frame(Grid, D, Cells),
% Count Color occurrences.
    findall(1, (member(R-C, Cells), grid_frame_cell_(Grid, R, C, Color)), Ones),
    length(Ones, Count).

% grid_frame_uniform_frames(+Grid, -Ds)
% Ds is the list of frame depths (in ascending order) for which the frame is uniform.
grid_frame_uniform_frames(Grid, Ds) :-
% Get max depth.
    grid_frame_max_depth(Grid, MaxD),
% Collect uniform depths.
    findall(D, (between(0, MaxD, D), grid_frame_frame_uniform(Grid, D, yes)), Ds).

% grid_frame_fill_frames(+Grid, +Colors, -Result)
...
% Build result: for each cell, use Colors[depth] if available.
    grid_frame_build_(H, W,
        [R, C, V]>>(grid_frame_depth_(H, W, R, C, D),
                    grid_frame_cell_(Grid, R, C, Orig),
                    (nth0(D, Colors, FillColor) -> V = FillColor ; V = Orig)),
        Result).

% grid_frame_innermost(+Grid, -Cells)
...
% Get max depth.
    grid_frame_max_depth(Grid, MaxD),
% Frame at max depth.
    grid_frame_frame(Grid, MaxD, Cells).

% grid_frame_onion_layers(+Grid, -Layers)
