...
:- begin_tests(grid_frame).

% --- grid_frame_cell_depth ---
test(cell_depth_corner_3x3, []) :-
    g3x3(G),
    % Corner (0,0): min(0,2,0,2) = 0
    grid_frame_cell_depth(G, 0, 0, 0).

test(cell_depth_center_3x3, []) :-
    g3x3(G),
    % Center (1,1): min(1,1,1,1) = 1
    grid_frame_cell_depth(G, 1, 1, 1).
...
test(cell_depth_center_5x5, []) :-
    ...
    grid_frame_cell_depth(G, 2, 2, 2).

% --- grid_frame_frame cell counts ---
test(frame0_3x3_count, []) :-
    g3x3(G),
    grid_frame_frame(G, 0, Cells),
    % 3x3 border: 8 cells
    length(Cells, 8).

test(frame1_3x3_count, []) :-
    g3x3(G),
    grid_frame_frame(G, 1, Cells),
    % 3x3 center: 1 cell
    length(Cells, 1).
...
test(frame2_5x5_count, []) :-
    g5x5(G),
    grid_frame_frame(G, 2, Cells),
    % 5x5 center: 1 cell
    length(Cells, 1).

% --- grid_frame_max_depth ---
test(max_depth_3x3, []) :-
    g3x3(G),
    grid_frame_max_depth(G, 1).

test(max_depth_5x5, []) :-
    g5x5(G),
    grid_frame_max_depth(G, 2).
...
test(max_depth_2x4, []) :-
    g2x4(G),
    % min(2,4)=2; (2-1)//2=0
    grid_frame_max_depth(G, 0).

% --- grid_frame_frame_count ---
test(frame_count_3x3, []) :-
    g3x3(G),
    grid_frame_frame_count(G, 2).

test(frame_count_5x5, []) :-
    g5x5(G),
    grid_frame_frame_count(G, 3).

test(frame_count_1x1, []) :-
    g1x1(G),
    grid_frame_frame_count(G, 1).

% --- grid_frame_peel ---
test(peel_3x3_size, []) :-
    g3x3_rx(G),
    grid_frame_peel(G, Inner),
    % 3x3 peeled = 1x1
    length(Inner, 1),
    Inner = [[x]].
...
% --- grid_frame_all_frames ---
test(all_frames_3x3_count, []) :-
    g3x3(G),
    grid_frame_all_frames(G, Frames),
...
test(frame_uniform_0_3x3_no, []) :-
    ...
    grid_frame_frame_uniform(G, 0, no).

% --- grid_frame_frame_colors ---
test(frame_colors_0_rx, []) :-
    g3x3_rx(G),
    grid_frame_frame_colors(G, 0, Colors),
    Colors = [r].

test(frame_colors_2_onion, []) :-
    g5x5_onion(G),
    grid_frame_frame_colors(G, 2, Colors),
    Colors = [b].
...
% --- grid_frame_set_frame ---
test(set_frame_0_border, []) :-
    g3x3_rx(G),
    grid_frame_set_frame(G, 0, y, Result),
    % Border becomes y
...
% --- grid_frame_onion_layers ---
test(onion_layers_rx, []) :-
    g3x3_rx(G),
    grid_frame_onion_layers(G, Layers),
    Layers = [0-r, 1-x].
