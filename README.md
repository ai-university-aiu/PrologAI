...
| `causal_planner` | Causal Planning (causal_planner_*, Layer 370, WP-395) — backward causal-chain planning over the Causalontology CRO graph: procedures as sequence-cause relations, backward planning whose steps must be achievable and never avoided, bounded backward graph search, and honest plan execution. Kept distinct from the flagship HTN `planner` pack (they are different planning paradigms). |
