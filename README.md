...
| `safety_governor` | Causalontology Safety Governor — the conscience that owns the last word: an immutable constitution of forbidden action patterns (variables allowed, so forbid touch(_) bans touching anything) plus learned hazards mirrored from co_core's preventive relations, a pre-action check that returns allow or veto(Reason), an append-only veto log the mind's own learning cannot rewrite, and no predicate to weaken a constraint except an explicit operator reset (co_*, Layer 389) (WP-414) |
