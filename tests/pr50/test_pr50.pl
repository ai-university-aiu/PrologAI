/*  PrologAI — PR 50 Imaginative Memory: Mindscapes, Tableaux, Reveries Acceptance Tests
    ...
    AC-PR50-001: pai_mindscape_new creates a mindscape held in the imagined reality.
    AC-PR50-002: pai_mindscape_new rejects the observed reality (sandbox guard).
    AC-PR50-003: pai_tableau_add binds elements and returns a tableau on the mindscape.
    AC-PR50-004: pai_tableau_add rejects an element of an unknown kind.
    AC-PR50-005: pai_tableau_ground reuses a percept as a grounded figure.
    AC-PR50-006: pai_reverie_render produces exactly Steps frames.
    ...
    AC-PR50-009: pai_mindscape_clear empties the canvas of tableaux and reveries.
    AC-PR50-010: pai_imagine_fresh creates, binds, and renders in one call.
*/
...
:- use_module(library(imagination), [
    % Supply 'pai_mindscape_new/2' as the next argument to the expression above.
    pai_mindscape_new/2,
    % Supply 'pai_mindscape_reality/2' as the next argument to the expression above.
    pai_mindscape_reality/2,
    % Supply 'pai_tableau_add/3' as the next argument to the expression above.
    pai_tableau_add/3,
    % Supply 'pai_tableau_ground/3' as the next argument to the expression above.
    pai_tableau_ground/3,
    % Supply 'pai_reverie_render/3' as the next argument to the expression above.
    pai_reverie_render/3,
    % Supply 'pai_reverie_frames/2' as the next argument to the expression above.
    pai_reverie_frames/2,
    % Supply 'pai_mindscape_clear/1' as the next argument to the expression above.
    pai_mindscape_clear/1,
...
test(mindscape_new_imagined) :-
    ...
    pai_mindscape_reality(M, imagined).
...
test(tableau_rejects_bad_kind, [fail]) :-
    ...
    pai_tableau_add(M, [element(sprite, ball, [])], _T).
...
test(tableau_ground) :-
    ...
    pai_tableau_add(M, [element(vantage, eye_level, [])], T),
    % Ground a perceived object 'cup_07' as a figure on the tableau.
    pai_tableau_ground(T, cup_07, Ref),
...
test(reverie_frame_count) :-
    ...
    % Render five frames.
    pai_reverie_render(M, 5, R),
    % Retrieve the frames.
    pai_reverie_frames(R, Frames),
...
test(reverie_vantage) :-
    ...
    pai_tableau_add(M, [element(vantage, birds_eye, []),
                        element(figure, ball, [pos(0,0)])], _T),
    % Render two frames.
    pai_reverie_render(M, 2, R),
    % Retrieve the frames.
    pai_reverie_frames(R, Frames),
...
test(mindscape_clear) :-
    ...
    pai_mindscape_new(imagined, M),
    % Add a tableau.
    pai_tableau_add(M, [element(figure, ball, [pos(0,0)])], _T),
    % Render a reverie from it.
    pai_reverie_render(M, 2, R),
    % Clear the mindscape.
    pai_mindscape_clear(M),
    % Confirm the reverie is gone (its frames can no longer be retrieved).
    \+ pai_reverie_frames(R, _Frames).
...
test(imagine_fresh) :-
    ...
    pai_imagine_fresh(hypothetical,
