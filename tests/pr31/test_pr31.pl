/*  PrologAI — PR 31 Developmental Language Acquisition Acceptance Tests
    ...
    AC-PR31-004: acquisition_ground links a word form to a percept reference.
    AC-PR31-005: Words heard before referents persist ungrounded; later
                 grounding succeeds.
    AC-PR31-006: acquisition_symbolize records a signifier→signified link.
    AC-PR31-007: Repeated symbolization increments the count.
    AC-PR31-008: acquisition_chain_promote combines units into a next-tier composite.
    AC-PR31-009: acquisition_word_of queries by phoneme sequence.
*/
...
test(grounded_word_from_repeated_series) :-
    ...
    forall(
        ...
        between(1, 3, _),
        % Continue the multi-line expression started above.
        once(acquisition_chain_phonemes([k, ae, t], _))
    % Close the expression opened above.
    ),
    % Ground to percept
    % State a fact for 'once' with the arguments listed below.
    once(acquisition_ground('k-ae-t', cat_percept_001, GId)),
    % State a fact for 'nonvar' with the arguments listed below.
    nonvar(GId),
    % State the fact: once(acquisition_grounding_of('k-ae-t', cat_percept_001)).
    once(acquisition_grounding_of('k-ae-t', cat_percept_001)).
...
test(word_promoted_at_threshold) :-
    ...
    forall(
        ...
        between(1, 3, _),
        % Continue the multi-line expression started above.
        once(acquisition_chain_phonemes([d, ae, g], _))
    % Close the expression opened above.
    ),
    % State a fact for 'once' with the arguments listed below.
    once(acquisition_chain_phonemes([d, ae, g], Candidates)),  % 4th call returns candidate
...
test(no_word_before_threshold) :-
    % State a fact for 'once' with the arguments listed below.
    once(acquisition_chain_phonemes([b, ir, d], C1)),
    % Check that 'C1' is unifiable with '[]'.
    C1 = [],
    % State a fact for 'once' with the arguments listed below.
    once(acquisition_chain_phonemes([b, ir, d], C2)),
    % Check that 'C2' is unifiable with '[]'.
    C2 = [].

%  AC-PR31-004: acquisition_ground links word to percept
% Define a clause for 'test': succeed when the following conditions hold.
test(ground_links_to_percept) :-
    % State a fact for 'once' with the arguments listed below.
    once(acquisition_ground(apple, apple_image_42, GId)),
    % State a fact for 'nonvar' with the arguments listed below.
    nonvar(GId),
    % State the fact: once(acquisition_grounding_of(apple, apple_image_42)).
    once(acquisition_grounding_of(apple, apple_image_42)).
...
test(ungrounded_word_grounds_later) :-
    ...
    forall(
        ...
        between(1, 3, _),
        % Continue the multi-line expression started above.
        once(acquisition_chain_phonemes([tr, ii], _))
    % Close the expression opened above.
    ),
    ...
    % State a fact for 'once' with the arguments listed below.
    once(acquisition_ground('tr-ii', tree_percept_007, _)),
...
%  AC-PR31-006: acquisition_symbolize records signifier → signified
% Define a clause for 'test': succeed when the following conditions hold.
test(symbolize_records_link) :-
    % State a fact for 'once' with the arguments listed below.
    once(acquisition_symbolize(sunrise, hope, SId)),
    % State a fact for 'nonvar' with the arguments listed below.
    nonvar(SId),
...
test(symbolize_increments_count) :-
    % State a fact for 'once' with the arguments listed below.
    once(acquisition_symbolize(storm, danger, SId1)),
    % State a fact for 'once' with the arguments listed below.
    once(acquisition_symbolize(storm, danger, SId2)),
...
%  AC-PR31-008: acquisition_chain_promote builds next-tier composite
% Define a clause for 'test': succeed when the following conditions hold.
test(chain_promote_builds_composite) :-
    ...
    % State a fact for 'once' with the arguments listed below.
    once(acquisition_chain_promote(Units, NextTier)),
...
test(word_of_queries_sequence) :-
    ...
    forall(
        ...
        between(1, 3, _),
        % Continue the multi-line expression started above.
        once(acquisition_chain_phonemes([f, au, ks], _))
    % Close the expression opened above.
    ),
    % State a fact for 'once' with the arguments listed below.
    once(acquisition_word_of([f, au, ks], Form)),
