/*  PrologAI — PR 43 Agent Interoperability (A2A and Mail) Acceptance Tests
    ...
    AC-PR43-003: agent_to_agent_agent_card includes capabilities registered for the agent.
    AC-PR43-004: agent_to_agent_task submits a task and returns completed status.
    AC-PR43-005: agent_to_agent_task with unknown skill returns failed status.
    AC-PR43-006: agent_to_agent_peer_mail_send stores mail persistently.
    AC-PR43-007: agent_to_agent_peer_mail_fetch returns only mail for the recipient.
    AC-PR43-008: Delivered mail does not reappear in subsequent fetch.
    AC-PR43-009: agent_to_agent_agent_card opacity — card has no Lattice contents.
*/
...
test(mail_arrives_on_fetch, [setup(pr43_setup)]) :-
    ...
    once(agent_to_agent_peer_mail_send(recipient43, hello_subject, greeting43)),
    % Recipient comes online and fetches
    % State a fact for 'once' with the arguments listed below.
    once(agent_to_agent_peer_mail_fetch(recipient43, quarantine43, Messages)),
...
test(task_unknown_skill, [setup(pr43_setup)]) :-
    % State a fact for 'pai a2a task' with the arguments listed below.
    agent_to_agent_task(task_002_43, unknown_skill_43xyz, input(x, y), Status),
...
test(fetch_recipient_isolation, [setup(pr43_setup)]) :-
    % State a fact for 'once' with the arguments listed below.
    once(agent_to_agent_peer_mail_send(alice43, msg_for_alice, hello_alice43)),
    % State a fact for 'once' with the arguments listed below.
    once(agent_to_agent_peer_mail_send(bob43,   msg_for_bob,   hello_bob43)),
    % State a fact for 'once' with the arguments listed below.
    once(agent_to_agent_peer_mail_fetch(alice43, scope43, AliceMsgs)),
    % State a fact for 'once' with the arguments listed below.
    once(agent_to_agent_peer_mail_fetch(bob43,   scope43, BobMsgs)),
...
test(delivered_mail_not_refetched, [setup(pr43_setup)]) :-
    % State a fact for 'once' with the arguments listed below.
    once(agent_to_agent_peer_mail_send(carol43, subj43, body_carol43)),
    % State a fact for 'once' with the arguments listed below.
    once(agent_to_agent_peer_mail_fetch(carol43, scope43, First)),
    % Unify '1' with the number of elements in list 'First'.
    length(First, 1),
    % State a fact for 'once' with the arguments listed below.
    once(agent_to_agent_peer_mail_fetch(carol43, scope43, Second)),
