...
:- use_module(library(markers), [
    % Supply 'pai_marker_stamp/2' as the next argument to the expression above.
    pai_marker_stamp/2,
    % Supply 'pai_marker_of/2' as the next argument to the expression above.
    pai_marker_of/2,
    % Supply 'pai_marker_filter/3' as the next argument to the expression above.
    pai_marker_filter/3,
    % Supply 'pai_marker_decay/0' as the next argument to the expression above.
    pai_marker_decay/0,
    % Supply 'pai_marker_override/2' as the next argument to the expression above.
...
test(decay_moves_toward_neutral) :-
    ...
    % Call the goal 'pai_marker_decay'.
    pai_marker_decay,
    % State a fact for 'pai marker of' with the arguments listed below.
