/*  PrologAI — PR 26 Staged Appraisal and Coping (EMA) Acceptance Tests
    ...
    AC-PR26-004: appraisal_emotion_from_appraisal maps negative+self → shame.
    AC-PR26-005: appraisal_emotion_from_appraisal maps negative+other → anger.
    AC-PR26-006: appraisal_appraisal_decay reduces intensity over ticks.
    AC-PR26-007: safety_critical event gets desirability adjusted, not denied.
    AC-PR26-008: appraisal_causal_model asserts an event into the causal model.
...
test(shame_for_negative_self) :-
    ...
    % State a fact for 'once' with the arguments listed below.
    once(appraisal_emotion_from_appraisal(Appraisal, Emotion)),
...
test(appraisal_intensity_decays) :-
    ...
    I0 > 0.0,
    % Call the goal 'appraisal_appraisal_decay'.
...
%  AC-PR26-008: appraisal_causal_model asserts into causal interpretation
% Define a clause for 'test': succeed when the following conditions hold.
...
test(unknown_event_neutral_desirability) :-
    % State a fact for 'once' with the arguments listed below.
    once(appraisal_appraise(completely_unknown_event_xyz, [], Appraisal)),
