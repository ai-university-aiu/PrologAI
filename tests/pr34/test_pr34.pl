/*  PrologAI — PR 34 Generic Chainer and Meta-Reasoning Acceptance Tests
    ...
    AC-PR34-001: Given a causal rule base and a goal state, when chainer_chain is
    ...
    AC-PR34-009: chainer_control_policy returns stats for a named policy.
*/
...
:- use_module(library(chainer), [
    % Supply 'chainer_chain/5' as the next argument to the expression above.
    chainer_chain/5,
    % Supply 'chainer_rule_base/2' as the next argument to the expression above.
    chainer_rule_base/2,
    % Supply 'chainer_control_policy/3' as the next argument to the expression above.
...
test(backward_chain_causal, [setup(pr34_setup)]) :-
    ...
    chainer_chain(backward, rb34_causal, slippery_road, Result1, [max_depth(5), max_steps(100)]),
    ...
    % Unreachable goal
    % State a fact for 'pai chain' with the arguments listed below.
    chainer_chain(backward, rb34_causal, sunshine, Result2, [max_depth(5), max_steps(100)]),
...
test(control_policy_reliability, [setup(pr34_setup)]) :-
    ...
    chainer_control_policy(rb34_pol, shallow34, _),
    % State a fact for 'pai control policy' with the arguments listed below.
    chainer_control_policy(rb34_pol, deep34,    _),
    ...
    forall(between(1, 25, _),
           % Continue the multi-line expression started above.
           chainer_chain(backward, rb34_pol, b34, _, [max_depth(1), policy(shallow34)])),
    % Verify that for every solution of the Condition, the Action also holds.
    forall(between(1, 25, _),
           % Continue the multi-line expression started above.
           chainer_chain(backward, rb34_pol, c34, _, [max_depth(1), policy(shallow34)])),
    % 50 hard (c34, depth 5 → succeeds) under deep
    % Verify that for every solution of the Condition, the Action also holds.
    forall(between(1, 50, _),
           % Continue the multi-line expression started above.
           chainer_chain(backward, rb34_pol, c34, _, [max_depth(5), policy(deep34)])),
    % Stats updated: shallow < 50 successes, deep = 50 successes
    % State a fact for 'pai control policy' with the arguments listed below.
    chainer_control_policy(rb34_pol, shallow34, policy_stats(shallow34, S1, 50, _)),
    % State a fact for 'pai control policy' with the arguments listed below.
    chainer_control_policy(rb34_pol, deep34,    policy_stats(deep34,    50, 50, _)),
    % Check that 'S1' is less than '50'.
    S1 < 50,
    % Chainer prefers deep34 (higher reliability)
    % State a fact for 'pai control policy' with the arguments listed below.
    chainer_control_policy(rb34_pol, BestPolicy, _),
...
test(backward_chain_base_fact, [setup(pr34_setup)]) :-
    % State a fact for 'pai rule base' with the arguments listed below.
    chainer_rule_base(rb34_base, [fact(sunshine34)]),
    % State a fact for 'pai chain' with the arguments listed below.
    chainer_chain(backward, rb34_base, sunshine34, Result, [max_depth(1), max_steps(10)]),
...
test(forward_chain_derives_consequences, [setup(pr34_setup)]) :-
    ...
    % State a fact for 'pai chain' with the arguments listed below.
    chainer_chain(forward, rb34_fwd, [], Result, [max_steps(100)]),
...
test(backward_chain_depth_limit, [setup(pr34_setup)]) :-
    ...
    % State a fact for 'pai chain' with the arguments listed below.
    chainer_chain(backward, rb34_deep, leaf34, Result, [max_depth(1), max_steps(100)]),
...
test(budget_respected, [setup(pr34_setup)]) :-
    ...
    % State a fact for 'pai chain' with the arguments listed below.
    chainer_chain(backward, rb34_budg, s534, Result, [max_depth(10), max_steps(3)]),
...
test(provenance_complete, [setup(pr34_setup)]) :-
    % State a fact for 'pai rule base' with the arguments listed below.
    chainer_rule_base(rb34_prov, [fact(p_start), rule(p_start, p_goal)]),
    % State a fact for 'pai chain' with the arguments listed below.
    chainer_chain(backward, rb34_prov, p_goal, Result, [max_depth(5), max_steps(100)]),
...
test(rule_bases_isolated, [setup(pr34_setup)]) :-
    % State a fact for 'pai rule base' with the arguments listed below.
    chainer_rule_base(rb34_x, [fact(x34), rule(x34, x_result34)]),
    % State a fact for 'pai rule base' with the arguments listed below.
    chainer_rule_base(rb34_y, [fact(y34), rule(y34, y_result34)]),
    % x_result34 reachable from rb34_x
    % State a fact for 'pai chain' with the arguments listed below.
    chainer_chain(backward, rb34_x, x_result34, R1, [max_depth(5)]),
    % Check that 'R1' is unifiable with 'chain(_)'.
    R1 = chain(_),
    % x_result34 NOT reachable from rb34_y
    % State a fact for 'pai chain' with the arguments listed below.
    chainer_chain(backward, rb34_y, x_result34, R2, [max_depth(5)]),
...
test(control_policy_stats, [setup(pr34_setup)]) :-
    % State a fact for 'pai control policy' with the arguments listed below.
    chainer_control_policy(rb34_q, my_policy34, Stats),
