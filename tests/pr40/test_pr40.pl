...
test(two_defaults, [setup(pr40_setup)]) :-
    ...
    % State the fact: once(pai_defeasible_query(rb40b, eats40(cat40),  BG, answer(yes, _))).
...
test(multiple_exceptions, [setup(pr40_setup)]) :-
    ...
    % State the fact: once(pai_defeasible_query(rb40c, active40(bear40), BG, answer(no, _))).
