...
% Load the built-in 'prologai_similarity' library so its predicates are available here.
:- use_module(library(prologai_similarity)).
% Load the built-in 'prologai_types' library so its predicates are available here.
:- use_module(library(prologai_types)).
% Load the built-in 'prologai_collections' library so its predicates are available here.
:- use_module(library(prologai_collections)).
% Load the built-in 'prologai_convenience' library so its predicates are available here.
...
test(similar_signed) :-
    % Check that 'prologai_similar_signed(a, a, S), S' is greater than '0.5'.
    prologai_similar_signed(a, a, S), S > 0.5.

% Define a clause for 'test': succeed when the following conditions hold.
test(closest_match) :-
    % State a fact for 'pai closest match' with the arguments listed below.
    prologai_closest_match(apple, [apple, pear, carburetor], 2, Ranked),
...
test(keyword_group) :-
    ...
    prologai_keyword_group_member(colors, red).
...
test(case_match) :-
    ...
    prologai_case(foo, [foo -> bar, default -> baz], bar).

% Define a clause for 'test': succeed when the following conditions hold.
test(critical_section) :-
    % State the fact: pai critical(test_mutex, true).
    prologai_critical(test_mutex, true).
...
test(config_store) :-
    ...
    prologai_config_set(test_cfg, key1, value1),
    % State a fact for 'pai config get' with the arguments listed below.
    prologai_config_get(test_cfg, key1, value1),
