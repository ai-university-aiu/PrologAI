/*  PrologAI — PR 32 Attention Economy (ECAN) Acceptance Tests
    ...
    AC-PR32-001: Contributor node's LTI outlasts idle node's LTI after 100
                 banker cycles; attention_evict_lowest_lti removes idle first.
    ...
    AC-PR32-006: attention_spread transfers STI fraction to neighbors.
    AC-PR32-007: attention_evict_lowest_lti removes lowest-LTI non-protected nodes.
    AC-PR32-008: attention_metrics returns non-negative totals.
    AC-PR32-009: attention_link is idempotent — no duplicate edges.
*/
...
:- use_module(library(attention), [
    % Supply 'attention_level/3' as the next argument to the expression above.
    attention_level/3,
    % Supply 'attention_wage/3' as the next argument to the expression above.
    attention_wage/3,
    % Supply 'attention_spread/2' as the next argument to the expression above.
    attention_spread/2,
    % Supply 'attention_banker_cycle/0' as the next argument to the expression above.
    attention_banker_cycle/0,
    % Supply 'attention_metrics/1' as the next argument to the expression above.
    attention_metrics/1,
    % Supply 'attention_evict_lowest_lti/1' as the next argument to the expression above.
    attention_evict_lowest_lti/1,
    % Supply 'attention_link/2' as the next argument to the expression above.
    attention_link/2
...
test(contributor_outlasts_idle, [setup(reset_attention)]) :-
    ...
    attention_wage(contributor32, 1.0, _),
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(idle32, sti, 0.0),
    % Verify that for every solution of the Condition, the Action also holds.
    forall(between(1, 100, _), attention_banker_cycle),
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(contributor32, lti, ContLTI),
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(idle32, lti, IdleLTI),
...
test(conservation_sti_plus_reserve, [setup(reset_attention)]) :-
    % State a fact for 'pai wage' with the arguments listed below.
    attention_wage(cons32, 1.0, _),
    % State a fact for 'pai attention metrics' with the arguments listed below.
    attention_metrics(metrics(TotalSTI, _, Reserve)),
...
test(conservation_after_cycle, [setup(reset_attention)]) :-
    % State a fact for 'pai wage' with the arguments listed below.
    attention_wage(cycle32, 2.0, _),
    % Call the goal 'attention_banker_cycle'.
...
test(wages_from_reserve, [setup(reset_attention)]) :-
    ...
    % State a fact for 'pai wage' with the arguments listed below.
    attention_wage(wage32, 2.0, credits(ActualSTI, _)),
...
test(rent_decays_sti, [setup(reset_attention)]) :-
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(rent32, sti, 100.0),
    % Call the goal 'attention_banker_cycle'.
    attention_banker_cycle,
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(rent32, sti, NewSTI),
...
test(spread_transfers_sti, [setup(reset_attention)]) :-
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(src32, sti, 100.0),
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(nbr32, sti, 0.0),
    % State a fact for 'pai attention spread' with the arguments listed below.
    attention_spread(src32, [nbr32]),
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(nbr32, sti, NbrSTI),
...
test(evict_lowest_lti, [setup(reset_attention)]) :-
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(ev_hi32, lti, 20.0),
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(ev_lo32, lti,  5.0),
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(ev_md32, lti, 10.0),
    % State a fact for 'pai evict lowest lti' with the arguments listed below.
    attention_evict_lowest_lti(1),
...
test(metrics_non_negative, [setup(reset_attention)]) :-
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(ma32, sti, 50.0),
    % State a fact for 'pai attention' with the arguments listed below.
    attention_level(mb32, sti, 30.0),
    % State a fact for 'pai attention metrics' with the arguments listed below.
