...
% Import [sentinel_list/2] from the built-in 'sentinels' library.
...
pr13_cleanup :-
    % Call the goal 'uninstall_reflection_actors'.
...
test(install_starts_all_cyclic_actors) :-
    % Call the goal 'install_reflection_actors'.
...
test(motivation_cycle_inscribes_objective) :-
    ...
    anchor_node(percept_signal,
                ...
                _),
    % Call the goal 'motivation_cycle'.
    motivation_cycle,
...
test(exploration_cycle_inscribes_explore_objective) :-
    % Call the goal 'exploration_cycle'.
    exploration_cycle,
    % State a fact for 'default nexus' with the arguments listed below.
    default_nexus(Nx),
...
test(meta_control_cycle_runs) :-
    % State the zero-argument fact 'meta_control_cycle'.
    meta_control_cycle.
...
test(regulation_cycle_classifies_outcome) :-
    ...
    anchor_node(percept_signal,
                ...
                % Supply '_' as the next argument to the expression above.
                _),
    % Call the goal 'regulation_cycle'.
    regulation_cycle,
