/*  PrologAI — PR 13 Reflection Pattern Actors Acceptance Tests
    ...
    AC-PR13-001: reflex_actors_install_actors/0 starts all 10 cyclic actors.
    AC-PR13-002: reflex_actors_uninstall_actors/0 stops them all.
    AC-PR13-003: reflex_actors_motivation_cycle/0 inscribes an objective node_fact when
                 a homeostatic delta exceeds threshold.
    AC-PR13-004: reflex_actors_exploration_cycle/0 inscribes an explore_objective.
    AC-PR13-005: reflex_actors_meta_control_cycle/0 runs without error on a fresh nexus.
    AC-PR13-006: reflex_actors_regulation_cycle/0 classifies a confirmed outcome.
    ...
    AC-PR13-009: reflex_actors_impasse_cycle/0 inscribes subgoal for objective without plan.
*/
...
:- prolog_load_context(directory, TestDir),
   ...
   % State a fact for 'atomic list concat' with the arguments listed below.
   atomic_list_concat([ProjectRoot, '/packs/reflex_actors/prolog'],     ReflectionPath),
...
:- use_module(library(reflex_actors),  [reflex_actors_install_actors/0,
                                     ...
                                     % Continue the multi-line expression started above.
                                     reflex_actors_motivation_cycle/0, reflex_actors_exploration_cycle/0,
                                     % Continue the multi-line expression started above.
                                     reflex_actors_meta_control_cycle/0, reflex_actors_regulation_cycle/0,
                                     % Continue the multi-line expression started above.
...
% State a fact for 'reflex actor names' with the arguments listed below.
reflex_actors_actor_names([
    % Continue the multi-line expression started above.
...
test(install_starts_all_cyclic_actors) :-
    ...
    % State a fact for 'reflex actor names' with the arguments listed below.
    reflex_actors_actor_names(Expected),
...
test(motivation_cycle_inscribes_objective) :-
    ...
    anchor_node(percept_signal,
                ...
                _),
    % Call the goal 'reflex_actors_motivation_cycle'.
...
test(regulation_cycle_classifies_outcome) :-
    ...
    anchor_node(percept_signal,
                ...
                % Supply '_' as the next argument to the expression above.
                _),
    % Call the goal 'reflex_actors_regulation_cycle'.
...
test(impasse_cycle_inscribes_subgoal) :-
    ...
    % Call the goal 'reflex_actors_impasse_cycle'.
