/*  PrologAI — PR 46 ROS 2 Robot Bridge Acceptance Tests
    ...
    AC-PR46-003: robot_operating_system_bridge_enroll registers a robot body with its topics.
    AC-PR46-004: robot_operating_system_bridge maps a camera message to a visual_percept.
    AC-PR46-005: robot_operating_system_bridge maps a laser scan to a spatial_percept.
    AC-PR46-006: robot_operating_system_bridge maps an odometry message to proprioceptive.
    AC-PR46-007: robot_operating_system_bridge_observe returns buffered percepts and clears buffer.
    AC-PR46-008: robot_operating_system_bridge_act drive in simulation is confirmed.
    AC-PR46-009: robot_operating_system_bridge_act with excessive speed is vetoed by constitutional gate.
*/
...
test(observe_plan_drive_confirm, [setup(pr46_setup)]) :-
    ...
    % State a fact for 'once' with the arguments listed below.
    once(robot_operating_system_bridge(message(BodyId, '/camera/image_raw', frame(colored_object46)), CamPercept)),
    ...
    % State a fact for 'once' with the arguments listed below.
    once(robot_operating_system_bridge_observe(Percepts)),
    % State a fact for 'once' with the arguments listed below.
    once(member(perception_signal(_, BodyId, visual_percept(image, _)), Percepts)),
    % Issue a drive command toward the object (simulated)
    % State a fact for 'once' with the arguments listed below.
    once(robot_operating_system_bridge_act(drive(BodyId, 0.5, 0.0), DriveConf)),
    ...
    % State a fact for 'once' with the arguments listed below.
    once(robot_operating_system_bridge(message(BodyId, '/odom', pose(at_object46)), OdomPercept)),
    ...
    % State a fact for 'once' with the arguments listed below.
    once(robot_operating_system_bridge_observe(Feedback)),
...
test(physical_actuation_withheld, [setup(pr46_setup)]) :-
    ...
    % Succeed only if 'catch(robot_operating_system_bridge_enroll(Robot, none, _), _, fail' cannot be proved (negation as failure).
    \+ catch(robot_operating_system_bridge_enroll(Robot, none, _), _, fail).
...
test(camera_to_visual_percept, [setup(pr46_setup)]) :-
    ...
    once(robot_operating_system_bridge_enroll(Robot, gazebo, BodyId)),
    % State a fact for 'once' with the arguments listed below.
    once(robot_operating_system_bridge(message(BodyId, '/camera/image_raw', img_data46), Percept)),
...
test(lidar_to_spatial_percept, [setup(pr46_setup)]) :-
    ...
    % State a fact for 'once' with the arguments listed below.
    once(robot_operating_system_bridge(message(BodyId, '/scan', scan_data46), Percept)),
...
test(odom_to_proprioceptive, [setup(pr46_setup)]) :-
    ...
    once(robot_operating_system_bridge_enroll(Robot, gazebo, BodyId)),
    % State a fact for 'once' with the arguments listed below.
    once(robot_operating_system_bridge(message(BodyId, '/odom', odom_data46), Percept)),
...
test(observe_clears_buffer, [setup(pr46_setup)]) :-
    ...
    once(robot_operating_system_bridge_enroll(Robot, webots, BodyId)),
    % State a fact for 'once' with the arguments listed below.
    once(robot_operating_system_bridge(message(BodyId, '/camera/image_raw', frame46), _)),
    % State a fact for 'once' with the arguments listed below.
...
test(excessive_speed_vetoed, [setup(pr46_setup)]) :-
    ...
    % State a fact for 'once' with the arguments listed below.
    once(robot_operating_system_bridge_enroll(Robot, gazebo, BodyId)),
    % Speed 10.0 m/s exceeds the 2.0 limit in robot_operating_system_bridge_constitutional_gate
    % State a fact for 'once' with the arguments listed below.
